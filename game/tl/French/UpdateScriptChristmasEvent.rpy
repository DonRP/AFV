#     old ""
#     ""
# "Plural-Forms: nplurals=2; plural=(n > 1);\n"
# "X-french-Project: a-family-venture\n"
# "X-french-Project-ID: 492487\n"
# "X-french-Language: fr\n"
# "X-french-File: /[DonRP.AFV] main/game/tl/french/UpdateScriptChristmasEvent.po\n"
# "X-french-File-ID: 3273\n"
# "Project-Id-Version: a-family-venture\n"
# "Content-Type: text/plain; charset=UTF-8\n"
# "Language-Team: French\n"
# "Language: fr_FR\n"
# "PO-Revision-Date: 2022-04-23 09:35\n"

# TODO: Translation updated at 2022-04-23 HH:HH

# game/UpdateScriptChristmasEvent.rpy:247
translate french xmas_dream_387f76a6:

    # Welcome to our Christmas event! WARNING!! This event has elements of NTR, that are completely avoidable! Choices that lead to NTR content are clearly marked!"
    "Bienvenue à notre événement de Noël! ATTENTION !! Cet événement a des éléments de NTR (tricherie / inceste), qui sont complètement évitables! Les choix qui mènent au contenu NTR sont clairement marqués!"

# game/UpdateScriptChristmasEvent.rpy:248
translate french xmas_dream_fbb90359:

    # Do not choose an NTR option if it offends you, only to complain about it later on some online forum!!"
    "Ne choisissez pas une option NTR (trahison / inceste), si elle vous offense, pour vous en plaindre plus tard sur un forum en ligne!!"

# game/UpdateScriptChristmasEvent.rpy:249
translate french xmas_dream_074aaa65:

    # This event also has pregnancy content."
    "Cet événement a également un contenu de grossesse."

# game/UpdateScriptChristmasEvent.rpy:250
translate french xmas_dream_835e42bc:

    # Would you like to play the Christmas event at this time? (Choosing \"No\" will complete this event and make it so you can only watch it in the gallery menu.)"
    "Voulez-vous jouer à l'événement de Noël maintenant? (En choisissant 'Non', vous terminerez cet événement et vous ne pourrez pas le voir que dans le menu de la galerie.)"

# game/UpdateScriptChristmasEvent.rpy:266
translate french xmas_dream_64c08e3f:

    # {i}A tribute to Charles Dickens and his timeless classic, \"A Christmas Carol.\"{/i}"
    "{i}Un hommage à Charles Dickens et à son classique intemporel, 'A Christmas Carol.'{/i}"

# game/UpdateScriptChristmasEvent.rpy:267
translate french xmas_dream_a20cefa7:

    # ..."
    "..."

# game/UpdateScriptChristmasEvent.rpy:268
translate french xmas_dream_41c1aef9:

    # RT "{i}Thank God the Christmas break is coming! I'm so tired of school.{/i}"
    RT "{i}Dieu merci, les vacances de Noël arrivent! J'en ai marre de l'école.{/i}"

# game/UpdateScriptChristmasEvent.rpy:269
translate french xmas_dream_93bf0873:

    # RT "{i}Now I'll have even more time to work on seducing the girls.{/i}"
    RT "{i}Maintenant, j'aurai encore plus de temps pour séduire les filles.{/i}"

# game/UpdateScriptChristmasEvent.rpy:270
translate french xmas_dream_0c80fdb1:

    # RT "{i}I've got to really work hard if I want to unwrap those beautiful boxes for the holidays.{/i}"
    RT "{i}Je dois vraiment travailler dur si je veux déballer ces belles boîtes pour les vacances.{/i}"

# game/UpdateScriptChristmasEvent.rpy:271
translate french xmas_dream_7c3c9a34:

    # RT "{i}And those are some great mental images to fall asleep to.... zzzzzz.... {/i}"
    RT "{i}Et ce sont de grandes images mentales sur lesquelles s'endormir.... zzzzz.... {/i}"

# game/UpdateScriptChristmasEvent.rpy:272
translate french xmas_dream_e27ce5cd:

    # RT "{i}zzzzzzz.... {/i}"
    RT "{i}zzzzzzz.... {/i}"

# game/UpdateScriptChristmasEvent.rpy:273
translate french xmas_dream_a20cefa7_1:

    # ..."
    "..."

# game/UpdateScriptChristmasEvent.rpy:274
translate french xmas_dream_03f26b44:

    # X "[ryan]..."
    X "[ryan]..."

# game/UpdateScriptChristmasEvent.rpy:275
translate french xmas_dream_6bb809b3:

    # X "Hey [ryan]..."
    X "Hé [ryan]..."

# game/UpdateScriptChristmasEvent.rpy:280
translate french xmas_dream_1e1e85d8:

    # X "Wake up [ryan]!..."
    X "[ryan], réveille-toi!..."

# game/UpdateScriptChristmasEvent.rpy:285
translate french xmas_dream_4231b67a:

    # X "Hey [ryan]!.... Wake up!"
    X "Hé [ryan]!.... Réveille-toi!"

# game/UpdateScriptChristmasEvent.rpy:286
translate french xmas_dream_e3b1781d:

    # RT "{i}Huh.... what?....{/i}"
    RT "{i}Huh.... quoi ?....{/i}"

# game/UpdateScriptChristmasEvent.rpy:297
translate french xmas_dream_cfc9079c:

    # RT "{i}What the hell?{/i}"
    RT "{i}C'est quoi ce bordel?{/i}"

# game/UpdateScriptChristmasEvent.rpy:298
translate french xmas_dream_beca06f1:

    # RT "{i}What a weird dream! There's a tiny Mom on my chest{/i}"
    RT "{i}Quel rêve bizarre! Il y a une petite maman sur ma poitrine {/i}"

# game/UpdateScriptChristmasEvent.rpy:299
translate french xmas_dream_bc20437e:

    # X "[ryan].... this isn't a dream.... and I'm not a tiny Mom"
    X "[ryan].... ce n'est pas un rêve.... et je ne suis pas une petite maman"

# game/UpdateScriptChristmasEvent.rpy:300
translate french xmas_dream_df474a97:

    # RT "{i}You have to be a dream.... you can read my thoughts....{/i}"
    RT "{i}Tu devines que c'est un rêve.... Tu peux lire mes pensées....{/i}"

# game/UpdateScriptChristmasEvent.rpy:301
translate french xmas_dream_2e1d4afb:

    # X "[ryan].... I'm the ghost of Christmas past."
    X "[ryan].... Je suis le fantôme de Noël passé."

# game/UpdateScriptChristmasEvent.rpy:302
translate french xmas_dream_056794e2:

    # R "What?.... Bullshit!.... Then why do you look like a half-naked version of my mom?"
    R "Quoi?.... Conneries !.... Alors pourquoi tu ressembles à une version à moitié nue de ma mère?"

# game/UpdateScriptChristmasEvent.rpy:303
translate french xmas_dream_34e0ad87:

    # GP "Well.... I'm a ghost.... so my true form might have scared you to death."
    GP "Bien.... Je suis un fantôme.... donc ma vraie forme pourrait t'avoir fait peur à mort."

# game/UpdateScriptChristmasEvent.rpy:304
translate french xmas_dream_8f36dc49:

    # GP "I've been given the power to take the form of the person in your life that brings you the most comfort."
    GP "On m'a donné le pouvoir de prendre la forme de la personne dans tu vie qui t'apporte le plus de réconfort."

# game/UpdateScriptChristmasEvent.rpy:305
translate french xmas_dream_d10c4215:

    # GP "What I'm wearing is also a product of what your mind would prefer to see this person wearing, mixed with a little Christmas flair for the season."
    GP "Ce que je porte est aussi un produit de ce que ton esprit préférerait voir cette personne porter, mélangé avec un peu de flair de Noël pour la saison."

# game/UpdateScriptChristmasEvent.rpy:306
translate french xmas_dream_86038b67:

    # GP "I look like your mom, you say?"
    GP "Je ressemble à ta mère, dis-tu ?"

# game/UpdateScriptChristmasEvent.rpy:307
translate french xmas_dream_42561a27:

    # GP "..."
    GP "..."

# game/UpdateScriptChristmasEvent.rpy:308
translate french xmas_dream_4bb6e3c8:

    # GP "You sick fuck!"
    GP "Putain de malade!"

# game/UpdateScriptChristmasEvent.rpy:309
translate french xmas_dream_25013382:

    # R "This is so weird!"
    R "C'est tellement bizarre!"

# game/UpdateScriptChristmasEvent.rpy:310
translate french xmas_dream_a870c1e2:

    # R "Though I'm still about 90 percent sure that this is just a dream."
    R "Bien que je sois toujours sûr à 90% que ce n'est qu'un rêve."

# game/UpdateScriptChristmasEvent.rpy:311
translate french xmas_dream_9254ac2e:

    # R "You're just something I ate.... an undigested piece of pizza..."
    R "Tu es juste quelque chose que j'ai mangé.... un morceau de pizza non digéré..."

# game/UpdateScriptChristmasEvent.rpy:312
translate french xmas_dream_542d68e0:

    # R "Yeah.... that's all you are..."
    R "Ouais.... c'est tout ce que tu es..."

# game/UpdateScriptChristmasEvent.rpy:313
translate french xmas_dream_9d857c40:

    # R "Which means I can do whatever I want to you."
    R "Ce qui signifie que je peux faire ce que je veux pour toi."

# game/UpdateScriptChristmasEvent.rpy:316
translate french xmas_dream_658b300f:

    # GP "Ooofff..."
    GP "Ooofff..."

# game/UpdateScriptChristmasEvent.rpy:317
translate french xmas_dream_26b29a71:

    # R "Holy shit!.... I can touch you!"
    R "Putain de merde!.... Je peux te toucher!"

# game/UpdateScriptChristmasEvent.rpy:321
translate french xmas_dream_4c6c2e11:

    # R "Oh, shit!.... Where did she go?"
    R "Oh, merde!.... Où est-elle allée ?"

# game/UpdateScriptChristmasEvent.rpy:324
translate french xmas_dream_f0276cc4:

    # R "She's gone!..."
    R "Elle est partie!..."

# game/UpdateScriptChristmasEvent.rpy:325
translate french xmas_dream_f7dde659:

    # R "I guess I was dreaming after all."
    R "Je suppose que je rêvais après tout."

# game/UpdateScriptChristmasEvent.rpy:329
translate french xmas_dream_a70c5134:

    # GP "You son of a bitch!"
    GP "Fils de pute!"

# game/UpdateScriptChristmasEvent.rpy:330
translate french xmas_dream_e93aa33f:

    # GP "You try a move like that again and I'll cut your cock off!"
    GP "Tu essaies encore un mouvement comme ça et je te coupe la bite!"

# game/UpdateScriptChristmasEvent.rpy:331
translate french xmas_dream_b53dbcfb:

    # GP "If you still think you're dreaming, come at me!"
    GP "Si tu penses encore rêver, viens me voir !"

# game/UpdateScriptChristmasEvent.rpy:332
translate french xmas_dream_6e0ad0bc:

    # R "Ok.... ok.... I won't grab you again!.... Just let me go..."
    R "D'accord.... OK.... Je ne t'attraperai plus!.... Laisse-moi partir..."

# game/UpdateScriptChristmasEvent.rpy:335
translate french xmas_dream_e7f5cc38:

    # GP "Woah.... holy shit!"
    GP "Ouah.... putain de merde!"

# game/UpdateScriptChristmasEvent.rpy:336
translate french xmas_dream_3d8b8547:

    # GP "Haha.... you like it when I'm rough with you, don't you?"
    GP "Haha.... tu aimes quand je suis dur avec toi, n'est-ce pas?"

# game/UpdateScriptChristmasEvent.rpy:337
translate french xmas_dream_e27ab8db:

    # GP "You dirty boy!"
    GP "Sale garçon!"

# game/UpdateScriptChristmasEvent.rpy:338
translate french xmas_dream_675b0c7a:

    # R "Yes.... I mean no!.... Can you please let my cock go?"
    R "Oui.... Je veux dire non!.... Tu peux laisser partir ma bite s'il te plait ?"

# game/UpdateScriptChristmasEvent.rpy:339
translate french xmas_dream_02efabac:

    # GP "Hmmmm.... I guess I could, if you promise not to touch me, and to sit there and listen to me."
    GP "Hmmmm.... Je suppose que je pourrais, si tu promets de ne pas me toucher, et de rester assis là et de m'écouter."

# game/UpdateScriptChristmasEvent.rpy:340
translate french xmas_dream_07269c20:

    # R "Ok, I promise!"
    R "Ok, promis !"

# game/UpdateScriptChristmasEvent.rpy:341
translate french xmas_dream_69db1379:

    # RT "{i}I wonder if I could capture her in a jar or something.{/i}"
    RT "{i}Je me demande si je pourrais la capturer dans un bocal ou quelque chose comme ça.{/i}"

# game/UpdateScriptChristmasEvent.rpy:342
translate french xmas_dream_58b351f3:

    # GP "[ryan]! I can hear your thoughts, remember? And no, I'd just disappear out of a jar."
    GP "[ryan]! J'entends tes pensées, tu te souviens ? Et non, je disparaîtrais d'un bocal."

# game/UpdateScriptChristmasEvent.rpy:343
translate french xmas_dream_695ebf96:

    # R "Sorry.... I won't do anything stupid."
    R "Désolé.... Je ne ferai rien de stupide."

# game/UpdateScriptChristmasEvent.rpy:346
translate french xmas_dream_8ee0bd1d:

    # GP "Great!.... Because you've got a very busy night ahead of you."
    GP "Génial!.... Parce que tu as une nuit très chargée devant toi."

# game/UpdateScriptChristmasEvent.rpy:347
translate french xmas_dream_2afb9948:

    # R "Huh?....I can't move!..."
    R "Hein?....Je ne peux pas bouger !..."

# game/UpdateScriptChristmasEvent.rpy:348
translate french xmas_dream_ab8aa8e8:

    # GP "Just a safety precaution."
    GP "Juste une précaution de sécurité."

# game/UpdateScriptChristmasEvent.rpy:349
translate french xmas_dream_7c173e55:

    # GP "You will be visited by three spirits, including myself, obviously."
    GP "Tu recevras la visite de trois esprits, dont moi-même, évidemment."

# game/UpdateScriptChristmasEvent.rpy:350
translate french xmas_dream_b1b8ff54:

    # R "What?.... But why?"
    R "Quoi?.... Mais pourquoi?"

# game/UpdateScriptChristmasEvent.rpy:351
translate french xmas_dream_af34e7f3:

    # GP "Your actions have caught the attention of \"The Woman Upstairs.\""
    GP "Tes actions ont attiré l'attention de 'La Femme à l'étage.'"

# game/UpdateScriptChristmasEvent.rpy:352
translate french xmas_dream_854e1f6c:

    # R "What really?.... So God's a woman?"
    R "Quoi vraiment?.... Alors Dieu est une femme ?"

# game/UpdateScriptChristmasEvent.rpy:353
translate french xmas_dream_bce74b59:

    # GP "Actually a woman that can grow a penis.... I think you would call her a \"futa.\""
    GP "En fait une femme qui peut faire pousser un pénis.... Je pense que tu l'appellerais un 'futa.'"

# game/UpdateScriptChristmasEvent.rpy:354
translate french xmas_dream_5f1e8af6:

    # R "Wow!"
    R "Wow!"

# game/UpdateScriptChristmasEvent.rpy:355
translate french xmas_dream_2deedd58:

    # R "And what have I done to get her attention on me?"
    R "Et qu'ai-je fait pour attirer son attention sur moi?"

# game/UpdateScriptChristmasEvent.rpy:356
translate french xmas_dream_f2158951:

    # GP "What do you think?"
    GP "Qu'en penses-tu?"

# game/UpdateScriptChristmasEvent.rpy:357
translate french xmas_dream_a3914c92:

    # GP "I'll give you three guesses.... and they all live in this house."
    GP "Je vais te donner trois suppositions.... et ils vivent tous dans cette maison."

# game/UpdateScriptChristmasEvent.rpy:358
translate french xmas_dream_34bb179c:

    # GP "And we can sense that you're about to turn up your devious efforts a few notches over the Christmas holidays."
    GP "Et nous pouvons sentir que tu es sur le point d'augmenter tes efforts sournois de quelques crans pendant les vacances de Noël."

# game/UpdateScriptChristmasEvent.rpy:359
translate french xmas_dream_ef99f76e:

    # R "Oh.... you guys have been listening to my thoughts, huh?"
    R "Oh.... Tu as écouté mes pensées, hein ?"

# game/UpdateScriptChristmasEvent.rpy:360
translate french xmas_dream_78001064:

    # GP "Oh, yes.... and we're here to show you the consequences of your actions."
    GP "Oh, oui.... et nous sommes là pour te montrer les conséquences de tes actions."

# game/UpdateScriptChristmasEvent.rpy:361
translate french xmas_dream_f75d2307:

    # GP "While we don't have problems with sexual promiscuity in general,"
    GP "Alors que nous n'avons pas de problèmes de promiscuité sexuelle en général,"

# game/UpdateScriptChristmasEvent.rpy:362
translate french xmas_dream_2983ab07:

    # GP "we tend to frown on fucking your own family."
    GP "nous avons tendance à froncer les sourcils en baisant ta propre famille."

# game/UpdateScriptChristmasEvent.rpy:363
translate french xmas_dream_124e0baa:

    # GP "It just causes complications down the line."
    GP "Cela provoque juste des complications sur toute la ligne."

# game/UpdateScriptChristmasEvent.rpy:366
translate french xmas_dream_166b430c:

    # GP "Although, I do see why the women in your family are so tempting for you."
    GP "Bien que, je vois pourquoi les femmes de ta famille sont si tentantes pour toi."

# game/UpdateScriptChristmasEvent.rpy:367
translate french xmas_dream_af5a552a:

    # GP "I mean, look at that ass!"
    GP "Je veux dire, regarde ce cul!"

# game/UpdateScriptChristmasEvent.rpy:368
translate french xmas_dream_4e5927f0:

    # R "I know, right?..."
    R "Je sais, non?..."

# game/UpdateScriptChristmasEvent.rpy:369
translate french xmas_dream_d0e4ac34:

    # GP "I haven't had a good taste of ass in a millennium."
    GP "Je n'ai pas eu un bon goût de cul depuis un millénaire."

# game/UpdateScriptChristmasEvent.rpy:370
translate french xmas_dream_7c9da831:

    # GP "I've just been stuck in the spirit realm serving \"Her\", ever since I died."
    GP "Je viens d'être coincé dans le royaume des esprits au service de 'Sa', depuis que je suis mort."

# game/UpdateScriptChristmasEvent.rpy:371
translate french xmas_dream_140b90e2:

    # GP "No tasty asses there, only dead ones."
    GP "Pas de culs savoureux là-bas, seulement des culs morts."

# game/UpdateScriptChristmasEvent.rpy:372
translate french xmas_dream_2866a634:

    # R "Weren't we talking about the consequences of my actions?"
    R "Ne parlait-on pas des conséquences de mes actes ?"

# game/UpdateScriptChristmasEvent.rpy:373
translate french xmas_dream_f7282719:

    # GP "Were we?.... Uh.... just give me a second."
    GP "Étions-nous?.... Euh.... donne-moi juste une seconde."

# game/UpdateScriptChristmasEvent.rpy:376
translate french xmas_dream_03d2f74e:

    # R "You can fly?..."
    R "Tu peux voler?..."

# game/UpdateScriptChristmasEvent.rpy:377
translate french xmas_dream_2ad8bfeb:

    # GP "Of course!.... You really don't know much about ghosts, do you?"
    GP "Bien sûr!.... Tu ne connais vraiment pas grand-chose aux fantômes, n'est-ce pas ?"

# game/UpdateScriptChristmasEvent.rpy:378
translate french xmas_dream_70cf7f50:

    # R "What are you doing?"
    R "Que fais-tu?"

# game/UpdateScriptChristmasEvent.rpy:379
translate french xmas_dream_73ce20fa:

    # GP "I said just give me a second."
    GP "J'ai dit donne-moi juste une seconde."

# game/UpdateScriptChristmasEvent.rpy:382
translate french xmas_dream_c6841c25:

    # R "..."
    R "..."

# game/UpdateScriptChristmasEvent.rpy:383
translate french xmas_dream_1961d0cf:

    # R "Hey!.... What are you doing?"
    R "Hé!.... Qu'es-tu en train de faire?"

# game/UpdateScriptChristmasEvent.rpy:384
translate french xmas_dream_5869b57e:

    # GP "I'm sorry!.... I just can't resist"
    GP "Je suis désolé!.... Je ne peux pas résister"

# game/UpdateScriptChristmasEvent.rpy:385
translate french xmas_dream_f488d935:

    # R "But that's your daughter.... I mean my sister!"
    R "Mais c'est ta fille.... Je veux dire ma sœur!"

# game/UpdateScriptChristmasEvent.rpy:386
translate french xmas_dream_ddb6fcdd:

    # GP "What's the matter [ryan], jealous?"
    GP "Quel est le problème [ryan], jaloux?"

# game/UpdateScriptChristmasEvent.rpy:387
translate french xmas_dream_7d5b3f44:

    # GP "Then why don't you come and stop me?"
    GP "Alors pourquoi ne viens-tu pas m'arrêter?"

# game/UpdateScriptChristmasEvent.rpy:388
translate french xmas_dream_4e4491b3:

    # R "Because I can't move!"
    R "Parce que je ne peux pas bouger!"

# game/UpdateScriptChristmasEvent.rpy:389
translate french xmas_dream_8061eddb:

    # GP "Haha.... that's right."
    GP "Haha.... c'est vrai."

# game/UpdateScriptChristmasEvent.rpy:390
translate french xmas_dream_9b25d42a:

    # GP "I guess you'll just have to watch."
    GP "Je suppose que tu auras juste à regarder."

# game/UpdateScriptChristmasEvent.rpy:393
translate french xmas_dream_436cded4:

    # GP "Oh, man!.... I've missed this!"
    GP "Oh, mec!.... J'ai raté ça !"

# game/UpdateScriptChristmasEvent.rpy:394
translate french xmas_dream_50ff98c2:

    # RT "{i}Holy shit.... I know that's not really Mom licking Sidney, but that's actually pretty hot!{/i}"
    RT "{i}Merde... Je sais que ce n'est pas vraiment Maman qui lèche Sidney, mais c'est en fait assez chaud!{/i}"

# game/UpdateScriptChristmasEvent.rpy:395
translate french xmas_dream_3d4820cb:

    # GP "Mmmppphhh.... haha.... remrembrere rhat I cran strill hear your throts..."
    GP "Mmmppphhh.... haha.... rapelle toi ça je peux entendre tes gaz..."

# game/UpdateScriptChristmasEvent.rpy:396
translate french xmas_dream_1a7e6af3:

    # R "Shit!"
    R "Merde!"

# game/UpdateScriptChristmasEvent.rpy:397
translate french xmas_dream_2ae0aebe:

    # S "(sleepily mumbling) mmmmmmm.... [ryan].... that feels really.... nice.... "
    S "(marmonement endormi) mmmmmmm.... [ryan].... ça se sent vraiment.... bon.... "

# game/UpdateScriptChristmasEvent.rpy:400
translate french xmas_dream_600c0015:

    # GP "There.... my spiritual batteries are all re-charged."
    GP "Là.... mes batteries spirituelles sont toutes rechargées."

# game/UpdateScriptChristmasEvent.rpy:401
translate french xmas_dream_d076ceaf:

    # GP "So, are you ready for our journey?"
    GP "Alors, es-tu prêt pour notre voyage?"

# game/UpdateScriptChristmasEvent.rpy:402
translate french xmas_dream_6db5a4cd:

    # R "What journey?"
    R "Quel voyage?"

# game/UpdateScriptChristmasEvent.rpy:403
translate french xmas_dream_1e232d09:

    # R "I'm not going anywhere with you."
    R "Je ne vais nulle part avec toi."

# game/UpdateScriptChristmasEvent.rpy:406
translate french xmas_dream_b2ac3ad5:

    # GP "I'm going to take you on a journey into Christmas past."
    GP "Je vais t'emmener dans un voyage dans le passé de Noël."

# game/UpdateScriptChristmasEvent.rpy:407
translate french xmas_dream_7d056303:

    # GP "To a time when your parents first met."
    GP "À un moment où tes parents se sont rencontrés pour la première fois."

# game/UpdateScriptChristmasEvent.rpy:408
translate french xmas_dream_570b7afa:

    # GP "I want you to see first hand, the beginning of the relationship that concieved you and your sisters, and the relationship you're currently trying to destroy."
    GP "Je veux que tu voies de première main, le début de la relation que toi et tes sœurs avez conçue, et la relation que tu essaies actuellement de détruire."

# game/UpdateScriptChristmasEvent.rpy:409
translate french xmas_dream_cd70090f:

    # R "Can't you just tell me about it? I really need to get my sleep. I've got a busy day of seducing ahead of me tomorrow."
    R "Tu ne peux pas m'en parler ? J'ai vraiment besoin de dormir. J'ai une journée bien remplie de séduction devant moi demain."

# game/UpdateScriptChristmasEvent.rpy:410
translate french xmas_dream_409de082:

    # GP "Haha.... you lazy, cheeky, asshole."
    GP "Haha.... espèce de fainéant, effronté, connard."

# game/UpdateScriptChristmasEvent.rpy:411
translate french xmas_dream_e050aa95:

    # GP "You don't have a choice."
    GP "Tu n'as pas le choix."

# game/UpdateScriptChristmasEvent.rpy:412
translate french xmas_dream_0e8c45de:

    # GP "Now get ready, because this could possibly give you motion sickness."
    GP "Maintenant, prépares-toi, car cela pourrait te donner le mal des transports."

# game/UpdateScriptChristmasEvent.rpy:425
translate french xmas_dream_cb7d94ff:

    # RT "{i}Ahhhh!.... Where the hell am I?{/i}"
    RT "{i}Ahhhh!.... Où suis-je, bordel ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:426
translate french xmas_dream_de5d1b43:

    # GP "You're dissolving through the wormholes of time."
    GP "Tu te dissous à travers les trous de ver du temps."

# game/UpdateScriptChristmasEvent.rpy:427
translate french xmas_dream_2d200f0c:

    # GP "Just relax, we'll be there in just a couple more seconds."
    GP "Détends-toi, nous serons là dans quelques secondes."

# game/UpdateScriptChristmasEvent.rpy:432
translate french xmas_present_e27ce5cd:

    # RT "{i}zzzzzzz.... {/i}"
    RT "{i}zzzzzzz.... {/i}"

# game/UpdateScriptChristmasEvent.rpy:433
translate french xmas_present_a20cefa7:

    # ..."
    "..."

# game/UpdateScriptChristmasEvent.rpy:434
translate french xmas_present_03f26b44:

    # X "[ryan]..."
    X "[ryan]..."

# game/UpdateScriptChristmasEvent.rpy:435
translate french xmas_present_6bb809b3:

    # X "Hey [ryan]..."
    X "Hé [ryan]..."

# game/UpdateScriptChristmasEvent.rpy:440
translate french xmas_present_1e1e85d8:

    # X "Wake up [ryan]!..."
    X "[ryan], réveille-toi!..."

# game/UpdateScriptChristmasEvent.rpy:445
translate french xmas_present_4231b67a:

    # X "Hey [ryan]!.... Wake up!"
    X "Hé [ryan]!.... Réveille-toi!"

# game/UpdateScriptChristmasEvent.rpy:446
translate french xmas_present_e3b1781d:

    # RT "{i}Huh.... what?....{/i}"
    RT "{i}Hein.... Quoi?....{/i}"

# game/UpdateScriptChristmasEvent.rpy:457
translate french xmas_present_f34a6aa1:

    # R "Lauren?..."
    R "Lauren?..."

# game/UpdateScriptChristmasEvent.rpy:458
translate french xmas_present_203c7bd4:

    # X "Guess again."
    X "Devine à nouveau."

# game/UpdateScriptChristmasEvent.rpy:459
translate french xmas_present_e889f985:

    # R "Oh, fuck!.... I thought I was done with this dream."
    R "Oh, putain!.... Je pensais en avoir fini avec ce rêve."

# game/UpdateScriptChristmasEvent.rpy:460
translate french xmas_present_2d2097dd:

    # X "Maybe it's not a dream."
    X "Ce n'est peut-être pas un rêve."

# game/UpdateScriptChristmasEvent.rpy:461
translate french xmas_present_2c5ccf90:

    # R "Wouldn't you just tell me it's not a dream, if it weren't really a dream?"
    R "Ne me dirais-tu pas que ce n'est pas un rêve, si ce n'était pas vraiment un rêve ?"

# game/UpdateScriptChristmasEvent.rpy:462
translate french xmas_present_9b304d48:

    # X "That wouldn't be any fun, if we took out the wonder."
    X "Ce ne serait pas amusant, si on sortait la merveille."

# game/UpdateScriptChristmasEvent.rpy:463
translate french xmas_present_7691e859:

    # X "We're doing this for your welfare."
    X "Nous faisons cela pour ton bien-être."

# game/UpdateScriptChristmasEvent.rpy:464
translate french xmas_present_54b995ef:

    # R "Wouldn't a night of uninterrupted sleep be better for my welfare?"
    R "Une nuit de sommeil ininterrompu ne serait-elle pas meilleure pour mon bien-être?"

# game/UpdateScriptChristmasEvent.rpy:465
translate french xmas_present_294f5843:

    # X "For your salvation then."
    X "Pour ton salut alors."

# game/UpdateScriptChristmasEvent.rpy:466
translate french xmas_present_ebe8af21:

    # R "Salvation from what?"
    R "Salut de quoi?"

# game/UpdateScriptChristmasEvent.rpy:467
translate french xmas_present_e32c8be5:

    # X "From being such a creepy little pervert."
    X "D'être un petit pervers si effrayant."

# game/UpdateScriptChristmasEvent.rpy:468
translate french xmas_present_e7606f6b:

    # RT "{i}Hah.... good luck!{/i}"
    RT "{i}Hah.... Bonne chance!{/i}"

# game/UpdateScriptChristmasEvent.rpy:469
translate french xmas_present_19f0bf7f:

    # X "I can read thoughts."
    X "Je peux lire les pensées."

# game/UpdateScriptChristmasEvent.rpy:470
translate french xmas_present_c3a6fe17:

    # R "Oh, yeah..."
    R "Oh, oui..."

# game/UpdateScriptChristmasEvent.rpy:471
translate french xmas_present_6b1ac578:

    # R "So, are you \"The Leprechaun of Christmas Present\", or something?"
    R "Alors, es-tu 'Le Lutin du Cadeau de Noël', ou quelque chose comme ça?"

# game/UpdateScriptChristmasEvent.rpy:474
translate french xmas_present_3661e54a:

    # X "I'm the \"Ghost of Christmas Present.\""
    X "Je suis le 'Fantôme du cadeau de Noël.'"

# game/UpdateScriptChristmasEvent.rpy:475
translate french xmas_present_12d923ca:

    # R "Well, of course you are."
    R "Eh bien, bien sûr que tu l'es."

# game/UpdateScriptChristmasEvent.rpy:476
translate french xmas_present_98f10171:

    # GCP "I see my sister's anger at you was not misplaced."
    GCP "Je vois que la colère de ma sœur contre toi n'était pas déplacée."

# game/UpdateScriptChristmasEvent.rpy:477
translate french xmas_present_9fb9da09:

    # GCP "After the experience with the first ghost, a client is usually well on his way to being reclaimed, but my sister thinks that you actually regressed with your first visitation."
    GCP "Après l'expérience avec le premier fantôme, un client est généralement en bonne voie d'être récupéré, mais ma sœur pense que tu as réellement régressé lors de ta première visite."

# game/UpdateScriptChristmasEvent.rpy:479
translate french xmas_present_9ea38abf:

    # GCP "She also warned me to stay away from the end of your penis."
    GCP "Elle m'a également averti de rester à l'écart de l'extrémité de ton pénis."

# game/UpdateScriptChristmasEvent.rpy:480
translate french xmas_present_2e229525:

    # GCP "Haha.... I've never seen her so mad."
    GCP "Haha.... Je ne l'ai jamais vue aussi folle depuis longtemps."

# game/UpdateScriptChristmasEvent.rpy:483
translate french xmas_present_9587cd81:

    # GCP "She did tell me that I must try the local flavor while I'm here in town though."
    GCP "Elle m'a dit que je devais essayer la saveur locale pendant que je suis ici en ville."

# game/UpdateScriptChristmasEvent.rpy:486
translate french xmas_present_f6261ae8:

    # GCP "Oh, yes!.... And there she is."
    GCP "Oh, oui!.... Et la voilà."

# game/UpdateScriptChristmasEvent.rpy:489
translate french xmas_present_63cd985a:

    # GCP "Oh, my.... It looks like she was right. That is one tasty looking ass."
    GCP "Oh, mon.... On dirait qu'elle avait raison. C'est un cul savoureux."

# game/UpdateScriptChristmasEvent.rpy:490
translate french xmas_present_3f08d124:

    # GCP "This job has very few benefits, but it does have it's moments sometimes."
    GCP "Ce travail a très peu d'avantages, mais il a parfois ses moments."

# game/UpdateScriptChristmasEvent.rpy:493
translate french xmas_present_da75778f:

    # R "Dammit, I'm stuck again!"
    R "Bon sang, je suis à nouveau coincé!"

# game/UpdateScriptChristmasEvent.rpy:494
translate french xmas_present_6dc6eb97:

    # R "Stop eating my sister's ass!"
    R "Arrête de manger le cul de ma sœur!"

# game/UpdateScriptChristmasEvent.rpy:495
translate french xmas_present_1a4f473f:

    # GCP "Hahaha.... yeah the Ghrost urf Chrismras prast warned mre that thris wourd priss ru orff."
    GCP "Hahaha.... oui, le Ghrost urf Chrismras prast a averti mre que thris wourd priss ru orff."

# game/UpdateScriptChristmasEvent.rpy:496
translate french xmas_present_1a76db25:

    # GCP "Sro I trook the proper precrautions."
    GCP "Sro et trook les précrautions appropriées."

# game/UpdateScriptChristmasEvent.rpy:497
translate french xmas_present_04d954e4:

    # R "What is it with ghosts and eating ass?"
    R "Qu'est-ce que c'est avec des fantômes et manger du cul?"

# game/UpdateScriptChristmasEvent.rpy:498
translate french xmas_present_82ea9983:

    # S "(sleepily muttering).... ohhhhh.... [ryan].... since when do you like eating ass?.... That feels soooo good!"
    S "(murmure endormi).... ohhhhhh.... [ryan].... depuis quand tu aimes manger du cul?.... Ça fait tellement du bien!"

# game/UpdateScriptChristmasEvent.rpy:499
translate french xmas_present_cddbe70d:

    # R "And why is it ok for you to be sexually depraved, but you want to stop me from doing just that."
    R "Et pourquoi est-ce correct pour toi d'être dépravée sexuellement, mais tu veux m'empêcher de faire exactement cela."

# game/UpdateScriptChristmasEvent.rpy:502
translate french xmas_present_69b1c2ea:

    # GCP "Well, the difference is she's not my sister."
    GCP "Eh bien, la différence, c'est qu'elle n'est pas ma sœur."

# game/UpdateScriptChristmasEvent.rpy:503
translate french xmas_present_011eb99a:

    # R "Well, I guess you've got me there."
    R "Eh bien, je suppose que tu m'as là."

# game/UpdateScriptChristmasEvent.rpy:504
translate french xmas_present_b51236a3:

    # GCP "Great! So, are you ready for your next adventure?"
    GCP "Génial! Alors, es-tu prêt pour ta prochaine aventure?"

# game/UpdateScriptChristmasEvent.rpy:505
translate french xmas_present_cc060025:

    # R "I'd rather keep sleeping, to be honest."
    R "Je préfère continuer à dormir, pour être honnête."

# game/UpdateScriptChristmasEvent.rpy:506
translate french xmas_present_63bdf9a4:

    # GCP "Come on, party pooper!"
    GCP "Allez, fais la fête!"

# game/UpdateScriptChristmasEvent.rpy:507
translate french xmas_present_432dafb9:

    # GCP "We're going to the Christmas Present."
    GCP "Nous allons au Cadeau de Noël."

# game/UpdateScriptChristmasEvent.rpy:508
translate french xmas_present_eff10d5f:

    # R "So, the Christmas that's about to happen this year?"
    R "Alors, le Noël qui va arriver cette année ?"

# game/UpdateScriptChristmasEvent.rpy:509
translate french xmas_present_28e0e5b3:

    # GCP "That's right."
    GCP "C'est vrai."

# game/UpdateScriptChristmasEvent.rpy:512
translate french xmas_present_598b69fa:

    # GCP "So, get ready!"
    GCP "Alors, prépares-toi!"

# game/UpdateScriptChristmasEvent.rpy:513
translate french xmas_present_d575a006:

    # GCP "And try not to puke!"
    GCP "Et essaies de ne pas vomir!"

# game/UpdateScriptChristmasEvent.rpy:524
translate french xmas_present_b20ecff0:

    # R "Oh, I hate this part!"
    R "Oh, je déteste cette partie!"

# game/UpdateScriptChristmasEvent.rpy:525
translate french xmas_present_f0bc3e3e:

    # GCP "Yeah, you never really get used to it either."
    GCP "Oui, on ne s'y habitue jamais vraiment non plus."

# game/UpdateScriptChristmasEvent.rpy:526
translate french xmas_present_d0d2a0e1:

    # GCP "Just relax, we'll be there in just a couple more seconds."
    GCP "Détends-toi, nous serons là dans quelques secondes."

# game/UpdateScriptChristmasEvent.rpy:532
translate french xmas_future_2eaa5bac:

    # Shlurp.... shluurp.... smack"
    "Shlurp.... shluurp.... smack"

# game/UpdateScriptChristmasEvent.rpy:533
translate french xmas_future_d71f5f49:

    # RT "{i}What is that noise?{/i}"
    RT "{i}Quel est ce bruit?{/i}"

# game/UpdateScriptChristmasEvent.rpy:538
translate french xmas_future_aa2b5fee:

    # Shlurp, shlirrk, shmork"
    "Shlurp, shlirrk, shmork"

# game/UpdateScriptChristmasEvent.rpy:539
translate french xmas_future_378eed57:

    # S "(sleepily) Again!.... Oh [ryan]!.... That feels so good!"
    S "(encore endormi)!.... Oh [ryan]!.... Ça fait tellement de bien !"

# game/UpdateScriptChristmasEvent.rpy:546
translate french xmas_future_9b8fdbd9:

    # Shlirk, shmlack"
    "Shlirk, shmlack"

# game/UpdateScriptChristmasEvent.rpy:547
translate french xmas_future_8cc7d32b:

    # R "What the hell is going on?"
    R "Que se passe-t-il ?"

# game/UpdateScriptChristmasEvent.rpy:550
translate french xmas_future_bc53ebd9:

    # X "Oh.... hey [ryan]!"
    X "Oh.... hé [ryan]!"

# game/UpdateScriptChristmasEvent.rpy:551
translate french xmas_future_7477d7b3:

    # X "Sorry I woke you!"
    X "Désolé de t'avoir réveillé!"

# game/UpdateScriptChristmasEvent.rpy:552
translate french xmas_future_9795a042:

    # X "My sisters both told me I had to try out the all you can eat breakfast buffet."
    X "Mes sœurs m'ont toutes deux dit que je devais essayer le buffet du petit-déjeuner à volonté."

# game/UpdateScriptChristmasEvent.rpy:553
translate french xmas_future_2409e36d:

    # X "They said you got a little hot under the collar when they did this."
    X "Elles ont dit que tu avais un peu chaud sous le col quand elles ont fait ça."

# game/UpdateScriptChristmasEvent.rpy:554
translate french xmas_future_344c907f:

    # X "So, I thought I'd just get it out of the way before you woke up."
    X "Donc, j'ai pensé que je l'enlèverais juste avant que tu te réveilles."

# game/UpdateScriptChristmasEvent.rpy:555
translate french xmas_future_e6436c83:

    # X "But I think I've had my fill. Should we get down to business?"
    X "Mais je pense que j'ai fait le plein. Devrions-nous nous mettre au travail?"

# game/UpdateScriptChristmasEvent.rpy:561
translate french xmas_future_c931e5f6:

    # X "Do I need to introduce myself, or do you already know who I am?"
    X "Dois-je me présenter, ou sais-tu déjà qui je suis?"

# game/UpdateScriptChristmasEvent.rpy:562
translate french xmas_future_9758310c:

    # R "The ghost of Christmas Future, I presume?"
    R "Le fantôme de l'avenir de Noël, je présume?"

# game/UpdateScriptChristmasEvent.rpy:563
translate french xmas_future_3f1cd82f:

    # GF "You are correct!"
    GF "Tu as raison!"

# game/UpdateScriptChristmasEvent.rpy:564
translate french xmas_future_d602a554:

    # GF "And do you know why I'm here?"
    GF "Et sais-tu pourquoi je suis ici?"

# game/UpdateScriptChristmasEvent.rpy:565
translate french xmas_future_1b90be74:

    # R "To stop me from fucking my family?"
    R "Pour m'empêcher de baiser ma famille?"

# game/UpdateScriptChristmasEvent.rpy:566
translate french xmas_future_cb9ec460:

    # GF "Haha.... that's pretty blunt and to the point."
    GF "Haha.... c'est assez direct et précis."

# game/UpdateScriptChristmasEvent.rpy:567
translate french xmas_future_91c59f05:

    # GF "But that's actually where you're wrong."
    GF "Mais c'est en fait là que tu as tort."

# game/UpdateScriptChristmasEvent.rpy:568
translate french xmas_future_5593d8de:

    # GF "I'm not here to stop you from doing anything."
    GF "Je ne suis pas là pour t'empêcher de faire quoi que ce soit."

# game/UpdateScriptChristmasEvent.rpy:569
translate french xmas_future_ac0f7c0b:

    # GF "In fact, I don't give a fuck about what you choose to do with your life."
    GF "En fait, je m'en fous de ce que tu choisis de faire de ta vie."

# game/UpdateScriptChristmasEvent.rpy:570
translate french xmas_future_fa289250:

    # GF "I'm just here as a guide, to take you a couple of years into the Christmas future, to show you the consequences of your actions."
    GF "Je suis juste ici comme guide, pour t'emmener quelques années dans le futur de Noël, pour te montrer les conséquences de tes actions."

# game/UpdateScriptChristmasEvent.rpy:571
translate french xmas_future_190ae3f3:

    # GF "If you like the results, great! If not, you can decide to make different choices."
    GF "Si tu aimes les résultats, super! Sinon, tu peux décider de faire des choix différents."

# game/UpdateScriptChristmasEvent.rpy:572
translate french xmas_future_b9cd7c7f:

    # GF "Whatever tickles your pickle."
    GF "Tout ce qui chatouille ton cornichon."

# game/UpdateScriptChristmasEvent.rpy:573
translate french xmas_future_f1bd82db:

    # R "Well, I've got to say it's nice to meet a ghost that's not so judgy."
    R "Eh bien, je dois dire que c'est agréable de rencontrer un fantôme qui n'est pas si sérieux."

# game/UpdateScriptChristmasEvent.rpy:574
translate french xmas_future_e09f122f:

    # GF "Yeah, my sisters can be real condescending bitches."
    GF "Oui, mes sœurs peuvent être de vraies salopes condescendantes."

# game/UpdateScriptChristmasEvent.rpy:575
translate french xmas_future_27c970d7:

    # GF "Haha.... but you brought them down a peg."
    GF "Haha.... mais tu les as fait tomber une cheville."

# game/UpdateScriptChristmasEvent.rpy:576
translate french xmas_future_8a834488:

    # GF "God's not going to be super happy with them after tonight's over."
    GF "Dieu ne sera pas très heureux avec elles après la fin de ce soir."

# game/UpdateScriptChristmasEvent.rpy:577
translate french xmas_future_0270a7ad:

    # GF "I wouldn't be surprised if she sends them to the sex dungeon for a decade."
    GF "Je ne serais pas surpris si elle les envoyait au cachot sexuel pendant une décennie."

# game/UpdateScriptChristmasEvent.rpy:578
translate french xmas_future_398ece5f:

    # R "Oh, no!.... That sounds awful! Now I feel really bad that I didn't try to follow their game plan better."
    R "Oh, non!.... Ça a l'air horrible ! Maintenant, je me sens vraiment mal de ne pas avoir essayé de mieux suivre leur plan de jeu."

# game/UpdateScriptChristmasEvent.rpy:579
translate french xmas_future_8203c425:

    # GF "Ahhh.... don't worry about it, everyone secretly likes the sex dungeon anyways."
    GF "Ahhh.... ne t'inquiètes pas, tout le monde aime secrètement le donjon sexuel de toute façon."

# game/UpdateScriptChristmasEvent.rpy:580
translate french xmas_future_5d5f5092:

    # GF "I've been there so many times, they've made me an honorary torturer."
    GF "J'y suis allé tant de fois, ils ont fait de moi un tortionnaire honoraire."

# game/UpdateScriptChristmasEvent.rpy:581
translate french xmas_future_3c0bfcfd:

    # R "Is that why you don't shine as bright as your sisters?"
    R "Est-ce pour cela que tu ne brilles pas autant que tes sœurs?"

# game/UpdateScriptChristmasEvent.rpy:582
translate french xmas_future_0fb39396:

    # GF "Naaahhh.... It's just because I'm supposed to be the more scary ghost."
    GF "Naaahhh.... C'est juste parce que je suis censé être le fantôme le plus effrayant."

# game/UpdateScriptChristmasEvent.rpy:583
translate french xmas_future_63db340d:

    # GF "You see, most people fear their future, so they gave me this whole facade to make me more scary."
    GF "Tu vois, la plupart des gens craignent leur avenir, alors ils m'ont donné toute cette façade pour me faire plus peur."

# game/UpdateScriptChristmasEvent.rpy:584
translate french xmas_future_cb064996:

    # GF "Do I seem scarier to you?"
    GF "Est-ce que je te semble plus effrayante?"

# game/UpdateScriptChristmasEvent.rpy:585
translate french xmas_future_bcfcfdcb:

    # R "Nah.... making you look like my hot sister Sidney, kind of takes away the whole fear factor."
    R "Non.... te faire ressembler à ma sœur Sidney, ça enlève tout le facteur de peur."

# game/UpdateScriptChristmasEvent.rpy:586
translate french xmas_future_51167278:

    # GF "See! That's what I've been saying! Let me wear a holocaust robe, and give me a skull for a head, you know, like they did in the old days."
    GF "Voit! C'est ce que j'ai dit ! Laisses-moi porter une robe de l'holocauste, et donnes-moi un crâne pour une tête, tu saais, comme ils le faisaient autrefois."

# game/UpdateScriptChristmasEvent.rpy:587
translate french xmas_future_34bd417c:

    # GF "Too much PC shit in Heaven. We're not allowed to scare anyone too badly anymore."
    GF "Trop de merde de PC au Paradis. On n'a plus le droit de faire trop peur à personne."

# game/UpdateScriptChristmasEvent.rpy:588
translate french xmas_future_41d69bb4:

    # GF "Ahhh.... but we shouldn't be discussing heavenly politics. I've got a job to do, and not a lot of time to get it done."
    GF "Ahhh.... mais nous ne devrions pas discuter de politique céleste. J'ai un travail à faire, et pas beaucoup de temps pour le faire."

# game/UpdateScriptChristmasEvent.rpy:589
translate french xmas_future_eb7bbf9a:

    # GF "Are you ready to go to the future?"
    GF "Es-tu prêt à aller vers le futur?"

# game/UpdateScriptChristmasEvent.rpy:590
translate french xmas_future_718c1490:

    # R "I guess! Go ahead and blow that magic fairy dust on me"
    R "Je suppose! vas-y et souffles cette poussière de fée magique sur moi"

# game/UpdateScriptChristmasEvent.rpy:591
translate french xmas_future_0117dc30:

    # GF "Yeah.... I'm not into that flowery bull-shit. I do things a little differently."
    GF "Ouais.... Je ne suis pas dans cette merde fleurie. Je fais les choses un peu différemment."

# game/UpdateScriptChristmasEvent.rpy:594
translate french xmas_future_70cf7f50:

    # R "What are you doing?"
    R "Que fais-tu?"

# game/UpdateScriptChristmasEvent.rpy:595
translate french xmas_future_84adaa77:

    # GF "Just give me a second."
    GF "Donne-moi juste une seconde."

# game/UpdateScriptChristmasEvent.rpy:598
translate french xmas_future_20c89c81:

    # GF "Huhhnngghhhh..."
    GF "Huhhnngghhhh..."

# game/UpdateScriptChristmasEvent.rpy:599
translate french xmas_future_6cf37ce8:

    # R "Oh, no!"
    R "Oh, non!"

# game/UpdateScriptChristmasEvent.rpy:603
translate french xmas_future_b7bdf01a:

    # R "Ohhhh..."
    R "Ohhhh..."

# game/UpdateScriptChristmasEvent.rpy:604
translate french xmas_future_9e8fea21:

    # R "Oh, my God!..."
    R "Oh mon Dieu!..."

# game/UpdateScriptChristmasEvent.rpy:605
translate french xmas_future_ca2d6884:

    # R "What in Futa God's name?..."
    R "Qu'en est-il du nom de Futa Dieu?..."

# game/UpdateScriptChristmasEvent.rpy:606
translate french xmas_future_8e95cc4a:

    # R "Huh.... weird.... It.... smells like.... candy canes."
    R "Hein.... étrange.... Il.... ça sent bon.... cannes de bonbons."

# game/UpdateScriptChristmasEvent.rpy:612
translate french xmas_future_1b3e2c8f:

    # RT "{i}What the.... I'm still in bed.... I don't think I've even moved.... these dreams are getting weird.{/i}"
    RT "{i}C'est quoi ce.... Je suis toujours au lit.... Je ne pense même pas avoir déménagé.... ces rêves deviennent bizarres.{/i}"

# game/UpdateScriptChristmasEvent.rpy:613
translate french xmas_future_8a905434:

    # M "Wake up [ryan]!.... It's Christmas!"
    M "Réveilles-toi [ryan]!.... C'est Noël !"

# game/UpdateScriptChristmasEvent.rpy:616
translate french xmas_future_b4a9dbe2:

    # M "Good morning [ryan]!"
    M "Bonjour [ryan]!"

# game/UpdateScriptChristmasEvent.rpy:617
translate french xmas_future_86b04e69:

    # R "....Mmm.... Mom?"
    R "Ok maman, je viens tout de suite."

# game/UpdateScriptChristmasEvent.rpy:618
translate french xmas_future_dee82788:

    # M "That's right, honey.... merry Christmas!.... I wanted to be the first to give you a present."
    M "C'est vrai, chérie.... Joyeux Noël!.... Je voulais être le premier à te faire un cadeau."

# game/UpdateScriptChristmasEvent.rpy:620
translate french xmas_future_6852ae54:

    # M "I hope you enjoy it!"
    M "J'espère que cela te plaira!"

# game/UpdateScriptChristmasEvent.rpy:623
translate french xmas_future_5fcfc07c:

    # R "Ohhh.... Mom!"
    R "Ohhh.... Maman!"

# game/UpdateScriptChristmasEvent.rpy:624
translate french xmas_future_dd65c4b4:

    # RT "{i}Ok.... Mom's giving me a morning handy!.... I'm definitely in the future.{/i}"
    RT "{i}Ok.... Maman me rend le matin agréable!.... Je suis définitivement dans le futur.{/i}"

# game/UpdateScriptChristmasEvent.rpy:625
translate french xmas_future_4fbfbc43:

    # RT "{i}What the hell? If I get to wake up to this in the morning, why would I want to change anything?{/i}"
    RT "{i}Qu'est-ce que l'enfer? Si je me réveille le matin, pourquoi voudrais-je changer quelque chose?{/i}"

# game/UpdateScriptChristmasEvent.rpy:626
translate french xmas_future_c7938b4c:

    # RT "{i}Ohhh.... that feels so good!{/i}"
    RT "{i}Ohhh.... ça fait tellement de bien !{/i}"

# game/UpdateScriptChristmasEvent.rpy:627
translate french xmas_future_720f8f02:

    # RT "{i}And the fact that Mom is doing it is a dream come true. I wonder what else we do together in the future.{/i}"
    RT "{i}Et le fait que maman le fasse est un rêve devenu réalité. Je me demande ce que nous ferons d'autre ensemble à l'avenir.{/i}"

# game/UpdateScriptChristmasEvent.rpy:628
translate french xmas_future_c5f8cf08:

    # M "Oh, that's such a handful!"
    M "Oh, c'est une telle poignée!"

# game/UpdateScriptChristmasEvent.rpy:629
translate french xmas_future_ca97c1c1:

    # M "You're my big boy!"
    M "Tu es mon grand garçon!"

# game/UpdateScriptChristmasEvent.rpy:630
translate french xmas_future_be5faabb:

    # RT "{i}Ohhh.... she knows how to talk sexy too.{/i}"
    RT "{i}Ohhh.... elle sait aussi parler sale.{/i}"

# game/UpdateScriptChristmasEvent.rpy:631
translate french xmas_future_d9a992fd:

    # RT "{i}I shouldn't be surprised, I guess. {/i}"
    RT "{i}Je ne devrais pas être surpris, je suppose. {/i}"

# game/UpdateScriptChristmasEvent.rpy:632
translate french xmas_future_55041951:

    # M "Are you going to blow your huge load all over my face?"
    M "Vas-tu souffler ton énorme charge sur mon visage?"

# game/UpdateScriptChristmasEvent.rpy:633
translate french xmas_future_886f4e46:

    # R "Yes!.... I want to sooo bad!"
    R "Oui!.... Je veux tellement mal!"

# game/UpdateScriptChristmasEvent.rpy:634
translate french xmas_future_68637f27:

    # M "Your cock is just pulsing in my hand! Are you close?"
    M "Ta bite ne fait que battre dans ma main! Tu es proche ?"

# game/UpdateScriptChristmasEvent.rpy:635
translate french xmas_future_e30c4ea4:

    # R "Yes, Mom!.... I'm so close!"
    R "Oui, maman!.... Je suis si proche !"

# game/UpdateScriptChristmasEvent.rpy:638
translate french xmas_future_c6841c25:

    # R "..."
    R "..."

# game/UpdateScriptChristmasEvent.rpy:639
translate french xmas_future_52b9d91e:

    # R "Why did you stop?"
    R "Pourquoi tu t'es arrêtée?"

# game/UpdateScriptChristmasEvent.rpy:640
translate french xmas_future_566163f1:

    # M "Because it's barely morning! It's a big day today. You're going to need your stamina."
    M "Parce que c'est à peine le matin ! C'est un grand jour aujourd'hui. Tu vas avoir besoin de ton endurance."

# game/UpdateScriptChristmasEvent.rpy:641
translate french xmas_future_eeedc834:

    # R "But.... but that's just cruel!"
    R "Mais.... mais c'est juste cruel!"

# game/UpdateScriptChristmasEvent.rpy:642
translate french xmas_future_0b9c8fb4:

    # M "Haha.... no, It will put you in the right mood for the rest of the day! I'll see you in just a bit. I've got to run use the bathroom."
    M "Haha.... non, ça va te mettre dans la bonne humeur pour le reste de la journée! Je vais te voir un peu. Je dois utiliser la salle de bain."

# game/UpdateScriptChristmasEvent.rpy:646
translate french xmas_future_1c538c65:

    # RT "{i}What in the!....{/i}"
    RT "{i}Quoi dans le!....{/i}"

# game/UpdateScriptChristmasEvent.rpy:647
translate french xmas_future_8f597427:

    # RT "{i}Holy shit!....{/i}"
    RT "{i}Merde!.... {/i}"

# game/UpdateScriptChristmasEvent.rpy:648
translate french xmas_future_9c45fea9:

    # RT "{i}Is Mom?....{/i}"
    RT "{i}C'est maman ?....{/i}"

# game/UpdateScriptChristmasEvent.rpy:652
translate french xmas_future_fc67f521:

    # GF "Holy shit, dude! Is your mom pregnant?"
    GF "Merde, mec! Ta mère est enceinte?"

# game/UpdateScriptChristmasEvent.rpy:653
translate french xmas_future_9853d4e7:

    # RT "{i}Uhhh.... It kind of looks that way!{/i}"
    RT "{i}Uhhh .... Ça ressemble à ça ! {/i}"

# game/UpdateScriptChristmasEvent.rpy:654
translate french xmas_future_02153a21:

    # RT "{i}Unless she ate a really big breakfast burrito.{/i}"
    RT "{i}À moins qu'elle ait mangé un très gros burrito au petit déjeuner.{/i}"

# game/UpdateScriptChristmasEvent.rpy:655
translate french xmas_future_8e3e9946:

    # GF "Holy shit! I wonder if it's yours!"
    GF "Putain de merde! Je me demande si c'est à toi !"

# game/UpdateScriptChristmasEvent.rpy:656
translate french xmas_future_76daf13f:

    # RT "{i}Could I have knocked up my own mother?{/i}"
    RT "{i}Aurais-je pu assommer ma propre mère?{/i}"

# game/UpdateScriptChristmasEvent.rpy:657
translate french xmas_future_ca17a601:

    # GF "That would be fucking legendary!"
    GF "Ce serait vraiment légendaire !"

# game/UpdateScriptChristmasEvent.rpy:660
translate french xmas_future_4464a1ed:

    # R "Uhhh.... Mom!..."
    R "Uhhh.... Maman!..."

# game/UpdateScriptChristmasEvent.rpy:661
translate french xmas_future_85e2424e:

    # M "Hnnnhhh...Yeah honey?"
    M "Hnnnhhh...Ouais chérie ?"

# game/UpdateScriptChristmasEvent.rpy:662
translate french xmas_future_8e6a196b:

    # R "I need to talk to you a little before you leave!"
    R "J'ai besoin de te parler un peu avant que tu partes !"

# game/UpdateScriptChristmasEvent.rpy:665
translate french xmas_future_264e383c:

    # M "Alright, well make it fast."
    M "D'accord, fais vite."

# game/UpdateScriptChristmasEvent.rpy:666
translate french xmas_future_83ea7f7f:

    # M "You know how a pregnant woman's bladder is."
    M "Tu sais comment est la vessie d'une femme enceinte."

# game/UpdateScriptChristmasEvent.rpy:667
translate french xmas_future_8a166c84:

    # M "And unless you're into water sports..."
    M "Et sauf si tu aimes les sports nautiques..."

# game/UpdateScriptChristmasEvent.rpy:668
translate french xmas_future_56155da4:

    # R "Mom!..."
    R "Maman!..."

# game/UpdateScriptChristmasEvent.rpy:669
translate french xmas_future_0ac3fbf8:

    # M "Haha.... what is it you want?"
    M "Haha.... qu'est-ce que tu veux?"

# game/UpdateScriptChristmasEvent.rpy:670
translate french xmas_future_bfaae458:

    # R "Uhhh.... I hope this isn't too indelicate to ask?"
    R "Uhhh.... J'espère que ce n'est pas trop indélicat pour demander?"

# game/UpdateScriptChristmasEvent.rpy:671
translate french xmas_future_0c4b2c27:

    # R "But.... how far along are you?"
    R "Mais.... jusqu'où en es-tu ?"

# game/UpdateScriptChristmasEvent.rpy:672
translate french xmas_future_0990ad27:

    # M "Hmmm.... let me think..."
    M "Hmmm.... laisse-moi réfléchir..."

# game/UpdateScriptChristmasEvent.rpy:673
translate french xmas_future_68b99fd8:

    # M "I should be about 7 months along now."
    M "Je devrais être à environ 7 mois maintenant."

# game/UpdateScriptChristmasEvent.rpy:674
translate french xmas_future_b88f373e:

    # R "Wow!.... I mean you still look great!"
    R "Wow!.... Je veux dire que tu as toujours fière allure!"

# game/UpdateScriptChristmasEvent.rpy:675
translate french xmas_future_3604ddab:

    # M "Haha.... thanks hon!"
    M "Haha.... merci chéri!"

# game/UpdateScriptChristmasEvent.rpy:676
translate french xmas_future_7db48149:

    # R "Uhhhh.... and do we think it's mine?"
    R "Uhhhh.... et pense-t-on que c'est à moi ?"

# game/UpdateScriptChristmasEvent.rpy:679
translate french xmas_future_1204ec42:

    # M "Are you serious?"
    M "Es-tu sérieux?"

# game/UpdateScriptChristmasEvent.rpy:680
translate french xmas_future_d4047d77:

    # R "Uhhh.... yeah.... I mean..."
    R "Uhhh.... ouais.... Je veux dire..."

# game/UpdateScriptChristmasEvent.rpy:681
translate french xmas_future_4c9eb629:

    # M "You must still have foggy morning brain, or pent up sexual energy brain, or something."
    M "Tu dois toujours avoir un cerveau matinal brumeux, ou un cerveau d'énergie sexuelle refoulé, ou quelque chose comme ça."

# game/UpdateScriptChristmasEvent.rpy:682
translate french xmas_future_7dbd44c3:

    # R "Well?..."
    R "Eh bien?..."

# game/UpdateScriptChristmasEvent.rpy:684
translate french xmas_future_66b3e72c:

    # M "Of course it's not yours!"
    M "Bien sûr, ce n'est pas à toi !"

# game/UpdateScriptChristmasEvent.rpy:685
translate french xmas_future_b3cb0100:

    # M "How do you think you could get me pregnant, when I only let you have sex with my butt?"
    M "Comment penses-tu que tu pourrais me mettre enceinte, alors que je ne te laisse coucher qu'avec mes fesses?"

# game/UpdateScriptChristmasEvent.rpy:686
translate french xmas_future_fafcf564:

    # R "Well.... yeah.... I suppose that's a pretty effective form of birth control."
    R "Bien.... ouais.... Je suppose que c'est une forme de contrôle des naissances assez efficace."

# game/UpdateScriptChristmasEvent.rpy:687
translate french xmas_future_a7af5384:

    # M "You know my pussy is reserved for [dad_name]!"
    M "Tu sais que ma chatte est réservée pour [dad_name]!"

# game/UpdateScriptChristmasEvent.rpy:688
translate french xmas_future_cba198e4:

    # R "So Dad got you pregnant?"
    R "Alors Papa t'a mise enceinte?"

# game/UpdateScriptChristmasEvent.rpy:689
translate french xmas_future_195c86a5:

    # R "Did he get out of prison?"
    R "Est-il sorti de prison ?"

# game/UpdateScriptChristmasEvent.rpy:690
translate french xmas_future_f7e0da9c:

    # M "[ryan]? Are you sure you're ok? I wonder if you have a fever or something."
    M "[ryan]? Tu es sûr que ça va ? Je me demande si tu as de la fièvre ou quelque chose comme ça."

# game/UpdateScriptChristmasEvent.rpy:691
translate french xmas_future_4867c81f:

    # R "What? No.... I mean yes.... I'm ok, but is Dad out of prison?"
    R "Quoi? Non.... Je veux dire oui.... Je vais bien, mais papa est sorti de prison?"

# game/UpdateScriptChristmasEvent.rpy:694
translate french xmas_future_c923657a:

    # M "Of course not honey! I know you miss him.... but I'm afraid he's going to be gone a very long time."
    M "Bien sûr, que non chéri! Je sais qu'il te manque.... mais je crains qu'il ne soit parti très longtemps."

# game/UpdateScriptChristmasEvent.rpy:695
translate french xmas_future_351d154a:

    # RT "{i}Oh Thank God?{/i}"
    RT "{i}Oh Dieu Merci?{/i}"

# game/UpdateScriptChristmasEvent.rpy:696
translate french xmas_future_60b6e8f7:

    # R "So, how did you get pregnant?"
    R "Alors, comment es-tu tombée enceinte?"

# game/UpdateScriptChristmasEvent.rpy:697
translate french xmas_future_b41c9d22:

    # M "I know you've heard of conjugal visits."
    M "Je sais que tu as entendu parler des visites conjugales."

# game/UpdateScriptChristmasEvent.rpy:698
translate french xmas_future_f2bc701c:

    # R "And you're ok raising him as a single mom?"
    R "Et tu es d'accord pour l'élever comme une mère célibataire?"

# game/UpdateScriptChristmasEvent.rpy:699
translate french xmas_future_39f6d57d:

    # M "No silly. You said you'd help! You'll be like his second daddy!"
    M "Pas idiot. Tu as dit que tu aiderais! Tu seras comme son deuxième papa !"

# game/UpdateScriptChristmasEvent.rpy:700
translate french xmas_future_5bd81822:

    # RT "{i}You've got to be fucking kidding me!{/i}"
    RT "{i}Tu te moques de moi !{/i}"

# game/UpdateScriptChristmasEvent.rpy:701
translate french xmas_future_e44df2e7:

    # GF "Oh man, and you're going to have to support his kid financially! You totally got cucked!"
    GF "Oh mec, et tu vas devoir soutenir financièrement son enfant! Tu t'es fait avoir !"

# game/UpdateScriptChristmasEvent.rpy:702
translate french xmas_future_3e57b230:

    # RT "{i}Oh shit! I can't believe this is happening!{/i}"
    RT "{i}Oh merde! Je n'arrive pas à croire que ça arrive!{/i}"

# game/UpdateScriptChristmasEvent.rpy:703
translate french xmas_future_99a3b605:

    # GF "Hey, don't sweat it too bad! This hasn't happened yet. Now you can take steps to prevent this."
    GF "Hé, ne transpire pas trop! Ce n'est pas encore arrivé. Tu peux maintenant prendre des mesures pour éviter cela."

# game/UpdateScriptChristmasEvent.rpy:704
translate french xmas_future_4078e072:

    # GF "You know, like rolling back scenes in one of those crappy incest H-games you like, so you can make the better decision?"
    GF "Tu sais, comme revenir sur des scènes dans l'un de ces jeux H d'inceste merdiques que tu aimes, pour que tu puisses prendre la meilleure décision?"

# game/UpdateScriptChristmasEvent.rpy:705
translate french xmas_future_469785ad:

    # GF "You can fix this!"
    GF "Tu peux réparer cela !"

# game/UpdateScriptChristmasEvent.rpy:706
translate french xmas_future_f7d1b00c:

    # RT "{i}Yeah, you're right! Oh Thank Futa God!{/i}"
    RT "{i}Ouais, tu as raison! Oh merci mon Dieu!{/i}"

# game/UpdateScriptChristmasEvent.rpy:707
translate french xmas_future_780bae4c:

    # M "Don't worry! You'll be the best surrogate dad!"
    M "Ne t'inquiètes pas! Tu seras le meilleur père de substitution!"

# game/UpdateScriptChristmasEvent.rpy:709
translate french xmas_future_bbadc54d:

    # M "Well, of course it's yours!"
    M "Eh bien, bien sûr, c'est à toi!"

# game/UpdateScriptChristmasEvent.rpy:710
translate french xmas_future_b727c63f:

    # M "Who else's would it be?"
    M "A qui d'autre serait-ce?"

# game/UpdateScriptChristmasEvent.rpy:711
translate french xmas_future_55735b7b:

    # M "Your father is in prison."
    M "Ton père est en prison."

# game/UpdateScriptChristmasEvent.rpy:712
translate french xmas_future_39df12b1:

    # M "And between posing for pictures for you, and working at the school, I have no time to meet any other men."
    M "Et entre poser pour des photos pour toi et travailler à l'école, je n'ai pas le temps de rencontrer d'autres hommes."

# game/UpdateScriptChristmasEvent.rpy:715
translate french xmas_future_d454c2c7:

    # M "And I'm kind of freaking out here because I'm carrying your baby."
    M "Et j'ai un peu peur ici parce que je porte ton bébé."

# game/UpdateScriptChristmasEvent.rpy:716
translate french xmas_future_b5b5dd11:

    # M "And your father is going to murder me when he gets out, and meet's his grandbaby slash step-son!"
    M "Et ton père va me tuer quand il sortira, et rencontrer son petit-fils!"

# game/UpdateScriptChristmasEvent.rpy:717
translate french xmas_future_cf228ddc:

    # M "Meanwhile, I have to raise and care for another baby! I thought I was done with that stage of my life!"
    M "Pendant ce temps, je dois élever et prendre soin d'un autre bébé! Je pensais en avoir fini avec cette étape de ma vie!"

# game/UpdateScriptChristmasEvent.rpy:718
translate french xmas_future_95a1d292:

    # R "Hey.... hey.... relax!.... It's ok!..."
    R "Hé .... Hé .... détends-toi!... C'est ok !..."

# game/UpdateScriptChristmasEvent.rpy:719
translate french xmas_future_931364c9:

    # R "I'm going to be there to help take care of the baby!"
    R "Je vais être là pour aider à prendre soin du bébé!"

# game/UpdateScriptChristmasEvent.rpy:720
translate french xmas_future_d6c27e1a:

    # R "I'll take care of you financially."
    R "Je m'occuperai de toi financièrement."

# game/UpdateScriptChristmasEvent.rpy:723
translate french xmas_future_ce262a77:

    # R "I have it on good authority that Dad's not getting out of prison any time before we can cut all ties to him."
    R "J'ai la bonne autorité que Papa ne sorte pas de prison avant que nous puissions couper tous les liens avec lui."

# game/UpdateScriptChristmasEvent.rpy:724
translate french xmas_future_a3a09770:

    # R "It's all going to be alright!"
    R "Tout va bien se passer !"

# game/UpdateScriptChristmasEvent.rpy:725
translate french xmas_future_f962aa48:

    # RT "{i}Holy shit!.... I'm going to be a daddy!{/i}"
    RT "{i}Merde!.... Je vais être papa!{/i}"

# game/UpdateScriptChristmasEvent.rpy:726
translate french xmas_future_20596f70:

    # GF "Sure! If that's the way you want it.... that's the beauty of this gift you're receiving from us."
    GF "Bien sûr! Si c'est comme ça que tu le veux.... c'est la beauté de ce cadeau que tu reçois de notre part."

# game/UpdateScriptChristmasEvent.rpy:727
translate french xmas_future_66657724:

    # GF "It's like having the ability to roll back life. You know, like one of those crappy incest H-games you like, where you can roll back to make the better choice if you fuck up."
    GF "C'est comme avoir la capacité de faire reculer la vie. Tu sais, comme un de ces jeux d'inceste merdiques que tu aimes, où tu peux revenir en arrière pour faire le meilleur choix si tu baises."

# game/UpdateScriptChristmasEvent.rpy:728
translate french xmas_future_4186683f:

    # GF "If you want to be a dad, good job. If you decide you don't want to be. You can decide to wear a condom, or only have anal sex."
    GF "Si tu veux être papa, bon travail. Si tu décides que tu ne veux pas l'être. Tu peux décider de porter un préservatif ou de n'avoir que des relations sexuelles anales."

# game/UpdateScriptChristmasEvent.rpy:729
translate french xmas_future_e8a946e5:

    # RT "{i}Thank you! That is a pretty sweet gift.{/i}"
    RT "{i}Merci! C'est un joli cadeau.{/i}"

# game/UpdateScriptChristmasEvent.rpy:730
translate french xmas_future_b696d066:

    # M "Thanks for the pep talk honey!"
    M "Merci pour le miel de poivre !"

# game/UpdateScriptChristmasEvent.rpy:733
translate french xmas_future_91d26595:

    # M "If that's it, I'm going to run to the bathroom before I piss myself."
    M "Si c'est ça, je vais courir aux toilettes avant de pisser."

# game/UpdateScriptChristmasEvent.rpy:734
translate french xmas_future_5ef77632:

    # M "You should get out of your room, and go see Lauren and Sidney. I think Lauren is baking cookies in the kitchen, and I think Sidney is in the lounge making sure everything is ready for our Christmas gift exchange."
    M "Tu devrais sortir de ta chambre et aller voir Lauren et Sidney. Je pense que Lauren prépare des biscuits dans la cuisine, et je pense que Sidney est dans le salon pour s'assurer que tout est prêt pour notre échange de cadeaux de Noël."

# game/UpdateScriptChristmasEvent.rpy:735
translate french xmas_future_5c5a541a:

    # M "After that, we'll all meet in the lounge for the gift exchange. I think you'll like the gift we're all going to give you."
    M "Après cela, nous nous retrouverons tous dans le salon pour l'échange de cadeaux. Je pense que tu aimeras le cadeau que nous allons tous t'offrir."

# game/UpdateScriptChristmasEvent.rpy:749
translate french xmas_future_215b43ac:

    # GF "Hahahaha.... holy shit!"
    GF "Hahahaha.... putain de merde!"

# game/UpdateScriptChristmasEvent.rpy:751
translate french xmas_future_c5847833:

    # GF "In all the years I've been showing people their Christmas futures, I've never seen anyone cucked as hard as that!"
    GF "Pendant toutes les années où j'ai montré aux gens leur avenir de Noël, je n'ai jamais vu quelqu'un aussi dur que ça!"

# game/UpdateScriptChristmasEvent.rpy:752
translate french xmas_future_71402f87:

    # GF "Are you ok? Should I call an ambulance so the crew can come fuck your mom and then take you to the hospital?"
    GF "Ça va ? Dois-je appeler une ambulance pour que l'équipe puisse venir baiser ta mère et ensuite t'emmener à l'hôpital?"

# game/UpdateScriptChristmasEvent.rpy:753
translate french xmas_future_a3245df1:

    # RT "{i}Fuck off!{/i}"
    RT "{i}Va te faire foutre!{/i}"

# game/UpdateScriptChristmasEvent.rpy:754
translate french xmas_future_d51f080a:

    # GF "I'm sorry! I really do feel sorry for you, man!"
    GF "Je suis désolé! Je suis vraiment désolé pour toi, mec!"

# game/UpdateScriptChristmasEvent.rpy:755
translate french xmas_future_3718dcab:

    # GF "But like I said. This is not final. And even if it were, at least you're getting some side action from that hot piece of ass."
    GF "Mais comme je l'ai dit. Ce n'est pas définitif. Et même si c'était le cas, au moins tu obtiens une action secondaire de ce morceau de cul chaud."

# game/UpdateScriptChristmasEvent.rpy:757
translate french xmas_future_b3c91e0c:

    # GF "You are amazing!"
    GF "Tu es incroyable!"

# game/UpdateScriptChristmasEvent.rpy:758
translate french xmas_future_6f4136d0:

    # GF "In all the years I've been showing people their Christmas futures, I've never seen anyone be surprised with impregnating their mom!"
    GF "Pendant toutes les années où j'ai montré aux gens leur avenir de Noël, je n'ai jamais vu personne être surpris d'imprégner leur mère!"

# game/UpdateScriptChristmasEvent.rpy:759
translate french xmas_future_a567f2ae:

    # GF "Ohhh.... hoho.... the look on your face!"
    GF "Ohhh.... hoho.... le regard sur ton visage!"

# game/UpdateScriptChristmasEvent.rpy:760
translate french xmas_future_812410b5:

    # GF "I've got to say! That's pretty damn awesome! What better way to claim a woman for yourself, than to impregnate her."
    GF "Je dois dire! C'est sacrément génial! Quelle meilleure façon de réclamer une femme pour toi-même, que de l'imprégner."

# game/UpdateScriptChristmasEvent.rpy:761
translate french xmas_future_f65d566d:

    # GF "I don't know if I'd change a thing. Unless you hate kids or something."
    GF "Je ne sais pas si je changerais quelque chose. Sauf si tu détestes les enfants ou quelque chose comme ça."

# game/UpdateScriptChristmasEvent.rpy:762
translate french xmas_future_fe0f8165:

    # GF "So, are you ready to move on?"
    GF "Alors, es-tu prêt à passer à autre chose?"

# game/UpdateScriptChristmasEvent.rpy:763
translate french xmas_future_bc4a4085:

    # GF "I think your mom said something about cookies downstairs. Let's go see if they're done, and we can talk to Lauren while we're at it."
    GF "Je pense que ta mère a parlé des cookies en bas. Allons voir si ils sont fini, et on pourra parler à Lauren pendant qu'on y est."

# game/UpdateScriptChristmasEvent.rpy:771
translate french xmas_goodbye_09930380:

    # GF "Sorry that we weren't able to stay longer."
    GF "Désolé de ne pas pouvoir rester plus longtemps."

# game/UpdateScriptChristmasEvent.rpy:772
translate french xmas_goodbye_f933813d:

    # GF "Sometimes at these things you can't wait to get away, and sometimes you wish you could stay forever."
    GF "Parfois, à ces choses, tu as hâte de t'échapper, et parfois tu souhaites pouvoir rester pour toujours."

# game/UpdateScriptChristmasEvent.rpy:773
translate french xmas_goodbye_6f85596e:

    # GF "You definitely left on a high note though."
    GF "Tu es définitivement parti sur une note élevée cependant."

# game/UpdateScriptChristmasEvent.rpy:774
translate french xmas_goodbye_0fd8f28a:

    # GF "So, did you learn anything?"
    GF "Alors, as-tu appris quelque chose?"

# game/UpdateScriptChristmasEvent.rpy:775
translate french xmas_goodbye_6b7b3eee:

    # R "I learned that life is like a Ren'Py game.... It's full of choices, and I just need to make the ones that make me the happiest."
    R "J'ai appris que la vie est comme un jeu de Ren'Py.... C'est plein de choix, et j'ai juste besoin de faire ceux qui me rendent le plus heureux."

# game/UpdateScriptChristmasEvent.rpy:776
translate french xmas_goodbye_fbf455fa:

    # R "The best choice, usually leads to the outcome that makes the other NPC's happy too."
    R "Le meilleur choix conduit généralement au résultat qui rend aussi heureux les autres PNJ."

# game/UpdateScriptChristmasEvent.rpy:777
translate french xmas_goodbye_b6210002:

    # R "No one can tell me which choices are right for me, because each player can make whatever choices makes them the happiest."
    R "Personne ne peut me dire quels choix me conviennent, car chaque joueur peut faire tous les choix qui le rendent le plus heureux."

# game/UpdateScriptChristmasEvent.rpy:778
translate french xmas_goodbye_618ab44e:

    # R "And if they want to judge me, like some troll in some incest game forum."
    R "Et s'ils veulent me juger, comme un troll dans un forum de jeu d'inceste."

# game/UpdateScriptChristmasEvent.rpy:779
translate french xmas_goodbye_e2467697:

    # R "I just need to ignore them and keep living life the way I want."
    R "J'ai juste besoin de les ignorer et de continuer à vivre la vie comme je le veux."

# game/UpdateScriptChristmasEvent.rpy:780
translate french xmas_goodbye_5080414f:

    # GF "That's a great lesson, and anyone who tries to impose their version of morals on you can go fuck themselves."
    GF "C'est une grande leçon, et quiconque essaie de t'imposer sa version de la morale peut aller se faire foutre."

# game/UpdateScriptChristmasEvent.rpy:781
translate french xmas_goodbye_cd2a812a:

    # GF "Haha.... just like Futa God is going to be fucking my sisters as soon as Christmas is over."
    GF "Haha.... tout comme Futa God va baiser mes sœurs dès que Noël sera terminé."

# game/UpdateScriptChristmasEvent.rpy:782
translate french xmas_goodbye_412bf53b:

    # GF "I hope your upcoming Christmases are all better than the last."
    GF "J'espère que tes prochains Noëls seront tous meilleurs que les derniers."

# game/UpdateScriptChristmasEvent.rpy:783
translate french xmas_goodbye_36b7b10a:

    # GF "It's been fun!"
    GF "Ça a été amusant!"

# game/UpdateScriptChristmasEvent.rpy:784
translate french xmas_goodbye_cd4e359c:

    # GF "Keep it up, and maybe Futa God will send us back next year!"
    GF "Continues, et peut-être que Futa Dieu nous renverra l'année prochaine!"

# game/UpdateScriptChristmasEvent.rpy:785
translate french xmas_goodbye_0ddb3810:

    # GF "Merry Christmas!"
    GF "Joyeux Noël!"

# game/UpdateScriptChristmasEvent.rpy:789
translate french xmas_goodbye_a244e88f:

    # RT "{i}Holy shit!.... Dream or not, I don't want to ever forget that experience.{/i}"
    RT "{i}Merde!.... Rêve ou pas, je ne veux jamais oublier cette expérience.{/i}"

# game/UpdateScriptChristmasEvent.rpy:790
translate french xmas_goodbye_97bb1701:

    # RT "{i}Oh Futa God.... I'm so tired now.... but will I even remember this dream when I wake up?{/i}"
    RT "{i}Oh mon Dieu.... Je suis tellement fatiguée maintenant.... mais vais-je me souvenir de ce rêve quand je me réveillerais ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:807
translate french tutorial_c287fc63:

    # R "Testing.... testing.... ok.... It looks like it's recording."
    R "Essais.... test.... OK.... On dirait qu'il enregistre."

# game/UpdateScriptChristmasEvent.rpy:810
translate french tutorial_628908b8:

    # R "My name is [ryan].... and if you are watching this recording, it means that I am dead."
    R "Mon nom est [ryan].... et si vous regardez cet enregistrement, cela signifie que je suis mort."

# game/UpdateScriptChristmasEvent.rpy:811
translate french tutorial_480f96f3:

    # R "Or I guess it could mean the extremely unlikely scenario that my computer was hacked, and now some schmuck is using my story for financial gain."
    R "Ou je suppose que cela pourrait signifier le scénario extrêmement improbable selon lequel mon ordinateur a été piraté, et maintenant un imbécile utilise mon histoire pour un gain financier."

# game/UpdateScriptChristmasEvent.rpy:814
translate french tutorial_03ba063a:

    # R "Because the series of events I'm going to tell you about is so unbelievable, it would make a great movie or TV series."
    R "Parce que la série d'événements dont je vais vous parler est tellement incroyable, cela ferait un grand film ou une série TÉLÉVISÉE."

# game/UpdateScriptChristmasEvent.rpy:815
translate french tutorial_f5a2cacf:

    # R "But probably the kind of TV series you could only see on HBO."
    R "Mais probablement le genre de série télévisée que vous ne pouviez voir que sur HBO."

# game/UpdateScriptChristmasEvent.rpy:818
translate french tutorial_047a76a4:

    # R "Please God, just don't let anybody make a crappy H-game out of this."
    R "S'il te plaît dieu, ne laisses personne en faire un jeu hentai merdique dehors de ça."

# game/UpdateScriptChristmasEvent.rpy:822
translate french tutorial_be9784f7:

    # WT "Ummm.... hi!"
    WT "Ummm .... Salut !"

# game/UpdateScriptChristmasEvent.rpy:823
translate french tutorial_327b2971:

    # WT "Sorry to interrupt."
    WT "Désolé de vous interrompre."

# game/UpdateScriptChristmasEvent.rpy:824
translate french tutorial_3f364fff:

    # WT "My name is Will Tylor and I just wanted to take this opportunity to welcome you to our \"awesome\" H-game, \"A Family Venture!\""
    WT "Je m’appelle Will Tylor et je voulais juste profiter de cette occasion pour vous souhaiter la bienvenue dans notre « génial » jeu hentai, « A Family Venture! »"

# game/UpdateScriptChristmasEvent.rpy:825
translate french tutorial_a1128353:

    # WT "Back to you, [ryan]."
    WT "Reviens à toi, [ryan]."

# game/UpdateScriptChristmasEvent.rpy:830
translate french tutorial_23e6109b:

    # R "I wanted to record my version of the remarkable events that have occurred over the last little while."
    R "Je voulais enregistrer ma version des événements remarquables qui se sont produits ces derniers temps."

# game/UpdateScriptChristmasEvent.rpy:831
translate french tutorial_f9858483:

    # R "I know many wouldn't understand why I've been doing the things I'm doing, if I didn't explain myself, so this is my attempt to justify my actions."
    R "je sais que beaucoup ne comprendraient pas pourquoi j’ai fait les choses que je fais, si je ne m’expliquais pas, alors c’est ma tentative de justifier mes actions."

# game/UpdateScriptChristmasEvent.rpy:832
translate french tutorial_15e2c4a9:

    # R "I've been acquiring many items to help me achieve my goals, of saving those I love, and helping that love to become even more realized."
    R "j’ai acquis de nombreux objets pour m’aider à atteindre mes objectifs, à sauver ceux que j’aime et à aider cet amour à se réaliser encore plus."

# game/UpdateScriptChristmasEvent.rpy:835
translate french tutorial_7be5f7b4:

    # WT "Thanks [ryan]. Let me do a little explaining from here."
    WT "Merci [ryan]. Permettez-moi de faire quelques explications à partir d’ici."

# game/UpdateScriptChristmasEvent.rpy:838
translate french tutorial_308a5fdb:

    # WT "This is a typical in-game screen. As [ryan] mentioned, you will acquire many items in the game."
    WT "Il s'agit d'un écran typique du jeu. Comme [ryan] l'a mentionné, tu acquérais de nombreux objets dans le jeu."

# game/UpdateScriptChristmasEvent.rpy:841
translate french tutorial_c3f79235:

    # WT "You can keep track of the items you acquire by clicking on this blue backpack."
    WT "Vous pouvez garder une trace des objets que vous acquérez en cliquant sur ce sac à dos bleu."

# game/UpdateScriptChristmasEvent.rpy:844
translate french tutorial_0556842f:

    # WT "This will open up your inventory,"
    WT "Cela ouvrira votre inventaire,"

# game/UpdateScriptChristmasEvent.rpy:847
translate french tutorial_d644837a:

    # WT "where you will be able to see how much money you have earned,"
    WT "où vous pourrez voir combien d’argent vous avez gagné,"

# game/UpdateScriptChristmasEvent.rpy:850
translate french tutorial_25ecef9b:

    # WT "and what items you have acquired."
    WT "et quels objets vous avez acquis."

# game/UpdateScriptChristmasEvent.rpy:853
translate french tutorial_48f3259e:

    # WT "There are several other important items on the game screen I'd like to draw your attention to."
    WT "Il y a plusieurs autres éléments importants sur l’écran du jeu sur lesquels j’aimerais attirer votre attention."

# game/UpdateScriptChristmasEvent.rpy:856
translate french tutorial_25fc2c00:

    # WT "This symbol shows you what day of the week it is,"
    WT "Ce symbole vous indique de quel jour de la semaine il s’agit,"

# game/UpdateScriptChristmasEvent.rpy:859
translate french tutorial_75aeed33:

    # WT "while this symbol shows you the time of day."
    WT "alors que ce symbole t'indique l'heure de la journée."

# game/UpdateScriptChristmasEvent.rpy:862
translate french tutorial_2c6857c0:

    # WT "I'll let you get back to [ryan] again."
    WT "Je vous laisse revenir à [ryan]."

# game/UpdateScriptChristmasEvent.rpy:865
translate french tutorial_4f74ea21:

    # R "I've had to become very good at managing my money. I've got debts I have to pay, and items I need to buy, to help influence those around me."
    R "j’ai dû devenir très bon dans la gestion de mon argent. J’ai des dettes que je dois payer, et des articles que je dois acheter, pour aider à influencer ceux qui m’entourent."

# game/UpdateScriptChristmasEvent.rpy:868
translate french tutorial_88c4b13a:

    # R "I've become aware that someone, or several someones, have been tracking my movements. So, I have learned to be stealthy, and to watch my back wherever I go."
    R "j’ai pris conscience que quelqu’un, ou plusieurs personnes, suivaient mes mouvements. J’ai donc appris à être furtif et à surveiller mes arrières partout où je vais."

# game/UpdateScriptChristmasEvent.rpy:871
translate french tutorial_33df29b8:

    # WT "Let me just pause you again for a second. [ryan] brings up a good point about getting around."
    WT "Permettez-moi de vous interrompre à nouveau une seconde. [ryan] soulève un bon point sur le fait de se déplacer."

# game/UpdateScriptChristmasEvent.rpy:872
translate french tutorial_c69197d0:

    # WT "You will find that navigation is very important in this type of game, so let me just show you how that's done."
    WT "Vous constaterez que la navigation est très importante dans ce type de jeu, alors laissez-moi vous montrer comment cela se fait."

# game/UpdateScriptChristmasEvent.rpy:875
translate french tutorial_f5ab5820:

    # WT "On the game screen, you can get around to many locations by using the travel buttons."
    WT "Sur l'écran de jeu, Tu peux te déplacer à de nombreux endroits en utilisant les boutons voyage."

# game/UpdateScriptChristmasEvent.rpy:878
translate french tutorial_4f84e77b:

    # WT "This button right here, will open up a map of the house, where you can pick any room you would like to go to."
    WT "Ce bouton ici, ouvrira une carte de la maison, où tu pourras choisir n'importe quelle pièce dans laquelle tu souhaites aller."

# game/UpdateScriptChristmasEvent.rpy:881
translate french tutorial_1e2916f7:

    # WT "You can also hover your cursor over this button, to open up the quick travel options."
    WT "Vous pouvez également passer votre curseur sur ce bouton, pour ouvrir les options de déplacement rapide."

# game/UpdateScriptChristmasEvent.rpy:882
translate french tutorial_65da6e59:

    # WT "These buttons will take you to any room in the house, as well as the world map."
    WT "Ces boutons vous mèneront à n’importe quelle pièce de la maison, ainsi qu’à la carte du monde."

# game/UpdateScriptChristmasEvent.rpy:883
translate french tutorial_82d216eb:

    # WT "Exploring the AFV world is vitally important, as you will find that some events only happen at certain times, and in certain places."
    WT "Explorer le monde AFV est d’une importance vitale, car vous constaterez que certains événements ne se produisent qu’à certains moments et à certains endroits."

# game/UpdateScriptChristmasEvent.rpy:884
translate french tutorial_60f71c6f:

    # WT "So, have fun exploring!"
    WT "Alors, amuses-toi à explorer!"

# game/UpdateScriptChristmasEvent.rpy:890
translate french tutorial_c83ea849:

    # R "I never can be sure who is following me, even though I have my suspicions."
    R "je ne peux jamais être sûr de qui me suit, même si j’ai mes soupçons."

# game/UpdateScriptChristmasEvent.rpy:891
translate french tutorial_422bf6c7:

    # R "Always having to watch my back has made me grow closer to the few people I can trust."
    R "Avoir toujours à surveiller mon dos m'a fait me rapprocher des quelques personnes en qui je peux avoir confiance."

# game/UpdateScriptChristmasEvent.rpy:892
translate french tutorial_51e7cbc3:

    # R "And who can I trust better than those that live right here with me?"
    R "Et à qui puis-je faire confiance mieux que ceux qui vivent ici avec moi ?"

# game/UpdateScriptChristmasEvent.rpy:893
translate french tutorial_033e4640:

    # {i}\"(knock knock knock....)\"{/i}"
    "{i}'(Toc toc toc....)'{/i}"

# game/UpdateScriptChristmasEvent.rpy:894
translate french tutorial_c1bfb181:

    # M "{i}(muffled voice){/i} [ryan]? Why is the door locked? You're not looking at porn and masturbating again are you?"
    M "{i}(voix étouffée){/i} [ryan]? Pourquoi la porte est-elle verrouillée? Tu ne regardes pas du porno et tu ne te masturbes pas, n’est-ce pas?"

# game/UpdateScriptChristmasEvent.rpy:897
translate french tutorial_80ba08b4:

    # R "No, Mom!.... But I'm really busy right now!"
    R "Non, maman!.... Mais je suis vraiment occupé en ce moment!"

# game/UpdateScriptChristmasEvent.rpy:898
translate french tutorial_ecfc0255:

    # M "With what?"
    M "Avec quoi?"

# game/UpdateScriptChristmasEvent.rpy:899
translate french tutorial_0fcacb7c:

    # R "Ummm.... homework.... of course!"
    R "Ummm.... devoirs.... bien sûr!"

# game/UpdateScriptChristmasEvent.rpy:900
translate french tutorial_0bb56762:

    # M "Well, ok.... but you know you don't need to take care of yourself like that anymore, right?"
    M "Eh bien, d'accord.... mais tu sais que tu n'as plus besoin de prendre soin de toi comme ça, non ?"

# game/UpdateScriptChristmasEvent.rpy:901
translate french tutorial_189f9a50:

    # M "You know that if you need some relief, you just need to let me know. And if I'm too busy, I'm sure Lauren or Sidney would be happy to help you out."
    M "Tu sais que si tu as besoin d'un soulagement, il te suffit de me le faire savoir. Et si je suis trop occupé, je suis sûr que Lauren ou Sidney seraient heureuse de t'aider."

# game/UpdateScriptChristmasEvent.rpy:904
translate french tutorial_1584e8be:

    # R "Ok!.... Thanks, Mom!"
    R "Ok!.... Merci, Maman!"

# game/UpdateScriptChristmasEvent.rpy:905
translate french tutorial_b0ff8df9:

    # R "Please, don't judge me until you've heard my side of the story."
    R "S'il te plait, ne me juges pas avant d'avoir entendu ma version des faits."

# game/UpdateScriptChristmasEvent.rpy:908
translate french tutorial_0c911552:

    # R "It's like I said.... with so many shady characters around tracking my movements, I've needed to grow closer to the people around me."
    R "C'est comme je l'ai dit.... avec tant de personnages louches qui suivent mes mouvements, j'ai eu besoin de me rapprocher des gens autour de moi."

# game/UpdateScriptChristmasEvent.rpy:911
translate french tutorial_0f68a40c:

    # WT "Let me just jump in here one more time."
    WT "Permettez-moi d’intervenir ici une fois de plus."

# game/UpdateScriptChristmasEvent.rpy:914
translate french tutorial_c7f3d7ec:

    # WT "To be successful, [ryan] has to influence those around him."
    WT "Pour réussir, [ryan] doit influencer ceux qui l’entourent."

# game/UpdateScriptChristmasEvent.rpy:915
translate french tutorial_762d3d13:

    # WT "Relationship management is the most important part of this game."
    WT "La gestion des relations est la partie la plus importante de ce jeu."

# game/UpdateScriptChristmasEvent.rpy:916
translate french tutorial_6141dd6f:

    # WT "You will need to make choices throughout the game that will influence the way the girls in your life feel about you."
    WT "Vous devrez faire des choix tout au long du jeu qui influenceront la façon dont les filles de votre vie se sentent à votre sujet."

# game/UpdateScriptChristmasEvent.rpy:917
translate french tutorial_4329ffca:

    # WT "So, it's pretty important that you keep track of those stats."
    WT "Donc, il est assez important que tu gardais une trace de ces statistiques."

# game/UpdateScriptChristmasEvent.rpy:918
translate french tutorial_12be52d2:

    # WT "You can access a girl's stats,"
    WT "Tu peux accéder aux statistiques d'une fille,"

# game/UpdateScriptChristmasEvent.rpy:921
translate french tutorial_9491e3d1:

    # WT "by pressing the \"P\" button on your keyboard."
    WT "en appuyant sur le bouton « P » de votre clavier."

# game/UpdateScriptChristmasEvent.rpy:924
translate french tutorial_b769c99b:

    # WT "You can cycle through the girls in the game by clicking on the left or right arrows."
    WT "Vous pouvez parcourir les filles du jeu en cliquant sur les flèches gauche ou droite."

# game/UpdateScriptChristmasEvent.rpy:927
translate french tutorial_c8ef2a6f:

    # WT "Once you've found the girl you want to track, you can figure out her stats by looking at the icons by her name and picture."
    WT "Une fois que vous avez trouvé la fille que vous souhaitez suivre, vous pouvez prendre connaissance de ses statistiques en regardant les icônes sous son nom et sa photo."

# game/UpdateScriptChristmasEvent.rpy:930
translate french tutorial_418686a5:

    # WT "The heart icon stands for love."
    WT "L’icône du cœur représente l’amour."

# game/UpdateScriptChristmasEvent.rpy:933
translate french tutorial_c3bbac67:

    # WT "These hands represent respect."
    WT "Celles des mains représentent le respect."

# game/UpdateScriptChristmasEvent.rpy:936
translate french tutorial_0b232737:

    # WT "The angry emoji stands for a girl's anger towards you. It's hard to get them to do anything for you when they are mad."
    WT "L'emoji en colère représente la colère d'une fille envers toi. Il est difficile de les amener à faire quoi que ce soit pour toi quand elles sont énervées."

# game/UpdateScriptChristmasEvent.rpy:939
translate french tutorial_b93cb628:

    # WT "This whip icon, represents a girl's submission to you."
    WT "Cette icône de fouet représente la soumission d'une fille envers toi."

# game/UpdateScriptChristmasEvent.rpy:942
translate french tutorial_f8468e18:

    # WT "And these lips represent a girl's libido."
    WT "Et ces lèvres représentent la libido d'une fille."

# game/UpdateScriptChristmasEvent.rpy:945
translate french tutorial_d4d20a80:

    # WT "To close the stats window, just press the \"P\" on your keyboard again."
    WT "Pour fermer la fenêtre des statistiques, appuyez à nouveau sur le « P » sur votre clavier."

# game/UpdateScriptChristmasEvent.rpy:946
translate french tutorial_d933deaa:

    # WT "Another handy way to track a girl's stats is through your phone."
    WT "Un autre moyen pratique de suivre les statistiques d'une fille est via ton téléphone."

# game/UpdateScriptChristmasEvent.rpy:949
translate french tutorial_bb9483d0:

    # WT "You can access your phone by clicking on this button."
    WT "Vous pouvez accéder à votre téléphone en cliquant sur ce bouton."

# game/UpdateScriptChristmasEvent.rpy:952
translate french tutorial_b2eba79a:

    # WT "Once your phone is pulled up, you can track a girl's stats,"
    WT "Une fois que ton téléphone est tiré, tu peux suivre les statistiques d'une fille,"

# game/UpdateScriptChristmasEvent.rpy:955
translate french tutorial_d3037095:

    # WT "by clicking on this button right here."
    WT "en cliquant sur ce bouton ici."

# game/UpdateScriptChristmasEvent.rpy:958
translate french tutorial_6d29e98a:

    # WT "There are also some other handy things you can do with your phone as well."
    WT "Il y a aussi d’autres choses pratiques que vous pouvez faire avec votre téléphone."

# game/UpdateScriptChristmasEvent.rpy:961
translate french tutorial_c65434ed:

    # WT "For those of you who support us at the \"Friend\" tier on SubscribeStar or Patreon, you can install a mod that you can access by pressing on this button."
    WT "Pour ceux d’entre vous qui nous soutiennent au niveau «Ami» sur SubscribeStar ou Patreon, vous pouvez installer un mod auquel vous pouvez accéder en appuyant sur ce bouton."

# game/UpdateScriptChristmasEvent.rpy:962
translate french tutorial_c33007ea:

    # WT "The mod is pretty awesome! It allows you to change the name of your character, manipulate the stats of other characters, add and remove items, and add money to your backpack."
    WT "Le mod est assez génial! Il vous permet de changer le nom de votre personnage, de manipuler les statistiques d’autres personnages, d’ajouter et de supprimer des objets et d’ajouter de l’argent à votre sac à dos."

# game/UpdateScriptChristmasEvent.rpy:965
translate french tutorial_aadd8f32:

    # WT "This button allows you to track your progress. When you click on it,"
    WT "Ce bouton vous permet de suivre vos progrès. Lorsque vous cliquez dessus,"

# game/UpdateScriptChristmasEvent.rpy:968
translate french tutorial_69912aa8:

    # WT "A screen that lists events will pop up."
    WT "Un écran qui répertorie les événements apparaîtra."

# game/UpdateScriptChristmasEvent.rpy:971
translate french tutorial_7a7c9308:

    # WT "And when you click on the three question marks \"???\", you will receive hints to help you know how to complete that event."
    WT "Et lorsque vous cliquez sur les trois points d’interrogation «???», vous recevrez des conseils pour vous aider à savoir comment terminer cet événement."

# game/UpdateScriptChristmasEvent.rpy:974
translate french tutorial_caf2f581:

    # WT "A couple other useful buttons are on the bottom of the phone."
    WT "Quelques autres boutons utiles se trouvent au bas du téléphone."

# game/UpdateScriptChristmasEvent.rpy:977
translate french tutorial_ce5b1c76:

    # WT "This button lets you see any pictures of girls that you may take throughout the course of the game."
    WT "Ce bouton vous permet de voir toutes les photos de filles que vous pouvez prendre tout au long du jeu."

# game/UpdateScriptChristmasEvent.rpy:980
translate french tutorial_631f34ab:

    # WT "And this extremely useful button lets you move forward in time."
    WT "Et ce bouton extrêmement utile te permet d'avancer dans le temps."

# game/UpdateScriptChristmasEvent.rpy:983
translate french tutorial_bc435dec:

    # WT "So, that's all the tips we have for you. Everything else you will have to figure out on your own, or I guess you could always find the walkthrough, or ask for help online."
    WT "Voilà donc tous les conseils que nous avons pour toi. Tout le reste, tu devras le découvrir toi-même, ou je suppose que tu peux toujours trouver la procédure pas à pas, ou demander de l'aide en ligne."

# game/UpdateScriptChristmasEvent.rpy:984
translate french tutorial_d431e493:

    # WT "However you decide to play, we hope you enjoy the game!"
    WT "Quelle que soit la façon dont tu décides de jouer, nous espérons que tu apprécieras le jeu!"

# game/UpdateScriptChristmasEvent.rpy:987
translate french tutorial_bd43ab24:

    # WT "Go ahead and take it from here, [ryan]!"
    WT "Allez-y et reprenez-le d'ici, [ryan]!"

# game/UpdateScriptChristmasEvent.rpy:990
translate french tutorial_d6f0f5d3:

    # R "I guess the best thing I can do is to tell you my story, and let whomever watches this to be the judge of my actions."
    R "Je suppose que la meilleure chose que je puisse faire est de vous raconter mon histoire, et que quiconque regarde cela soit le juge de mes actions."

# game/UpdateScriptChristmasEvent.rpy:991
translate french tutorial_ec630762:

    # R "And once again, hope this story gets turned into a kick-ass HBO series, and not a fucking lame-ass H-game."
    R "Et encore une fois, j'espère que cette histoire se transformera en une série HBO kick-ass, et non en un putain de H-game boiteux."

# game/UpdateScriptChristmasEvent.rpy:994
translate french tutorial_e6d5853d:

    # R ".... So, until fairly recently, I was just a normal teenage kid like everybody else."
    R ".... Donc, jusqu’à assez récemment, j’étais juste un adolescent normal comme tout le monde."

# game/UpdateScriptChristmasEvent.rpy:995
translate french tutorial_ce33c5bc:

    # R "Until I was awoken really early one morning by a phone call that would change my life forever..."
    R "Jusqu'à ce que je sois réveillé très tôt un matin par un appel téléphonique qui allait changer ma vie pour toujours..."

# game/UpdateScriptChristmasEvent.rpy:996
translate french tutorial_c6841c25:

    # R "..."
    R "..."

# game/UpdateScriptChristmasEvent.rpy:999
translate french tutorial_076a53a8:

    # End of tutorial."
    "Fin du tutoriel."

# game/UpdateScriptChristmasEvent.rpy:1007
translate french sidney_prego_too_0674578f:

    # GF "Oh, look! There she is, adjusting something on the Christmas tree."
    GF "Oh, regarde! La voilà, ajustant quelque chose sur le sapin de Noël."

# game/UpdateScriptChristmasEvent.rpy:1008
translate french sidney_prego_too_b6f8da23:

    # RT "I'll wager money that when she turns around, she'll have a pregnant belly too."
    RT "Je vais parier de l'argent que quand elle se retournera, elle aura aussi un ventre de femme enceinte."

# game/UpdateScriptChristmasEvent.rpy:1009
translate french sidney_prego_too_64bce564:

    # GF "Yeah, I'm not betting you. I think the odds are too much in your favor."
    GF "Ouais, je ne te parie pas. Je pense que les chances sont trop grandes en ta faveur."

# game/UpdateScriptChristmasEvent.rpy:1012
translate french sidney_prego_too_cf8fb58b:

    # RT "{i}Ha!.... Called it!{/i}"
    RT "{i}Ha!.... Je l'ai appelé!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1013
translate french sidney_prego_too_32bac789:

    # GF "Congratulations for being able to see obvious plot points in simple-minded stories."
    GF "Félicitations pour avoir pu voir des points d'intrigue évidents dans des histoires simples d'esprit."

# game/UpdateScriptChristmasEvent.rpy:1014
translate french sidney_prego_too_f27d2943:

    # RT "{i}Huh?....{/i}"
    RT "{i}Hein ?....{/i}"

# game/UpdateScriptChristmasEvent.rpy:1015
translate french sidney_prego_too_bda134b9:

    # GF "Never mind."
    GF "Peu importe."

# game/UpdateScriptChristmasEvent.rpy:1016
translate french sidney_prego_too_fba13aed:

    # S "Merry Christmas, [ryan]!"
    S "Joyeux Noël, [ryan]!"

# game/UpdateScriptChristmasEvent.rpy:1019
translate french sidney_prego_too_f881299b:

    # R "Oh, hey Sidney! Merry Christmas! I love your sweater!"
    R "Oh, hé Sidney! Joyeux Noël! J'adore ton pull !"

# game/UpdateScriptChristmasEvent.rpy:1020
translate french sidney_prego_too_94588f10:

    # S "Thanks! I made it myself. I thought it was pretty festive."
    S "Merci! Je l'ai fait moi-même. Je trouvais ça plutôt festif."

# game/UpdateScriptChristmasEvent.rpy:1021
translate french sidney_prego_too_80b53318:

    # S "But it's too damn hot!"
    S "Mais il fait trop chaud !"

# game/UpdateScriptChristmasEvent.rpy:1022
translate french sidney_prego_too_2af513d9:

    # S "This baby is like having my own personal space heater attached to me."
    S "Ce bébé, c'est comme si j'avais mon propre chauffage personnel attaché à moi."

# game/UpdateScriptChristmasEvent.rpy:1025
translate french sidney_prego_too_df41d2a2:

    # S "I can't seem to get cool, no matter what I do."
    S "Je n'arrive pas à devenir cool, peu importe ce que je fais."

# game/UpdateScriptChristmasEvent.rpy:1026
translate french sidney_prego_too_27b52a67:

    # RT "{i}I think I'm starting to understand why guys get pregnancy fetishes.{/i}"
    RT "{i}Je pense que je commence à comprendre pourquoi les gars ont des fétiches sur la grossesse.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1027
translate french sidney_prego_too_fe30230a:

    # RT "{i}Every single one of the girls still looks sexy as hell, even with their huge baby bumps.{/i}"
    RT "{i}Chacune des filles a toujours l'air sexy, même avec leurs énormes bébés.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1028
translate french sidney_prego_too_f8f7bca8:

    # RT "{i}I guess I should just add this fetish to the list.{/i}"
    RT "{i}Je suppose que je devrais simplement ajouter ce fétiche à la liste.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1029
translate french sidney_prego_too_7deaa026:

    # GF "They should hand out fetish merit badges. You'd be one of the most decorated perverts out there."
    GF "Ils devraient distribuer des badges de mérite fétiche. Tu serais l'un des pervers les plus décorés du monde."

# game/UpdateScriptChristmasEvent.rpy:1030
translate french sidney_prego_too_6d0a575f:

    # RT "{i}And to think, I was a totally normal kid, not too long ago.{/i}"
    RT "{i}Et à penser, j'étais un enfant tout à fait normal, il n'y a pas si longtemps.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1031
translate french sidney_prego_too_5c1dc3f6:

    # GF "Well, every pervert's got to start somewhere."
    GF "Eh bien, chaque pervers doit commencer quelque part."

# game/UpdateScriptChristmasEvent.rpy:1032
translate french sidney_prego_too_52b9c6d2:

    # RT "I've got to find out who's the father of Sidney's baby."
    RT "Je dois savoir qui est le père du bébé de Sidney."

# game/UpdateScriptChristmasEvent.rpy:1033
translate french sidney_prego_too_7908d1ea:

    # GF "Maybe you could try the \"Keeping a History of Events for the Year,\" route with Sidney."
    GF "Peut-être pourrais-tu essayer la route 'Garder une histoire des événements de l'année' avec Sidney."

# game/UpdateScriptChristmasEvent.rpy:1034
translate french sidney_prego_too_59643a6d:

    # GF "That worked well with Lauren."
    GF "Cela a bien fonctionné avec Lauren."

# game/UpdateScriptChristmasEvent.rpy:1035
translate french sidney_prego_too_ef45f454:

    # R "So, I've been trying to record all of the notable events for this year. You know.... kind of a journal, so we can look back and remember the good times."
    R "J'ai donc essayé d'enregistrer tous les événements notables de cette année. Tu sais.... une sorte de journal, pour que nous puissions regarder en arrière et nous souvenir des bons moments."

# game/UpdateScriptChristmasEvent.rpy:1036
translate french sidney_prego_too_299cce03:

    # R "After talking with Mom and Sidney, I think I've got most of the events covered. The last thing I need for the record, is your version of how you got pregnant."
    R "Après avoir parlé avec Maman et Sidney, je pense avoir couvert la plupart des événements. La dernière chose dont j'ai besoin pour le dossier, c'est ta version de la façon dont tu es tombée enceinte."

# game/UpdateScriptChristmasEvent.rpy:1040
translate french sidney_prego_too_9373f743:

    # S "Nice try, [ryan]! I know you're only bringing this up so you can reprimand me again for what stupid decisions I make."
    S "Bel essai, [ryan]! Je sais que tu ne fais que parler de ça pour que tu puisses me réprimander à nouveau pour les décisions stupides que je prends."

# game/UpdateScriptChristmasEvent.rpy:1041
translate french sidney_prego_too_7bb6da73:

    # S "I already know it was stupid of me to follow Mom to Joey's strip club."
    S "Je sais déjà que c'était stupide de ma part de suivre maman au club de strip-tease de Joey."

# game/UpdateScriptChristmasEvent.rpy:1042
translate french sidney_prego_too_8b640395:

    # S "I already know it was stupid of me to accept the work that Joey was offering."
    S "Je sais déjà que c'était stupide de ma part d'accepter le travail que Joey offrait."

# game/UpdateScriptChristmasEvent.rpy:1043
translate french sidney_prego_too_d3f0ac06:

    # S "I know it was stupid of me to let him buy me nice things."
    S "Je sais que c'était stupide de ma part de le laisser m'acheter de belles choses."

# game/UpdateScriptChristmasEvent.rpy:1044
translate french sidney_prego_too_8eaa0f5b:

    # S "I know it was stupid of me to think he would ever leave his wife for me."
    S "Je sais que c'était stupide de ma part de penser qu'il quitterait un jour sa femme pour moi."

# game/UpdateScriptChristmasEvent.rpy:1045
translate french sidney_prego_too_cf8896cb:

    # S "I don't know how, or why, I let him manipulate me to become his tool to blackmail powerful men."
    S "Je ne sais pas comment, ni pourquoi, je l'ai laissé me manipuler pour devenir son outil de chantage aux hommes puissants."

# game/UpdateScriptChristmasEvent.rpy:1046
translate french sidney_prego_too_5b92748b:

    # S "I'd sleep with them, and Joey would record it all, to use the pictures later for blackmail."
    S "Je couchais avec eux, et Joey enregistrait tout, pour utiliser les photos plus tard pour faire du chantage."

# game/UpdateScriptChristmasEvent.rpy:1049
translate french sidney_prego_too_38d6435e:

    # S "And now I've slept with most of the powerful men in the state, and I don't know if my baby's father is Senator McFeely, Archbishop SaintFister, Governor LeGroper,"
    S "Et maintenant j'ai couché avec la plupart des hommes puissants de l'État, et je ne sais pas si le père de mon bébé est le sénateur McFeely, l'archevêque SaintFister, le gouverneur LeGroper,"

# game/UpdateScriptChristmasEvent.rpy:1050
translate french sidney_prego_too_b501d30f:

    # S "Chief O'CummingHand, Don BiggusDickus.... (rambling on)"
    S "Capo O'CummingHand, Don BiggusDickus.... (escalade sur)"

# game/UpdateScriptChristmasEvent.rpy:1051
translate french sidney_prego_too_9b662b95:

    # GF "Oh, damn! Holy shit! You're zero for three."
    GF "Oh, putain! Putain de merde! Tu es zéro pour trois."

# game/UpdateScriptChristmasEvent.rpy:1052
translate french sidney_prego_too_ee7d035c:

    # GF "I don't want to sound too intrusive or judgy, but you might want to do a few things to try and change this future."
    GF "Je ne veux pas paraître trop intrusif ou trop sérieuse, mais tu voudrais peut-être faire quelques choses pour essayer de changer cet avenir."

# game/UpdateScriptChristmasEvent.rpy:1053
translate french sidney_prego_too_2910dec5:

    # GF "Unless you get off on other men fucking your women. Like I said, I don't judge."
    GF "À moins que tu ne te débarrasses d'autres hommes qui baisent tes femmes. Comme je l'ai dit, je ne juge pas."

# game/UpdateScriptChristmasEvent.rpy:1054
translate french sidney_prego_too_1cb0423e:

    # RT "{i}Trust me, I'm keeping a mental list of things I need to change.{/i}"
    RT "{i}Crois-moi, je garde une liste mentale de choses que je dois changer.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1055
translate french sidney_prego_too_b8544f5a:

    # RT "{i}Though they're not the kind of changes your sisters would want me to make.{/i}"
    RT "{i}Bien qu'ils ne soient pas le genre de changements que vos sœurs voudraient que je fasse.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1056
translate french sidney_prego_too_f580bdf7:

    # GF "Ahhh.... Fuck 'em."
    GF "Ahhh.... Je les emmerde."

# game/UpdateScriptChristmasEvent.rpy:1057
translate french sidney_prego_too_76068caa:

    # RT "{i}They're too small.{/i}"
    RT "{i}Ils sont trop petits.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1058
translate french sidney_prego_too_91c29a5d:

    # GF "Hahaha.... you'd be surprised at how much they can stretch."
    GF "Hahaha.... tu serais surpris de voir à quel point ils peuvent s'étirer."

# game/UpdateScriptChristmasEvent.rpy:1059
translate french sidney_prego_too_0fb1a83e:

    # S "Principle Wankright, School board member Will Tylor."
    S "Principe Wankright, membre du conseil scolaire Will Tylor."

# game/UpdateScriptChristmasEvent.rpy:1060
translate french sidney_prego_too_f4399385:

    # R "Sidney! Shut up for a second"
    R "Sidney! Tais-toi une seconde"

# game/UpdateScriptChristmasEvent.rpy:1063
translate french sidney_prego_too_03ffe137:

    # R "I understand you slept with half the state, but is there any possible way that I could be the daddy?"
    R "Je comprends que tu as couché avec la moitié de l'État, mais y a-t-il un moyen possible que je sois le papa?"

# game/UpdateScriptChristmasEvent.rpy:1064
translate french sidney_prego_too_60356abf:

    # S "No.... I wasn't living at home at the time. I was too busy with the DeCapos. Joey had me living in his high rise apartment."
    S "Non.... Je ne vivais pas chez moi à l'époque. J'étais trop occupé avec les DeCapos. Joey m'a fait vivre dans son appartement en hauteur."

# game/UpdateScriptChristmasEvent.rpy:1065
translate french sidney_prego_too_19650fdb:

    # S "And you refused to even come visit me."
    S "Et tu refusais même de venir me rendre visite."

# game/UpdateScriptChristmasEvent.rpy:1068
translate french sidney_prego_too_997b8841:

    # S "But I was so grateful that you let me come back home, even though I was hooked on meth and had a vagina looser than a wizards sleeve."
    S "Mais j'étais tellement reconnaissante que tu me laissais rentrer à la maison, même si j'étais accro à la méthamphétamine et que j'avais un vagin plus lâche qu'une manche de sorciers."

# game/UpdateScriptChristmasEvent.rpy:1069
translate french sidney_prego_too_0d78224c:

    # R "Ok Sidney, just stop. I've got everything I need for the family history. And your section of it is going to be quite a legacy."
    R "Ok Sidney, arrête. J'ai tout ce dont j'ai besoin pour l'histoire familiale. Et ta section de celui-ci sera tout un héritage."

# game/UpdateScriptChristmasEvent.rpy:1070
translate french sidney_prego_too_6460ccc3:

    # S "Ahhh.... Fuck it. I know what I am."
    S "Ahhh.... Merde. Je sais ce que je suis."

# game/UpdateScriptChristmasEvent.rpy:1071
translate french sidney_prego_too_891bce20:

    # S "I was quite the prude at one time, then it all changed when I started molesting people in their sleep."
    S "J'étais assez prude à un moment donné, puis tout a changé quand j'ai commencé à agresser les gens dans leur sommeil."

# game/UpdateScriptChristmasEvent.rpy:1072
translate french sidney_prego_too_ccde9eee:

    # S "You remember that?"
    S "Tu t'en souviens ?"

# game/UpdateScriptChristmasEvent.rpy:1073
translate french sidney_prego_too_3e8b4a25:

    # R "Of course I do."
    R "Bien sûr que oui."

# game/UpdateScriptChristmasEvent.rpy:1074
translate french sidney_prego_too_50a1edf0:

    # S "Sorry if that messed you up at all."
    S "Désolé si cela t'a gâché du tout."

# game/UpdateScriptChristmasEvent.rpy:1075
translate french sidney_prego_too_738bf61b:

    # S "I know it did me."
    S "Je sais que c'est ce que j'ai fait."

# game/UpdateScriptChristmasEvent.rpy:1076
translate french sidney_prego_too_cef13795:

    # RT "{i}Shit.... I've ruined her life. And now she's apologizing to me, for something I made her do.{/i}"
    RT "{i}Merde.... J'ai ruiné sa vie. Et maintenant, elle s'excuse auprès de moi, pour quelque chose que je lui ai fait faire.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1077
translate french sidney_prego_too_e3e1083b:

    # RT "{i}Fuck! I'm ready to go back right now, and try to fix some things.{/i}"
    RT "{i}Merde ! Je suis prêt à y retourner maintenant, et à essayer de réparer certaines choses.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1078
translate french sidney_prego_too_fd2c6cb5:

    # GF "Well, you know how this works. We've got to wait in the future until our time is up. While you're waiting, go look around and maybe you can find something that will cheer you up."
    GF "Eh bien, tu sais comment cela fonctionne. Nous devons attendre à l'avenir jusqu'à ce que notre temps soit écoulé. Pendant que tu attends, allez regarder autour de toi et tu pourras peut-être trouver quelque chose qui te remontera le moral."

# game/UpdateScriptChristmasEvent.rpy:1079
translate french sidney_prego_too_9d8309db:

    # S "Oh look, Here comes Mom and Lauren."
    S "Oh regardes, voici Maman et Lauren."

# game/UpdateScriptChristmasEvent.rpy:1080
translate french sidney_prego_too_79b2c2bc:

    # S "I'm just going to go sit down and have a little break. My back is killing me."
    S "Je vais juste aller m'asseoir et faire une petite pause. Mon dos me tue."

# game/UpdateScriptChristmasEvent.rpy:1084
translate french sidney_prego_too_38dd5f9e:

    # S "Hahah.... nice try, [ryan]!"
    S "Hahah.... bon essai, [ryan]!"

# game/UpdateScriptChristmasEvent.rpy:1085
translate french sidney_prego_too_87eb9971:

    # S "I know you're only bringing that up because you like to gloat that you were able to knock up both me and Lauren."
    S "Je sais que tu ne fais que parler de ça parce que tu aimes te réjouir d'avoir été capable de me frapper à la fois Lauren et moi."

# game/UpdateScriptChristmasEvent.rpy:1086
translate french sidney_prego_too_66d20398:

    # GF "Ahhh! Way to go! Two out of three's not bad at all!"
    GF "Ahhh! Bien joué ! Deux sur trois, c'est pas mal du tout !"

# game/UpdateScriptChristmasEvent.rpy:1087
translate french sidney_prego_too_b29e8e88:

    # S "And yes, we all think you're amazing, and yes we're super grateful for all of the shit you've done for us."
    S "Et oui, nous pensons tous que tu es incroyable, et oui nous sommes super reconnaissants pour toute la merde que tu as faite pour nous."

# game/UpdateScriptChristmasEvent.rpy:1089
translate french sidney_prego_too_5c6e6e1c:

    # S "I hope you don't keep bringing it up because you don't feel appreciated!"
    S "J'espère que tu ne continues pas à en parler parce que tu ne te sens pas apprécié!"

# game/UpdateScriptChristmasEvent.rpy:1090
translate french sidney_prego_too_0572e331:

    # S "Because what you did to get us out from under the thumb of those Mafia goons!.... Well, it was so heroic."
    S "Parce que ce que tu as fait pour nous sortir de sous le pouce de ces crétins de la Mafia!.... C'était tellement héroïque."

# game/UpdateScriptChristmasEvent.rpy:1091
translate french sidney_prego_too_e20f3407:

    # S "Not many people are able to shrug off the Mafia like you did, without having to go into witness protection."
    S "Peu de gens sont capables de fuir la Mafia comme tu l'as fait, sans avoir à se mettre sous la protection des témoins."

# game/UpdateScriptChristmasEvent.rpy:1092
translate french sidney_prego_too_69c51d27:

    # S "And now, you've gained both their respect and their apathy."
    S "Et maintenant, tu as gagné à la fois leur respect et leur apathie."

# game/UpdateScriptChristmasEvent.rpy:1093
translate french sidney_prego_too_21335f68:

    # S "You really should write a book about it someday!"
    S "Tu devrais vraiment écrire un livre à ce sujet un jour!"

# game/UpdateScriptChristmasEvent.rpy:1094
translate french sidney_prego_too_325116af:

    # R "Really? Tell me!.... What did I do!?..."
    R "Vraiment? Dis-moi!.... Qu'ai-je fait!?..."

# game/UpdateScriptChristmasEvent.rpy:1096
translate french sidney_prego_too_6546b8b8:

    # S "Ohhhhh.... of course you know!.... And there's no need for false modesty."
    S "Ohhhhh.... bien sûr que tu sais!.... Et il n'y a pas besoin de fausse modestie."

# game/UpdateScriptChristmasEvent.rpy:1097
translate french sidney_prego_too_a1e26de1:

    # S "Needless to say. Lauren and I, are both very proud to be carrying your babies!"
    S "Inutile de le dire. Lauren et moi sommes toutes les deux très fières de porter tes bébés!"

# game/UpdateScriptChristmasEvent.rpy:1098
translate french sidney_prego_too_267cf485:

    # S "We don't give a fuck about what other people think."
    S "On s'en fout de ce que les autres pensent."

# game/UpdateScriptChristmasEvent.rpy:1099
translate french sidney_prego_too_b8d2ebed:

    # S "Though we still will probably lie about who their father is."
    S "Bien que nous allons encore probablement mentir sur qui est leur père."

# game/UpdateScriptChristmasEvent.rpy:1100
translate french sidney_prego_too_445dd637:

    # GF "Yeah, It will be confusing enough to try to explain to them that they are both cousins and siblings."
    GF "Oui, ce sera assez déroutant d'essayer de leur expliquer qu'ils sont à la fois cousins et frères et sœurs."

# game/UpdateScriptChristmasEvent.rpy:1101
translate french sidney_prego_too_6024fd79:

    # S "Oh look, here comes Mom and Lauren. I'm just going to go sit on the couch and take a little break. My back is killing me."
    S "Oh regarde, voici maman et Lauren. Je vais juste aller m'asseoir sur le canapé et faire une petite pause. Mon dos me tue."

# game/UpdateScriptChristmasEvent.rpy:1105
translate french sidney_prego_too_38dd5f9e_1:

    # S "Hahah.... nice try, [ryan]!"
    S "Hahah.... bon essai, [ryan]!"

# game/UpdateScriptChristmasEvent.rpy:1106
translate french sidney_prego_too_c339a28d:

    # S "I know you're only bringing that up because you like to gloat that you were able to knock up both me and Mom."
    S "Je sais que tu ne fais que parler de ça parce que tu aimes te réjouir d'avoir été capable de me frapper moi et maman."

# game/UpdateScriptChristmasEvent.rpy:1107
translate french sidney_prego_too_66d20398_1:

    # GF "Ahhh! Way to go! Two out of three's not bad at all!"
    GF "Ahhh! Bien joué ! Deux sur trois, c'est pas mal du tout !"

# game/UpdateScriptChristmasEvent.rpy:1108
translate french sidney_prego_too_b29e8e88_1:

    # S "And yes, we all think you're amazing, and yes we're super grateful for all of the shit you've done for us."
    S "Et oui, nous pensons tous que tu es incroyable, et oui nous sommes super reconnaissants pour toute la merde que tu as faite pour nous."

# game/UpdateScriptChristmasEvent.rpy:1110
translate french sidney_prego_too_5c6e6e1c_1:

    # S "I hope you don't keep bringing it up because you don't feel appreciated!"
    S "J'espère que tu ne continues pas à en parler parce que tu ne te sens pas apprécié!"

# game/UpdateScriptChristmasEvent.rpy:1111
translate french sidney_prego_too_0572e331_1:

    # S "Because what you did to get us out from under the thumb of those Mafia goons!.... Well, it was so heroic."
    S "Parce que ce que tu as fait pour nous sortir de sous le pouce de ces crétins de la Mafia!.... C'était tellement héroïque."

# game/UpdateScriptChristmasEvent.rpy:1112
translate french sidney_prego_too_e20f3407_1:

    # S "Not many people are able to shrug off the Mafia like you did, without having to go into witness protection."
    S "Peu de gens sont capables de fuir la Mafia comme tu l'as fait, sans avoir à se mettre sous la protection des témoins."

# game/UpdateScriptChristmasEvent.rpy:1113
translate french sidney_prego_too_69c51d27_1:

    # S "And now, you've gained both their respect and their apathy."
    S "Et maintenant, tu as gagné à la fois leur respect et leur apathie."

# game/UpdateScriptChristmasEvent.rpy:1114
translate french sidney_prego_too_21335f68_1:

    # S "You really should write a book about it someday!"
    S "Tu devrais vraiment écrire un livre à ce sujet un jour!"

# game/UpdateScriptChristmasEvent.rpy:1115
translate french sidney_prego_too_325116af_1:

    # R "Really? Tell me!.... What did I do!?..."
    R "Vraiment? Dis-moi!.... Qu'ai-je fait!?..."

# game/UpdateScriptChristmasEvent.rpy:1117
translate french sidney_prego_too_6546b8b8_1:

    # S "Ohhhhh.... of course you know!.... And there's no need for false modesty."
    S "Ohhhhh.... bien sûr que tu sais!.... Et il n'y a pas besoin de fausse modestie."

# game/UpdateScriptChristmasEvent.rpy:1118
translate french sidney_prego_too_f7484d4f:

    # S "Needless to say. Mom and I, are both very proud to be carrying your babies!"
    S "Inutile de le dire. Maman et moi sommes toutes les deux très fières de porter tes bébés!"

# game/UpdateScriptChristmasEvent.rpy:1119
translate french sidney_prego_too_267cf485_1:

    # S "We don't give a fuck about what other people think."
    S "On s'en fout de ce que les autres pensent."

# game/UpdateScriptChristmasEvent.rpy:1120
translate french sidney_prego_too_b8d2ebed_1:

    # S "Though we still will probably lie about who their father is."
    S "Bien que nous allons encore probablement mentir sur qui est leur père."

# game/UpdateScriptChristmasEvent.rpy:1121
translate french sidney_prego_too_0ad48d46:

    # GF "Yeah, It will be confusing enough to try to explain to them that they are simultaneously aunt or uncle, niece or nephew, and siblings."
    GF "Oui, Ce sera assez déroutant d'essayer de leur expliquer qu'ils sont à la fois tante ou oncle, nièce ou neveu et frères et sœurs."

# game/UpdateScriptChristmasEvent.rpy:1122
translate french sidney_prego_too_6024fd79_1:

    # S "Oh look, here comes Mom and Lauren. I'm just going to go sit on the couch and take a little break. My back is killing me."
    S "Oh regarde, voici maman et Lauren. Je vais juste aller m'asseoir sur le canapé et faire une petite pause. Mon dos me tue."

# game/UpdateScriptChristmasEvent.rpy:1126
translate french sidney_prego_too_38dd5f9e_2:

    # S "Hahah.... nice try, [ryan]!"
    S "Hahah.... bon essai, [ryan]!"

# game/UpdateScriptChristmasEvent.rpy:1127
translate french sidney_prego_too_7561ef78:

    # S "I know you're only bringing that up because you like to gloat that you were able to knock up all three of us."
    S "Je sais que tu n'évoques cela que parce que tu aimes jubiler d'avoir pu nous frapper tous les trois."

# game/UpdateScriptChristmasEvent.rpy:1128
translate french sidney_prego_too_b76b9213:

    # S "Yes, you have amazing swimmers!"
    S "Oui, tu as des nageurs incroyables!"

# game/UpdateScriptChristmasEvent.rpy:1129
translate french sidney_prego_too_90d97660:

    # GF "All three!!!.... You sir, are a fucking legend!"
    GF "Tous les trois!!!.... Tu es une putain de légende !"

# game/UpdateScriptChristmasEvent.rpy:1130
translate french sidney_prego_too_b29e8e88_2:

    # S "And yes, we all think you're amazing, and yes we're super grateful for all of the shit you've done for us."
    S "Et oui, nous pensons tous que tu es incroyable, et oui nous sommes super reconnaissants pour toute la merde que tu as faite pour nous."

# game/UpdateScriptChristmasEvent.rpy:1132
translate french sidney_prego_too_5c6e6e1c_2:

    # S "I hope you don't keep bringing it up because you don't feel appreciated!"
    S "J'espère que tu ne continues pas à en parler parce que tu ne te sens pas apprécié!"

# game/UpdateScriptChristmasEvent.rpy:1133
translate french sidney_prego_too_0572e331_2:

    # S "Because what you did to get us out from under the thumb of those Mafia goons!.... Well, it was so heroic."
    S "Parce que ce que tu as fait pour nous sortir de sous le pouce de ces crétins de la Mafia!.... C'était tellement héroïque."

# game/UpdateScriptChristmasEvent.rpy:1134
translate french sidney_prego_too_e20f3407_2:

    # S "Not many people are able to shrug off the Mafia like you did, without having to go into witness protection."
    S "Peu de gens sont capables de fuir la Mafia comme tu l'as fait, sans avoir à se mettre sous la protection des témoins."

# game/UpdateScriptChristmasEvent.rpy:1135
translate french sidney_prego_too_69c51d27_2:

    # S "And now, you've gained both their respect and their apathy."
    S "Et maintenant, tu as gagné à la fois leur respect et leur apathie."

# game/UpdateScriptChristmasEvent.rpy:1136
translate french sidney_prego_too_21335f68_2:

    # S "You really should write a book about it someday!"
    S "Tu devrais vraiment écrire un livre à ce sujet un jour!"

# game/UpdateScriptChristmasEvent.rpy:1137
translate french sidney_prego_too_325116af_2:

    # R "Really? Tell me!.... What did I do!?..."
    R "Vraiment? Dis-moi!.... Qu'ai-je fait!?..."

# game/UpdateScriptChristmasEvent.rpy:1139
translate french sidney_prego_too_6546b8b8_2:

    # S "Ohhhhh.... of course you know!.... And there's no need for false modesty."
    S "Ohhhhh.... bien sûr que tu sais!.... Et il n'y a pas besoin de fausse modestie."

# game/UpdateScriptChristmasEvent.rpy:1140
translate french sidney_prego_too_e33d9bf0:

    # S "Needless to say. All three of us are very proud to be carrying your babies!"
    S "Inutile de le dire. Nous sommes tous les trois très fiers de porter tes bébés!"

# game/UpdateScriptChristmasEvent.rpy:1141
translate french sidney_prego_too_267cf485_2:

    # S "We don't give a fuck about what other people think."
    S "On s'en fout de ce que les autres pensent."

# game/UpdateScriptChristmasEvent.rpy:1142
translate french sidney_prego_too_b8d2ebed_2:

    # S "Though we still will probably lie about who their father is."
    S "Bien que nous allons encore probablement mentir sur qui est leur père."

# game/UpdateScriptChristmasEvent.rpy:1143
translate french sidney_prego_too_9fe34f58:

    # GF "Yeah, It will be confusing enough to try to explain to them that they are simultaneously aunt or uncle, nieces or nephews, cousins, and siblings.."
    GF "Oui, Ce sera assez déroutant d'essayer de leur expliquer qu'ils sont simultanément tante ou oncle, nièces ou neveux, cousins et frères et sœurs.."

# game/UpdateScriptChristmasEvent.rpy:1144
translate french sidney_prego_too_6024fd79_2:

    # S "Oh look, here comes Mom and Lauren. I'm just going to go sit on the couch and take a little break. My back is killing me."
    S "Oh regarde, voici maman et Lauren. Je vais juste aller m'asseoir sur le canapé et faire une petite pause. Mon dos me tue."

# game/UpdateScriptChristmasEvent.rpy:1149
translate french sidney_prego_too_4e1e603e:

    # GF "Looks like the ladies are all worn out."
    GF "On dirait que les dames sont toutes usées."

# game/UpdateScriptChristmasEvent.rpy:1150
translate french sidney_prego_too_daff3da8:

    # RT "{i}Yeah, I hear pregnancy takes a lot out of you.{/i}"
    RT "{i}Ouais, j'ai entendu dire que la grossesse te prends beaucoup de temps.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1151
translate french sidney_prego_too_ae5b5ba0:

    # R "You girls look beat!"
    R "Vous les filles avez l'air abattues!"

# game/UpdateScriptChristmasEvent.rpy:1154
translate french sidney_prego_too_2aa16dd5:

    # R "Is there anything I can do for any of you?"
    R "Y a-t-il quelque chose que je puisse faire pour l'un d'entre vous?"

# game/UpdateScriptChristmasEvent.rpy:1155
translate french sidney_prego_too_9f41459a:

    # L "Ohh.... that's so sweet!"
    L "Ohh.... c'est tellement gentil!"

# game/UpdateScriptChristmasEvent.rpy:1156
translate french sidney_prego_too_986eb505:

    # S "And we're not totally beat. We're just conserving our energy for when we give you our present."
    S "Et nous ne sommes pas totalement abattus. Nous ne faisons que conserver notre énergie pour t'offrir notre cadeau."

# game/UpdateScriptChristmasEvent.rpy:1157
translate french sidney_prego_too_644f8ad8:

    # GF "I like that sound of that!.... But I will warn you.... you better hurry. Our time here is almost up."
    GF "J'aime ce son de ça!.... Mais je vais te prévenir.... dépêche-toi. Notre temps ici est presque écoulé."

# game/UpdateScriptChristmasEvent.rpy:1158
translate french sidney_prego_too_c086638e:

    # M "So, are you ready now for our present?"
    M "Alors, es-tu prêt maintenant pour notre présent?"

# game/UpdateScriptChristmasEvent.rpy:1159
translate french sidney_prego_too_ad749128:

    # R "More ready than anything!"
    R "Plus prêt que tout!"

# game/UpdateScriptChristmasEvent.rpy:1160
translate french sidney_prego_too_22287755:

    # M "Ok.... all you have to do, is take off your clothes, lie on the coffee table, and pick one of us."
    M "D'accord.... tout ce que tu as à faire, c'est d'enlever tes vêtements, de t'allonger sur la table basse et de choisir l'une de nous."

# game/UpdateScriptChristmasEvent.rpy:1161
translate french sidney_prego_too_d24c7aef:

    # R "Pick one of you?"
    R "Choisir l’un d’entre vous?"

# game/UpdateScriptChristmasEvent.rpy:1162
translate french sidney_prego_too_11f18668:

    # R "What for?"
    R "Pour quoi faire?"

# game/UpdateScriptChristmasEvent.rpy:1163
translate french sidney_prego_too_e909470c:

    # M "Just do as I say. Take off your clothes, lie down, and pick one of us."
    M "Fais ce que je dis. Enlèves tes vêtements, allonges-toi et choisis l'une de nous."

# game/UpdateScriptChristmasEvent.rpy:1164
translate french sidney_prego_too_9907a844:

    # RT "{i}How on earth will I choose?{/i}"
    RT "{i}Comment diable vais-je choisir?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1165
translate french sidney_prego_too_1d39457a:

    # GF "I'd pick Sidney, but then again, I'm slightly biased."
    GF "Je choisirais Sidney, mais là encore, je suis légèrement partial."

# game/UpdateScriptChristmasEvent.rpy:1195
translate french mom_xmas_fuck_5a51504a:

    # RT "{i}This might just be a dream, but to fuck Mom in the ass, makes it the best dream I've ever had.{/i}"
    RT "{i}C'est peut-être juste un rêve, mais baiser maman dans le cul, en fait le meilleur rêve que j'ai jamais eu.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1196
translate french mom_xmas_fuck_ffd1e9d3:

    # S "Wow Mom! You need to teach us to take a cock in the ass like that!"
    S "Wow Maman! Tu dois nous apprendre à prendre une bite dans le cul comme ça!"

# game/UpdateScriptChristmasEvent.rpy:1197
translate french mom_xmas_fuck_609e7073:

    # L "Yeah! [ryan]'s always been too big for us."
    L "Ouais! [ryan] a toujours été trop grand pour nous."

# game/UpdateScriptChristmasEvent.rpy:1227
translate french sidney_xmas_fuck_98ff4791:

    # RT "{i}Wow! I would be fine if this dream never ended.{/i}"
    RT "{i}Wow! Je serais bien si ce rêve ne se terminait jamais.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1228
translate french sidney_xmas_fuck_18f6021b:

    # GF "Or is it a dream?"
    GF "Ou est-ce un rêve?"

# game/UpdateScriptChristmasEvent.rpy:1229
translate french sidney_xmas_fuck_b30b859b:

    # M "Honey! You're doing so good! Not many pussies can take a cock as big as [ryan]'s"
    M "Chérie! Tu te débrouilles si bien ! Peu de chattes peuvent prendre une bite aussi grosse que [ryan]"

# game/UpdateScriptChristmasEvent.rpy:1230
translate french sidney_xmas_fuck_3572d9f7:

    # L "Well, mine can take even bigger."
    L "Eh bien, la mienne peut prendre encore plus grand."

# game/UpdateScriptChristmasEvent.rpy:1260
translate french lauren_xmas_fuck_cf793d87:

    # RT "{i}Well, if this is a dream, I have a feeling it's going to be a wet one.{/i}"
    RT "{i}Eh bien, si c'est un rêve, j'ai l'impression que ça va être humide.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1261
translate french lauren_xmas_fuck_8617aaf9:

    # S "Lauren, you've got the cutest little asshole!"
    S "Lauren, tu as le plus mignon petit trou du cul!"

# game/UpdateScriptChristmasEvent.rpy:1262
translate french lauren_xmas_fuck_08b8d7a0:

    # M "The trick is to get it so it can take a huge dick, but still look cute. It's not easy."
    M "L'astuce consiste à l'obtenir pour qu'il puisse prendre une énorme bite, mais toujours jolie. Ce n'est pas facile."

# game/UpdateScriptChristmasEvent.rpy:1276
translate french xmas_facial_85269cab:

    # M "Ok, baby!.... Drench us in your baby gravy!"
    M "Ok, bébé!.... Trempes-nous dans ta sauce pour bébé!"

# game/UpdateScriptChristmasEvent.rpy:1277
translate french xmas_facial_58161305:

    # S "Spray me first!"
    S "Vaporises-moi d'abord!"

# game/UpdateScriptChristmasEvent.rpy:1278
translate french xmas_facial_dd00ce0b:

    # L "No, me first!"
    L "Non, moi d'abord!"

# game/UpdateScriptChristmasEvent.rpy:1279
translate french xmas_facial_c9adaa4d:

    # R "Mom always gets to go first"
    R "Maman arrive toujours à partir en premier"

# game/UpdateScriptChristmasEvent.rpy:1280
translate french xmas_facial_2956163d:

    # R "Ohhhh.... here I cummmm!!!..."
    R "Ohhhh.... ici, je joooouuuiiiisss!!!..."

# game/UpdateScriptChristmasEvent.rpy:1301
translate french xmas_facial_0b2dfb15:

    # M "Good job my big boy!"
    M "Bon travail mon grand garçon!"

# game/UpdateScriptChristmasEvent.rpy:1302
translate french xmas_facial_7691efcf:

    # S "I just wish you'd covered me even more."
    S "J'aimerais que tu me couvres encore plus."

# game/UpdateScriptChristmasEvent.rpy:1303
translate french xmas_facial_562ef639:

    # L "Yeah.... usually you cum almost twice this much."
    L "Ouais.... habituellement, tu jouis presque deux fois plus."

# game/UpdateScriptChristmasEvent.rpy:1304
translate french xmas_facial_d51f4b98:

    # RT "{i}Is she fucking kidding me?{/i}"
    RT "{i}Elle se moque de moi ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1305
translate french xmas_facial_301d1a51:

    # RT "{i}I've never cum so much in my life.{/i}"
    RT "{i}Je n'ai jamais autant joui de ma vie.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1306
translate french xmas_facial_fe1249c7:

    # M "Now, are you ready to watch us make out, and lick all of this cum off each other?"
    M "Maintenant, es-tu prêt à nous regarder nous embrasser et nous lécher tout ce sperme les unes des autres?"

# game/UpdateScriptChristmasEvent.rpy:1307
translate french xmas_facial_ad6af4ee:

    # R "Oh God, yes!"
    R "Oh Mon Dieu, oui!"

# game/UpdateScriptChristmasEvent.rpy:1308
translate french xmas_facial_886e81e8:

    # S "Then we can grab the dildos and really get down to business!"
    S "Ensuite, nous pouvons attraper les godes et vraiment nous mettre au travail!"

# game/UpdateScriptChristmasEvent.rpy:1309
translate french xmas_facial_b901ef70:

    # L "Yay! A whole day of fucking!.... I love Christmas!"
    L "Ouah! Toute une journée de baise!.... J'adore Noël!"

# game/UpdateScriptChristmasEvent.rpy:1310
translate french xmas_facial_867a14a3:

    # GF "Sorry buddy, but you won't get to do any of that. Our time is up in 5.... 4..."
    GF "Désolé mon pote, mais tu n'auras rien à faire de tout ça. Notre temps est écoulé dans 5.... 4..."

# game/UpdateScriptChristmasEvent.rpy:1311
translate french xmas_facial_1db1b01a:

    # RT "{i}Noooo!.... Not yet!....{/i}"
    RT "{i}Noooonnn!.... Pas encore !....{/i}"

# game/UpdateScriptChristmasEvent.rpy:1312
translate french xmas_facial_f7f9aced:

    # GF "3.... 2..."
    GF "3.... 2..."

# game/UpdateScriptChristmasEvent.rpy:1313
translate french xmas_facial_5685aaa9:

    # RT "{i}Goodbye, my beauties! We'll continue this in the future!{/i}"
    RT "{i}Au revoir, mes beautés ! Nous continuerons cela plus tard!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1314
translate french xmas_facial_c8f267f6:

    # GF "1.... 0..."
    GF "1.... 0..."

# game/UpdateScriptChristmasEvent.rpy:1335
translate french cookies_in_oven_42d37d6b:

    # R "Oh, man! Look how Lauren's dressing now, just to do something as casual as making cookies."
    R "Oh, mec! Regarde comment Lauren s'habille maintenant, juste pour faire quelque chose d'aussi décontracté que de faire des biscuits."

# game/UpdateScriptChristmasEvent.rpy:1337
translate french cookies_in_oven_25e4e982:

    # R "At least this future isn't completely bad."
    R "Au moins, cet avenir n'est pas complètement mauvais."

# game/UpdateScriptChristmasEvent.rpy:1339
translate french cookies_in_oven_17dcb6e6:

    # R "I'm really liking this future so far."
    R "J'aime vraiment cet avenir jusqu'à présent."

# game/UpdateScriptChristmasEvent.rpy:1340
translate french cookies_in_oven_9d160ac8:

    # R "Good morning Lauren! Are the cookies ready yet?"
    R "Bonjour Lauren! Les cookies sont-ils prêts ?"

# game/UpdateScriptChristmasEvent.rpy:1341
translate french cookies_in_oven_300ca792:

    # L "Good morning [ryan]! Merry Christmas! I'm just getting the cookies in the oven, so they're going to be a few minutes."
    L "Bonjour [ryan]! Joyeux Noël! Je vais juste mettre les biscuits au four, donc ça va prendre quelques minutes."

# game/UpdateScriptChristmasEvent.rpy:1344
translate french cookies_in_oven_515944cb:

    # L "Ok.... I'll set the timer for twelve minutes."
    L "D'accord.... Je vais régler la minuterie pour douze minutes."

# game/UpdateScriptChristmasEvent.rpy:1347
translate french cookies_in_oven_5be51525:

    # L "Wow, cooking makes me get so hot when I'm like this!"
    L "Wow, la cuisine me fait tellement chaud quand je suis comme ça!"

# game/UpdateScriptChristmasEvent.rpy:1348
translate french cookies_in_oven_ce90d005:

    # RT "{i}No.... Fucking.... way!{/i}"
    RT "{i}Non.... Putain.... de manière!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1352
translate french cookies_in_oven_b897173c:

    # GF "Hahaha.... unbelievable!"
    GF "Hahaha.... Incroyable!"

# game/UpdateScriptChristmasEvent.rpy:1353
translate french cookies_in_oven_64eb836e:

    # RT "{i}You're telling me!{/i}"
    RT "{i}Tu me le dis !{/i}"

# game/UpdateScriptChristmasEvent.rpy:1354
translate french cookies_in_oven_8bc1e794:

    # RT "{i}I've.... I've got to be the father of this one, I'm sure!{/i}"
    RT "{i}J'ai.... Je dois être le père ici, j'en suis sûr !{/i}"

# game/UpdateScriptChristmasEvent.rpy:1355
translate french cookies_in_oven_0e567838:

    # GF "Ohhhh.... I don't know.... I'd tread lightly here."
    GF "Ohhhh.... Je ne sais pas.... Je marcherais légèrement ici."

# game/UpdateScriptChristmasEvent.rpy:1358
translate french cookies_in_oven_8a3d7dca:

    # R "So.... hey, Lauren!"
    R "Donc.... hé, Lauren!"

# game/UpdateScriptChristmasEvent.rpy:1359
translate french cookies_in_oven_02b36e49:

    # L "Yeah?..."
    L "Ouais?..."

# game/UpdateScriptChristmasEvent.rpy:1360
translate french cookies_in_oven_fa81b153:

    # R "What was the due date for your baby again?"
    R "Quelle était la date d'échéance pour ton bébé à nouveau?"

# game/UpdateScriptChristmasEvent.rpy:1363
translate french cookies_in_oven_9b48aef8:

    # L "Hmmmmm.... It's January 23rd, so I've got about a month left of being pregnant!"
    L "Hmmmmm.... C'est le 23 janvier, donc il me reste environ un mois de grossesse!"

# game/UpdateScriptChristmasEvent.rpy:1364
translate french cookies_in_oven_210cc05c:

    # L "Thank God.... I can't wait to get this thing out of me."
    L "Dieu merci.... J'ai hâte de m'en débarrasser."

# game/UpdateScriptChristmasEvent.rpy:1365
translate french cookies_in_oven_4241bc96:

    # L "But I guess I can't complain too much.... look!"
    L "Mais je suppose que je ne peux pas trop me plaindre.... Regarde!"

# game/UpdateScriptChristmasEvent.rpy:1368
translate french cookies_in_oven_a558aa88:

    # L "My tits have gotten even bigger in the last couple days."
    L "Mes seins sont devenus encore plus gros au cours des deux derniers jours."

# game/UpdateScriptChristmasEvent.rpy:1369
translate french cookies_in_oven_cd96d5a4:

    # L "Remember when they looked like swollen mosquito bites?"
    L "Rappelles-toi quand ils ressemblaient à des piqûres de moustiques enflées?"

# game/UpdateScriptChristmasEvent.rpy:1370
translate french cookies_in_oven_05d8f0bd:

    # R "Uhhh.... haha.... yeah.... It feels like yesterday."
    R "Uhhh.... haha.... ouais.... C'est comme hier."

# game/UpdateScriptChristmasEvent.rpy:1373
translate french cookies_in_oven_303e93c0:

    # L "Hhhhnnnnnn.... I just hope.... they get big enough to provide an entire meal for the baby."
    L "Hhhhnnnnnnn.... J'espère juste.... qu'ils deviennent assez grands pour fournir un repas entier au bébé."

# game/UpdateScriptChristmasEvent.rpy:1374
translate french cookies_in_oven_14596be5:

    # R "Do you want some help there?"
    R "Veux-tu de l'aide?"

# game/UpdateScriptChristmasEvent.rpy:1375
translate french cookies_in_oven_28011129:

    # L "hhnnn.... no.... I've got this."
    L "hhnnn.... non... Je l'ai compris."

# game/UpdateScriptChristmasEvent.rpy:1378
translate french cookies_in_oven_ad303024:

    # L "There.... I made it up!"
    L "Voilà... Je l'ai fait !"

# game/UpdateScriptChristmasEvent.rpy:1379
translate french cookies_in_oven_3b7e4402:

    # R "Wow! What a great accomplishment!"
    R "Wow ! Quelle belle réussite !"

# game/UpdateScriptChristmasEvent.rpy:1380
translate french cookies_in_oven_858089af:

    # L "Haha.... you smart ass!"
    L "Haha.... espèce d'idiot !"

# game/UpdateScriptChristmasEvent.rpy:1381
translate french cookies_in_oven_f1aa0898:

    # L "I hate these tall chairs now."
    L "Je déteste ces chaises hautes maintenant."

# game/UpdateScriptChristmasEvent.rpy:1382
translate french cookies_in_oven_2befb1d3:

    # R "So, have we settled on any names for the baby yet?"
    R "Alors, avons-nous encore des noms pour le bébé?"

# game/UpdateScriptChristmasEvent.rpy:1385
translate french cookies_in_oven_97e8db39:

    # L "Yeah.... remember? We talked about this."
    L "Ouais.... tu te souviens ? On en a parlé."

# game/UpdateScriptChristmasEvent.rpy:1386
translate french cookies_in_oven_7f897086:

    # R "Yeah, just refresh my memory."
    R "Ouais, rafraîchis-moi la mémoire."

# game/UpdateScriptChristmasEvent.rpy:1387
translate french cookies_in_oven_acc055e6:

    # L "If it's a boy, we'll name him [ryan], and if it's a girl, we'll name her after Mom."
    L "Si c'est un garçon, nous le nommerons [ryan], et si c'est une fille, nous la nommerons d'après maman."

# game/UpdateScriptChristmasEvent.rpy:1388
translate french cookies_in_oven_0754245a:

    # R "So [ryan].... after his..."
    R "Donc [ryan].... après le sien..."

# game/UpdateScriptChristmasEvent.rpy:1390
translate french cookies_in_oven_75b94cab:

    # L "Uncle.... you weirdo."
    L "Oncle.... espèce de cinglé."

# game/UpdateScriptChristmasEvent.rpy:1391
translate french cookies_in_oven_83b3d3b0:

    # GF "Haha.... that was smooth!"
    GF "Haha.... c'était lisse!"

# game/UpdateScriptChristmasEvent.rpy:1392
translate french cookies_in_oven_7a0aa253:

    # R "His uncle? You mean I'm not the?..."
    R "Son oncle? Tu veux dire que je ne suis pas le?..."

# game/UpdateScriptChristmasEvent.rpy:1395
translate french cookies_in_oven_18532eb7:

    # L "[ryan]! What the hell is wrong with you?"
    L "[ryan]! Qu'est-ce qui ne va pas chez toi ?"

# game/UpdateScriptChristmasEvent.rpy:1396
translate french cookies_in_oven_9ae63b5a:

    # L "Did you fall off your scooter and hit your head, or something?"
    L "Est-ce que tu es tombé de ton scooter et que tu t'es cogné la tête, ou quelque chose comme ça?"

# game/UpdateScriptChristmasEvent.rpy:1397
translate french cookies_in_oven_8fd5cad9:

    # R "I'm sorry.... just remind me.... who's the father of your baby?"
    R "Je suis désolé.... rappelle-moi juste.... qui est le père de tonn bébé?"

# game/UpdateScriptChristmasEvent.rpy:1398
translate french cookies_in_oven_fd705c6e:

    # GF "Well, that went from smooth to cringy in just a few seconds."
    GF "Eh bien, cela est passé de lisse à grinçant en quelques secondes."

# game/UpdateScriptChristmasEvent.rpy:1399
translate french cookies_in_oven_dbcd146d:

    # L "Is this because you're still angry at Agent Diaz?"
    L "Est-ce parce que tu es toujours en colère contre l'agent Diaz?"

# game/UpdateScriptChristmasEvent.rpy:1400
translate french cookies_in_oven_6b3a324e:

    # R "What does Diaz have to do with any of this?"
    R "Qu'est-ce que Diaz a à voir avec tout cela?"

# game/UpdateScriptChristmasEvent.rpy:1401
translate french cookies_in_oven_941acc92:

    # L "You know you've always been sore that we didn't let you father our child."
    L "Tu sais que tu as toujours été endolori que nous ne t'ayons pas laissé père de notre enfant."

# game/UpdateScriptChristmasEvent.rpy:1402
translate french cookies_in_oven_ec6cbd1d:

    # R "Our child.... you mean that you and Diaz are.... together?"
    R "Notre enfant.... tu veux dire que toi et Diaz l'êtes.... ensemble ?"

# game/UpdateScriptChristmasEvent.rpy:1405
translate french cookies_in_oven_9fb2b489:

    # L "Yep! We've been married for three weeks now."
    L "Oui! Nous sommes mariés depuis trois semaines maintenant."

# game/UpdateScriptChristmasEvent.rpy:1406
translate french cookies_in_oven_b1e34d58:

    # L "You were one of the witnesses."
    L "Tu étais l'un des témoins."

# game/UpdateScriptChristmasEvent.rpy:1407
translate french cookies_in_oven_6ca4704a:

    # L "Ohh.... this is some kind of joke or game isn't it."
    L "Ohh.... c'est une sorte de blague ou de jeu n'est-ce pas."

# game/UpdateScriptChristmasEvent.rpy:1408
translate french cookies_in_oven_92af3cdf:

    # R "Uhhh.... kind of.... I'm actually.... trying to document all of the important events that happened this year."
    R "Uhhh.... en quelque sorte.... Je le suis en fait.... essayer de documenter tous les événements importants qui se sont produits cette année."

# game/UpdateScriptChristmasEvent.rpy:1409
translate french cookies_in_oven_2869afd0:

    # R "You know, so we won't forget them in the future. I know a lot has happened to us these last couple years."
    R "Tu sais, donc nous ne les oublierons pas à l'avenir. Je sais que beaucoup de choses nous sont arrivées ces deux dernières années."

# game/UpdateScriptChristmasEvent.rpy:1410
translate french cookies_in_oven_d022717e:

    # R "So, if I ask you something that sounds like I should already know, just remember it's for the record."
    R "Donc, si je te demande quelque chose que je devrais déjà savoir, rappelles-toi simplement que c'est pour mémoire."

# game/UpdateScriptChristmasEvent.rpy:1411
translate french cookies_in_oven_0af0838a:

    # R "Here.... I'm recording you on my phone. Now will you please state for the record, how you got pregnant."
    R "Ici.... Je t'enregistre sur mon téléphone. Maintenant, veux-tu dire pour le compte rendu, comment tu es tombée enceinte."

# game/UpdateScriptChristmasEvent.rpy:1412
translate french cookies_in_oven_63be8f10:

    # R "And please give all the details."
    R "Et donnes tous les détails."

# game/UpdateScriptChristmasEvent.rpy:1415
translate french cookies_in_oven_59ebf449:

    # L "Well.... agent Diaz wanted our baby to have half of her genetics. So, we were going to have her brother fertilize one of my eggs."
    L "Bien.... l'agent Diaz voulait que notre bébé ait la moitié de sa génétique. Donc, nous allions demander à son frère de féconder un de mes ovules."

# game/UpdateScriptChristmasEvent.rpy:1416
translate french cookies_in_oven_cfd6e0e1:

    # L "You refused to pay for the treatment, so we did it the cheap and dirty way."
    L "Tu as refusé de payer le traitement, alors nous l'avons fait de manière sale et bon marché."

# game/UpdateScriptChristmasEvent.rpy:1417
translate french cookies_in_oven_f2cc61bb:

    # L "So, I fucked her older brother every day for two months..."
    L "J'ai donc baisé son frère aîné tous les jours pendant deux mois..."

# game/UpdateScriptChristmasEvent.rpy:1418
translate french cookies_in_oven_c0d0f83a:

    # GF "Oooohhhhh..."
    GF "Oooohhhhhh..."

# game/UpdateScriptChristmasEvent.rpy:1419
translate french cookies_in_oven_7bc564e4:

    # R "What?!.... Are you serious!?"
    R "Quoi?! .... T’es sérieux!?"

# game/UpdateScriptChristmasEvent.rpy:1420
translate french cookies_in_oven_27439301:

    # R "And I was ok with this?"
    R "Et j'étais d'accord avec ça?"

# game/UpdateScriptChristmasEvent.rpy:1421
translate french cookies_in_oven_1fd9872f:

    # L "Well, we didn't ask you."
    L "Eh bien, nous ne te l'avons pas demandé."

# game/UpdateScriptChristmasEvent.rpy:1422
translate french cookies_in_oven_3ee0ff4b:

    # L "So, when her older brother couldn't knock me up, we let her younger brother give it a try."
    L "Alors, quand son frère aîné n'a pas pu me sauter, nous avons laissé son frère cadet essayer."

# game/UpdateScriptChristmasEvent.rpy:1423
translate french cookies_in_oven_c5c6418a:

    # L "A few months later, and still no baby."
    L "Quelques mois plus tard, et toujours pas de bébé."

# game/UpdateScriptChristmasEvent.rpy:1424
translate french cookies_in_oven_5e1b91fd:

    # L "So, Diaz's father volunteered to knock me up."
    L "Donc, le père de Diaz s'est porté volontaire pour me baiser."

# game/UpdateScriptChristmasEvent.rpy:1427
translate french cookies_in_oven_d34288cf:

    # L "So, I fucked him for a while."
    L "Donc, je l'ai baisé pendant un moment."

# game/UpdateScriptChristmasEvent.rpy:1428
translate french cookies_in_oven_f603c571:

    # L "And for good measure I kept fucking her brothers too. Sometimes all of them at the same time."
    L "Et pour faire bonne mesure, j'ai continué à baiser ses frères aussi. Parfois tous en même temps."

# game/UpdateScriptChristmasEvent.rpy:1429
translate french cookies_in_oven_9cb5c0ca:

    # L "And so I wouldn't feel too awkward, Agent Diaz would join us sometimes."
    L "Et pour que je ne me sente pas trop gênée, l'agent Diaz nous rejoignait parfois."

# game/UpdateScriptChristmasEvent.rpy:1430
translate french cookies_in_oven_e0fd3193:

    # R "Holy shit!"
    R "Putain de merde!"

# game/UpdateScriptChristmasEvent.rpy:1433
translate french cookies_in_oven_dfb9def3:

    # L "Is this detailed enough for you?"
    L "Est-ce assez détaillé pour toi?"

# game/UpdateScriptChristmasEvent.rpy:1434
translate french cookies_in_oven_4f4abfd5:

    # R "Yeah, just skip to the part where you got pregnant."
    R "Oui, il suffit de passer à la partie où tu es tombé enceinte."

# game/UpdateScriptChristmasEvent.rpy:1435
translate french cookies_in_oven_65b3db75:

    # L "Well, there's not much more to the story."
    L "Eh bien, il n'y a pas grand-chose de plus dans l'histoire."

# game/UpdateScriptChristmasEvent.rpy:1436
translate french cookies_in_oven_856f2d49:

    # L "Eventually one of them slipped one past the goalie, and now I'm pregnant with one of their babies."
    L "Finalement, l'un d'eux en a glissé un devant le gardien de but, et maintenant je suis enceinte d'un de leurs bébés."

# game/UpdateScriptChristmasEvent.rpy:1437
translate french cookies_in_oven_fee8246c:

    # R "And you don't even know which one of them got you pregnant?"
    R "Et tu ne sais même pas lequel d'entre eux t'a fait tomber enceinte?"

# game/UpdateScriptChristmasEvent.rpy:1438
translate french cookies_in_oven_cb6f8ada:

    # L "It doesn't matter. It's Agent Diaz's and my baby not theirs."
    L "Peu importe. C'est à l'agent Diaz et c'est mon bébé pas le leurs."

# game/UpdateScriptChristmasEvent.rpy:1439
translate french cookies_in_oven_b6dfb8b4:

    # R "You guys are married, and you still call her Agent Diaz?"
    R "Tu es marié, et tu l'appelles toujours agent Diaz?"

# game/UpdateScriptChristmasEvent.rpy:1440
translate french cookies_in_oven_641159f3:

    # L "Well, yeah! It's respectful. I call her by her first name when we're in our apartment."
    L "Eh bien, oui! C'est respectueux. Je l'appelle par son prénom quand on est dans notre appartement."

# game/UpdateScriptChristmasEvent.rpy:1441
translate french cookies_in_oven_d4a66d19:

    # R "So, what is her first name?"
    R "Alors, quel est son prénom ?"

# game/UpdateScriptChristmasEvent.rpy:1444
translate french cookies_in_oven_60e17e3a:

    # L "I've already told you several times. If you can't remember, you're on your own."
    L "Je te l'ai déjà dit plusieurs fois. Si tu ne t'en souviens pas, tu es le seul."

# game/UpdateScriptChristmasEvent.rpy:1445
translate french cookies_in_oven_4e48cd6f:

    # R "This is for the record."
    R "Ceci est pour l'enregistrement."

# game/UpdateScriptChristmasEvent.rpy:1446
translate french cookies_in_oven_60e49b42:

    # L "well then state it for the record yourself."
    L "eh bien, indiques-le toi-même pour l'enregistrement."

# game/UpdateScriptChristmasEvent.rpy:1447
translate french cookies_in_oven_240f04cc:

    # R "Ok.... never mind."
    R "D'accord.... Ça ne fait rien."

# game/UpdateScriptChristmasEvent.rpy:1448
translate french cookies_in_oven_3374bc65:

    # R "Are you screwing anybody else?"
    R "Es-tu en train de baiser quelqu'un d'autre?"

# game/UpdateScriptChristmasEvent.rpy:1449
translate french cookies_in_oven_2af71d35:

    # L "Of course not!.... I only screwed Agent Diaz's brothers and father to get pregnant."
    L "Bien sûr que non!.... J'ai seulement baisé les frères et le père de l'agent Diaz pour tomber enceinte."

# game/UpdateScriptChristmasEvent.rpy:1450
translate french cookies_in_oven_b570d46e:

    # L "Although, to be totally honest, I'm still screwing them, so that their swimmers will still be strong when we're ready for baby number two."
    L "Bien que, pour être tout à fait honnête, je les baise encore, pour que leurs nageurs soient toujours forts quand nous serons prêts pour le bébé numéro deux."

# game/UpdateScriptChristmasEvent.rpy:1451
translate french cookies_in_oven_29dcfe45:

    # R "That doesn't make any sense."
    R "Cela n'a aucun sens."

# game/UpdateScriptChristmasEvent.rpy:1452
translate french cookies_in_oven_b05d712c:

    # L "Yeah it does. It makes perfect sense. I'll just have Agent Diaz explain it to you, the same way she explained it to me. You'll see."
    L "Oui, c'est le cas. C'est parfaitement logique. Je demanderai à l'agent Diaz de te l'expliquer, comme elle me l'a expliqué. Tu verras."

# game/UpdateScriptChristmasEvent.rpy:1453
translate french cookies_in_oven_013d2019:

    # GF "Holy shit! I thought you were the DaVinci of bullshit, but you're just a toddler compared with her!"
    GF "Putain de merde! Je pensais que tu étais le DaVinci des conneries, mais tu n'es qu'un bambin comparé à elle!"

# game/UpdateScriptChristmasEvent.rpy:1454
translate french cookies_in_oven_c61a17a1:

    # RT "{i}No shit!.... Once I go back to the present, I've got to remember to get that toxic woman out of our lives.{/i}"
    RT "{i}Pas de connerie !.... Une fois revenu au présent, je dois me souvenir de sortir cette femme toxique de nos vies.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1455
translate french cookies_in_oven_b3c9e61a:

    # RT "{i}What the hell? Screwing the entire Diaz family?.... I'm pretty sure she's still a virgin in the present.... hmmmm.... I wonder if....{/i}"
    RT "{i}C'est quoi ce bordel? Baiser toute la famille de Diaz?.... Je suis presque sûr qu'elle est encore vierge dans le présent.... hmmmm.... Je me demande si....{/i}"

# game/UpdateScriptChristmasEvent.rpy:1456
translate french cookies_in_oven_bc24cf93:

    # R "So.... since you got married. Are we ever.... you know..."
    R "Donc.... depuis votre mariage. Sommes-nous jamais.... tu sais..."

# game/UpdateScriptChristmasEvent.rpy:1457
translate french cookies_in_oven_e4128083:

    # L "Intimate?"
    L "Intime?"

# game/UpdateScriptChristmasEvent.rpy:1458
translate french cookies_in_oven_2948575d:

    # R "Yeah."
    R "Ouais."

# game/UpdateScriptChristmasEvent.rpy:1459
translate french cookies_in_oven_4206f7d5:

    # L "Oh.... of course! Diaz knows I would have never married her if she tried to take away what we have."
    L "Oh.... Pardi! Diaz sait que je ne l'aurais jamais épousée si elle avait essayé de nous enlever ce que nous avons."

# game/UpdateScriptChristmasEvent.rpy:1460
translate french cookies_in_oven_f8948aa8:

    # L "And I know you enjoy it when Diaz joins in."
    L "Et je sais que tu aimes quand Diaz se joint à nous."

# game/UpdateScriptChristmasEvent.rpy:1461
translate french cookies_in_oven_efc70f52:

    # L "Although I'm pretty sure you just like to hate fuck her."
    L "Bien que je sois presque sûr que tu aimes juste détester la baiser."

# game/UpdateScriptChristmasEvent.rpy:1462
translate french cookies_in_oven_6b85a50a:

    # GF "No wonder!"
    GF "Pas étonnant!"

# game/UpdateScriptChristmasEvent.rpy:1465
translate french cookies_in_oven_3d63ffd6:

    # L "She told me to tell you that if you ever fuck her ass that hard again, she's going to have an FBI drug dog bite your cock off."
    L "Elle m'a dit de te dire que si jamais tu la baises à nouveau aussi fort, elle va avoir un chien de drogue du FBI qui te mordra la bite."

# game/UpdateScriptChristmasEvent.rpy:1466
translate french cookies_in_oven_ff8a5a20:

    # GF "Well, at least you have that perk."
    GF "Eh bien, au moins tu as cet avantage."

# game/UpdateScriptChristmasEvent.rpy:1467
translate french cookies_in_oven_6837d0d1:

    # R "But if.... we're intimate.... how can you be sure the baby's not mine?"
    R "Mais si.... on est intimes.... comment peux-tu être sûr que le bébé n'est pas de moi?"

# game/UpdateScriptChristmasEvent.rpy:1468
translate french cookies_in_oven_7365351c:

    # L "Because we made you wear a condom until I was pregnant."
    L "Parce qu'on t'a fait porter un préservatif jusqu'à ce que je sois enceinte."

# game/UpdateScriptChristmasEvent.rpy:1469
translate french cookies_in_oven_e4b301f5:

    # L "So, unless you were poking holes in the condom."
    L "Donc, à moins que tu ne percais des trous dans le préservatif."

# game/UpdateScriptChristmasEvent.rpy:1470
translate french cookies_in_oven_54df3aab:

    # L "And I warn you that you better not have!"
    L "Et je te préviens que tu ferais mieux de ne pas l'avoir fait!"

# game/UpdateScriptChristmasEvent.rpy:1471
translate french cookies_in_oven_7f0786d3:

    # L "If Agent Diaz suspects that child isn't half hers, she'll do a DNA test, and if you're the father, she'll deport both of you to Cuba."
    L "Si l'Agent Diaz soupçonne que l'enfant n'est pas à moitié le sien, elle fera un test ADN, et si tu es le père, elle vous expulsera tous les deux à Cuba."

# game/UpdateScriptChristmasEvent.rpy:1472
translate french cookies_in_oven_7e368287:

    # GF "Agent Diaz makes the Mafia look like the boy scouts."
    GF "L'agent Diaz fait ressembler la Mafia aux scouts."

# game/UpdateScriptChristmasEvent.rpy:1473
translate french cookies_in_oven_10017616:

    # DING"
    "DING"

# game/UpdateScriptChristmasEvent.rpy:1474
translate french cookies_in_oven_58552e0f:

    # L "Oh, there goes the cookies. Why don't you run and see if Sidney's almost ready for the gift exchange, and I'll get the cookies ready, and bring them out when I'm done."
    L "Oh, voilà les cookies. Pourquoi ne courrais-tu pas voir si Sidney est presque prêt pour l'échange de cadeaux, et je préparerai les biscuits, et je les sortirai quand j'aurai fini."

# game/UpdateScriptChristmasEvent.rpy:1475
translate french cookies_in_oven_2e2caf83:

    # R "Yeah, ok."
    R "Oui, d'accord."

# game/UpdateScriptChristmasEvent.rpy:1478
translate french cookies_in_oven_d858d039:

    # L "Ohhh.... these stupid, tall chairs!"
    L "Ohhh.... ces stupides chaises hautes!"

# game/UpdateScriptChristmasEvent.rpy:1479
translate french cookies_in_oven_2ff4ef7f:

    # L "And where did I throw that apron?.... Baking in the buff is not a good idea."
    L "Et où ai-je jeté ce tablier?.... La cuisson dans le buff n'est pas une bonne idée."

# game/UpdateScriptChristmasEvent.rpy:1480
translate french cookies_in_oven_8a9f9b57:

    # RT "{i}Shit!.... I can't even believe what Lauren just told me!{/i}"
    RT "{i}Merde!.... Je n'arrive même pas à croire ce que Lauren vient de me dire !{/i}"

# game/UpdateScriptChristmasEvent.rpy:1481
translate french cookies_in_oven_42abb657:

    # RT "{i}How am I going to get rid of that woman, before she can do that much damage? She's got my balls in a vice as it is! I've got to search for some kind of opportunity.{/i}"
    RT "{i}Comment vais-je me débarrasser de cette femme, avant qu'elle puisse faire autant de dégâts? Elle me met les couilles dans un étau en faisant ça ! Je dois chercher une opportunité.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1482
translate french cookies_in_oven_e634a1eb:

    # GF "Or at least help Lauren see how evil she is."
    GF "Ou au moins aider Lauren à voir à quel point elle est diabolique."

# game/UpdateScriptChristmasEvent.rpy:1485
translate french cookies_in_oven_fa3c1d20:

    # RT "{i}That's definitely something from the future that has to change!{/i}"
    RT "{i}C'est certainement quelque chose du futur qui doit changer!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1486
translate french cookies_in_oven_579b2ef2:

    # GF "Yeah that sucks!.... Should we go see what the damage is with Sidney?"
    GF "Ouais ça craint!.... On va voir les dégâts avec Sidney ?"

# game/UpdateScriptChristmasEvent.rpy:1489
translate french cookies_in_oven_37224b02:

    # RT "{i}Yeah, although at this point, I'm almost afraid to ask.{/i}"
    RT "{i}Oui, bien qu'à ce stade, j'ai presque peur de demander.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1494
translate french cookies_in_oven_65847b74:

    # L "Father, you weirdo!"
    L "Père, espèce de cinglé!"

# game/UpdateScriptChristmasEvent.rpy:1495
translate french cookies_in_oven_cd47a34d:

    # GF "Haha.... that was pretty smooth."
    GF "Haha.... c'était plutôt fluide."

# game/UpdateScriptChristmasEvent.rpy:1496
translate french cookies_in_oven_8f1622e6:

    # RT "{i}Thanks.{/i}"
    RT "{i}Merci.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1497
translate french cookies_in_oven_dc96db1f:

    # R "And are you ok with getting pregnant from your brother?"
    R "Et es-tu d'accord pour tomber enceinte de ton frère?"

# game/UpdateScriptChristmasEvent.rpy:1498
translate french cookies_in_oven_f4dba509:

    # L "[ryan]!.... We've already talked about this!"
    L "[ryan]!.... Nous en avons déjà parlé !"

# game/UpdateScriptChristmasEvent.rpy:1499
translate french cookies_in_oven_64acbe9a:

    # L "Is this some kind of trick, or Christmas game?"
    L "Est-ce une sorte de tour, ou un jeu de Noël?"

# game/UpdateScriptChristmasEvent.rpy:1500
translate french cookies_in_oven_92af3cdf_1:

    # R "Uhhh.... kind of.... I'm actually.... trying to document all of the important events that happened this year."
    R "Uhhh.... en quelque sorte.... Je le suis en fait.... essayer de documenter tous les événements importants qui se sont produits cette année."

# game/UpdateScriptChristmasEvent.rpy:1501
translate french cookies_in_oven_2869afd0_1:

    # R "You know, so we won't forget them in the future. I know a lot has happened to us these last couple years."
    R "Tu sais, donc nous ne les oublierons pas à l'avenir. Je sais que beaucoup de choses nous sont arrivées ces deux dernières années."

# game/UpdateScriptChristmasEvent.rpy:1502
translate french cookies_in_oven_d022717e_1:

    # R "So, if I ask you something that sounds like I should already know, just remember it's for the record."
    R "Donc, si je te demande quelque chose que je devrais déjà savoir, rappelles-toi simplement que c'est pour mémoire."

# game/UpdateScriptChristmasEvent.rpy:1503
translate french cookies_in_oven_0406b030:

    # R "Here.... I'm recording you on my phone. Now will you please state for the record, the story of your pregnancy."
    R "Ici.... Je t'enregistre sur mon téléphone. Maintenant, vas-tu indiquer pour le compte rendu, l'histoire de ta grossesse."

# game/UpdateScriptChristmasEvent.rpy:1504
translate french cookies_in_oven_e560be0c:

    # L "Well, it's not much of a story."
    L "Eh bien, ce n'est pas vraiment une histoire."

# game/UpdateScriptChristmasEvent.rpy:1505
translate french cookies_in_oven_5b560295:

    # L "I just kept falling more and more in love with you, as I watched you work so hard to take care of us."
    L "Je ne cessais de tomber de plus en plus amoureuse de toi, alors que je te regardais travailler si dur pour prendre soin de nous."

# game/UpdateScriptChristmasEvent.rpy:1506
translate french cookies_in_oven_b7a8afd5:

    # L "Even though at any point, you could have done the easy thing, and left us to care for ourselves."
    L "Même si à tout moment, tu aurais pu faire la chose facile et nous laisser prendre soin de nous-mêmes."

# game/UpdateScriptChristmasEvent.rpy:1509
translate french cookies_in_oven_011cdeeb:

    # L "But I think the moment I was head over heels for you, was when you finally were able to get Agent Diaz off our backs, or I guess out of my pants in my case."
    L "Mais je pense que le moment où j'ai été éperdue pour toi, c'est quand tu as finalement pu retirer l'Agent Diaz de notre dos, ou je suppose que de mon pantalon dans mon cas."

# game/UpdateScriptChristmasEvent.rpy:1510
translate french cookies_in_oven_75ccf71c:

    # R "Ooohhh, that sounds like a good story!.... Why don't you tell me about that!.... For the record."
    R "Ooohhh, cela ressemble à une bonne histoire!.... Parles-moi de ça !.... Pour mémoire."

# game/UpdateScriptChristmasEvent.rpy:1511
translate french cookies_in_oven_9100225b:

    # L "You already know the story!.... And I'm pretty sure we don't want that on any kind of record."
    L "Tu connais déjà l'histoire !.... Et je suis presque sûr qu'on ne veut pas ça sur un disque."

# game/UpdateScriptChristmasEvent.rpy:1512
translate french cookies_in_oven_b53866e6:

    # L "You know.... Incriminating evidence and that sort of thing."
    L "Tu sais.... Des preuves incriminantes et ce genre de choses."

# game/UpdateScriptChristmasEvent.rpy:1513
translate french cookies_in_oven_ca83ee53:

    # RT "{i}Damn!{/i}"
    RT "{i}Merde!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1514
translate french cookies_in_oven_3b7568b2:

    # RT "{i}It sure would be helpful to know how I'm going to finally nail that bitch!{/i}"
    RT "{i}Il serait certainement utile de savoir comment je vais enfin épingler cette chienne!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1515
translate french cookies_in_oven_223c0f55:

    # GF "Well, it looks like you were able to figure it out on your own, so when you go back to the present, just keep an eye out for that opportunity."
    GF "Eh bien, il semble que tu aies pu le comprendre par toi-même, alors quand tu reviens au présent, gardes juste un œil sur cette opportunité."

# game/UpdateScriptChristmasEvent.rpy:1516
translate french cookies_in_oven_dcf373f8:

    # L "But I'm so glad you let me be part of her weekly punishment."
    L "Mais je suis si content que tu me laisses faire partie de sa punition hebdomadaire."

# game/UpdateScriptChristmasEvent.rpy:1519
translate french cookies_in_oven_945ae6fd:

    # L "I know this sounds cruel, but I love humiliating that bitch every week!"
    L "Je sais que cela semble cruel, mais j'adore humilier cette chienne chaque semaine!"

# game/UpdateScriptChristmasEvent.rpy:1520
translate french cookies_in_oven_fb009ddb:

    # RT "{i}Oh.... I want to know so bad!{/i}"
    RT "{i}Oh.... Je veux tellement savoir!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1521
translate french cookies_in_oven_74ebcc85:

    # R "So, did you get pregnant on purpose?"
    R "Alors, es-tu tombée enceinte exprès?"

# game/UpdateScriptChristmasEvent.rpy:1522
translate french cookies_in_oven_beaf5b8d:

    # L "No.... but when you fuck as often as we do, we were bound to slip up one of those times."
    L "Non.... mais quand tu baises aussi souvent que nous, nous étions obligés de glisser une de ces fois."

# game/UpdateScriptChristmasEvent.rpy:1525
translate french cookies_in_oven_a7bbc898:

    # L "When it happened though, It just made me happy!"
    L "Quand c'est arrivé, Ça m'a juste rendu heureuse!"

# game/UpdateScriptChristmasEvent.rpy:1526
translate french cookies_in_oven_ca1412d8:

    # L "And I'm so glad that you're ok with being a daddy!"
    L "Et je suis tellement contente que tu sois d'accord pour être papa!"

# game/UpdateScriptChristmasEvent.rpy:1528
translate french cookies_in_oven_0994739e:

    # R "How do you feel about the fact that I got Mom pregnant too?"
    R "Que penses-tu du fait que je sois aussi père de l'enfant de maman?"

# game/UpdateScriptChristmasEvent.rpy:1531
translate french cookies_in_oven_e1b24fd1:

    # L "You know I'm fine with it!"
    L "Tu sais que ça me va!"

# game/UpdateScriptChristmasEvent.rpy:1532
translate french cookies_in_oven_67d01cdb:

    # L "Kind of happy even."
    L "Un peu heureuse même."

# game/UpdateScriptChristmasEvent.rpy:1536
translate french cookies_in_oven_4fbdea30:

    # R "Hypothetically, would it bother you if I shared my affection with another?"
    R "Hypothétiquement, cela te dérangerait-il si je partageais mon affection avec une autre?"

# game/UpdateScriptChristmasEvent.rpy:1539
translate french cookies_in_oven_807a277c:

    # L "Haha.... \"Hypothetical\", my ass!"
    L "Haha.... « Hypothétique», mon cul!"

# game/UpdateScriptChristmasEvent.rpy:1540
translate french cookies_in_oven_8c1ce33e:

    # L "You know, that I know, that you're intimate with Mom and Sidney."
    L "Tu sais, que je sais, que tu es intime avec Maman et Sidney."

# game/UpdateScriptChristmasEvent.rpy:1541
translate french cookies_in_oven_e2f582db:

    # L "You work so hard for all of us, and it's only natural that we all have those kinds of feelings towards you."
    L "Tu travailles si dur pour nous tous, et il est naturel que nous ayons tous ce genre de sentiments envers toi."

# game/UpdateScriptChristmasEvent.rpy:1542
translate french cookies_in_oven_4e62b7ad:

    # L "And you deserve to receive all of our thanks for your dedication to us."
    L "Et tu mérites de recevoir tous nos remerciements pour ton dévouement envers nous."

# game/UpdateScriptChristmasEvent.rpy:1543
translate french cookies_in_oven_91f8489d:

    # GF "You sir, are the Ernest Hemingway of bullshit and manipulation. My Santa hat is off to you!"
    GF "Monsieur, tu es l'Ernest Hemingway des conneries et de la manipulation. Mon chapeau de Père Noël est à toi!"

# game/UpdateScriptChristmasEvent.rpy:1544
translate french cookies_in_oven_10017616_1:

    # DING"
    "DING"

# game/UpdateScriptChristmasEvent.rpy:1545
translate french cookies_in_oven_58552e0f_1:

    # L "Oh, there goes the cookies. Why don't you run and see if Sidney's almost ready for the gift exchange, and I'll get the cookies ready, and bring them out when I'm done."
    L "Oh, voilà les cookies. Pourquoi ne courrais-tu pas voir si Sidney est presque prêt pour l'échange de cadeaux, et je préparerai les biscuits, et je les sortirai quand j'aurai fini."

# game/UpdateScriptChristmasEvent.rpy:1546
translate french cookies_in_oven_2e2caf83_1:

    # R "Yeah, ok."
    R "Oui, d'accord."

# game/UpdateScriptChristmasEvent.rpy:1549
translate french cookies_in_oven_d858d039_1:

    # L "Ohhh.... these stupid, tall chairs!"
    L "Ohhh.... ces stupides chaises hautes!"

# game/UpdateScriptChristmasEvent.rpy:1550
translate french cookies_in_oven_2ff4ef7f_1:

    # L "And where did I throw that apron?.... Baking in the buff is not a good idea."
    L "Et où ai-je jeté ce tablier?.... La cuisson dans le buff n'est pas une bonne idée."

# game/UpdateScriptChristmasEvent.rpy:1552
translate french cookies_in_oven_5f995f3c:

    # RT "{i}I don't know how I feel about taking care of two babies, but so far, this future is looking pretty awesome.{/i}"
    RT "{i}Je ne sais pas ce que je pense de prendre soin de deux bébés, mais jusqu'à présent, cet avenir s'annonce plutôt génial.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1554
translate french cookies_in_oven_2b1600b8:

    # RT "{i}I'm not super excited about Mom being pregnant by Dad, but so far this future doesn't seem too bad.{/i}"
    RT "{i}Je ne suis pas super excité à l'idée que maman soit enceinte de papa, mais jusqu'à présent, cet avenir ne semble pas trop mauvais.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1555
translate french cookies_in_oven_0a589716:

    # GF "Yeah, and if you're not super jazzed about taking care of babies, you can always just get a vasectomy when you're back to the present."
    GF "Oui, et si tu n'es pas très enthousiaste à l'idée de prendre soin des bébés, tu peux toujours subir une vasectomie lorsque tu reviens au présent."

# game/UpdateScriptChristmasEvent.rpy:1556
translate french cookies_in_oven_75586cbc:

    # GF "Should we go see what the damage is with Sidney?"
    GF "Faut-il aller voir quels sont les dégâts avec Sidney?"

# game/UpdateScriptChristmasEvent.rpy:1559
translate french cookies_in_oven_93b36a9a:

    # RT "{i}Yeah, I wonder what my future has in store with her.{/i}"
    RT "{i}Oui, je me demande ce que mon avenir lui réserve.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1575
translate french christmas_shoot_7e5940b2:

    # R "Aaahhh..."
    R "Aaahhh..."

# game/UpdateScriptChristmasEvent.rpy:1578
translate french christmas_shoot_079d0025:

    # R "Ooofff.... "
    R "Ooofff.... "

# game/UpdateScriptChristmasEvent.rpy:1579
translate french christmas_shoot_f12a060c:

    # GCP "Oh, no!.... Not again!"
    GCP "Oh, non!.... Pas encore!"

# game/UpdateScriptChristmasEvent.rpy:1582
translate french christmas_shoot_0e264417:

    # GCP "That's so embarrassing!"
    GCP "C'est tellement embarrassant!"

# game/UpdateScriptChristmasEvent.rpy:1583
translate french christmas_shoot_e5e42ff3:

    # GCP "I'm so sorry! I didn't mean to."
    GCP "Je suis tellement désolé! Je ne le voulais pas."

# game/UpdateScriptChristmasEvent.rpy:1584
translate french christmas_shoot_a75a4ed3:

    # R "Are you sure the Ghost of Christmas Past didn't put you up to it?"
    R "Es-tu sûr que le Fantôme de Noël passé ne t'a pas mis à la hauteur?"

# game/UpdateScriptChristmasEvent.rpy:1585
translate french christmas_shoot_4a1eea03:

    # GCP "Haha.... no, but I'm sure it will help cheer her up when I tell her."
    GCP "Haha.... non, mais je suis sûr que ça l'aidera à lui remonter le moral quand je lui dirai."

# game/UpdateScriptChristmasEvent.rpy:1586
translate french christmas_shoot_a5d9cc33:

    # R "This is my photo studio.... what are we doing here?"
    R "C'est mon studio photo.... qu'est-ce qu'on fait ici ?"

# game/UpdateScriptChristmasEvent.rpy:1587
translate french christmas_shoot_410d7744:

    # GCP "That's a good question. Why on earth would you make your sisters do a photo shoot on Christmas Eve?"
    GCP "C'est une bonne question. Pourquoi diable ferais-tu faire une séance photo à tes sœurs la veille de Noël?"

# game/UpdateScriptChristmasEvent.rpy:1588
translate french christmas_shoot_96eca841:

    # R "Uhhhh.... well.... I don't have any plans for a photo shoot as of right now.... but, Christmas is coming really soon..."
    R "Uhhhh.... Bien.... Je n'ai pas prévu de séance photo pour le moment.... mais, Noël arrive très bientôt..."

# game/UpdateScriptChristmasEvent.rpy:1589
translate french christmas_shoot_71e57e95:

    # R "That's not a bad idea! There's just enough time for Sidney to make some sexy Christmas costumes."
    R "Ce n'est pas une mauvaise idée! Il y a juste assez de temps pour Sidney pour faire des costumes de Noël sexy."

# game/UpdateScriptChristmasEvent.rpy:1590
translate french christmas_shoot_326ac472:

    # R "And if I post them on Christmas Eve night, They'll pick up tons of fans and likes!"
    R "Et si je les poste le soir de la veille de Noël, Ils ramasseront des tonnes de fans et de likes!"

# game/UpdateScriptChristmasEvent.rpy:1591
translate french christmas_shoot_2359c046:

    # R "Are you sure we're not doing this photo shoot because you gave me the idea?"
    R "Es-tu sûr que nous ne faisons pas cette séance photo parce que tu m'en as donné l'idée?"

# game/UpdateScriptChristmasEvent.rpy:1592
translate french christmas_shoot_8aa22782:

    # GCP "Of course not!.... I don't think..."
    GCP "Bien sûr que non!.... Je ne pense pas..."

# game/UpdateScriptChristmasEvent.rpy:1593
translate french christmas_shoot_74113f39:

    # GCP "But anyways, it's a bad idea!"
    GCP "Mais de toute façon, c'est une mauvaise idée!"

# game/UpdateScriptChristmasEvent.rpy:1594
translate french christmas_shoot_1f7635df:

    # GCP "And I want you to see how much they resent you for it."
    GCP "Et je veux que tu voies à quel point elles t'en veulent."

# game/UpdateScriptChristmasEvent.rpy:1595
translate french christmas_shoot_b50bb370:

    # GCP "And how much they resent you for making them show off their bodies for money."
    GCP "Et combien elles t'en veulent de leur avoir fait montrer leur corps pour de l'argent."

# game/UpdateScriptChristmasEvent.rpy:1596
translate french christmas_shoot_458a8191:

    # R "How will you be able to show me that?"
    R "Comment vas-tu pouvoir me montrer cela?"

# game/UpdateScriptChristmasEvent.rpy:1597
translate french christmas_shoot_1ccfbd26:

    # GCP "I'm giving you the power to read their thoughts."
    GCP "Je te donne le pouvoir de lire leurs pensées."

# game/UpdateScriptChristmasEvent.rpy:1598
translate french christmas_shoot_8b520856:

    # R "What?.... That's awesome!"
    R "Quoi?.... C'est génial!"

# game/UpdateScriptChristmasEvent.rpy:1599
translate french christmas_shoot_e1da6a32:

    # GCP "It will only work while we're here in the \"Christmas Present\". And we'll only be here a very short while."
    GCP "Cela ne fonctionnera que pendant que nous sommes ici dans le 'Cadeau de Noël'. Et nous ne serons là que très peu de temps."

# game/UpdateScriptChristmasEvent.rpy:1600
translate french christmas_shoot_93b68d81:

    # GCP "And you may not enjoy it as much as you think, once you hear how your sisters really feel towards you, and your slimy manipulations."
    GCP "Et tu ne l'apprécieras peut-être pas autant que tu le penses, une fois que tu auras entendu ce que tes sœurs ressentent vraiment envers toi et tes manipulations visqueuses."

# game/UpdateScriptChristmasEvent.rpy:1601
translate french christmas_shoot_2e21f803:

    # GCP "After you hear how they feel about you, you should send them home, instead of making them do the photo shoot."
    GCP "Après avoir entendu ce qu'elles pensent de toi, tu devrais les renvoyer chez elles, au lieu de leur faire faire la séance photo."

# game/UpdateScriptChristmasEvent.rpy:1602
translate french christmas_shoot_297981d0:

    # GCP "I think once you hear how happy that will make them, you'll have a stronger desire to treat them, and their bodies, with respect."
    GCP "Je pense qu'une fois que tu auras entendu à quel point cela les rendra heureuse, tu auras un désir plus fort de les traiter, ainsi que leur corps, avec respect."

# game/UpdateScriptChristmasEvent.rpy:1603
translate french christmas_shoot_2408ed15:

    # R "Alright.... well let's see what happens."
    R "D'accord.... voyons ce qui se passe."

# game/UpdateScriptChristmasEvent.rpy:1604
translate french christmas_shoot_cbd180fc:

    # S "Who are you talking too?"
    S "De qui parles-tu aussi?"

# game/UpdateScriptChristmasEvent.rpy:1607
translate french christmas_shoot_25d1a7b6:

    # L "Were you just talking to yourself?"
    L "Étais-tu en train de te parler?"

# game/UpdateScriptChristmasEvent.rpy:1610
translate french christmas_shoot_be0beffe:

    # R "Ummmm.... yeah, I do that every once in a while to organize my thoughts..."
    R "UmmMm.... oui, je fais ça de temps en temps pour organiser mes pensées..."

# game/UpdateScriptChristmasEvent.rpy:1611
translate french christmas_shoot_691ba193:

    # ST "{i}Yep.... what a simple-minded idiot!{/i}"
    ST "{i}Oui.... quel idiot simple d'esprit!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1612
translate french christmas_shoot_2f3e4124:

    # R "What did you call me?"
    R "Comment m'as-tu appelé?"

# game/UpdateScriptChristmasEvent.rpy:1613
translate french christmas_shoot_424c2851:

    # S "Huh?.... I didn't call you anything!"
    S "Hein?.... Je ne t'ai rien appelé !"

# game/UpdateScriptChristmasEvent.rpy:1614
translate french christmas_shoot_c8c85304:

    # GCP "[ryan]! She just thought that, she didn't actually say it, you simple-minded idiot!"
    GCP "[ryan]! Elle a juste pensé ça, elle ne l'a pas dit, espèce d'idiot simple d'esprit !"

# game/UpdateScriptChristmasEvent.rpy:1615
translate french christmas_shoot_7fbb3df6:

    # RT "{i}Oh right!.... Whoops!{/i}"
    RT "{i}Oh non !.... Oups!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1616
translate french christmas_shoot_4e18b37a:

    # R "Uhhh.... never mind."
    R "Uhhh.... Ça ne fait rien."

# game/UpdateScriptChristmasEvent.rpy:1617
translate french christmas_shoot_8034e877:

    # R "You girls look incredible by the way!"
    R "Vous les filles avez l'air incroyables au fait!"

# game/UpdateScriptChristmasEvent.rpy:1620
translate french christmas_shoot_abfe77b0:

    # R "Sidney, you never fail to amaze me!"
    R "Sidney, tu ne manques jamais de m'étonner!"

# game/UpdateScriptChristmasEvent.rpy:1621
translate french christmas_shoot_3cbdde6e:

    # LT "{i}Ohh.... that's sweet.... except for the fact that he's ogling us like a creep.{/i}"
    LT "{i}Ohh.... c'est gentil.... sauf qu'il nous regarde comme un sale type.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1622
translate french christmas_shoot_46632c48:

    # S "Thanks, These were actually pretty technical. The stitching took forever!"
    S "Merci, ceux-ci étaient en fait assez techniques. La couture a pris une éternité!"

# game/UpdateScriptChristmasEvent.rpy:1623
translate french christmas_shoot_0f916946:

    # S "I should have charged you more than the $1,500 dollars you gave me."
    S "J'aurais dû te facturer plus que les 1 500 dollars que tu m'as donnés."

# game/UpdateScriptChristmasEvent.rpy:1624
translate french christmas_shoot_940a15be:

    # R "(Cough).... excuse me? How much?"
    R "(Toux).... excusez-moi ? Combien?"

# game/UpdateScriptChristmasEvent.rpy:1625
translate french christmas_shoot_c81a9ac3:

    # S "$1,500! You remember!.... You made almost as big a deal of it as you are now."
    S "1'500$ ! Tu te souviens !... Tu as fait presque une affaire aussi importante que maintenant."

# game/UpdateScriptChristmasEvent.rpy:1626
translate french christmas_shoot_16b5bfbc:

    # ST "{i}I know that cheapskate's making more money off of us then he's telling us, and he's still complaining every time I tell him the price of an outfit!{/i}"
    ST "{i}Je sais que cheapskate gagne plus d'argent avec nous qu'il nous le dit, et il se plaint toujours chaque fois que je lui dis le prix d'une tenue!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1627
translate french christmas_shoot_38e04ef1:

    # ST "{i}I can tell he doesn't trust or appreciate me.{/i}"
    ST "{i}Je peux dire qu'il ne me fait pas confiance ni ne m'apprécie.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1628
translate french christmas_shoot_9435aa1c:

    # RT "{i}She's right.... I do make a big deal every time I have to pay for an outfit. I need to change that.{/i}"
    RT "{i}Elle a raison.... Je fais beaucoup d'effort chaque fois que je dois payer pour un costume. Je dois changer ça.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1629
translate french christmas_shoot_3e5308b4:

    # R "I'm sorry, Sidney. I shouldn't have made such a big deal about that.... looking at you both in these outfits, I realize they were worth every penny."
    R "Je suis désolé, Sidney. Je n'aurais pas dû en faire une si grosse affaire.... en vous regardant tous les deux dans ces tenues, je me rends compte qu'elles valaient chaque centime."

# game/UpdateScriptChristmasEvent.rpy:1632
translate french christmas_shoot_a1626485:

    # GCP "Aaahhhh.... that was really sweet! This is going so well already! The Ghost of Christmas Past is going to be so jealous!"
    GCP "Aaahhhh.... c'était vraiment gentil! Cela se passe déjà si bien! Le Fantôme de Noël passé va être si jaloux!"

# game/UpdateScriptChristmasEvent.rpy:1635
translate french christmas_shoot_1b40e870:

    # S "Thanks [ryan]! It's always nice to get some appreciation."
    S "Merci [ryan]! C'est toujours agréable d'être appréciée."

# game/UpdateScriptChristmasEvent.rpy:1637
translate french christmas_shoot_826e6d9a:

    # ST "{i}It's almost like he read my mind!{/i}"
    ST "{i}C'est presque comme s'il lisait dans mes pensées!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1638
translate french christmas_shoot_5dad9b80:

    # GCP "Yeah.... be careful.... If you always talk about what they are thinking, they'll get pretty weirded out."
    GCP "Ouais.... fais attention.... Si tu parles toujours de ce qu'ils pensent, elles seront assez bizarres."

# game/UpdateScriptChristmasEvent.rpy:1639
translate french christmas_shoot_02e7c009:

    # L "So, can we get this photoshoot done?"
    L "Alors, pouvons-nous faire cette séance photo?"

# game/UpdateScriptChristmasEvent.rpy:1640
translate french christmas_shoot_3fe9446c:

    # L "I want to get this over with, so I can get that Hard'n'long gift card you promised me."
    L "Je veux en finir avec ça, pour que je puisse obtenir cette carte-cadeau Hard'n'long que tu m'as promise."

# game/UpdateScriptChristmasEvent.rpy:1641
translate french christmas_shoot_b46d979e:

    # L "I've got a ton of Christmas shopping I need to do."
    L "J'ai une tonne de courses de Noël à faire."

# game/UpdateScriptChristmasEvent.rpy:1642
translate french christmas_shoot_89aee8da:

    # LT "{i}Only fucking reason he could get me here on Christmas Eve, I hate how he uses money to control us.{/i}"
    LT "{i}Seule putain de raison pour laquelle il pourrait me faire venir ici la veille de Noël, je déteste comment il utilise l'argent pour nous contrôler.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1643
translate french christmas_shoot_6f7f0b30:

    # RT "{i}Wow! You're right! They really do have a lot of negative feelings towards me.{/i}"
    RT "{i}Wow! Tu as raison ! Ils ont vraiment beaucoup de sentiments négatifs envers moi.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1644
translate french christmas_shoot_8203bd83:

    # GCP "Yep. I told ya. Now all you have to do is give them the day off, and for good measure, you could throw in those gift cards as Christmas gifts."
    GCP "Oui. Je te l'ai dit. Maintenant, tout ce que tu as à faire est de leur donner un jour de congé, et pour faire bonne mesure, tu peux donner ces cartes-cadeaux comme cadeaux de Noël."

# game/UpdateScriptChristmasEvent.rpy:1645
translate french christmas_shoot_e63a47d5:

    # GCP "You'll see how much better they think of you, and that should make you want to stop exploiting them."
    GCP "Tu verras à quel point elles pensent mieux de toi, et cela devrait te donner envie d'arrêter de les exploiter."

# game/UpdateScriptChristmasEvent.rpy:1646
translate french christmas_shoot_71d37fda:

    # RT "{i}Alright, I'll give it a try.{/i}"
    RT "{i}D'accord, je vais essayer.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1647
translate french christmas_shoot_42d71c1f:

    # RT "..."
    RT "..."

# game/UpdateScriptChristmasEvent.rpy:1648
translate french christmas_shoot_8d5b26f2:

    # R "Uhh.... I've actually been thinking."
    R "Heu.... En fait, j'ai réfléchi."

# game/UpdateScriptChristmasEvent.rpy:1649
translate french christmas_shoot_ded8b596:

    # R "Asking you to come in on Christmas Eve wasn't very cool of me."
    R "Te demander de venir la veille de Noël n'était pas très cool de ma part."

# game/UpdateScriptChristmasEvent.rpy:1650
translate french christmas_shoot_a5f6c71f:

    # R "Christmas only comes once a year, and I manipulated you into spending it with me, at work."
    R "Noël ne vient qu'une fois par an, et je vous ai manipulé pour le passer avec moi, au travail."

# game/UpdateScriptChristmasEvent.rpy:1651
translate french christmas_shoot_799f9322:

    # R "Why don't you take the day off, and you can even have those gift cards I promised you as a gift, instead of as a wage for today's work."
    R "Pourquoi ne prenez-vous pas un jour de congé, et vous pouvez même avoir ces cartes-cadeaux que je vous ai promises comme cadeau, au lieu de servir de salaire pour le travail d'aujourd'hui."

# game/UpdateScriptChristmasEvent.rpy:1657
translate french christmas_shoot_a1fc4f51:

    # L "Wow, [ryan]! That's so sweet of you!"
    L "Wow, [ryan]! C'est si gentil de ta part!"

# game/UpdateScriptChristmasEvent.rpy:1658
translate french christmas_shoot_73bd7c12:

    # LT "{i}Maybe I should think better of him. He really is trying his best to be a good brother. Plus, he has all the responsibility of providing for us.{/i}"
    LT "{i}Je devrais peut-être mieux penser à lui. Il fait vraiment de son mieux pour être un bon frère. De plus, il a toute la responsabilité de subvenir à nos besoins.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1661
translate french christmas_shoot_9957624c:

    # S "Yeah, [ryan]! That's amazing of you!"
    S "Ouais, [ryan]! C'est incroyable de ta part!"

# game/UpdateScriptChristmasEvent.rpy:1662
translate french christmas_shoot_15d12944:

    # ST "{i}Well, I can't let this outfit go to waste.... I know.... I can wear it for [ryan] tonight in bed.{/i}"
    ST "{i}Eh bien, je ne peux pas laisser cette tenue se perdre.... Je sais.... Je peux la porter pour [ryan] ce soir au lit.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1663
translate french christmas_shoot_f1c07e1b:

    # RT "{i}Oh my.... spirit?.... Can I please see that part of the \"Christmas Present\" timeline?{/i}"
    RT "{i}Oh mon dieu.... esprit ?.... Puis-je voir cette partie de la chronologie du 'Cadeau de Noël'?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1664
translate french christmas_shoot_83ecc93d:

    # GCP "No you cannot! What a hussy! No wonder you're struggling so much with this!"
    GCP "Non, tu ne peux pas! Quelle coquine! Pas étonnant que tu luttais tellement avec ça!"

# game/UpdateScriptChristmasEvent.rpy:1668
translate french christmas_shoot_5b16d5aa:

    # L "Oh, shit! It's Agent Diaz!.... What is she doing here?"
    L "Oh, merde! C'est l'agent Diaz !.... Que fait-elle ici ?"

# game/UpdateScriptChristmasEvent.rpy:1669
translate french christmas_shoot_51a32b90:

    # R "Yeah, what the fuck?"
    R "Ouais, putain ?"

# game/UpdateScriptChristmasEvent.rpy:1670
translate french christmas_shoot_afd55f19:

    # GCP "Who's Agent Diaz? I didn't see her named in my file."
    GCP "Qui est l'agent Diaz? Je ne l'ai pas vue nommée dans mon dossier."

# game/UpdateScriptChristmasEvent.rpy:1671
translate french christmas_shoot_097d0070:

    # RT "{i}She's an FBI agent that has been extorting us for money.{/i}"
    RT "{i}C'est un agent du FBI qui nous extorque de l'argent.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1672
translate french christmas_shoot_c689c401:

    # RT "{i}I think I've learned my lesson. Can we just disappear now?{/i}"
    RT "{i}Je pense avoir appris ma leçon. Puis-je disparaître maintenant ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1673
translate french christmas_shoot_34e0a31a:

    # GCP "No.... remember. Just like in the past, We're stuck here for a certain amount of time."
    GCP "Non... souvenez-vous. Comme par le passé, nous sommes restés ici pendant un certain temps."

# game/UpdateScriptChristmasEvent.rpy:1674
translate french christmas_shoot_fe7aab64:

    # GCP "Let's hurry and head her off before she ruins everything I've taught you here."
    GCP "Dépêchons-nous de la chasser avant qu'elle ne ruine tout ce que je t'ai appris ici."

# game/UpdateScriptChristmasEvent.rpy:1675
translate french christmas_shoot_c6841c25:

    # R "..."
    R "..."

# game/UpdateScriptChristmasEvent.rpy:1676
translate french christmas_shoot_4500ce88:

    # GCP "Run!"
    GCP "Cours!"

# game/UpdateScriptChristmasEvent.rpy:1682
translate french christmas_shoot_b7f18c05:

    # R "(breathing heavily) What are you doing here?"
    R "(respire fortement) Que fais-tu ici?"

# game/UpdateScriptChristmasEvent.rpy:1683
translate french christmas_shoot_555f21f1:

    # AD "Now that's not a very nice way to greet me!"
    AD "Maintenant, ce n'est pas une très belle façon de me dire bonjour!"

# game/UpdateScriptChristmasEvent.rpy:1684
translate french christmas_shoot_d4b329c2:

    # R "Sorry!.... What the hell are you doing here.... ma'am."
    R "Désolé!.... Que diable fais-tu ici.... madame."

# game/UpdateScriptChristmasEvent.rpy:1685
translate french christmas_shoot_e9d12ed2:

    # AD "Well, I came by the house for my usual payoff, and found that everyone was gone."
    AD "Eh bien, je suis venue à la maison pour mon salaire habituel, et j'ai constaté que tout le monde était parti."

# game/UpdateScriptChristmasEvent.rpy:1686
translate french christmas_shoot_594253b8:

    # R "Shit! It's Thursday?.... But it's Christmas Eve!"
    R "Merde! C'est jeudi ?.... Mais c'est la veille de Noël!"

# game/UpdateScriptChristmasEvent.rpy:1687
translate french christmas_shoot_e1675db3:

    # AD "Yeah, and I wanted my present. So, I tracked you here."
    AD "Oui, et je voulais mon cadeau. Donc, je t'ai suivi ici."

# game/UpdateScriptChristmasEvent.rpy:1688
translate french christmas_shoot_2efefd9a:

    # R "How!?"
    R "Comment!?"

# game/UpdateScriptChristmasEvent.rpy:1689
translate french christmas_shoot_a64feca2:

    # AD "I work for the FBI.... I know a hundred ways to track you."
    AD "Je travaille pour le FBI.... Je connais cent façons de te suivre."

# game/UpdateScriptChristmasEvent.rpy:1690
translate french christmas_shoot_b81a0386:

    # AD "And I'm so glad I did!"
    AD "Et je suis tellement contente de l'avoir fait!"

# game/UpdateScriptChristmasEvent.rpy:1693
translate french christmas_shoot_d4ffbcfe:

    # AD "Now I get to see where the magic happens."
    AD "Maintenant, je peux voir où la magie se produit."

# game/UpdateScriptChristmasEvent.rpy:1694
translate french christmas_shoot_15584b21:

    # GCP "Get rid of her!"
    GCP "Débarrasses-toi d'elle!"

# game/UpdateScriptChristmasEvent.rpy:1695
translate french christmas_shoot_bd4dc978:

    # R "Uhhh.... I suppose you want your money."
    R "Uhhh.... Je suppose que tu veux ton argent."

# game/UpdateScriptChristmasEvent.rpy:1696
translate french christmas_shoot_9f495968:

    # AD "Actually, there's something I want even more than my money."
    AD "En fait, il y a quelque chose que je veux encore plus que mon argent."

# game/UpdateScriptChristmasEvent.rpy:1697
translate french christmas_shoot_3864a5ae:

    # AD "If you'll let me be the director and photographer of your photo shoot, you won't have to pay me for this week, or next week."
    AD "Si tu me laisses être le réalisateur et le photographe de ta séance photo, tu n'auras pas à me payer cette semaine, ni la semaine prochaine."

# game/UpdateScriptChristmasEvent.rpy:1698
translate french christmas_shoot_34fe4869:

    # {i}\"Offer only valid in the Christmas present.\"{/i}"
    "{i}'Offre valable uniquement dans le cadeau de Noël.'{/i}"

# game/UpdateScriptChristmasEvent.rpy:1699
translate french christmas_shoot_39d647c5:

    # AD "So, what do you say?"
    AD "Alors, qu'en dis-tu?"

# game/UpdateScriptChristmasEvent.rpy:1700
translate french christmas_shoot_7aeb6eec:

    # GCP "Pay her, and get her out of here!"
    GCP "Paye-la, et sors-la d'ici!"

# game/UpdateScriptChristmasEvent.rpy:1701
translate french christmas_shoot_89d19b30:

    # RT "{i}Why is she making you so nervous?{/i}"
    RT "{i}Pourquoi te rend-elle si nerveuse?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1702
translate french christmas_shoot_669676e0:

    # GCP "Because she's an unknown entity. I have no idea what will happen if she meddles."
    GCP "Parce qu'elle est une entité inconnue. Je n'ai aucune idée de ce qui se passera si elle s'en mêle."

# game/UpdateScriptChristmasEvent.rpy:1705
translate french christmas_shoot_91e035b2:

    # R "Yeah, I don't think so! Here's the $500, and why don't you take your broom and fly away."
    R "Ouais, je ne pense pas! Voici les 500$, et pourquoi ne pas prendre ton balai et t'envoler."

# game/UpdateScriptChristmasEvent.rpy:1708
translate french christmas_shoot_24e0e0c3:

    # AD "Fine!.... But make sure you're home next week! If I have to track you down again, I'm going to be taking both forms of payment."
    AD "Bien!.... Mais assures-toi d'être à la maison la semaine prochaine! Si non, je te retrouve et je te prendrais les deux moyens de paiement."

# game/UpdateScriptChristmasEvent.rpy:1709
translate french christmas_shoot_60388158:

    # GCP "Good job! Now let's go give the girls their gift cards, and we'll call today a success!"
    GCP "Bon travail! Maintenant, allons donner aux filles leurs cartes-cadeaux, et nous appellerons aujourd'hui un succès!"

# game/UpdateScriptChristmasEvent.rpy:1712
translate french christmas_shoot_9eabab67:

    # S "What's going on, [ryan]?"
    S "Que se passe-t-il, [ryan]?"

# game/UpdateScriptChristmasEvent.rpy:1713
translate french christmas_shoot_0da086dc:

    # S "Is she gone?"
    S "Est-elle partie ?"

# game/UpdateScriptChristmasEvent.rpy:1714
translate french christmas_shoot_e9a348eb:

    # R "Yeah.... you know, it was just some woman looking for her cat."
    R "Ouais... vous savez, c'était juste une femme à la recherche de son chat."

# game/UpdateScriptChristmasEvent.rpy:1716
translate french christmas_shoot_7d6c73d1:

    # S "You don't have to lie.... Lauren just told me what's going on."
    S "Tu n'as pas à mentir.... Lauren vient de me dire ce qui se passe."

# game/UpdateScriptChristmasEvent.rpy:1717
translate french christmas_shoot_69a8a4b6:

    # R "You told her?"
    R "Tu lui as dit?"

# game/UpdateScriptChristmasEvent.rpy:1718
translate french christmas_shoot_267aee7f:

    # L "Well, not everything, but most of it."
    L "Eh bien, pas tout, mais la plupart."

# game/UpdateScriptChristmasEvent.rpy:1719
translate french christmas_shoot_a3ffc063:

    # R "Did you tell her what happens when I don't have enough money to pay her?"
    R "As-tu dit ce qui se passe quand je n'ai pas assez d'argent pour la payer?"

# game/UpdateScriptChristmasEvent.rpy:1720
translate french christmas_shoot_626c45d7:

    # L "Yeah.... well, not all the details."
    L "Ouais.... eh bien, pas tous les détails."

# game/UpdateScriptChristmasEvent.rpy:1721
translate french christmas_shoot_7505507d:

    # LT "{i}I didn't tell her that I'm mostly scared that I kind of like it.{/i}"
    LT "{i}Je ne lui ai pas dit que j'avais surtout peur d'aimer ça.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1722
translate french christmas_shoot_fec7dbda:

    # RT "{i}Whoa! What was that?{/i}"
    RT "{i}Ouah! C'était quoi ça ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1723
translate french christmas_shoot_50aee4b9:

    # GCP "Yeah.... all of you are a bunch of deviants."
    GCP "Ouais.... vous êtes tous une bande de déviants."

# game/UpdateScriptChristmasEvent.rpy:1724
translate french christmas_shoot_a3a1c994:

    # S "That disgusting old sleaze hole!"
    S "Ce vieux trou de manche répugnant !"

# game/UpdateScriptChristmasEvent.rpy:1725
translate french christmas_shoot_d5774408:

    # R "Well, the good news is that I paid her, and now I can give you these gift cards, and we can all go enjoy the Christmas holiday."
    R "Eh bien, la bonne nouvelle est que je l'ai payée, et maintenant je peux vous donner ces cartes-cadeaux, et nous pouvons tous aller profiter des vacances de Noël."

# game/UpdateScriptChristmasEvent.rpy:1726
translate french christmas_shoot_0411c235:

    # GCP "Yay!"
    GCP "Ouah!"

# game/UpdateScriptChristmasEvent.rpy:1727
translate french christmas_shoot_e0356c7b:

    # S "Yeah.... I don't think so."
    S "Ouais.... Je ne pense pas."

# game/UpdateScriptChristmasEvent.rpy:1728
translate french christmas_shoot_44fefe39:

    # GCP "Wait, what?"
    GCP "Attends, quoi?"

# game/UpdateScriptChristmasEvent.rpy:1729
translate french christmas_shoot_eb92663b:

    # S "I'm sorry [ryan].... I had no idea you had this added pressure on you, on top of everything else you're trying to do to keep a roof over our heads."
    S "Je suis désolée [ryan].... Je ne savais pas que tu avais cette pression supplémentaire sur toi, en plus de tout ce que tu essaies de faire pour garder un toit au-dessus de nos têtes."

# game/UpdateScriptChristmasEvent.rpy:1730
translate french christmas_shoot_d87e8231:

    # ST "{i}Now that's a real man!{/i}"
    ST "{i}Maintenant c'est un vrai homme!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1731
translate french christmas_shoot_176235aa:

    # RT "{i}That's so nice!{/i}"
    RT "{i}C'est tellement sympa!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1732
translate french christmas_shoot_a04ab9ef:

    # ST "{i}Even if he doesn't look like it.{/i}"
    ST "{i}Même s'il ne ressemble pas à ça. {/i}"

# game/UpdateScriptChristmasEvent.rpy:1733
translate french christmas_shoot_e3b57d2b:

    # RT "{i}Ouch!{/i}"
    RT "{i}Aïe!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1734
translate french christmas_shoot_1f925d9d:

    # GCP "No! This can't be happening."
    GCP "Non! Ça ne peut pas arriver."

# game/UpdateScriptChristmasEvent.rpy:1735
translate french christmas_shoot_c495d151:

    # S "We're going to do the photo shoot!"
    S "Nous allons faire la séance photo!"

# game/UpdateScriptChristmasEvent.rpy:1736
translate french christmas_shoot_f65598f0:

    # S "I know it has the potential to bring in a lot of money, and I want to help you out by doing my part."
    S "Je sais qu'elle a le potentiel de rapporter beaucoup d'argent, et je veux t'aider en faisant ma part."

# game/UpdateScriptChristmasEvent.rpy:1737
translate french christmas_shoot_b3ac0b52:

    # S "Are you in, Lauren?"
    S "Tu es là, Lauren ?"

# game/UpdateScriptChristmasEvent.rpy:1738
translate french christmas_shoot_c376958a:

    # L "Yeah, and I think we can make it fun!"
    L "Ouais, et je pense qu'on peut rendre ça amusant!"

# game/UpdateScriptChristmasEvent.rpy:1739
translate french christmas_shoot_821db4a0:

    # S "We'll think of a good pose to start with."
    S "Nous allons penser à une bonne pose pour commencer."

# game/UpdateScriptChristmasEvent.rpy:1742
translate french christmas_shoot_88823a4a:

    # LT "{i}It's not everyday I get to spend time with my sister. At least not dressed like total sluts. Damn, she looks hot!{/i}"
    LT "{i}Ce n'est pas tous les jours que je passe du temps avec ma sœur. Du moins pas habillées comme des salopes totales. Putain, elle a l'air sexy!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1743
translate french christmas_shoot_a44b827d:

    # GCP "You degenerates!"
    GCP "Tu dégénères !"

# game/UpdateScriptChristmasEvent.rpy:1744
translate french christmas_shoot_173b0be5:

    # RT "{i}What's the matter? This is awesome! They now love and respect me more, and they're going to do the photo shoot without having to be manipulated.{/i}"
    RT "{i}Quel est le problème? C'est génial! Maintenant, ils m'aiment et me respectent davantage, et elles vont faire la séance photo sans avoir à être manipulées.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1745
translate french christmas_shoot_68b67806:

    # RT "{i}Isn't this what you wanted?{/i}"
    RT "{i}N'est-ce pas ce que tu voulais ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1748
translate french christmas_shoot_04f0462d:

    # GCP "No!.... I wanted you to see the error of your ways, and stop taking sexy pictures of your sisters!"
    GCP "Non!.... Je voulais que tu voies les erreurs de tes manières, et que tu arrêtes de prendre des photos sexy de tes sœurs!"

# game/UpdateScriptChristmasEvent.rpy:1749
translate french christmas_shoot_c79946fa:

    # RT "But why? They're into it now. They're fully consenting adults."
    RT "Mais pourquoi? Elles sont dedans maintenant. Ce sont des adultes pleinement consentantes."

# game/UpdateScriptChristmasEvent.rpy:1750
translate french christmas_shoot_5593a202:

    # GCP "I've had it with your family!"
    GCP "Je l'ai eu avec ta famille!"

# game/UpdateScriptChristmasEvent.rpy:1751
translate french christmas_shoot_4d318ef6:

    # GCP "I'm going to see if I can pillage some liquor from the bar down the street!"
    GCP "Je vais voir si je peux piller de l'alcool dans le bar de la rue!"

# game/UpdateScriptChristmasEvent.rpy:1752
translate french christmas_shoot_a744df60:

    # GCP "I'll be back when it's time to go."
    GCP "Je reviendrai quand il sera temps de partir."

# game/UpdateScriptChristmasEvent.rpy:1755
translate french christmas_shoot_57b367f6:

    # RT "{i}Shoot! I made another ghost disappointed.{/i}"
    RT "{i}Merde! J'ai déçu un autre fantôme.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1758
translate french christmas_shoot_aeef8a08:

    # L "So, are you ready to shoot us in sexy poses?"
    L "Alors, es-tu prêt à nous photographier dessus dans des poses sexy?"

# game/UpdateScriptChristmasEvent.rpy:1759
translate french christmas_shoot_a724a720:

    # S "And how's this for a first pose?"
    S "Et comment ça se passe pour une première pose ?"

# game/UpdateScriptChristmasEvent.rpy:1760
translate french christmas_shoot_472bee99:

    # R "Great! I like the buddy-buddy pose."
    R "Génial! J'aime la pose ami-ami."

# game/UpdateScriptChristmasEvent.rpy:1761
translate french christmas_shoot_d295a5a8:

    # R "That will make a great first pic."
    R "Qui fera une grande première photo."

# game/UpdateScriptChristmasEvent.rpy:1766
translate french christmas_shoot_6451a299:

    # R "I think I'll add a homey Christmas background."
    R "Je pense que je vais ajouter un fond de Noël chaleureux."

# game/UpdateScriptChristmasEvent.rpy:1771
translate french christmas_shoot_652b7528:

    # R "I can see it all now!"
    R "Je peux tout voir maintenant!"

# game/UpdateScriptChristmasEvent.rpy:1772
translate french christmas_shoot_42d71c1f_1:

    # RT "..."
    RT "..."

# game/UpdateScriptChristmasEvent.rpy:1773
translate french christmas_shoot_710888da:

    # R "Ok.... now I want you to do some Christmassy things."
    R "D'accord.... maintenant, je veux que tu fasses des choses de Noël."

# game/UpdateScriptChristmasEvent.rpy:1774
translate french christmas_shoot_c0db9358:

    # L "I can make it look like I'm hanging Christmas tree ornaments."
    L "Je peux faire croire que j'accroche des ornements d'arbre de Noël."

# game/UpdateScriptChristmasEvent.rpy:1775
translate french christmas_shoot_d146882e:

    # S "And I'll make it look like I'm hanging stockings."
    S "Et je vais faire croire que j'accroche des bas."

# game/UpdateScriptChristmasEvent.rpy:1776
translate french christmas_shoot_1582bafc:

    # R "Sounds good, but do it in a sexy way."
    R "Ça sonne bien, mais faites-le de manière sexy."

# game/UpdateScriptChristmasEvent.rpy:1777
translate french christmas_shoot_5600fdf4:

    # L "Well, duh..."
    L "Eh bien, duh..."

# game/UpdateScriptChristmasEvent.rpy:1780
translate french christmas_shoot_53759230:

    # R "Nice! That looks super sexy!"
    R "Sympa! Ça a l'air super sexy!"

# game/UpdateScriptChristmasEvent.rpy:1785
translate french christmas_shoot_941a6b4d:

    # R "Beautiful!"
    R "Magnifique!"

# game/UpdateScriptChristmasEvent.rpy:1786
translate french christmas_shoot_f97248a9:

    # LT "{i}It's kind of fun posing with my sister.{/i}"
    LT "{i}C'est un peu amusant de poser avec ma sœur.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1787
translate french christmas_shoot_2bd1e168:

    # ST "{i}My sister looks sexy as hell in that costume I made for her.{/i}"
    ST "{i}Ma sœur a l'air sexy comme l'enfer dans ce costume que j'ai fait pour elle.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1788
translate french christmas_shoot_40b9edd3:

    # RT "{i}This is awesome! I wish I could read minds all the time.{/i}"
    RT "{i}C'est génial! J'aimerais pouvoir lire dans les pensées tout le temps.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1789
translate french christmas_shoot_6c71cc26:

    # R "Ok.... moving on.... I think I see some prop presents against the wall."
    R "D'accord.... on passe à autre chose.... Je crois voir des cadeaux contre le mur."

# game/UpdateScriptChristmasEvent.rpy:1790
translate french christmas_shoot_0b4c4e8e:

    # R "Let's do a scene with you opening them"
    R "Faisons une scène avec vous en les ouvrant"

# game/UpdateScriptChristmasEvent.rpy:1791
translate french christmas_shoot_7da240c4:

    # S "Those aren't actually props. Sidney and I decided to give each other our Christmas presents early, since, well.... at least my present isn't appropriate to open under the tree in front of Mom."
    S "Ce ne sont pas des accessoires. Sidney et moi avons décidé de nous offrir nos cadeaux de Noël tôt, depuis, eh bien.... au moins, mon cadeau n'est pas approprié pour s'ouvrir sous l'arbre devant maman."

# game/UpdateScriptChristmasEvent.rpy:1792
translate french christmas_shoot_ae729c76:

    # L "Haha, yeah.... mine isn't, either."
    L "Haha, ouais... ce n'est pas le mien."

# game/UpdateScriptChristmasEvent.rpy:1793
translate french christmas_shoot_c2fc30e3:

    # R "Do you guys mind opening them now, for the photo shoot?"
    R "Ça vous dérange de les ouvrir maintenant, pour la séance photo?"

# game/UpdateScriptChristmasEvent.rpy:1794
translate french christmas_shoot_d5f16439:

    # S "No, we don't mind."
    S "Non, ça ne nous dérange pas."

# game/UpdateScriptChristmasEvent.rpy:1795
translate french christmas_shoot_8434226f:

    # R "Geat! Run and grab them, and then get into a good pose."
    R "Geat! Allez et attrapez-les, puis prenez une bonne pose."

# game/UpdateScriptChristmasEvent.rpy:1800
translate french christmas_shoot_53b0adca:

    # R "You two are becoming naturals at this."
    R "Vous deux devenez naturels à cela."

# game/UpdateScriptChristmasEvent.rpy:1805
translate french christmas_shoot_f17a945f:

    # R "Once you get them open, show each other what you got."
    R "Une fois que vous les ouvrez, montrez-vous ce que vous avez."

# game/UpdateScriptChristmasEvent.rpy:1808
translate french christmas_shoot_0e0c01a9:

    # S "Haha.... I can't believe we got each other the same thing!"
    S "Haha.... Je n'arrive pas à croire qu'on se soit fait la même chose !"

# game/UpdateScriptChristmasEvent.rpy:1809
translate french christmas_shoot_7cd28faa:

    # L "Yeah.... just different colors!"
    L "Ouais.... juste des couleurs différentes!"

# game/UpdateScriptChristmasEvent.rpy:1810
translate french christmas_shoot_508109d6:

    # R "I guess horny minds think alike."
    R "Je suppose que les esprits excités se ressemblent."

# game/UpdateScriptChristmasEvent.rpy:1811
translate french christmas_shoot_59571a3b:

    # R "This is great! The natural reaction makes an even better picture than if you girls were acting!"
    R "C'est génial! La réaction naturelle fait une image encore meilleure que si vous agissiez!"

# game/UpdateScriptChristmasEvent.rpy:1816
translate french christmas_shoot_eab0694d:

    # ST "{i}I can't wait to get home and try it.{/i}"
    ST "{i}J'ai hâte de rentrer à la maison et de l'essayer.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1817
translate french christmas_shoot_43d804d1:

    # LT "{i}I wish I could try this right now!{/i}"
    LT "{i}J'aimerais pouvoir essayer cela maintenant!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1818
translate french christmas_shoot_b4af2796:

    # R "Alright.... why don't you thank each other with a passionate kiss."
    R "D'accord.... remerciez-vous les unes les autres avec un baiser passionné."

# game/UpdateScriptChristmasEvent.rpy:1821
translate french christmas_shoot_feb8d233:

    # S "What!?..."
    S "Quoi!?..."

# game/UpdateScriptChristmasEvent.rpy:1822
translate french christmas_shoot_9c7eec0d:

    # R "For the photo shoot!"
    R "Pour la séance photo!"

# game/UpdateScriptChristmasEvent.rpy:1823
translate french christmas_shoot_14b67a14:

    # R "Not for real."
    R "Pas pour de vrai."

# game/UpdateScriptChristmasEvent.rpy:1824
translate french christmas_shoot_64b300f5:

    # R "You don't actually have to make out."
    R "Vous n'avez pas vraiment à vous distinguer."

# game/UpdateScriptChristmasEvent.rpy:1825
translate french christmas_shoot_6a046414:

    # R "Just make it look like you are."
    R "Fais simplement comme tu es."

# game/UpdateScriptChristmasEvent.rpy:1826
translate french christmas_shoot_237e9ea2:

    # L "But we're sisters!"
    L "Mais nous sommes sœurs!"

# game/UpdateScriptChristmasEvent.rpy:1827
translate french christmas_shoot_46a50aa7:

    # R "Yeah, well, none of your online fans know that. And even if they did, it would just make you two even more popular."
    R "Ouais, eh bien, aucun de tes fans en ligne ne le sait. Et même s'ils le savaient, cela te rendrait encore plus populaires."

# game/UpdateScriptChristmasEvent.rpy:1828
translate french christmas_shoot_853d43ee:

    # RT "{i}Maybe I should put that in their bios.{/i}"
    RT "{i}Peut-être que je devrais mettre cela dans leur bio. {/i}"

# game/UpdateScriptChristmasEvent.rpy:1829
translate french christmas_shoot_719ba3e8:

    # R "If you want to learn to be good models, you'll have to learn to go outside your comfort zones a little."
    R "Si vous voulez apprendre à être de bons modèles, vous devrez apprendre à sortir un peu de vos zones de confort."

# game/UpdateScriptChristmasEvent.rpy:1830
translate french christmas_shoot_82ff560c:

    # S "Uhhh.... this is outside a lot!"
    S "Uhhh.... ceci est en dehors de beaucoup !"

# game/UpdateScriptChristmasEvent.rpy:1831
translate french christmas_shoot_827334e2:

    # LT "{i}Oh my God! Will Sidney agree to do this with me? Why am I getting butterflies in my pussy when I think about it?{/i}"
    LT "{i}Oh mon Dieu! Sidney acceptera-t-elle de faire ça avec moi? Pourquoi ai-je des papillons dans la chatte quand j'y pense?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1832
translate french christmas_shoot_8e3fc8df:

    # L "Come on Sidney, Let's just get it over with."
    L "Allez Sidney, finissons-en."

# game/UpdateScriptChristmasEvent.rpy:1833
translate french christmas_shoot_c6190b95:

    # S "Fine!"
    S "Bien!"

# game/UpdateScriptChristmasEvent.rpy:1834
translate french christmas_shoot_c99d6dd1:

    # R "Lauren, why don't you just straddle Sidney and..."
    R "Lauren, pourquoi ne pas chevaucher Sidney et..."

# game/UpdateScriptChristmasEvent.rpy:1837
translate french christmas_shoot_879ff214:

    # R "Oh, yeah.... that's perfect!"
    R "Oh, oui.... c'est parfait !"

# game/UpdateScriptChristmasEvent.rpy:1838
translate french christmas_shoot_f1b3db03:

    # R "Wow! That's pretty hot!"
    R "Wow! C'est assez chaud!"

# game/UpdateScriptChristmasEvent.rpy:1839
translate french christmas_shoot_826b8a70:

    # S "Just hurry up and take the picture!"
    S "Dépêches-toi et prends la photo!"

# game/UpdateScriptChristmasEvent.rpy:1842
translate french christmas_shoot_4deb5453:

    # ST "{i}Before my pussy betrays me and leaves a puddle on the floor!{/i}"
    ST "{i}Avant que ma chatte ne me trahisse et ne laisse une flaque d'eau sur le sol!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1843
translate french christmas_shoot_843e7d14:

    # ST "{i}This is so wrong!{/i}"
    ST "{i}C'est tellement mal!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1844
translate french christmas_shoot_4cb66561:

    # ST "{i}Why is it making my pussy tingle so much?{/i}"
    ST "{i}Pourquoi fait-il tant picoter ma chatte?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1845
translate french christmas_shoot_19da5e64:

    # LT "{i}Why does this turn me on so bad?{/i}"
    LT "{i}Pourquoi est-ce si mauvais d'être excitée par ça?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1846
translate french christmas_shoot_61919a30:

    # LT "{i}I've already been having confusing thoughts about women, but now I'm getting turned on by my sister?{/i}"
    LT "{i}J'ai déjà eu des pensées confuses sur les femmes, mais maintenant je suis excitée par ma sœur?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1847
translate french christmas_shoot_a9761b3c:

    # LT "{i}God, I hope I don't drip all over her lap.{/i}"
    LT "{i}Dieu, j'espère que je ne dégouline pas sur ses genoux.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1848
translate french christmas_shoot_38a2ac68:

    # LT "{i}I can't believe she made a costume that doesn't include panties.{/i}"
    LT "{i}Je n'arrive pas à croire qu'elle ait fait un costume qui n'inclut pas de culotte.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1849
translate french christmas_shoot_9d6d529c:

    # RT "{i}Wow! They're kind of into each other.{/i}"
    RT "{i}Wow! Ils sont en quelque sorte l'un dans l'autre.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1850
translate french christmas_shoot_bf987008:

    # RT "{i}It sounds like they just need a little persuading.{/i}"
    RT "{i}On dirait qu'ils ont juste besoin d'un peu de persuasion.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1855
translate french christmas_shoot_c4a8ef7a:

    # R "I think we're ready for the next pose."
    R "Je pense que nous sommes prêts pour la prochaine pose."

# game/UpdateScriptChristmasEvent.rpy:1857
translate french christmas_shoot_82b56236:

    # R "Unless you want to keep kissing, but I'm thinking we should take a few shots with you playing with your new toys."
    R "Sauf si tu veux continuer à t'embrasser, mais je pense que nous devrions prendre quelques photos avec toi en train de jouer avec tes nouveaux jouets."

# game/UpdateScriptChristmasEvent.rpy:1860
translate french christmas_shoot_fe31079c:

    # S "Play with our..."
    S "Jouer avec notre..."

# game/UpdateScriptChristmasEvent.rpy:1861
translate french christmas_shoot_f7e421e8:

    # L "(Gulp) Toys?..."
    L "(Gulp) Jouets?..."

# game/UpdateScriptChristmasEvent.rpy:1862
translate french christmas_shoot_e4f25d36:

    # S "How do you mean?"
    S "Que veux-tu dire?"

# game/UpdateScriptChristmasEvent.rpy:1863
translate french christmas_shoot_44e4da00:

    # LT "{i}Oh my.... he wants us to dildo each other.... shit!.... I think I just dripped onto Sidney's leg!{/i}"
    LT "{i}Oh mon dieu.... il veut qu'on se gode.... Merde!.... Je crois que j'ai dégouliné de plaisir sur la jambe de Sidney!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1864
translate french christmas_shoot_64458996:

    # LT "{i}I hope she doesn't notice.{/i}"
    LT "{i}J'espère qu'elle ne le remarque pas.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1865
translate french christmas_shoot_7dcc4672:

    # ST "{i}Holy shit!.... I think Lauren just leaked onto my leg!.... Does the thought of us playing with toys together get her excited?{/i}"
    ST "{i}Merde!.... Je pense que Lauren vient de jouir sur ma jambe!.... Est-ce que l'idée de jouer avec des jouets ensemble l'a excitée?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1866
translate french christmas_shoot_ace84175:

    # R "Haha.... I'm not expecting you to screw each other with your new dildos."
    R "Haha.... Je ne m'attends pas à ce que vous vous embrassiez mutuellement avec vos nouveaux godes."

# game/UpdateScriptChristmasEvent.rpy:1867
translate french christmas_shoot_c50482e5:

    # RT "{i}These girls are going to make me nut in my pants!{/i}"
    RT "{i}Ces filles vont me rendre fou dans mon pantalon!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1868
translate french christmas_shoot_d644fc26:

    # R "Just get into a position so it looks like Sidney is penetrating Lauren with the dildo."
    R "Mets-toi juste dans une position pour qu'il semble que Sidney pénètre Lauren avec le gode."

# game/UpdateScriptChristmasEvent.rpy:1869
translate french christmas_shoot_2544b3de:

    # R "But you won't actually penetrate."
    R "Mais vous ne pénétrerez pas réellement."

# game/UpdateScriptChristmasEvent.rpy:1870
translate french christmas_shoot_dba4d304:

    # R "Just make it look like it."
    R "Il suffit de lui donner l'air."

# game/UpdateScriptChristmasEvent.rpy:1871
translate french christmas_shoot_acd04ee7:

    # R "Unless you want to keep making out!"
    R "Sauf si vous voulez continuer à faire!"

# game/UpdateScriptChristmasEvent.rpy:1872
translate french christmas_shoot_0b2b81d3:

    # LT "{i}I kind of do.{/i}"
    LT "{i}Je le fais en quelque sorte.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1873
translate french christmas_shoot_55a83b56:

    # S "Uuuhhh.... ok Lauren.... let's.... get.... into position."
    S "Uuuuhhh.... ok Lauren.... mettrons-nous en position."

# game/UpdateScriptChristmasEvent.rpy:1877
translate french christmas_shoot_f53eaa6f:

    # S "Does this look ok?"
    S "Est-ce que ça a l'air correct?"

# game/UpdateScriptChristmasEvent.rpy:1878
translate french christmas_shoot_152213b3:

    # R "Yeah, that's pretty convincing."
    R "Ouais, c'est assez convaincant."

# game/UpdateScriptChristmasEvent.rpy:1879
translate french christmas_shoot_512abdf3:

    # R "Let me get a quick shot of it."
    R "Permettez-moi d'en prendre une photo rapide."

# game/UpdateScriptChristmasEvent.rpy:1884
translate french christmas_shoot_1c0ca9e8:

    # R "Great! Now move the dildo a little closer."
    R "Génial! Maintenant, rapproches le gode un peu plus près."

# game/UpdateScriptChristmasEvent.rpy:1885
translate french christmas_shoot_d1b8db6f:

    # R "And if Lauren's ok with it, just kind of prod and push the dildo between her thigh gap."
    R "Et si Lauren est d'accord avec ça, juste une sorte de prod et pousser le gode entre son espace de cuisse."

# game/UpdateScriptChristmasEvent.rpy:1886
translate french christmas_shoot_ddf72a39:

    # R "From my perspective, it should look pretty convincing."
    R "De mon point de vue, cela devrait être assez convaincant."

# game/UpdateScriptChristmasEvent.rpy:1889
translate french christmas_shoot_1979dd13:

    # ST "{i}Hmmmm.... I think I should use this opportunity to tease Lauren.{/i}"
    ST "{i}Hmmmm.... Je pense que je devrais profiter de cette occasion pour taquiner Lauren.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1890
translate french christmas_shoot_3584c134:

    # ST "{i}After all, she did let her pussy leak her naughty juices all over my leg.{/i}"
    ST "{i}Après tout, elle a laissé sa chatte couler sur ma jambe.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1891
translate french christmas_shoot_9c1aca6d:

    # RT "{i}Oh, no way!.... I always thought Sidney was too much of a prude to do anything like that to her own sister!{/i}"
    RT "{i}Oh, pas question!.... J'ai toujours pensé que Sidney était trop prude pour faire quelque chose comme ça à sa propre sœur!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1892
translate french christmas_shoot_e7e3fc50:

    # RT "{i}I wonder what she'll do to her.{/i}"
    RT "{i}Je me demande ce qu'elle va lui faire.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1895
translate french christmas_shoot_82960bd2:

    # LT "{i}Oh.... my.... God!....{/i}"
    LT "{i}Oh.... mon.... Dieu !....{/i}"

# game/UpdateScriptChristmasEvent.rpy:1896
translate french christmas_shoot_564f8cf1:

    # LT "{i}What the hell is Sidney thinking?{/i}"
    LT "{i}À quoi pense Sidney ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1897
translate french christmas_shoot_0fbb788f:

    # LT "{i}Where does she get the balls to do this to her own sister?{/i}"
    LT "{i}Où trouve-t-elle les couilles pour faire ça à sa propre sœur?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1898
translate french christmas_shoot_0e1f9820:

    # LT "{i}Keep quiet.... not a sound.... that would be so embarassing if [ryan] knew!{/i}"
    LT "{i}Gardez le silence.... pas un bruit.... ce serait tellement embarrassant si [ryan] savait!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1901
translate french christmas_shoot_044435c0:

    # ST "{i}Haha.... look at her squirm.... this is driving her crazy!{/i}"
    ST "{i}Haha.... regarde-la se tortiller.... ça la rend folle !{/i}"

# game/UpdateScriptChristmasEvent.rpy:1902
translate french christmas_shoot_39b98dee:

    # RT "{i}Damn, from this angle I can't see what she's doing.{/i}"
    RT "{i}Merde, sous cet angle, je ne vois pas ce qu'elle fait.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1905
translate french christmas_shoot_70c5ab8d:

    # RT "{i}I can only imagine what's going on back there.{/i}"
    RT "{i}Je ne peux qu'imaginer ce qui se passe là-bas.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1907
translate french christmas_shoot_89c32e02:

    # LT "{i}Why does it feel so much better when Sidney does that to me, then when I do it to myself?{/i}"
    LT "{i}Pourquoi se sent-il tellement mieux quand Sidney me fait ça, plutôt que quand je le fais à moi-même?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1908
translate french christmas_shoot_d1bccbfc:

    # LT "{i}It's really starting to feel good!{/i}"
    LT "{i}Ça commence vraiment à faire du bien !{/i}"

# game/UpdateScriptChristmasEvent.rpy:1910
translate french christmas_shoot_428b441f:

    # LT "{i}Oh God!.... This is driving me crazy!.... What if that ho gives me an orgasm, right here in front of [ryan]?{/i}"
    LT "{i}Oh mon dieu!.... Ça me rend folle!.... Et si ce 'ho' me donne un orgasme, juste devant [ryan]?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1911
translate french christmas_shoot_5a55f1ca:

    # S "(whispering) This is what you get for dripping your dirty juices all over my leg."
    S "(murmure) C'est ce que tu obtiens pour laisser dégouliner ton sale jus sur ma jambe."

# game/UpdateScriptChristmasEvent.rpy:1912
translate french christmas_shoot_101be609:

    # L "(whispering) Oh my God.... you.... noticed that?"
    L "(murmure) Oh mon Dieu.... tu.... tu as vu ça?"

# game/UpdateScriptChristmasEvent.rpy:1913
translate french christmas_shoot_b365dbad:

    # S "(whispering) Yes, I noticed that!.... And now I'm going to do this until [ryan] tells me to stop."
    S "(murmure) Oui, j'ai vu!.... Et maintenant, je vais le faire jusqu'à ce que [ryan] me dise d'arrêter."

# game/UpdateScriptChristmasEvent.rpy:1914
translate french christmas_shoot_115cb496:

    # RT "{i}Whatever Sidney is doing to Lauren, I should let it continue for a little while.{/i}"
    RT "{i}Quoi que Sidney fasse à Lauren, je devrais la laisser continuer un petit moment.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1915
translate french christmas_shoot_a13aaded:

    # R "Can you girls keep holding that pose for a bit? There's something funny going on with this camera, just hold that until I get it working."
    R "Pouvez-vous les filles garder cette pose un peu? Il se passe quelque chose de drôle avec cet appareil photo, tenez-le jusqu'à ce que je le fasse fonctionner."

# game/UpdateScriptChristmasEvent.rpy:1918
translate french christmas_shoot_fadaf516:

    # LT "{i}You've got to be kidding me!{/i}"
    LT "{i}Tu dois te moquer de moi!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1919
translate french christmas_shoot_c46e5f40:

    # S "(whispering) Oh my God Sidney!.... Today is your lucky day."
    S "(murmure) Oh mon dieu Sidney!.... Aujourd'hui est ton jour de chance."

# game/UpdateScriptChristmasEvent.rpy:1920
translate french christmas_shoot_1ad20c45:

    # L "(whispering) You're so mean today!"
    L "(murmure) Tu es si méchant aujourd'hui!"

# game/UpdateScriptChristmasEvent.rpy:1921
translate french christmas_shoot_c057f23d:

    # S "(whispering) I'll bet your slutty little pussy wishes I'd go further in, doesn't it?"
    S "(murmure) Je parie que ta petite chatte de salope souhaite que j'aille plus loin, n'est-ce pas?"

# game/UpdateScriptChristmasEvent.rpy:1922
translate french christmas_shoot_6673b4e8:

    # L "(whispering) Nnnn....nnoo...that's disgusting!"
    L "(murmure) Nonnn....nnonn...c'est dégoûtant!"

# game/UpdateScriptChristmasEvent.rpy:1923
translate french christmas_shoot_333e953d:

    # L "(whispering) I.... I don't.... want my own sister.... to fuck my pussy!"
    L "(murmure) Je.... Je ne le sais pas.... je veux ma propre sœur.... pour baiser sa chatte!"

# game/UpdateScriptChristmasEvent.rpy:1924
translate french christmas_shoot_a160c384:

    # S "(whispering) You don't sound very convincing."
    S "(murmure) Tu n'as pas l'air très convaincante."

# game/UpdateScriptChristmasEvent.rpy:1926
translate french christmas_shoot_6371a0d1:

    # L "(whispering) When did you become such a pervert?"
    L "(murmure) Quand es-tu devenue une telle perverse?"

# game/UpdateScriptChristmasEvent.rpy:1927
translate french christmas_shoot_cb122c21:

    # S "(whispering) hmmm.... well.... lately there's been some things going on, that have opened me up to be a little more sexually adventurous."
    S "(murmure) hmmm.... Bien.... dernièrement, il s'est passé des choses qui m'ont ouvert à être un peu plus aventureuse sexuellement."

# game/UpdateScriptChristmasEvent.rpy:1928
translate french christmas_shoot_4502d900:

    # ST "{i}Thank you [ryan].{/i}"
    ST "{i}Merci [ryan].{/i}"

# game/UpdateScriptChristmasEvent.rpy:1929
translate french christmas_shoot_08befc3a:

    # L "(whispering) You mean like molesting me in my sleep?"
    L "(murmure) Tu veux dire comme me molester dans mon sommeil?"

# game/UpdateScriptChristmasEvent.rpy:1930
translate french christmas_shoot_59158bb7:

    # L "Did you do that on purpose!?"
    L "As-tu fait cela exprès!?"

# game/UpdateScriptChristmasEvent.rpy:1931
translate french christmas_shoot_42d4d163:

    # S "Ssshhhh.... (whispering loudly) Of course I didn't."
    S "Ssshhhh.... (murmure fort) Bien sûr que non."

# game/UpdateScriptChristmasEvent.rpy:1932
translate french christmas_shoot_8482ecc8:

    # S "(whispering) But it did start a chain of events that brought me to where I am now."
    S "(murmure) Mais cela a déclenché une chaîne d'événements qui m'a amenée là où je suis maintenant."

# game/UpdateScriptChristmasEvent.rpy:1933
translate french christmas_shoot_c9f13534:

    # S "(whispering) Maybe I'll tell you some day."
    S "(murmure) Peut-être que je te le dirai un jour."

# game/UpdateScriptChristmasEvent.rpy:1934
translate french christmas_shoot_45599a4a:

    # S "(whispering) Maybe it would help you be a little more adventurous."
    S "(murmure) Peut-être que cela t'aiderait à être un peu plus aventureuse."

# game/UpdateScriptChristmasEvent.rpy:1935
translate french christmas_shoot_2a3d9838:

    # LT "{i}She has no idea!{/i}"
    LT "{i}Elle n'en a aucune idée!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1936
translate french christmas_shoot_cbfe314c:

    # LT "{i}She would probably think I'm such a dirty pervert if she knew that I've been dry humping my own brother.{/i}"
    LT "{i}Elle penserait probablement que je suis une sale perverese si elle savait que j'ai baisé avec mon propre frère.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1937
translate french christmas_shoot_94dccf8b:

    # LT "{i}Oh I wish it was his cock rubbing against my pussy!{/i}"
    LT "{i}Oh j'aimerais que ce soit sa bite qui se frotte contre ma chatte!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1938
translate french christmas_shoot_0598d82b:

    # RT "{i}Holy shit!.... If this isn't a dream, I could really ramp things up between the two of them.{/i}"
    RT "{i}Merde!.... Si ce n'est pas un rêve, je pourrais vraiment accélérer les choses entre eux deux.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1939
translate french christmas_shoot_949d8bde:

    # RT "{i}But how will I ever be sure?{/i}"
    RT "{i}Mais comment en serai-je jamais sûr ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1940
translate french christmas_shoot_8a480287:

    # L "(whispering loudly) Aaahhhhh.... Fuck you, Sidney.... you're driving me completely crazy! Just fuck my pussy already!"
    L "(murmure fort) Aaahhhhh.... Va te faire foutre, Sidney.... tu me rends complètement folle! Baise déjà ma chatte!"

# game/UpdateScriptChristmasEvent.rpy:1947
translate french christmas_shoot_20100ce8:

    # S "(whispering) Are you fucking kidding me?"
    S "(murmure) Tu te fous de moi?"

# game/UpdateScriptChristmasEvent.rpy:1948
translate french christmas_shoot_f5050d2f:

    # L "(whispering) No.... you ho! You started this."
    L "(murmure) Non.... toi ho! Tu as commencé ça."

# game/UpdateScriptChristmasEvent.rpy:1949
translate french christmas_shoot_0683e499:

    # L "(whispering) I'm literally dying with lust right now!"
    L "(murmure) Je meurs littéralement de convoitise en ce moment!"

# game/UpdateScriptChristmasEvent.rpy:1950
translate french christmas_shoot_36929394:

    # S "(whispering) Is it really turning you on that bad to have your sister rub your pussy?"
    S "(murmure) Est-ce que ça t'excite vraiment d'avoir ta sœur qui se frotte ta chatte?"

# game/UpdateScriptChristmasEvent.rpy:1951
translate french christmas_shoot_70aaaab5:

    # L "(whispering) Well, obviously I'm thinking of someone else while you're doing that."
    L "(murmure) Eh bien, évidemment, je pense à quelqu'un d'autre pendant que tu fais ça."

# game/UpdateScriptChristmasEvent.rpy:1952
translate french christmas_shoot_14c6461d:

    # LT "{i} Like my brother's cock.... shit!.... That feels sooo wrong when I think those things!.... So why does it turn me on so bad?{/i}"
    LT "{i} Comme la bite de mon frère.... Merde!.... C'est tellement mal quand je pense à ces choses!.... Alors pourquoi ça m'excite autant?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1953
translate french christmas_shoot_737b42e7:

    # L "(whispering) I will admit, having you do this to me, right in front of [ryan], and he has no idea, is making this so much hotter!"
    L "(murmure) Je vais admettre que tu me fais ça, juste en face de [ryan], et qu'il n'en a aucune idée, rend cela tellement plus chaud!"

# game/UpdateScriptChristmasEvent.rpy:1954
translate french christmas_shoot_9bb79db8:

    # S "(whispering) Aren't you worried that he'll notice?"
    S "(murmure) Ne crais-tu pas qu'il s'en aperçoive?"

# game/UpdateScriptChristmasEvent.rpy:1955
translate french christmas_shoot_4f355d2f:

    # L "(whispering) No.... he'll think you're just sliding it between my thighs like he suggested.... plus he's got all his attention focused on his camera right now."
    L "(murmure) Non.... il pensera que tu glisses entre mes cuisses comme il l'a suggérée.... de plus, toute son attention est concentrée sur son appareil photo en ce moment."

# game/UpdateScriptChristmasEvent.rpy:1956
translate french christmas_shoot_4e912e41:

    # S "(whispering) Alright.... you asked for it!"
    S "(murmure) D'accord.... tu l'as demandé!"

# game/UpdateScriptChristmasEvent.rpy:1962
translate french christmas_shoot_f2ab2857:

    # L "Oh, my fuck!..."
    L "Oh, mon putain!..."

# game/UpdateScriptChristmasEvent.rpy:1963
translate french christmas_shoot_7b4188c4:

    # R "Are you ok over there?"
    R "Tu vas bien ?"

# game/UpdateScriptChristmasEvent.rpy:1964
translate french christmas_shoot_9005b5b9:

    # S "(whispering loudly) Lauren!.... What the fuck?"
    S "(murmure fort) Lauren!.... C'est quoi ce bordel?"

# game/UpdateScriptChristmasEvent.rpy:1965
translate french christmas_shoot_4055e236:

    # L "Yeah.... I'm.... ok.... I think I just.... kneeled on a.... pebble."
    L "Ouais.... Je le suis.... OK.... Je crois juste.... agenouillé sur un.... galet."

# game/UpdateScriptChristmasEvent.rpy:1966
translate french christmas_shoot_dafa6357:

    # R "Ok good!.... I'm glad that's all it is."
    R "Ok bon!.... Je suis content que ce soit tout."

# game/UpdateScriptChristmasEvent.rpy:1967
translate french christmas_shoot_718cc5d5:

    # R "I'll be done with the camera soon. Just hang on a little longer!"
    R "J'en aurai bientôt fini avec la caméra. Accrochez-vous un peu plus longtemps!"

# game/UpdateScriptChristmasEvent.rpy:1968
translate french christmas_shoot_59b848b0:

    # RT "{i}Holy Futa God!.... Sidney penetrated Lauren before I got to!{/i}"
    RT "{i}Sainte mère de Dieu !.... Sidney a pénétré Lauren avant que je n'arrive!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1969
translate french christmas_shoot_b525f35e:

    # RT "{i}I'm kind of jealous!{/i}"
    RT "{i}Je suis un peu jaloux!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1970
translate french christmas_shoot_cdd27b77:

    # RT "{i}But at the same time.... that's pretty fucking hot!{/i}"
    RT "{i}Mais en même temps.... c'est assez chaud !{/i}"

# game/UpdateScriptChristmasEvent.rpy:1973
translate french christmas_shoot_233f0c3f:

    # LT "{i}Holy fuck!.... My sister is fucking me.... right in front of my brother.... and he has no idea!....{/i}"
    LT "{i}Putain!.... Ma soeur me baise.... juste devant mon frère.... et il n'en a aucune idée !....{/i}"

# game/UpdateScriptChristmasEvent.rpy:1974
translate french christmas_shoot_da540a13:

    # RT "{i}Well, maybe more than you think.{/i}"
    RT "{i}Eh bien, peut-être plus que tu ne le penses.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1975
translate french christmas_shoot_f604a0b5:

    # LT "{i}I've never been more turned on in my life!{/i}"
    LT "{i}Je n'ai jamais été aussi excitée de ma vie!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1976
translate french christmas_shoot_1ba4d501:

    # LT "{i}Oh....no.... what if I cum?.... It will be so obvious if I squirt all over the green screen!{/i}"
    LT "{i}Oh....no .... et si je jouis?.... Ce sera tellement évident si je gicle partout sur l'écran vert!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1977
translate french christmas_shoot_bc81d670:

    # LT "{i}Fuck!.... What will.... [ryan] think?....{/i}"
    LT "{i}Merde!.... Qu'est-ce qui.... que [ryan] en pensent?....{/i}"

# game/UpdateScriptChristmasEvent.rpy:1980
translate french christmas_shoot_2f5d577e:

    # LT "{i}I can't believe Sidney is into this!.... Now we have a secret together.{/i}"
    LT "{i}Je n'arrive pas à croire que Sidney soit dans ce domaine!.... Maintenant, nous avons un secret ensemble.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1981
translate french christmas_shoot_2da5c7c2:

    # LT "{i}Now we can do some real sisterly bonding in the future!{/i}"
    LT "{i}Maintenant, nous pouvons créer de véritables liens fraternels à l'avenir!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1982
translate french christmas_shoot_e87aa4e7:

    # LT "{i}I wonder if she'll be up for using Mr. Snuggles' strap-on?{/i}"
    LT "{i}Je me demande si elle sera prête à utiliser le gode ceinture de m. Snuggles?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1983
translate french christmas_shoot_c2423b8e:

    # LT "{i}Fuck!.... I can just imagine her pounding me from behind!{/i}"
    LT "{i}Merde! .... Je peux juste imaginer qu'elle me pousse à partir de derrière!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1986
translate french christmas_shoot_138222af:

    # ST "{i}Why am I enjoying this so much?{/i}"
    ST "{i}Pourquoi est-ce que j'apprécie autant cela?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1987
translate french christmas_shoot_d7137e18:

    # ST "{i}It's not like it's my pussy that's getting all the attention.{/i}"
    ST "{i}Ce n'est pas comme si c'était ma chatte qui attirait toute l'attention.{/i}"

# game/UpdateScriptChristmasEvent.rpy:1988
translate french christmas_shoot_fafcd75b:

    # ST "{i}Good God!.... I can't believe this is happening!{/i}"
    ST "{i}Mon Dieu!... Je n'arrive pas à croire que ça arrive!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1989
translate french christmas_shoot_f9c59704:

    # ST "{i}One day I'm sucking my brother's dick, and the next day I'm pounding my sister's pussy!{/i}"
    ST "{i}Un jour, je suce la bite de mon frère et le lendemain, je lèche la chatte de ma sœur!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1992
translate french christmas_shoot_507659a1:

    # ST "{i}This is so bad!{/i}"
    ST "{i}C'est tellement mal!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1993
translate french christmas_shoot_fe968288:

    # ST "{i}I'm not supposed to be turned on by my siblings!{/i}"
    ST "{i}Je ne suis pas censé être allumée par mes frères et sœurs!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1994
translate french christmas_shoot_afd65054:

    # ST "{i}What's next? Fantasies about my mom?{/i}"
    ST "{i}Et maintenant ? Des fantasmes à propos de ma mère ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:1995
translate french christmas_shoot_c47b528a:

    # ST "{i}Shit, Sidney!.... Don't even go there!{/i}"
    ST "{i}Merde, Sidney!.... N'y vas pas !{/i}"

# game/UpdateScriptChristmasEvent.rpy:1996
translate french christmas_shoot_0b6cd0f2:

    # ST "{i}With her perfect ass and titties!{/i}"
    ST "{i}Avec son cul et ses nichons parfaits!{/i}"

# game/UpdateScriptChristmasEvent.rpy:1999
translate french christmas_shoot_8df0d47b:

    # ST "{i}Aaahhh.... maybe I should just embrace my perversions.{/i}"
    ST "{i}Aaahhh.... je devrais peut-être assumer mes perversions.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2000
translate french christmas_shoot_424bd220:

    # ST "{i}[ryan] doesn't seem to follow any kind of code of ethics.{/i}"
    ST "{i}[ryan] ne semble suivre aucune sorte de code d'éthique.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2001
translate french christmas_shoot_2c23660a:

    # RT "{i}Ouch!.... Yeah.... I kind of let those ethics die the night I saw Mom at the club.... and started letting my cock do all my thinking.{/i}"
    RT "{i}Aïe!.... Ouais.... J'ai un peu laissé mourir cette éthique la nuit où j'ai vu maman au club.... et j'ai commencé à laisser ma bite faire toute ma réflexion.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2004
translate french christmas_shoot_c9b33e31:

    # RT "{i}Maybe poor self control is a weakness in our genetics.{/i}"
    RT "{i}Peut-être qu'une mauvaise maîtrise de soi est une faiblesse de notre lignée génétique.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2005
translate french christmas_shoot_9e99cc58:

    # RT "{i}Hmmmm.... maybe a weakness that I can take advantage of.{/i}"
    RT "{i}Hmmmm.... peut-être une faiblesse dont je peux profiter.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2007
translate french christmas_shoot_3cb6a5d9:

    # LT "{i}Fuck.... Fuck.... Fuck.... Fuck....{/i}"
    LT "{i}Merde.... Merde.... Merde.... Merde....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2008
translate french christmas_shoot_48963a34:

    # LT "{i}I'm getting sooo close!{/i}"
    LT "{i}Je suis tellllemmmenttt proche d'arriver!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2010
translate french christmas_shoot_3cb6a5d9_1:

    # LT "{i}Fuck.... Fuck.... Fuck.... Fuck....{/i}"
    LT "{i}Merde.... Merde.... Merde.... Merde....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2011
translate french christmas_shoot_677c960b:

    # LT "{i}What am I going to do?{/i}"
    LT "{i}Qu'est-ce que je vais faire?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2012
translate french christmas_shoot_db3f8241:

    # LT "{i}I can't control if I squirt or not!{/i}"
    LT "{i}Je ne peux pas me contrôler si je gicle ou non!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2013
translate french christmas_shoot_eb2b349a:

    # LT "{i}And as turned on as I am, it's basically a sure thing!{/i}"
    LT "{i}Et aussi ouverte que je le suis, c'est fondamentalement une bonne chose!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2014
translate french christmas_shoot_94a71be6:

    # RT "{i}I should stop her before she makes a mess!.... Plus if she's not allowed a release, maybe she'll be dying to do something with me later.{/i}"
    RT "{i}Je devrais l'arrêter avant qu'elle ne fasse une connerie !.... De plus, si elle n'a pas le droit à jouir, elle mourra peut-être d'envie de faire quelque chose avec moi plus tard.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2015
translate french christmas_shoot_28515740:

    # R "I got the camera working! Good job girls! That's a very convincing pose."
    R "J'ai fait fonctionner la caméra! Bon travail les filles! C'est une pose très convaincante."

# game/UpdateScriptChristmasEvent.rpy:2020
translate french christmas_shoot_072e9c85:

    # LT "{i}Fuck.... I was so close!{/i}"
    LT "{i}Merde.... J'étais si proche d'arriver!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2023
translate french christmas_shoot_05e9429f:

    # LT "{i}Oh, thank God he stopped her!{/i}"
    LT "{i}Oh, Dieu merci, il l'a arrêtée!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2024
translate french christmas_shoot_dd0d8bf5:

    # LT "{i}As much as I wanted that.... that would have been a disaster.{/i}"
    LT "{i}Autant que je le voulais.... ça aurait été un désastre.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2027
translate french christmas_shoot_891a3d24:

    # LT "{i}But now I really need to get off! How am I going to take care of this?{/i}"
    LT "{i}Mais maintenant j'ai vraiment besoin de sortir! Comment vais-je le faire?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2031
translate french christmas_shoot_9e43fdad:

    # R "You girls did really good today!"
    R "Vous les filles avez vraiment bien fait aujourd'hui!"

# game/UpdateScriptChristmasEvent.rpy:2032
translate french christmas_shoot_d3d9d13a:

    # R "I think we need to have you two pose together more often."
    R "Je pense que nous devons vous faire poser ensemble plus souvent."

# game/UpdateScriptChristmasEvent.rpy:2033
translate french christmas_shoot_e67254ad:

    # L "I'm all for it!"
    L "Je suis pour ça !"

# game/UpdateScriptChristmasEvent.rpy:2034
translate french christmas_shoot_cb5ef409:

    # S "Yeah, that was kind of fun!"
    S "Ouais, c'était plutôt amusant!"

# game/UpdateScriptChristmasEvent.rpy:2037
translate french christmas_shoot_0ddfa8ae:

    # S "(whispering) Lauren? Are you ok?.... I know you didn't get to satisfy your itch."
    S "(murmure) Lauren? Est-ce que tu vas bien?.... Je sais que tu n'as pas pu satisfaire tes démangeaisons."

# game/UpdateScriptChristmasEvent.rpy:2040
translate french christmas_shoot_a61b4aad:

    # L "(whispering) No!.... I'm so horny right now!"
    L "(murmure) Non!.... Je suis tellement excitée en ce moment!"

# game/UpdateScriptChristmasEvent.rpy:2041
translate french christmas_shoot_63557893:

    # RT "{i}I'll help her take care of that.{/i}"
    RT "{i}Je vais l'aider à s'en occuper.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2044
translate french christmas_shoot_2796dcfe:

    # S "(whispering) When we go to the storage shed to change, I'll finish what I started.... I can't leave my own sister in such pain."
    S "(murmure) Quand nous irons au hangar de stockage pour nous changer, je terminerais ce que j'ai commencé.... Je ne peux pas laisser ma propre sœur dans un tel état d'exitation."

# game/UpdateScriptChristmasEvent.rpy:2047
translate french christmas_shoot_73b4864e:

    # L "(whispering) Oh my God!.... I have the best sister in the world.... I can't wait!"
    L "(murmure) Oh mon Dieu!.... J'ai la meilleure soeur du monde.... J'ai hâte!"

# game/UpdateScriptChristmasEvent.rpy:2048
translate french christmas_shoot_435f48e5:

    # RT "{i}Shit! Sidney's taking over!{/i}"
    RT "{i}Merde! Sidney prend le relais !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2051
translate french christmas_shoot_2e21b49f:

    # R "Uhh.... Sidney, we're all done here.... you can run home. Lauren?.... Would you mind sticking around here and helping me clean up?"
    R "Heu.... Sidney, on a tous fini ici.... tu peux rentrer chez toi. Lauren ?.... Pourrais-tu rester ici et m'aider à nettoyer?"

# game/UpdateScriptChristmasEvent.rpy:2052
translate french christmas_shoot_11e22be4:

    # L "Sorry [ryan], but Sidney and I have plans to go shopping right now. Normally I'd stay and help, but we need to get our shopping done before all the stores close."
    L "Désolé [ryan], mais Sidney et moi avons l'intention d'aller faire du shopping en ce moment. Normalement, je resterais et j'aiderais, mais nous devons faire nos courses avant que tous les magasins ne ferment."

# game/UpdateScriptChristmasEvent.rpy:2053
translate french christmas_shoot_81b201a3:

    # L "I'm really sorry!"
    L "Je suis vraiment désolé!"

# game/UpdateScriptChristmasEvent.rpy:2054
translate french christmas_shoot_d0a99d8c:

    # RT "{i}Yeah.... sure you are.... at least that storage shed has so many holes in it, it will be easy to spy on them.{/i}"
    RT "{i}Ouais.... bien sûr que tu l'es.... au moins ce hangar de stockage a tellement de trous dedans, il sera facile de les espionner.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2055
translate french christmas_shoot_33f13380:

    # R "Alright.... I understand. I'll take care of it myself. Go ahead and get changed."
    R "D'accord.... Je comprends. Je m'en occuperai moi-même. Allez-y et changez-vous."

# game/UpdateScriptChristmasEvent.rpy:2058
translate french christmas_shoot_3336f244:

    # {i}\"Sound of girls giggling as they run off.\"{/i}"
    "{i}'Bruit de filles qui ricanent alors qu'elles s'enfuient.'{/i}"

# game/UpdateScriptChristmasEvent.rpy:2059
translate french christmas_shoot_744aca6f:

    # RT "{i}Dammit!.... Sidney cock-blocked me!{/i}"
    RT "{i}Bon sang ! .... La bite de Sidney m'a bloqué ! {/i}"

# game/UpdateScriptChristmasEvent.rpy:2060
translate french christmas_shoot_80665c37:

    # RT "{i}I'll give them just a second, and then I'm definitely spying on them.{/i}"
    RT "{i}Je leur donnerai juste une seconde, puis je les espionnerai certainement.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2064
translate french christmas_shoot_01d5fc25:

    # R "Ahh.... shit! You scared me!"
    R "Ahh.... Merde! Tu m'as fait peur !"

# game/UpdateScriptChristmasEvent.rpy:2065
translate french christmas_shoot_5bf2102f:

    # R "Feeling any better?"
    R "Tu te sens mieux?"

# game/UpdateScriptChristmasEvent.rpy:2066
translate french christmas_shoot_f5149242:

    # GCP "Well, the alcohol has numbed me a little, but no, I'm still feeling pretty shitty."
    GCP "Eh bien, l'alcool m'a un peu engourdi, mais non, je me sens toujours assez merdique."

# game/UpdateScriptChristmasEvent.rpy:2067
translate french christmas_shoot_90b33d3c:

    # GCP "How were things while I was gone?"
    GCP "Comment étaient les choses pendant mon absence ?"

# game/UpdateScriptChristmasEvent.rpy:2068
translate french christmas_shoot_924fc367:

    # R "Oh, you know.... nothing eventful happened here."
    R "Oh, tu sais.... rien de mouvementé ne s'est passé ici."

# game/UpdateScriptChristmasEvent.rpy:2069
translate french christmas_shoot_2b0ac8ec:

    # GCP "You don't have to lie to me. All your thoughts were so sexually charged, I could hear them all the way in the bar down the street."
    GCP "Tu n'as pas à me mentir. Toutes tes pensées étaient si chargées sexuellement que je les entendais tout le long dans le bar de la rue."

# game/UpdateScriptChristmasEvent.rpy:2070
translate french christmas_shoot_8ad287a2:

    # GCP "I have never failed so spectacularly in all the decades I've been doing this."
    GCP "Je n'ai jamais échoué aussi spectaculairement dans toutes les décennies que j'ai fais cela."

# game/UpdateScriptChristmasEvent.rpy:2071
translate french christmas_shoot_f1c8567d:

    # GCP "I won't be gettin my wings anytime soon."
    GCP "Je ne serai pas de sitôt sous mes ailes."

# game/UpdateScriptChristmasEvent.rpy:2072
translate french christmas_shoot_b49cd84c:

    # GCP "Plus, God's definitely going to ream my tiny ass with her huge futa cock, when I get back."
    GCP "De plus, Dieu va certainement aléser mon petit cul avec son énorme coq de futa, quand je reviendrai."

# game/UpdateScriptChristmasEvent.rpy:2073
translate french christmas_shoot_63850416:

    # R "Oh, God!.... I'm so sorry!"
    R "Oh, mon Dieu!.... Je suis vraiment désolée !"

# game/UpdateScriptChristmasEvent.rpy:2074
translate french christmas_shoot_ce30431f:

    # R "But if it makes you feel better.... I learned a couple really valuable things."
    R "Mais si cela te fait te sentir mieux.... J'ai appris quelques choses vraiment précieuses."

# game/UpdateScriptChristmasEvent.rpy:2075
translate french christmas_shoot_6678e393:

    # GCP "Oh?.... Like what?"
    GCP "Oh?.... Comme quoi ?"

# game/UpdateScriptChristmasEvent.rpy:2076
translate french christmas_shoot_0d76341d:

    # R "I know that Sidney and Lauren are starting to think like me..."
    R "Je sais que Sidney et Lauren commencent à penser comme moi..."

# game/UpdateScriptChristmasEvent.rpy:2077
translate french christmas_shoot_e10e8622:

    # GCP "(sarcastically) Oh, wonderful."
    GCP "(sarcastiquement) Oh, merveilleux."

# game/UpdateScriptChristmasEvent.rpy:2078
translate french christmas_shoot_30e02bf7:

    # R "And I learned that I need to keep them from getting too intimate with each other right away, or they're going to cut me out of their sex lives."
    R "Et j'ai appris que je devais les empêcher de devenir trop intimes l'une avec l'autre tout de suite, sinon elles vont me couper de leur vie sexuelle."

# game/UpdateScriptChristmasEvent.rpy:2079
translate french christmas_shoot_dd8296f2:

    # R "I think I need to work on getting each one of them attached to me more, before I bring them together."
    R "Je pense que je dois travailler à m'attacher davantage à chacun d'elles, avant de les réunir."

# game/UpdateScriptChristmasEvent.rpy:2080
translate french christmas_shoot_a98dab55:

    # GCP "(sarcastically) What a treasure trove of moral lessons you've learned today."
    GCP "(sarcastiquement) Quel trésor de leçons morales tu as apprises aujourd'hui."

# game/UpdateScriptChristmasEvent.rpy:2081
translate french christmas_shoot_8f328987:

    # R "I really am sorry it didn't work out for you the way you wanted. But if you'll now excuse me. I've got a pretty hot event I need to go spy on."
    R "Je suis vraiment désolé que cela n'ait pas fonctionné pour toi comme tu le voulais. Mais excusez-moi maintenant. J'ai un événement assez chaud que je dois espionner."

# game/UpdateScriptChristmasEvent.rpy:2082
translate french christmas_shoot_57f382dc:

    # R "Haha.... yeah, you know what I'm talking about."
    R "Haha.... ouais, tu sais de quoi je parle."

# game/UpdateScriptChristmasEvent.rpy:2083
translate french christmas_shoot_0d29aec3:

    # GCP "Yeah, sorry about that, but you won't have time."
    GCP "Ouais, désolé pour ça, mais tu n'auras pas le temps."

# game/UpdateScriptChristmasEvent.rpy:2084
translate french christmas_shoot_4881927b:

    # R "What?"
    R "Quoi?"

# game/UpdateScriptChristmasEvent.rpy:2085
translate french christmas_shoot_2af19843:

    # GCP "Yeah, it's about time to go."
    GCP "Oui, il est temps de partir."

# game/UpdateScriptChristmasEvent.rpy:2086
translate french christmas_shoot_07a3f443:

    # GCP "You can expect the third, and final ghost, when the clock strikes three."
    GCP "Tu peux t'attendre au troisième et dernier fantôme lorsque l'horloge sonnera trois fois."

# game/UpdateScriptChristmasEvent.rpy:2087
translate french christmas_shoot_87163a6f:

    # R "No, I don't want to go yet!.... And once again, we don't have a clock tower."
    R "Non, je ne veux pas encore y aller!.... Et encore une fois, nous n'avons pas de tour d'horloge."

# game/UpdateScriptChristmasEvent.rpy:2088
translate french christmas_shoot_228ab36d:

    # GCP "In 5.... 4.... 3..."
    GCP "Dans 5.... 4.... 3..."

# game/UpdateScriptChristmasEvent.rpy:2089
translate french christmas_shoot_9e01f8fc:

    # R "Nooooo!..."
    R "Nooooonnn!..."

# game/UpdateScriptChristmasEvent.rpy:2090
translate french christmas_shoot_4b36f23f:

    # GCP "2.... 1..."
    GCP "2.... 1..."

# game/UpdateScriptChristmasEvent.rpy:2097
translate french christmas_shoot_cca4618a:

    # RT "{i}Oh Futa God.... I'm back in my bed?.... Another weird dream?{/i}"
    RT "{i}Oh mon Dieu.... Je suis de retour dans mon lit ?.... Un autre rêve bizarre ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2098
translate french christmas_shoot_636f2d5c:

    # RT "{i}At least, I think they were dreams!{/i}"
    RT "{i}Au moins, je pense que c’étaient des rêves ! {/i}"

# game/UpdateScriptChristmasEvent.rpy:2099
translate french christmas_shoot_4ac6c243:

    # RT "{i}They were both just so vivid! Huh.... {/i}"
    RT "{i}Ils étaient tous les deux si vifs! Huh.... {/i}"

# game/UpdateScriptChristmasEvent.rpy:2100
translate french christmas_shoot_91c22639:

    # RT "{i}I guess I'll have to wait till three o'clock to find out.{/i}"
    RT "{i}Je devrais attendre jusqu'à trois heures pour le savoir.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2101
translate french christmas_shoot_e44c0b82:

    # RT "{i}Ohhh.... so tired.... zzzzzzz....{/i}"
    RT "{i}Ohhh.... tellement fatigué.... zzzzzzz....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2107
translate french christmas_shoot_9ecfe47e:

    # R "Yeah, I don't think so! I'm going to pay you the $500, and then you can take your broom and fly away."
    R "Ouais, je ne pense pas! Je vais te payer les 500$, et tu pourras prendre ton balai et t'envoler."

# game/UpdateScriptChristmasEvent.rpy:2110
translate french christmas_shoot_6c3b1d98:

    # RT "{i}I'll just pull it out of my magic pockets.... shit!.... It's not working!{/i}"
    RT "{i}Je vais juste le sortir de mes poches magiques.... Merde!.... Ça ne marche pas !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2111
translate french christmas_shoot_4fa69d6c:

    # RT "{i}Ghost of Christmas Present! Why aren't my magic pockets working?{/i}"
    RT "{i}Cadeau Fantôme de Noël! Pourquoi mes poches magiques ne fonctionnent-elles pas ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2112
translate french christmas_shoot_b56a3418:

    # GCP "Magic Pockets?"
    GCP "Poches magiques ?"

# game/UpdateScriptChristmasEvent.rpy:2113
translate french christmas_shoot_e9599707:

    # RT "{i}Yeah, in the past, I could just think about what I needed, and then I could pull it out of my pockets.{/i}"
    RT "{i}Ouais, dans le passé, je pouvais juste penser à ce dont j'avais besoin, et ensuite je l'avais.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2114
translate french christmas_shoot_1909b358:

    # GCP "Oh, right!"
    GCP "Oh, c'est ça!"

# game/UpdateScriptChristmasEvent.rpy:2115
translate french christmas_shoot_2bca0929:

    # GCP "You see, each ghost has one magic trick. The Ghost of Christmas Past could make you produce anything you needed."
    GCP "Tu vois, chaque fantôme a un tour de magie. Le Fantôme de Noël passé pourrait te faire produire tout ce dont tu as besoin."

# game/UpdateScriptChristmasEvent.rpy:2116
translate french christmas_shoot_466b0a56:

    # GCP "But my magic trick is to let you read minds, so sorry! No magic pockets."
    GCP "Mais mon tour de magie est de te laisser lire dans les pensées, alors désolé! Pas de poches magiques."

# game/UpdateScriptChristmasEvent.rpy:2117
translate french christmas_shoot_38c90e91:

    # GCP "But damn, I wish you had them. We need this bitch to leave!"
    GCP "Mais bon sang, j'aimerais que tu les aies. Il faut que cette salope parte !"

# game/UpdateScriptChristmasEvent.rpy:2118
translate french christmas_shoot_32fcb17d:

    # R "Uhhh.... actually it seems that I don't have the money on me."
    R "Uhhh.... en fait, il semble que je n'ai pas l'argent sur moi."

# game/UpdateScriptChristmasEvent.rpy:2121
translate french christmas_shoot_8e7496c2:

    # AD "So.... I guess I'll show myself in then."
    AD "Alors, je suppose que je vais me montrer à ce moment-là."

# game/UpdateScriptChristmasEvent.rpy:2122
translate french christmas_shoot_37ce8e08:

    # RT "{i}Shit!{/i}"
    RT "{i}Merde!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2123
translate french christmas_shoot_e36900cf:

    # GCP "Shit!"
    GCP "Merde!"

# game/UpdateScriptChristmasEvent.rpy:2126
translate french christmas_shoot_1dac4b85:

    # R "Ummmm.... girls.... It seems that I forgot that I was bringing in an, ummm.... intern photography student.... who wants to help with the photo shoot today."
    R "UmmMm.... filles.... Il semble que j'ai oublié que j'apportais un, ummm.... étudiant en photographie stagiaire.... qui veut aider à la séance photo aujourd'hui."

# game/UpdateScriptChristmasEvent.rpy:2127
translate french christmas_shoot_a9572131:

    # {i}\"Furiously winking at Lauren.\"{/i}"
    "{i}'Clin d'œil furieux à Lauren.'{/i}"

# game/UpdateScriptChristmasEvent.rpy:2128
translate french christmas_shoot_7127d6ba:

    # S "You don't have to lie to us. Lauren just told me everything."
    S "Tu n'as pas à nous mentir. Lauren m'a tout dit."

# game/UpdateScriptChristmasEvent.rpy:2129
translate french christmas_shoot_92a483fa:

    # R "Everything?"
    R "Tout?"

# game/UpdateScriptChristmasEvent.rpy:2132
translate french christmas_shoot_6c722b76:

    # AD "Like how I'm extorting you for money and favors in exchange for letting you continue your legally questionable business and other activities, and also not throwing you in prison like your father?"
    AD "Comme la façon dont je t'extorque de l'argent et des faveurs en échange de te laisser poursuivre tes affaires légalement douteuses et d'autres activités, et de ne pas te jeter en prison comme ton père?"

# game/UpdateScriptChristmasEvent.rpy:2133
translate french christmas_shoot_d978f182:

    # AD "Is that what she told you?"
    AD "Est-ce ce qu'elle t'a dit?"

# game/UpdateScriptChristmasEvent.rpy:2134
translate french christmas_shoot_3115c941:

    # S "Basically."
    S "En gros."

# game/UpdateScriptChristmasEvent.rpy:2135
translate french christmas_shoot_9c607738:

    # AD "Great! I'm glad we're all up to speed."
    AD "Génial! Je suis content que nous soyons tous au courant."

# game/UpdateScriptChristmasEvent.rpy:2136
translate french christmas_shoot_72a6f832:

    # AD "And today is Thursday, and [ryan] failed to produce the needed cash for my buy off, so for my payment in favors, I get to take the pictures today instead."
    AD "Et aujourd'hui c'est jeudi, et [ryan] n'a pas réussi à obtenir l'argent nécessaire pour mon achat, donc pour mon paiement, je peux prendre les photos aujourd'hui à la place de ce payement."

# game/UpdateScriptChristmasEvent.rpy:2137
translate french christmas_shoot_5dd560f7:

    # GCP "No! If this photo shoot takes place, then I'll fail my part of the mission too!"
    GCP "Non! Si cette séance photo a lieu, alors j'échouerai aussi ma partie de la mission!"

# game/UpdateScriptChristmasEvent.rpy:2138
translate french christmas_shoot_362f5af3:

    # S "You can't do that!"
    S "Tu ne peux pas faire ça !"

# game/UpdateScriptChristmasEvent.rpy:2139
translate french christmas_shoot_07d4361c:

    # GCP "Yeah!"
    GCP "Ouais!"

# game/UpdateScriptChristmasEvent.rpy:2140
translate french christmas_shoot_909820f0:

    # AD "Oh, really? Because I happen to know that you make and get paid for costumes, as part of a business that is not in of itself illegal, but is hiding funds from the FBI to avoid garnishments."
    AD "Oh, vraiment? Parce que je sais que tu fabriques et tu es payé pour des costumes, dans le cadre d'une entreprise qui n'est pas en soi illégale, mais qui cache des fonds au FBI pour éviter les saisies."

# game/UpdateScriptChristmasEvent.rpy:2141
translate french christmas_shoot_5f1d3c14:

    # AD "Now, I can either keep this knowledge safe, and actually completely hide this illegal cash flow, or I can expose it, and you can do hard time in federal prison for financial fraud."
    AD "Maintenant, je peux soit garder cette information en sécurité, et en fait cacher complètement ces transactions financières illégal, soit je peux les révéler, et tu peux aller directement dans une prison fédérale pour fraude fiscale."

# game/UpdateScriptChristmasEvent.rpy:2142
translate french christmas_shoot_43fa7ba6:

    # S "You goddamn dirty cop!"
    S "Sale flic !"

# game/UpdateScriptChristmasEvent.rpy:2143
translate french christmas_shoot_5ae5d5a3:

    # GCP "You chode-munching, cock-gobbling, anal prolapse-licking, whore of Babylon!"
    GCP "Vous chode-grignotant, mangeant des bites, léchant des prolapsus anaux, pute de Babylone!"

# game/UpdateScriptChristmasEvent.rpy:2144
translate french christmas_shoot_add71553:

    # RT "{i}Are you ok?{/i}"
    RT "{i}Ça va ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2145
translate french christmas_shoot_c68a5876:

    # GCP "Uuuurrrggghhhh!!"
    GCP "Uuuurrrggghhhh!!"

# game/UpdateScriptChristmasEvent.rpy:2146
translate french christmas_shoot_90866e3e:

    # AD "(sarcastically) Ouch.... I can't tell you how much that hurts."
    AD "(ton sarcastique) Aïe.... Je ne peux pas te dire à quel point ça fait mal."

# game/UpdateScriptChristmasEvent.rpy:2147
translate french christmas_shoot_4b64039b:

    # AD "So, are you all going to play ball?"
    AD "Alors, allez-vous tous jouer au ballon?"

# game/UpdateScriptChristmasEvent.rpy:2148
translate french christmas_shoot_5594fc7c:

    # S "I don't see another choice."
    S "Je ne vois pas d'autre choix."

# game/UpdateScriptChristmasEvent.rpy:2149
translate french christmas_shoot_f82ef9e8:

    # AD "Good! I'm glad you're being sensible."
    AD "Bien! Je suis content que tu sois raisonnable."

# game/UpdateScriptChristmasEvent.rpy:2150
translate french christmas_shoot_420ba00f:

    # AD "[ryan], give me the camera, and girls, go do a nice pose! And make these pics good, or else!"
    AD "[ryan], donnes-moi la caméra, et les filles, allez faire une belle pose ! Et ces photos sont bonnes, sinon !"

# game/UpdateScriptChristmasEvent.rpy:2153
translate french christmas_shoot_7e7aadd2:

    # AD "Girls?.... While I'm figuring out this camera, run to my car and grab the two gifts in the back seat. They were for a couple of my girl friends, but I think we can make better use of them during the photo shoot."
    AD "Filles?.... Pendant que je cherche cet appareil photo, cours vers ma voiture et attrapes les deux cadeaux sur la banquette arrière. Ils étaient pour quelques-unes de mes amies, mais je pense que nous pouvons mieux les utiliser pendant la séance photo."

# game/UpdateScriptChristmasEvent.rpy:2154
translate french christmas_shoot_93462efb:

    # L "{i}I hate this bitch!{/i}"
    L "{i}Je déteste cette salope!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2155
translate french christmas_shoot_c3fe2f78:

    # GCP "I wish God was as vengeful to the wicked as she used to be. Turn her into a pillar of salt or better yet, a puddle of cum."
    GCP "J'aimerais que Dieu soit aussi vengeur envers les méchants qu'elle l'était auparavant. Transformes-la en un pilier de sel ou mieux encore, une flaque de sperme."

# game/UpdateScriptChristmasEvent.rpy:2161
translate french christmas_shoot_055a614c:

    # GCP "This is a disaster!"
    GCP "C'est un désastre!"

# game/UpdateScriptChristmasEvent.rpy:2162
translate french christmas_shoot_55d5db61:

    # RT "{i}Isn't there anything we can do to salvage things?{/i}"
    RT "{i}N'y a-t-il rien que nous puissions faire pour sauver les choses ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2163
translate french christmas_shoot_700ce378:

    # GCP "I don't know.... I give up.... try to keep your eyes out for some life lessons.... I guess."
    GCP "Je ne sais pas.... J'abandonne.... essaies de garder les yeux ouverts pour quelques leçons de vie.... Je suppose."

# game/UpdateScriptChristmasEvent.rpy:2164
translate french christmas_shoot_e4184402:

    # GCP "I'll be pillaging the bar down the street for some hard liquor. I'll be back when it's time to go."
    GCP "Je vais piller le bar dans la rue pour de l'alcool dur. Je reviendrai quand il sera temps de partir."

# game/UpdateScriptChristmasEvent.rpy:2168
translate french christmas_shoot_22415c11:

    # RT "{i}Wow, She really seems sad.{/i}"
    RT "{i}Wow, Elle semble vraiment triste.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2169
translate french christmas_shoot_9c06a9ee:

    # RT "{i}I hope Futa God isn't too hard on her.{/i}"
    RT "{i}J'espère que Dieu n'est pas trop dur avec elle.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2170
translate french christmas_shoot_88c2bc9c:

    # RT "{i}Haha....Futa God.... hard....{/i}"
    RT "{i}Haha....Putain de Dieu.... dur....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2171
translate french christmas_shoot_cc7297a3:

    # RT "{i}Too bad she left. That joke would have cheered her up.{/i}"
    RT "{i}Dommage qu'elle soit partie. Cette blague l'aurait encouragée.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2172
translate french christmas_shoot_9f1f2c39:

    # L "Ok, We're back!"
    L "Ok, nous sommes de retour!"

# game/UpdateScriptChristmasEvent.rpy:2173
translate french christmas_shoot_1cc4a049:

    # AD "Great! I think I've got the camera figured out, so go strike a pose."
    AD "Super! Je pense que j'ai trouvé la caméra, alors va prendre la pose."

# game/UpdateScriptChristmasEvent.rpy:2176
translate french christmas_shoot_e7ac152a:

    # L "Ok Agent Diaz, I think we're ready"
    L "Ok Agent Diaz, je pense que nous sommes prêtes"

# game/UpdateScriptChristmasEvent.rpy:2177
translate french christmas_shoot_88877bdd:

    # S "Is this ok?"
    S "Est-ce correct?"

# game/UpdateScriptChristmasEvent.rpy:2178
translate french christmas_shoot_6a383426:

    # AD "Great! I like the buddy-buddy pose."
    AD "Génial! J'aime la pose ami-ami."

# game/UpdateScriptChristmasEvent.rpy:2179
translate french christmas_shoot_d295a5a8_1:

    # R "That will make a great first pic."
    R "Qui fera une grande première photo."

# game/UpdateScriptChristmasEvent.rpy:2184
translate french christmas_shoot_67cae995:

    # AD "Can you add a homey Christmas background later or something?"
    AD "Peux-tu ajouter un arrière-plan de Noël plus tard ou quelque chose du genre?"

# game/UpdateScriptChristmasEvent.rpy:2185
translate french christmas_shoot_43f29d86:

    # R "Uh, yeah. That's what I'll do."
    R "Euh, ouais. C'est ce que je vais faire."

# game/UpdateScriptChristmasEvent.rpy:2190
translate french christmas_shoot_55158eaf:

    # AD "I can see it all now!"
    AD "Je peux tout voir maintenant!"

# game/UpdateScriptChristmasEvent.rpy:2192
translate french christmas_shoot_23b6798e:

    # AD "Ok.... now I want you to do some Christmassy things."
    AD "D'accord.... maintenant, je veux que tu fasses des choses de Noël."

# game/UpdateScriptChristmasEvent.rpy:2193
translate french christmas_shoot_f3e19616:

    # S "Like what?"
    S "Comme quoi?"

# game/UpdateScriptChristmasEvent.rpy:2194
translate french christmas_shoot_4e7a910e:

    # AD "I don't know. Do I have to think of everything for you?"
    AD "Je ne sais pas. Dois-je penser à tout pour toi?"

# game/UpdateScriptChristmasEvent.rpy:2195
translate french christmas_shoot_8f02dd6d:

    # L "I can make it look like I'm hanging Christmas tree ornaments, I guess."
    L "Je peux faire croire que j'accroche des ornements d'arbre de Noël, je suppose."

# game/UpdateScriptChristmasEvent.rpy:2196
translate french christmas_shoot_d146882e_1:

    # S "And I'll make it look like I'm hanging stockings."
    S "Et je vais faire croire que j'accroche des bas."

# game/UpdateScriptChristmasEvent.rpy:2197
translate french christmas_shoot_79decdaa:

    # AD "Yeah.... whatever! Just make it sexy!"
    AD "Ouais.... Si tu le dis! Rends-le sexy!"

# game/UpdateScriptChristmasEvent.rpy:2198
translate french christmas_shoot_a571aeb3:

    # ST "{i}Agent Diaz has a terrible disposition for a photographer.{/i}"
    ST "{i}L'agent Diaz a une disposition terrible pour un photographe.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2199
translate french christmas_shoot_31a2129a:

    # ST "{i}I miss how professional [ryan] is.{/i}"
    ST "{i}Ça me manque le professionnalisme de [ryan].{/i}"

# game/UpdateScriptChristmasEvent.rpy:2202
translate french christmas_shoot_6b544093:

    # AD "Nice! That looks super hot!"
    AD "Sympa! Ça a l'air super chaud!"

# game/UpdateScriptChristmasEvent.rpy:2207
translate french christmas_shoot_1a7bc2a0:

    # AD "I think I got a lady boner!"
    AD "Je pense que j'ai eu une érection féminine !"

# game/UpdateScriptChristmasEvent.rpy:2208
translate french christmas_shoot_af9dc667:

    # LT "{i}It's kind of fun posing with my sister, and Agent Diaz dominating this photo shoot is kind of turning me on.{/i}"
    LT "{i}C'est un peu amusant de poser avec ma sœur, et l'agent Diaz qui domine cette séance photo m'excite un peu.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2209
translate french christmas_shoot_8820a540:

    # ST "{i}My sister looks sexy as hell in that costume I made for her. Plus, it's kind of turning me on to pose in front of someone else besides [ryan].{/i}"
    ST "{i}Ma sœur a l'air sexy comme l'enfer dans ce costume que j'ai fait pour elle. De plus, ça m'excite de poser devant quelqu'un d'autre que [ryan].{/i}"

# game/UpdateScriptChristmasEvent.rpy:2210
translate french christmas_shoot_b9f19b8d:

    # RT "{i}This is insightful! I wish I could read minds all the time.{/i}"
    RT "{i}C'est perspicace! J'aimerais pouvoir lire dans les pensées tout le temps.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2211
translate french christmas_shoot_9e8995e1:

    # AD "Ok.... enough of that!.... I think it's time to pull out the presents I brought."
    AD "D'accord.... c'est assez!.... Je pense qu'il est temps de sortir les cadeaux que j'ai apportés."

# game/UpdateScriptChristmasEvent.rpy:2212
translate french christmas_shoot_295d36a0:

    # AD "Let's do some shots with you opening them."
    AD "Faisons quelques photos avec toi en les ouvrant."

# game/UpdateScriptChristmasEvent.rpy:2215
translate french christmas_shoot_7b30f61b:

    # AD "God, you two are sexy! I love those outfits, Sidney!"
    AD "Mon Dieu, vous deux êtes sexy! J'adore ces tenues, Sidney !"

# game/UpdateScriptChristmasEvent.rpy:2216
translate french christmas_shoot_be459f7f:

    # S "Uhh.... thanks?"
    S "Heu.... merci ?"

# game/UpdateScriptChristmasEvent.rpy:2217
translate french christmas_shoot_3e5af900:

    # AD "Yeah, I've got a great shot of both of your sexy young pussies."
    AD "Ouais, j'ai une superbe photo de tes deux jeunes chattes sexy."

# game/UpdateScriptChristmasEvent.rpy:2222
translate french christmas_shoot_dd777d5a:

    # AD "Once you get them open, show each other what was in them."
    AD "Une fois que tu les as ouverts, montres-nous ce qu'ils contenaient."

# game/UpdateScriptChristmasEvent.rpy:2223
translate french christmas_shoot_0cc71191:

    # AD "And try to look happy about what's inside!"
    AD "Et essaies de te réjouir de ce qui se trouve à l'intérieur !"

# game/UpdateScriptChristmasEvent.rpy:2226
translate french christmas_shoot_47cc17c7:

    # ST "{i}Oh God!.... Of course they're dildos{/i}"
    ST "{i}Oh Mon Dieu!.... Bien sûr, ce sont des godes {/i}"

# game/UpdateScriptChristmasEvent.rpy:2227
translate french christmas_shoot_27149ef4:

    # LT "{i}What a predatory pervert!{/i}"
    LT "{i}Quel prédateur pervers!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2228
translate french christmas_shoot_1f08d435:

    # AD "This is great! You two are great actresses!"
    AD "C'est génial! Vous êtes de grandes actrices !"

# game/UpdateScriptChristmasEvent.rpy:2233
translate french christmas_shoot_b6cdebda:

    # ST "{i}If she offers to screw me with it, I'm going home!{/i}"
    ST "{i}Si elle me propose de me baiser avec ça, je rentre à la maison!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2234
translate french christmas_shoot_8088d980:

    # LT "{i}What if she makes us use them?{/i}"
    LT "{i}Que faire si elle nous fait les utiliser? {/i}"

# game/UpdateScriptChristmasEvent.rpy:2235
translate french christmas_shoot_c46d2e6d:

    # AD "Alright.... why don't you two pretend to thank each other with a passionate kiss."
    AD "D'accord.... pourquoi ne pas vous remercier mutuellement avec un baiser passionné."

# game/UpdateScriptChristmasEvent.rpy:2238
translate french christmas_shoot_feb8d233_1:

    # S "What!?..."
    S "Quoi!?..."

# game/UpdateScriptChristmasEvent.rpy:2239
translate french christmas_shoot_784ed33e:

    # AD "For the photo shoot!"
    AD "Pour la séance photo!"

# game/UpdateScriptChristmasEvent.rpy:2240
translate french christmas_shoot_5d3fdfea:

    # AD "Not for real."
    AD "Pas pour de vrai."

# game/UpdateScriptChristmasEvent.rpy:2241
translate french christmas_shoot_f8f8dcdc:

    # AD "Though, I think you'd both enjoy a good makeout session with each other, or you can come make out with me if that doesn't do it for you."
    AD "Cependant, je pense que vous profiterez tous les deux d'une bonne session de maquillage l'une avec l'autre, ou vous pouvez venir vous faire plaisir avec moi si vous ne le faites pas pour vous."

# game/UpdateScriptChristmasEvent.rpy:2242
translate french christmas_shoot_c81fd647:

    # S "Gross!"
    S "Gros !"

# game/UpdateScriptChristmasEvent.rpy:2243
translate french christmas_shoot_d84cb42d:

    # S "I'd rather make out with Lauren, than with you."
    S "Je préfère sortir avec Lauren plutôt qu'avec toi."

# game/UpdateScriptChristmasEvent.rpy:2244
translate french christmas_shoot_fd2e7a77:

    # AD "Great, then go ahead and do that."
    AD "Génial, alors vas-y et fais-le."

# game/UpdateScriptChristmasEvent.rpy:2245
translate french christmas_shoot_237e9ea2_1:

    # L "But we're sisters!"
    L "Mais nous sommes sœurs!"

# game/UpdateScriptChristmasEvent.rpy:2246
translate french christmas_shoot_1e140025:

    # AD "Yeah, well, none of your online fans know that. And even if they did, it would just make you two even more popular."
    AD "Ouais, eh bien, aucun de tes fans en ligne ne le sait. Et même s'ils le savaient, cela te rendrait encore plus populaires."

# game/UpdateScriptChristmasEvent.rpy:2247
translate french christmas_shoot_853d43ee_1:

    # RT "{i}Maybe I should put that in their bios.{/i}"
    RT "{i}Peut-être que je devrais mettre cela dans leur bio. {/i}"

# game/UpdateScriptChristmasEvent.rpy:2248
translate french christmas_shoot_b1cc84fd:

    # AD "Are you gonna sit there like little bitches, or can we get on with this?"
    AD "Vas-tu t'asseoir là comme des petites salopes, ou pouvons-nous continuer?"

# game/UpdateScriptChristmasEvent.rpy:2249
translate french christmas_shoot_827334e2_1:

    # LT "{i}Oh my God! Will Sidney agree to do this with me? Why am I getting butterflies in my pussy when I think about it?{/i}"
    LT "{i}Oh mon Dieu! Sidney acceptera-t-elle de faire ça avec moi? Pourquoi ai-je des papillons dans la chatte quand j'y pense?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2250
translate french christmas_shoot_8e3fc8df_1:

    # L "Come on Sidney, Let's just get it over with."
    L "Allez Sidney, finissons-en."

# game/UpdateScriptChristmasEvent.rpy:2251
translate french christmas_shoot_c6190b95_1:

    # S "Fine!"
    S "Bien!"

# game/UpdateScriptChristmasEvent.rpy:2252
translate french christmas_shoot_4ec03217:

    # AD "Lauren, why don't you just straddle Sidney and..."
    AD "Lauren, pourquoi ne pas chevaucher Sidney et..."

# game/UpdateScriptChristmasEvent.rpy:2255
translate french christmas_shoot_2a7eae35:

    # AD "Oh, yeah.... that's perfect!"
    AD "Oh, oui.... c'est parfait !"

# game/UpdateScriptChristmasEvent.rpy:2256
translate french christmas_shoot_f1b3db03_1:

    # R "Wow! That's pretty hot!"
    R "Wow! C'est assez chaud!"

# game/UpdateScriptChristmasEvent.rpy:2257
translate french christmas_shoot_393537ee:

    # AD "I know, right?"
    AD "Je sais, non?"

# game/UpdateScriptChristmasEvent.rpy:2258
translate french christmas_shoot_826b8a70_1:

    # S "Just hurry up and take the picture!"
    S "Dépêches-toi et prends la photo!"

# game/UpdateScriptChristmasEvent.rpy:2261
translate french christmas_shoot_4deb5453_1:

    # ST "{i}Before my pussy betrays me and leaves a puddle on the floor!{/i}"
    ST "{i}Avant que ma chatte ne me trahisse et ne laisse une flaque d'eau sur le sol!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2262
translate french christmas_shoot_843e7d14_1:

    # ST "{i}This is so wrong!{/i}"
    ST "{i}C'est tellement mal!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2263
translate french christmas_shoot_4cb66561_1:

    # ST "{i}Why is it making my pussy tingle so much?{/i}"
    ST "{i}Pourquoi fait-il tant picoter ma chatte?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2264
translate french christmas_shoot_19da5e64_1:

    # LT "{i}Why does this turn me on so bad?{/i}"
    LT "{i}Pourquoi est-ce si mauvais d'être excitée par ça?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2265
translate french christmas_shoot_4dc43422:

    # LT "{i}I've already been having confusing thoughts about women, thanks to Agent Diaz, but now I'm getting turned on by my sister?{/i}"
    LT "{i}J'ai déjà eu des pensées confuses sur les femmes, grâce à l'agent Diaz, mais maintenant je suis excitée par ma sœur?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2266
translate french christmas_shoot_a9761b3c_1:

    # LT "{i}God, I hope I don't drip all over her lap.{/i}"
    LT "{i}Dieu, j'espère que je ne dégouline pas sur ses genoux.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2267
translate french christmas_shoot_38a2ac68_1:

    # LT "{i}I can't believe she made a costume that doesn't include panties.{/i}"
    LT "{i}Je n'arrive pas à croire qu'elle ait fait un costume qui n'inclut pas de culotte.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2268
translate french christmas_shoot_9d6d529c_1:

    # RT "{i}Wow! They're kind of into each other.{/i}"
    RT "{i}Wow! Ils sont en quelque sorte l'un dans l'autre.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2269
translate french christmas_shoot_bf987008_1:

    # RT "{i}It sounds like they just need a little persuading.{/i}"
    RT "{i}On dirait qu'ils ont juste besoin d'un peu de persuasion.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2274
translate french christmas_shoot_7ff41c1e:

    # AD "Great! I think we're ready for the next pose."
    AD "Génial! Je pense que nous sommes prêts pour la prochaine pose."

# game/UpdateScriptChristmasEvent.rpy:2276
translate french christmas_shoot_28c3dbd7:

    # AD "Unless you want to keep kissing, but I'm thinking we should take a few shots playing with your new toys."
    AD "Sauf si tu veux continuer à t'embrasser, mais je pense que nous devrions prendre quelques photos en jouant avec tes nouveaux jouets."

# game/UpdateScriptChristmasEvent.rpy:2278
translate french christmas_shoot_fe31079c_1:

    # S "Play with our..."
    S "Jouer avec notre..."

# game/UpdateScriptChristmasEvent.rpy:2279
translate french christmas_shoot_f7e421e8_1:

    # L "(Gulp) Toys?..."
    L "(Gulp) Jouets?..."

# game/UpdateScriptChristmasEvent.rpy:2280
translate french christmas_shoot_e4f25d36_1:

    # S "How do you mean?"
    S "Que veux-tu dire?"

# game/UpdateScriptChristmasEvent.rpy:2281
translate french christmas_shoot_200e626e:

    # LT "{i}Oh my.... she wants us to dildo each other.... shit!.... I think I just dripped onto Sidney's leg!{/i}"
    LT "{i}Oh mon dieu.... elle veut qu'on se gode.... Merde!.... Je crois que j'ai dégouliné de plaisir sur la jambe de Sidney!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2282
translate french christmas_shoot_64458996_1:

    # LT "{i}I hope she doesn't notice.{/i}"
    LT "{i}J'espère qu'elle ne le remarque pas.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2283
translate french christmas_shoot_7dcc4672_1:

    # ST "{i}Holy shit!.... I think Lauren just leaked onto my leg!.... Does the thought of us playing with toys together get her excited?{/i}"
    ST "{i}Merde!.... Je pense que Lauren vient de jouir sur ma jambe!.... Est-ce que l'idée de jouer avec des jouets ensemble l'a excitée?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2284
translate french christmas_shoot_02c43815:

    # AD "Haha.... I'm not expecting you to screw each other with your new dildos."
    AD "Haha.... Je ne m'attends pas à ce que vous vous embrassiez mutuellement avec vos nouveaux godes."

# game/UpdateScriptChristmasEvent.rpy:2285
translate french christmas_shoot_c50482e5_1:

    # RT "{i}These girls are going to make me nut in my pants!{/i}"
    RT "{i}Ces filles vont me rendre fou dans mon pantalon!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2286
translate french christmas_shoot_ce6a3b67:

    # AD "Just get into a position so it looks like Sidney is penetrating Lauren with the dildo."
    AD "Mets-toi juste dans une position pour qu'il semble que Sidney pénètre Lauren avec le gode."

# game/UpdateScriptChristmasEvent.rpy:2287
translate french christmas_shoot_612139ad:

    # AD "But you won't actually penetrate.... haha, unless you want to."
    AD "Mais tu ne pénétreras pas... haha, à moins que tu ne le veuilles."

# game/UpdateScriptChristmasEvent.rpy:2288
translate french christmas_shoot_5f488140:

    # AD "Whatever you choose, Just make it look like it."
    AD "Quoi que tu choisisses, Fais-le ressembler à ça."

# game/UpdateScriptChristmasEvent.rpy:2289
translate french christmas_shoot_814065ce:

    # AD "Unless you want to keep making out all day!"
    AD "Sauf si tu veux continuer à le faire toute la journée!"

# game/UpdateScriptChristmasEvent.rpy:2290
translate french christmas_shoot_0b2b81d3_1:

    # LT "{i}I kind of do.{/i}"
    LT "{i}Je le fais en quelque sorte.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2291
translate french christmas_shoot_b1735579:

    # S "Uuuhhh.... ok Lauren.... let's get.... into position."
    S "Uuuhhh.... ok Lauren.... allons-y.... en position."

# game/UpdateScriptChristmasEvent.rpy:2295
translate french christmas_shoot_f53eaa6f_1:

    # S "Does this look ok?"
    S "Est-ce que ça a l'air correct?"

# game/UpdateScriptChristmasEvent.rpy:2296
translate french christmas_shoot_c71cc244:

    # AD "Yeah, that's pretty convincing."
    AD "Ouais, c'est assez convaincant."

# game/UpdateScriptChristmasEvent.rpy:2297
translate french christmas_shoot_bdd3814a:

    # AD "Let me get a quick shot of it."
    AD "Laisses-moi y jeter un coup d’œil."

# game/UpdateScriptChristmasEvent.rpy:2302
translate french christmas_shoot_d86ddc78:

    # AD "Great! Now move the dildo a little closer."
    AD "Génial! Maintenant, rapproches le gode un peu plus près."

# game/UpdateScriptChristmasEvent.rpy:2303
translate french christmas_shoot_790a17c9:

    # AD "So.... just kind of prod and push the dildo between her thigh gap."
    AD "Donc .... juste une sorte de production et pousser le gode entre ses cuisses."

# game/UpdateScriptChristmasEvent.rpy:2304
translate french christmas_shoot_21784bd5:

    # AD "From my perspective, it should look pretty convincing."
    AD "De mon point de vue, cela devrait être assez convaincant."

# game/UpdateScriptChristmasEvent.rpy:2307
translate french christmas_shoot_b11b5772:

    # ST "{i}Hmmmm.... I think I should use this opportunity to punish Lauren for not telling me about Agent Diaz. This might not have been so shocking, had I been warned.{/i}"
    ST "{i}Hmmmm.... Je pense que je devrais profiter de cette occasion pour punir Lauren de ne pas m'avoir parlé de l'agent Diaz. Cela n'aurait peut-être pas été si choquant, si j'avais été prévenue.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2308
translate french christmas_shoot_43df5642:

    # ST "{i}Not only that, but she did let her pussy leak her naughty juices all over my leg.{/i}"
    ST "{i}Non seulement cela, mais elle a laissé sa chatte couler sur ma jambe.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2309
translate french christmas_shoot_9c1aca6d_1:

    # RT "{i}Oh, no way!.... I always thought Sidney was too much of a prude to do anything like that to her own sister!{/i}"
    RT "{i}Oh, pas question!.... J'ai toujours pensé que Sidney était trop prude pour faire quelque chose comme ça à sa propre sœur!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2310
translate french christmas_shoot_e7e3fc50_1:

    # RT "{i}I wonder what she'll do to her.{/i}"
    RT "{i}Je me demande ce qu'elle va lui faire.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2313
translate french christmas_shoot_82960bd2_1:

    # LT "{i}Oh.... my.... God!....{/i}"
    LT "{i}Oh.... mon.... Dieu !....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2314
translate french christmas_shoot_564f8cf1_1:

    # LT "{i}What the hell is Sidney thinking?{/i}"
    LT "{i}À quoi pense Sidney ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2315
translate french christmas_shoot_0fbb788f_1:

    # LT "{i}Where does she get the balls to do this to her own sister?{/i}"
    LT "{i}Où trouve-t-elle les couilles pour faire ça à sa propre sœur?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2316
translate french christmas_shoot_65ec1b87:

    # LT "{i}Keep quiet.... not a sound.... that would be so embarassing if [ryan] and Agent Diaz knew!{/i}"
    LT "{i}Gardez le silence.... pas un bruit.... ce serait tellement embarrassant si [ryan] et l'agent Diaz savaient!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2319
translate french christmas_shoot_044435c0_1:

    # ST "{i}Haha.... look at her squirm.... this is driving her crazy!{/i}"
    ST "{i}Haha.... regarde-la se tortiller.... ça la rend folle !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2320
translate french christmas_shoot_39b98dee_1:

    # RT "{i}Damn, from this angle I can't see what she's doing.{/i}"
    RT "{i}Merde, sous cet angle, je ne vois pas ce qu'elle fait.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2323
translate french christmas_shoot_70c5ab8d_1:

    # RT "{i}I can only imagine what's going on back there.{/i}"
    RT "{i}Je ne peux qu'imaginer ce qui se passe là-bas.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2325
translate french christmas_shoot_89c32e02_1:

    # LT "{i}Why does it feel so much better when Sidney does that to me, then when I do it to myself?{/i}"
    LT "{i}Pourquoi se sent-il tellement mieux quand Sidney me fait ça, plutôt que quand je le fais à moi-même?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2326
translate french christmas_shoot_d1bccbfc_1:

    # LT "{i}It's really starting to feel good!{/i}"
    LT "{i}Ça commence vraiment à faire du bien !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2328
translate french christmas_shoot_a3d29c7f:

    # LT "{i}Oh God!.... This is driving me crazy!.... What if that ho gives me an orgasm, right here in front of [ryan] and Agent Diaz?{/i}"
    LT "{i}Oh mon dieu!.... Ça me rend folle!.... Et si ce 'ho' me donne un orgasme, juste devant [ryan] et l'agent Diaz?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2329
translate french christmas_shoot_39fd5af3:

    # S "(whispering) This is what you get for not telling me about Agent Diaz, and for dripping your dirty juices all over my leg."
    S "(murmure) C'est ce que tu obtiens pour ne pas m'avoir parlé de l'agent Diaz, et pour avoir laisser dégouliner ton sale jus sur ma jambe."

# game/UpdateScriptChristmasEvent.rpy:2330
translate french christmas_shoot_101be609_1:

    # L "(whispering) Oh my God.... you.... noticed that?"
    L "(murmure) Oh mon Dieu.... tu.... tu as vu ça?"

# game/UpdateScriptChristmasEvent.rpy:2331
translate french christmas_shoot_696d4e9a:

    # S "(whispering) Yes, I noticed that!.... And now I'm going to do this until Agent Diaz tells me to stop."
    S "(murmure) Oui, j'ai vu!.... Et maintenant, je vais le faire jusqu'à ce que l'agent Diaz me dise d'arrêter."

# game/UpdateScriptChristmasEvent.rpy:2332
translate french christmas_shoot_6d31b345:

    # RT "{i}Whatever Sidney is doing to Lauren, I should help it continue for a little while.{/i}"
    RT "{i}Quoi que Sidney fasse à Lauren, je devrais l'aider à continuer pendant un petit moment.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2333
translate french christmas_shoot_7cc22acc:

    # R "Agent Diaz? Can I quickly show you a couple of settings on that camera that will make the shots even better?"
    R "Agent Diaz ? Puis-je rapidement te montrer quelques réglages sur cet appareil photo qui rendront les prises de vue encore meilleures?"

# game/UpdateScriptChristmasEvent.rpy:2334
translate french christmas_shoot_1570a005:

    # AD "Yeah.... I want these pics to turn out well too."
    AD "Ouais.... Je veux que ces photos m'excitent aussi."

# game/UpdateScriptChristmasEvent.rpy:2335
translate french christmas_shoot_aa4cd1ac:

    # R "Great! Let me show you a few things."
    R "Génial! Laisses-moi te montrer quelques choses."

# game/UpdateScriptChristmasEvent.rpy:2338
translate french christmas_shoot_fadaf516_1:

    # LT "{i}You've got to be kidding me!{/i}"
    LT "{i}Tu dois te moquer de moi!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2339
translate french christmas_shoot_c46e5f40_1:

    # S "(whispering) Oh my God Sidney!.... Today is your lucky day."
    S "(murmure) Oh mon dieu Sidney!.... Aujourd'hui est ton jour de chance."

# game/UpdateScriptChristmasEvent.rpy:2340
translate french christmas_shoot_1ad20c45_1:

    # L "(whispering) You're so mean today!"
    L "(murmure) Tu es si méchante aujourd'hui!"

# game/UpdateScriptChristmasEvent.rpy:2341
translate french christmas_shoot_c057f23d_1:

    # S "(whispering) I'll bet your slutty little pussy wishes I'd go further in, doesn't it?"
    S "(murmure) Je parie que ta petite chatte de salope souhaite que j'aille plus loin, n'est-ce pas?"

# game/UpdateScriptChristmasEvent.rpy:2342
translate french christmas_shoot_6673b4e8_1:

    # L "(whispering) Nnnn....nnoo...that's disgusting!"
    L "(murmure) Nonnn....nnonn...c'est dégoûtant!"

# game/UpdateScriptChristmasEvent.rpy:2343
translate french christmas_shoot_333e953d_1:

    # L "(whispering) I.... I don't.... want my own sister.... to fuck my pussy!"
    L "(murmure) Je.... Je ne le sais pas.... je veux ma propre sœur.... pour baiser ma chatte!"

# game/UpdateScriptChristmasEvent.rpy:2344
translate french christmas_shoot_a160c384_1:

    # S "(whispering) You don't sound very convincing."
    S "(murmure) Tu n'as pas l'air très convaincante."

# game/UpdateScriptChristmasEvent.rpy:2346
translate french christmas_shoot_6371a0d1_1:

    # L "(whispering) When did you become such a pervert?"
    L "(murmure) Quand es-tu devenue une telle perverse?"

# game/UpdateScriptChristmasEvent.rpy:2347
translate french christmas_shoot_cb122c21_1:

    # S "(whispering) hmmm.... well.... lately there's been some things going on, that have opened me up to be a little more sexually adventurous."
    S "(murmure) hmmm.... Bien.... dernièrement, il s'est passé des choses qui m'ont ouvert à être un peu plus aventureuse sexuellement."

# game/UpdateScriptChristmasEvent.rpy:2348
translate french christmas_shoot_4502d900_1:

    # ST "{i}Thank you [ryan].{/i}"
    ST "{i}Merci [ryan].{/i}"

# game/UpdateScriptChristmasEvent.rpy:2349
translate french christmas_shoot_08befc3a_1:

    # L "(whispering) You mean like molesting me in my sleep?"
    L "(murmure) Tu veux dire comme me molester dans mon sommeil?"

# game/UpdateScriptChristmasEvent.rpy:2350
translate french christmas_shoot_59158bb7_1:

    # L "Did you do that on purpose!?"
    L "As-tu fait cela exprès!?"

# game/UpdateScriptChristmasEvent.rpy:2351
translate french christmas_shoot_42d4d163_1:

    # S "Ssshhhh.... (whispering loudly) Of course I didn't."
    S "Ssshhhh.... (murmure fort) Bien sûr que non."

# game/UpdateScriptChristmasEvent.rpy:2352
translate french christmas_shoot_8482ecc8_1:

    # S "(whispering) But it did start a chain of events that brought me to where I am now."
    S "(murmure) Mais cela a déclenché une chaîne d'événements qui m'a amenée là où je suis maintenant."

# game/UpdateScriptChristmasEvent.rpy:2353
translate french christmas_shoot_c9f13534_1:

    # S "(whispering) Maybe I'll tell you some day."
    S "(murmure) Peut-être que je te le dirai un jour."

# game/UpdateScriptChristmasEvent.rpy:2354
translate french christmas_shoot_45599a4a_1:

    # S "(whispering) Maybe it would help you be a little more adventurous."
    S "(murmure) Peut-être que cela t'aiderait à être un peu plus aventureuse."

# game/UpdateScriptChristmasEvent.rpy:2355
translate french christmas_shoot_2a3d9838_1:

    # LT "{i}She has no idea!{/i}"
    LT "{i}Elle n'en a aucune idée!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2356
translate french christmas_shoot_cbfe314c_1:

    # LT "{i}She would probably think I'm such a dirty pervert if she knew that I've been dry humping my own brother.{/i}"
    LT "{i}Elle penserait probablement que je suis une sale perverese si elle savait que j'ai baisé avec mon propre frère.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2357
translate french christmas_shoot_94dccf8b_1:

    # LT "{i}Oh I wish it was his cock rubbing against my pussy!{/i}"
    LT "{i}Oh j'aimerais que ce soit sa bite qui se frotte contre ma chatte!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2358
translate french christmas_shoot_0598d82b_1:

    # RT "{i}Holy shit!.... If this isn't a dream, I could really ramp things up between the two of them.{/i}"
    RT "{i}Merde!.... Si ce n'est pas un rêve, je pourrais vraiment accélérer les choses entre eux deux.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2359
translate french christmas_shoot_949d8bde_1:

    # RT "{i}But how will I ever be sure?{/i}"
    RT "{i}Mais comment en serai-je jamais sûr ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2360
translate french christmas_shoot_8a480287_1:

    # L "(whispering loudly) Aaahhhhh.... Fuck you, Sidney.... you're driving me completely crazy! Just fuck my pussy already!"
    L "(murmure fort) Aaahhhhh.... Va te faire foutre, Sidney.... tu me rends complètement folle! Baise déjà ma chatte!"

# game/UpdateScriptChristmasEvent.rpy:2367
translate french christmas_shoot_20100ce8_1:

    # S "(whispering) Are you fucking kidding me?"
    S "(murmure) Tu te fous de moi?"

# game/UpdateScriptChristmasEvent.rpy:2368
translate french christmas_shoot_f5050d2f_1:

    # L "(whispering) No.... you ho! You started this."
    L "(murmure) Non.... toi ho! Tu as commencé ça."

# game/UpdateScriptChristmasEvent.rpy:2369
translate french christmas_shoot_0683e499_1:

    # L "(whispering) I'm literally dying with lust right now!"
    L "(murmure) Je meurs littéralement de convoitise en ce moment!"

# game/UpdateScriptChristmasEvent.rpy:2370
translate french christmas_shoot_36929394_1:

    # S "(whispering) Is it really turning you on that bad to have your sister rub your pussy?"
    S "(murmure) Est-ce que ça t'excite vraiment d'avoir ta sœur qui se frotte ta chatte?"

# game/UpdateScriptChristmasEvent.rpy:2371
translate french christmas_shoot_70aaaab5_1:

    # L "(whispering) Well, obviously I'm thinking of someone else while you're doing that."
    L "(murmure) Eh bien, évidemment, je pense à quelqu'un d'autre pendant que tu fais ça."

# game/UpdateScriptChristmasEvent.rpy:2372
translate french christmas_shoot_5559fdc7:

    # LT "{i}Like Agent Diaz spanking me in my room!.... That feels sooo wrong when I think those things!.... So why does it turn me on so bad?{/i}"
    LT "{i}Comme l'agent Diaz qui me gifle dans ma chambre!.... C'est tellement mal quand je pense à ces choses!.... Alors pourquoi ça m'excite si fort?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2373
translate french christmas_shoot_3fa2ca44:

    # L "(whispering) I will admit, having you do this to me, right in front of [ryan] and Agent Diaz, while they look on, completely clueless. Well, it's making this so much hotter!"
    L "(murmure) J'admets que tu me fasses cela, juste en face de [ryan] et de l'agent Diaz, pendant qu'ils regardent, complètement désemparés. Eh bien, ça fait tellement plus chaud!"

# game/UpdateScriptChristmasEvent.rpy:2374
translate french christmas_shoot_86026fa0:

    # S "(whispering) Aren't you worried they'll notice?"
    S "(murmure) Ne crains-tu pas qu'ils s'en aperçoivent?"

# game/UpdateScriptChristmasEvent.rpy:2375
translate french christmas_shoot_23f237f4:

    # L "(whispering) No.... they'll think you're just sliding it between my thighs like she suggested.... plus he's got all her attention focused on his camera right now."
    L "(murmure) Non.... ils penseront que tu glisses entre mes cuisses comme il l'a suggéré.... de plus, il a toute son attention concentrée sur son appareil photo en ce moment."

# game/UpdateScriptChristmasEvent.rpy:2376
translate french christmas_shoot_4e912e41_1:

    # S "(whispering) Alright.... you asked for it!"
    S "(murmure) D'accord.... tu l'as demandé!"

# game/UpdateScriptChristmasEvent.rpy:2382
translate french christmas_shoot_f2ab2857_1:

    # L "Oh, my fuck!..."
    L "Oh, mon putain!..."

# game/UpdateScriptChristmasEvent.rpy:2383
translate french christmas_shoot_7b4188c4_1:

    # R "Are you ok over there?"
    R "Tu vas bien ?"

# game/UpdateScriptChristmasEvent.rpy:2384
translate french christmas_shoot_9005b5b9_1:

    # S "(whispering loudly) Lauren!.... What the fuck?"
    S "(murmure fort) Lauren!.... C'est quoi ce bordel?"

# game/UpdateScriptChristmasEvent.rpy:2385
translate french christmas_shoot_4055e236_1:

    # L "Yeah.... I'm.... ok.... I think I just.... kneeled on a.... pebble."
    L "Ouais.... Je le suis.... OK.... Je crois juste.... agenouillé sur un.... galet."

# game/UpdateScriptChristmasEvent.rpy:2386
translate french christmas_shoot_dafa6357_1:

    # R "Ok good!.... I'm glad that's all it is."
    R "Ok bon!.... Je suis content que ce soit tout."

# game/UpdateScriptChristmasEvent.rpy:2387
translate french christmas_shoot_1944a818:

    # R "We'll be done with the camera soon, I just have a few more things to show Agent Diaz. Just hang on a little longer!"
    R "Nous en aurons bientôt fini avec la caméra, j'ai juste quelques autres choses à montrer à l'agent Diaz. Accrochez-vous un peu plus longtemps!"

# game/UpdateScriptChristmasEvent.rpy:2388
translate french christmas_shoot_59b848b0_1:

    # RT "{i}Holy Futa God!.... Sidney penetrated Lauren before I got to!{/i}"
    RT "{i}Sainte mère de Dieu !.... Sidney a pénétré Lauren avant que je n'arrive!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2389
translate french christmas_shoot_b525f35e_1:

    # RT "{i}I'm kind of jealous!{/i}"
    RT "{i}Je suis un peu jaloux!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2390
translate french christmas_shoot_cdd27b77_1:

    # RT "{i}But at the same time.... that's pretty fucking hot!{/i}"
    RT "{i}Mais en même temps.... c'est assez chaud !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2393
translate french christmas_shoot_d8807eb0:

    # LT "{i}Holy fuck!.... My sister is fucking me.... right in front of my brother and this perverted woman.... and they have no idea!....{/i}"
    LT "{i}Putain!.... Ma soeur me baise.... juste devant mon frère et cette femme perverse.... et ils n'en ont aucune idée !....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2394
translate french christmas_shoot_da540a13_1:

    # RT "{i}Well, maybe more than you think.{/i}"
    RT "{i}Eh bien, peut-être plus que tu ne le penses.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2395
translate french christmas_shoot_f604a0b5_1:

    # LT "{i}I've never been more turned on in my life!{/i}"
    LT "{i}Je n'ai jamais été aussi excitée de ma vie!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2396
translate french christmas_shoot_1ba4d501_1:

    # LT "{i}Oh....no.... what if I cum?.... It will be so obvious if I squirt all over the green screen!{/i}"
    LT "{i}Oh....no .... et si je jouis?.... Ce sera tellement évident si je gicle partout sur l'écran vert!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2397
translate french christmas_shoot_92fae9a0:

    # LT "{i}Fuck!.... What will.... they think?....{/i}"
    LT "{i}Merde!.... Qu'est-ce qui.... qu'ils en pensent?....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2400
translate french christmas_shoot_2f5d577e_1:

    # LT "{i}I can't believe Sidney is into this!.... Now we have a secret together.{/i}"
    LT "{i}Je n'arrive pas à croire que Sidney soit dans ce domaine!.... Maintenant, nous avons un secret ensemble.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2401
translate french christmas_shoot_2da5c7c2_1:

    # LT "{i}Now we can do some real sisterly bonding in the future!{/i}"
    LT "{i}Maintenant, nous pouvons créer de véritables liens fraternels à l'avenir!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2402
translate french christmas_shoot_e87aa4e7_1:

    # LT "{i}I wonder if she'll be up for using Mr. Snuggles' strap-on?{/i}"
    LT "{i}Je me demande si elle sera prête à utiliser le gode ceinture de m. Snuggles?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2403
translate french christmas_shoot_c2423b8e_1:

    # LT "{i}Fuck!.... I can just imagine her pounding me from behind!{/i}"
    LT "{i}Merde! .... Je peux juste imaginer qu'elle me pousse à partir de derrière!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2406
translate french christmas_shoot_138222af_1:

    # ST "{i}Why am I enjoying this so much?{/i}"
    ST "{i}Pourquoi est-ce que j'apprécie autant cela?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2407
translate french christmas_shoot_d7137e18_1:

    # ST "{i}It's not like it's my pussy that's getting all the attention.{/i}"
    ST "{i}Ce n'est pas comme si c'était ma chatte qui attirait toute l'attention.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2408
translate french christmas_shoot_fafcd75b_1:

    # ST "{i}Good God!.... I can't believe this is happening!{/i}"
    ST "{i}Mon Dieu!... Je n'arrive pas à croire que ça arrive!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2409
translate french christmas_shoot_f9c59704_1:

    # ST "{i}One day I'm sucking my brother's dick, and the next day I'm pounding my sister's pussy!{/i}"
    ST "{i}Un jour, je suce la bite de mon frère et le lendemain, je lèche la chatte de ma sœur!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2412
translate french christmas_shoot_507659a1_1:

    # ST "{i}This is so bad!{/i}"
    ST "{i}C'est tellement mal!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2413
translate french christmas_shoot_fe968288_1:

    # ST "{i}I'm not supposed to be turned on by my siblings!{/i}"
    ST "{i}Je ne suis pas censé être allumée par mes frères et sœurs!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2414
translate french christmas_shoot_afd65054_1:

    # ST "{i}What's next? Fantasies about my mom?{/i}"
    ST "{i}Et maintenant ? Des fantasmes à propos de ma mère ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2415
translate french christmas_shoot_c47b528a_1:

    # ST "{i}Shit, Sidney!.... Don't even go there!{/i}"
    ST "{i}Merde, Sidney!.... N'y vas pas !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2416
translate french christmas_shoot_0b6cd0f2_1:

    # ST "{i}With her perfect ass and titties!{/i}"
    ST "{i}Avec son cul et ses nichons parfaits!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2419
translate french christmas_shoot_8df0d47b_1:

    # ST "{i}Aaahhh.... maybe I should just embrace my perversions.{/i}"
    ST "{i}Aaahhh.... je devrais peut-être assumer mes perversions.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2420
translate french christmas_shoot_424bd220_1:

    # ST "{i}[ryan] doesn't seem to follow any kind of code of ethics.{/i}"
    ST "{i}[ryan] ne semble suivre aucune sorte de code d'éthique.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2421
translate french christmas_shoot_2c23660a_1:

    # RT "{i}Ouch!.... Yeah.... I kind of let those ethics die the night I saw Mom at the club.... and started letting my cock do all my thinking.{/i}"
    RT "{i}Aïe!.... Ouais.... J'ai un peu laissé mourir cette éthique la nuit où j'ai vu maman au club.... et j'ai commencé à laisser ma bite faire toute ma réflexion.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2424
translate french christmas_shoot_c9b33e31_1:

    # RT "{i}Maybe poor self control is a weakness in our genetics.{/i}"
    RT "{i}Peut-être qu'une mauvaise maîtrise de soi est une faiblesse de notre lignée génétique.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2425
translate french christmas_shoot_9e99cc58_1:

    # RT "{i}Hmmmm.... maybe a weakness that I can take advantage of.{/i}"
    RT "{i}Hmmmm.... peut-être une faiblesse dont je peux profiter.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2427
translate french christmas_shoot_3cb6a5d9_2:

    # LT "{i}Fuck.... Fuck.... Fuck.... Fuck....{/i}"
    LT "{i}Merde.... Merde.... Merde.... Merde....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2428
translate french christmas_shoot_48963a34_1:

    # LT "{i}I'm getting sooo close!{/i}"
    LT "{i}Je suis tellllemmmenttt proche d'arriver!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2430
translate french christmas_shoot_3cb6a5d9_3:

    # LT "{i}Fuck.... Fuck.... Fuck.... Fuck....{/i}"
    LT "{i}Merde.... Merde.... Merde.... Merde....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2431
translate french christmas_shoot_677c960b_1:

    # LT "{i}What am I going to do?{/i}"
    LT "{i}Qu'est-ce que je vais faire?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2432
translate french christmas_shoot_db3f8241_1:

    # LT "{i}I can't control if I squirt or not!{/i}"
    LT "{i}Je ne peux pas me contrôler si je gicle ou non!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2433
translate french christmas_shoot_eb2b349a_1:

    # LT "{i}And as turned on as I am, it's basically a sure thing!{/i}"
    LT "{i}Et aussi ouverte que je le suis, c'est fondamentalement une bonne chose!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2434
translate french christmas_shoot_1f301d67:

    # RT "{i}I should stop her before she makes a mess!.... I don't even want to know what Agent Diaz would do. Probably make Sidney lick it up.{/i}"
    RT "{i}Je devrais l'arrêter avant qu'elle ne fasse une connerie !.... Je ne veux même pas savoir ce que ferait l'agent Diaz. Je vais probablement lui faire lécher Sidney.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2435
translate french christmas_shoot_4e274397:

    # R "So that should be all! I think that will improve your pictures a lot!"
    R "Donc ça devrait être tout! Je pense que cela améliorera beaucoup tes photos!"

# game/UpdateScriptChristmasEvent.rpy:2436
translate french christmas_shoot_9ca5761e:

    # AD "Hmmmm.... that seemed like a lot of explaining for just a couple little adjustments."
    AD "Hmmmm.... cela semble être beaucoup d'explications pour quelques petits ajustements."

# game/UpdateScriptChristmasEvent.rpy:2437
translate french christmas_shoot_ff4e5326:

    # AD "Oh, well.... you girls are looking amazing. That dildo thigh humping looks pretty convincing, huh?"
    AD "Oh, eh bien.... vous avez l'air incroyables, les filles. Ce godemiché à la cuisse a l'air plutôt convaincant, hein ?"

# game/UpdateScriptChristmasEvent.rpy:2442
translate french christmas_shoot_072e9c85_1:

    # LT "{i}Fuck.... I was so close!{/i}"
    LT "{i}Merde.... J'étais si proche d'arriver!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2445
translate french christmas_shoot_745fbcfb:

    # LT "{i}Oh Thank God she stopped her!{/i}"
    LT "{i}Oh Dieu merci, elle l'a arrêtée!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2446
translate french christmas_shoot_dd0d8bf5_1:

    # LT "{i}As much as I wanted that.... that would have been a disaster.{/i}"
    LT "{i}Autant que je le voulais.... ça aurait été un désastre.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2449
translate french christmas_shoot_891a3d24_1:

    # LT "{i}But now I really need to get off! How am I going to take care of this?{/i}"
    LT "{i}Mais maintenant j'ai vraiment besoin de sortir! Comment vais-je le faire?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2453
translate french christmas_shoot_a4f4f00f:

    # AD "Ok, I have just one more pic for you girls to pose to."
    AD "Ok, j'ai juste une photo de plus pour vous les filles à faire."

# game/UpdateScriptChristmasEvent.rpy:2454
translate french christmas_shoot_d4393b4f:

    # AD "I'm going to be in this one, so [ryan], take the camera."
    AD "Je vais être dans celui-ci, alors [ryan], prends la caméra."

# game/UpdateScriptChristmasEvent.rpy:2455
translate french christmas_shoot_1e4a6e9c:

    # AD "And give me just a second to strip down to my panties."
    AD "Et donnes-moi juste une seconde pour me déshabiller jusqu'à mes sous-vêtements."

# game/UpdateScriptChristmasEvent.rpy:2456
translate french christmas_shoot_59dfa5fb:

    # RT "{i}Holy shit! Diaz is going to pose with them! I'll have pictures that will force the FBI to fire her.{/i}"
    RT "{i}Merde! Diaz va poser avec eux! J'aurai des photos qui forceront le FBI à la virer.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2457
translate french christmas_shoot_c8ff139b:

    # RT "{i}That is, assuming this isn't just a dream.... and.... shit, even if it isn't, this isn't happening right now, on my timeline.{/i}"
    RT "{i}C'est-à-dire, en supposant que ce n'est pas seulement un rêve.... et.... merde, même si ce n'est pas le cas, ça n'arrive pas en ce moment, sur mon calendrier.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2458
translate french christmas_shoot_12862598:

    # RT "{i}Dammit!{/i}"
    RT "{i}Bon sang!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2459
translate french christmas_shoot_10c6b01f:

    # S "Really? You're taking off your top too?"
    S "Vraiment? Tu enlèves ton haut aussi ?"

# game/UpdateScriptChristmasEvent.rpy:2460
translate french christmas_shoot_2dcd9f71:

    # AD "Haha.... you know you like what you see!"
    AD "Haha.... tu sais que tu aimes ce que tu vois!"

# game/UpdateScriptChristmasEvent.rpy:2461
translate french christmas_shoot_aa5db449:

    # LT "{i}She does have really sexy breasts.{/i}"
    LT "{i}Elle a des seins vraiment sexy.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2464
translate french christmas_shoot_9b564d73:

    # AD "Alright ladies. I want you to both get into a kneeling position, and then I want a shot where it looks like one of you is licking my crotch from the front, and one is licking my ass from behind."
    AD "Très bien mesdames. Je veux que vous vous mettiez tous les deux à genoux, puis je veux une photo où on dirait que l'un de vous me lèche l'entrejambe devant et que l'autre me lèche le cul par derrière."

# game/UpdateScriptChristmasEvent.rpy:2465
translate french christmas_shoot_aa0df7db:

    # S "Are you fucking kidding me?"
    S "Tu t’inquiétais pour moi?..."

# game/UpdateScriptChristmasEvent.rpy:2466
translate french christmas_shoot_238ab466:

    # RT "{i}Oh my God!....{/i}"
    RT "{i}Oh mon Dieu!....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2467
translate french christmas_shoot_f42ddd1b:

    # AD "If you complain too much, I'll take off the panties too."
    AD "Si tu te plains trop, j'enlève aussi la culotte."

# game/UpdateScriptChristmasEvent.rpy:2468
translate french christmas_shoot_c75d458e:

    # L "I call crotch!"
    L "Je suppose."

# game/UpdateScriptChristmasEvent.rpy:2469
translate french christmas_shoot_1d1d9d35:

    # AD "Ohh.... Lauren! I like the eagerness."
    AD "Ohh .... Lauren ! J'aime cette ardeur."

# game/UpdateScriptChristmasEvent.rpy:2470
translate french christmas_shoot_3158dc31:

    # L "I just don't want to get stuck with licking your ass."
    L "Je ne veux tout simplement pas rester coincé à te lécher le cul."

# game/UpdateScriptChristmasEvent.rpy:2471
translate french christmas_shoot_e0cf51d1:

    # AD "Haha.... I like the way you think!"
    AD "Haha.... J'aime ta façon de penser!"

# game/UpdateScriptChristmasEvent.rpy:2472
translate french christmas_shoot_218bad49:

    # AD "You ready, Sidney?"
    AD "Tu es prête, Sidney ?"

# game/UpdateScriptChristmasEvent.rpy:2473
translate french christmas_shoot_e90fa763:

    # S "I don't want to lick your ass!"
    S "Je ne veux pas te lécher le cul!"

# game/UpdateScriptChristmasEvent.rpy:2474
translate french christmas_shoot_f945238c:

    # AD "Then don't! Just make it look like you are."
    AD "Alors ne le fais pas! Il suffit de donner l'impression que tu l'es."

# game/UpdateScriptChristmasEvent.rpy:2475
translate french christmas_shoot_e509397a:

    # AD "Come on, it's the last picture, don't make me bring up my leverage over you."
    AD "Aller, c'est la dernière photo, ne m'oblige pas à augmenter mon influence sur toi."

# game/UpdateScriptChristmasEvent.rpy:2476
translate french christmas_shoot_c6190b95_2:

    # S "Fine!"
    S "Bien!"

# game/UpdateScriptChristmasEvent.rpy:2477
translate french christmas_shoot_bbf02d7a:

    # AD "Awesome! Ok [ryan], we'll let you know when we're in position."
    AD "Génial! Ok [ryan], nous t'informerons quand nous serons en position."

# game/UpdateScriptChristmasEvent.rpy:2478
translate french christmas_shoot_b802af47:

    # AD "How's it looking?"
    AD "À quoi ça ressemble ?"

# game/UpdateScriptChristmasEvent.rpy:2479
translate french christmas_shoot_89b26767:

    # R "Just make your face look more orgasmic, and I think it will look amazing."
    R "Rendez votre visage plus orgasmique, et je pense que cela aura l'air incroyable."

# game/UpdateScriptChristmasEvent.rpy:2482
translate french christmas_shoot_cf853d49:

    # ST "{i}How long do I have to hold this pose? Luckily it doesn't smell too much like ass. Mostly just sweat.{/i}"
    ST "{i}Combien de temps dois-je tenir cette pose? Heureusement, ça ne sent pas trop le cul. Surtout juste de la sueur.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2483
translate french christmas_shoot_28047363:

    # ST "{i}I guess it could be worse.{/i}"
    ST "{i}Je suppose que ça pourrait être pire.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2484
translate french christmas_shoot_92bdfad0:

    # LT "{i}This is the closest I've ever been to another girl's pussy.{/i}"
    LT "{i}C'est le plus proche que j'ai jamais été de la chatte d'une autre fille.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2485
translate french christmas_shoot_3503e6fe:

    # LT "{i}Her scent is different than mine, but still sweet, and.... sexy.{/i}"
    LT "{i}Son parfum est différent du mien, mais toujours doux, et.... sexy.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2486
translate french christmas_shoot_f887ab92:

    # LT "{i}Oh God!.... I feel like I'm going to squirt, and I'm not even getting my pussy played with.{/i}"
    LT "{i}Oh mon dieu!.... J'ai l'impression que je vais gicler, et je ne me fais même pas caresser la chatte.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2487
translate french christmas_shoot_f4a86f79:

    # LT "{i}Fuck.... I need a release sooooo bad!{/i}"
    LT "{i}Merde.... J'ai besoin d'une chose telllllemmmmentttt mauvaise!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2488
translate french christmas_shoot_caabec64:

    # R "There.... that pose is perfect!"
    R "Là.... cette pose est parfaite!"

# game/UpdateScriptChristmasEvent.rpy:2489
translate french christmas_shoot_e27625cc:

    # AD "This is going out as my Christmas card to my close girlfriends."
    AD "Cela ressemble à ma carte de Noël à mes amies proches."

# game/UpdateScriptChristmasEvent.rpy:2494
translate french christmas_shoot_8677b491:

    # R "Alright, I got it. I think that might end up being the best of the set."
    R "D'accord, je l'ai compris. Je pense que cela pourrait finir par être le meilleur du plateau."

# game/UpdateScriptChristmasEvent.rpy:2499
translate french christmas_shoot_b3b72fe0:

    # AD "Awesome! Too bad for you, it's not going with the rest of the set."
    AD "Génial! Dommage pour toi, ça ne va pas avec le reste de l'ensemble."

# game/UpdateScriptChristmasEvent.rpy:2500
translate french christmas_shoot_b91e5bf9:

    # R "What why not?"
    R "Et pourquoi pas?"

# game/UpdateScriptChristmasEvent.rpy:2501
translate french christmas_shoot_f4c0d074:

    # AD "Haha.... I'm sure you'd like that.... but I'm not an idiot. I know what damage that picture would cause in the wrong hands."
    AD "Haha.... Je suis sûre que ça te plairait.... mais je ne suis pas une idiote. Je sais quels dommages cette image causerait entre de mauvaises mains."

# game/UpdateScriptChristmasEvent.rpy:2504
translate french christmas_shoot_99415aa0:

    # AD "I'll just be taking the SD card."
    AD "Je vais juste prendre la carte SD."

# game/UpdateScriptChristmasEvent.rpy:2505
translate french christmas_shoot_a8aece6b:

    # R "What about the other pics?"
    R "Qu'en est-il des autres photos?"

# game/UpdateScriptChristmasEvent.rpy:2506
translate french christmas_shoot_10f25c04:

    # AD "You're a fan of my work, are you?"
    AD "Tu es fan de mon travail, n'est-ce pas?"

# game/UpdateScriptChristmasEvent.rpy:2507
translate french christmas_shoot_3da2af1b:

    # AD "I'll send you the rest."
    AD "Je t'enverrais le reste."

# game/UpdateScriptChristmasEvent.rpy:2508
translate french christmas_shoot_04fe04f7:

    # AD "After I get what I want off of it."
    AD "Après en avoir obtenu ce que je veux."

# game/UpdateScriptChristmasEvent.rpy:2509
translate french christmas_shoot_3b8d7f93:

    # AD "You girls did really good today!"
    AD "Vous les filles avez vraiment bien fait aujourd'hui!"

# game/UpdateScriptChristmasEvent.rpy:2510
translate french christmas_shoot_726816a9:

    # AD "I can't wait to see what kind of photo shoots [ryan] puts you together in down the road."
    AD "J'ai hâte de voir quel genre de séances photo [ryan] vous prépare."

# game/UpdateScriptChristmasEvent.rpy:2511
translate french christmas_shoot_a079e7f6:

    # AD "I am one of your subscribers on Cosplay Heaven, by the way."
    AD "Je suis d'ailleurs l'une de tes abonnées sur Cosplay Heaven."

# game/UpdateScriptChristmasEvent.rpy:2512
translate french christmas_shoot_f9378b2a:

    # AD "My user name is BigDykeDiaz, haha..."
    AD "Mon nom d'utilisateur est BigDykeDiaz, haha..."

# game/UpdateScriptChristmasEvent.rpy:2513
translate french christmas_shoot_a7715167:

    # L "Uhhh.... we'll look for you on there..."
    L "Uhhh.... on va te chercher là-bas..."

# game/UpdateScriptChristmasEvent.rpy:2514
translate french christmas_shoot_826251a9:

    # AD "Thank you all for your time and hard work!"
    AD "Merci à tous pour votre temps et votre travail acharné!"

# game/UpdateScriptChristmasEvent.rpy:2515
translate french christmas_shoot_6de81852:

    # AD "The pleasure was all mine!"
    AD "Le plaisir était pour moi!"

# game/UpdateScriptChristmasEvent.rpy:2516
translate french christmas_shoot_c735a41b:

    # AD "Later, perpetrators!"
    AD "Plus tard, les auteurs!"

# game/UpdateScriptChristmasEvent.rpy:2521
translate french christmas_shoot_0ddfa8ae_1:

    # S "(whispering) Lauren? Are you ok?.... I know you didn't get to satisfy your itch."
    S "(murmure) Lauren? Est-ce que tu vas bien?.... Je sais que tu n'as pas pu satisfaire tes démangeaisons."

# game/UpdateScriptChristmasEvent.rpy:2524
translate french christmas_shoot_a61b4aad_1:

    # L "(whispering) No!.... I'm so horny right now!"
    L "(murmure) Non!.... Je suis tellement excitée en ce moment!"

# game/UpdateScriptChristmasEvent.rpy:2525
translate french christmas_shoot_63557893_1:

    # RT "{i}I'll help her take care of that.{/i}"
    RT "{i}Je vais l'aider à s'en occuper.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2528
translate french christmas_shoot_2796dcfe_1:

    # S "(whispering) When we go to the storage shed to change, I'll finish what I started.... I can't leave my own sister in such pain."
    S "(murmure) Quand nous irons au hangar de stockage pour nous changer, je terminerais ce que j'ai commencé.... Je ne peux pas laisser ma propre sœur dans un tel état d'exitation."

# game/UpdateScriptChristmasEvent.rpy:2531
translate french christmas_shoot_73b4864e_1:

    # L "(whispering) Oh my God!.... I have the best sister in the world.... I can't wait!"
    L "(murmure) Oh mon Dieu!.... J'ai la meilleure soeur du monde.... J'ai hâte!"

# game/UpdateScriptChristmasEvent.rpy:2532
translate french christmas_shoot_435f48e5_1:

    # RT "{i}Shit! Sidney's taking over!{/i}"
    RT "{i}Merde! Sidney prend la relève!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2535
translate french christmas_shoot_2e21b49f_1:

    # R "Uhh.... Sidney, we're all done here.... you can run home. Lauren?.... Would you mind sticking around here and helping me clean up?"
    R "Heu.... Sidney, on a tous fini ici.... tu peux rentrer chez toi. Lauren ?.... Pourrais-tu rester ici et m'aider à nettoyer?"

# game/UpdateScriptChristmasEvent.rpy:2536
translate french christmas_shoot_11e22be4_1:

    # L "Sorry [ryan], but Sidney and I have plans to go shopping right now. Normally I'd stay and help, but we need to get our shopping done before all the stores close."
    L "Désolé [ryan], mais Sidney et moi avons l'intention d'aller faire du shopping en ce moment. Normalement, je resterais et j'aiderais, mais nous devons faire nos courses avant que tous les magasins ne ferment."

# game/UpdateScriptChristmasEvent.rpy:2537
translate french christmas_shoot_81b201a3_1:

    # L "I'm really sorry!"
    L "Je suis vraiment désolé!"

# game/UpdateScriptChristmasEvent.rpy:2538
translate french christmas_shoot_d0a99d8c_1:

    # RT "{i}Yeah.... sure you are.... at least that storage shed has so many holes in it, it will be easy to spy on them.{/i}"
    RT "{i}Ouais.... bien sûr que tu l'es.... au moins ce hangar de stockage a tellement de trous dedans, il sera facile de les espionner.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2539
translate french christmas_shoot_33f13380_1:

    # R "Alright.... I understand. I'll take care of it myself. Go ahead and get changed."
    R "D'accord.... Je comprends. Je m'en occuperai moi-même. Allez-y et changez-vous."

# game/UpdateScriptChristmasEvent.rpy:2542
translate french christmas_shoot_3336f244_1:

    # {i}\"Sound of girls giggling as they run off.\"{/i}"
    "{i}'Bruit de filles qui ricanent alors qu'elles s'enfuient.'{/i}"

# game/UpdateScriptChristmasEvent.rpy:2543
translate french christmas_shoot_52d98b53:

    # RT "{i}Holy hell!.... I can't believe that all just happened.{/i}"
    RT "{i}Enfer sacré!.... Je n'arrive pas à croire que tout s'est passé.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2544
translate french christmas_shoot_655f4f83:

    # RT "{i}Agent Diaz is even more unhinged than I thought!{/i}"
    RT "{i}L'agent Diaz est encore plus déstabilisé que je ne le pensais!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2545
translate french christmas_shoot_d7ab120b:

    # RT "{i}Maybe even more so than me.{/i}"
    RT "{i}Peut-être encore plus que moi.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2546
translate french christmas_shoot_6e55db9b:

    # RT "{i}And what's the deal with Sidney cock-blocking me?{/i}"
    RT "{i}Et quel est le problème avec Sidney qui me bloque?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2547
translate french christmas_shoot_80665c37_1:

    # RT "{i}I'll give them just a second, and then I'm definitely spying on them.{/i}"
    RT "{i}Je leur donnerai juste une seconde, puis je les espionnerai certainement.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2551
translate french christmas_shoot_01d5fc25_1:

    # R "Ahh.... shit! You scared me!"
    R "Ahh.... Merde! Tu m'as fait peur !"

# game/UpdateScriptChristmasEvent.rpy:2552
translate french christmas_shoot_5bf2102f_1:

    # R "Feeling any better?"
    R "Tu te sens mieux?"

# game/UpdateScriptChristmasEvent.rpy:2553
translate french christmas_shoot_f5149242_1:

    # GCP "Well, the alcohol has numbed me a little, but no, I'm still feeling pretty shitty."
    GCP "Eh bien, l'alcool m'a un peu engourdi, mais non, je me sens toujours assez merdique."

# game/UpdateScriptChristmasEvent.rpy:2554
translate french christmas_shoot_90b33d3c_1:

    # GCP "How were things while I was gone?"
    GCP "Comment étaient les choses pendant mon absence ?"

# game/UpdateScriptChristmasEvent.rpy:2555
translate french christmas_shoot_924fc367_1:

    # R "Oh, you know.... nothing eventful happened here."
    R "Oh, tu sais.... rien de mouvementé ne s'est passé ici."

# game/UpdateScriptChristmasEvent.rpy:2556
translate french christmas_shoot_2b0ac8ec_1:

    # GCP "You don't have to lie to me. All your thoughts were so sexually charged, I could hear them all the way in the bar down the street."
    GCP "Tu n'as pas à me mentir. Toutes tes pensées étaient si chargées sexuellement que je les entendais tout le long dans le bar de la rue."

# game/UpdateScriptChristmasEvent.rpy:2557
translate french christmas_shoot_99e807f9:

    # GCP "Fucking Diaz!"
    GCP "Putain de Diaz!"

# game/UpdateScriptChristmasEvent.rpy:2558
translate french christmas_shoot_0df59a27:

    # R "Fucking Diaz."
    R "Putain de Diaz."

# game/UpdateScriptChristmasEvent.rpy:2559
translate french christmas_shoot_8ad287a2_1:

    # GCP "I have never failed so spectacularly in all the decades I've been doing this."
    GCP "Je n'ai jamais échoué aussi spectaculairement dans toutes les décennies que j'ai fais cela."

# game/UpdateScriptChristmasEvent.rpy:2560
translate french christmas_shoot_f1c8567d_1:

    # GCP "I won't be gettin my wings anytime soon."
    GCP "Je ne serai pas de sitôt sous mes ailes."

# game/UpdateScriptChristmasEvent.rpy:2561
translate french christmas_shoot_b49cd84c_1:

    # GCP "Plus, God's definitely going to ream my tiny ass with her huge futa cock, when I get back."
    GCP "De plus, Dieu va certainement aléser mon petit cul avec son énorme coq de futa, quand je reviendrai."

# game/UpdateScriptChristmasEvent.rpy:2562
translate french christmas_shoot_63850416_1:

    # R "Oh, God!.... I'm so sorry!"
    R "Oh, mon Dieu!.... Je suis vraiment désolée !"

# game/UpdateScriptChristmasEvent.rpy:2563
translate french christmas_shoot_6df2766b:

    # R "But if it makes you feel better.... I learned a few really valuable lessons."
    R "Mais si cela vous fait vous sentir mieux.... J'ai appris quelques leçons très précieuses."

# game/UpdateScriptChristmasEvent.rpy:2564
translate french christmas_shoot_6678e393_1:

    # GCP "Oh?.... Like what?"
    GCP "Oh?.... Comme quoi ?"

# game/UpdateScriptChristmasEvent.rpy:2565
translate french christmas_shoot_5a5adc77:

    # R "I learned that Diaz is really dangerous, and I need to make sure I keep things under control with her."
    R "J'ai appris que Diaz est vraiment dangereuse, et je dois m'assurer de garder les choses sous contrôle avec elle."

# game/UpdateScriptChristmasEvent.rpy:2566
translate french christmas_shoot_174faae4:

    # R "I learned that Sidney and Lauren are starting to think like me."
    R "J'ai appris que Sidney et Lauren commencent à penser comme moi."

# game/UpdateScriptChristmasEvent.rpy:2567
translate french christmas_shoot_e10e8622_1:

    # GCP "(sarcastically) Oh, wonderful."
    GCP "(sarcastiquement) Oh, merveilleux."

# game/UpdateScriptChristmasEvent.rpy:2568
translate french christmas_shoot_30e02bf7_1:

    # R "And I learned that I need to keep them from getting too intimate with each other right away, or they're going to cut me out of their sex lives."
    R "Et j'ai appris que je devais les empêcher de devenir trop intimes l'une avec l'autre tout de suite, sinon elles vont me couper de leur vie sexuelle."

# game/UpdateScriptChristmasEvent.rpy:2569
translate french christmas_shoot_dd8296f2_1:

    # R "I think I need to work on getting each one of them attached to me more, before I bring them together."
    R "Je pense que je dois travailler à m'attacher davantage à chacun d'elles, avant de les réunir."

# game/UpdateScriptChristmasEvent.rpy:2570
translate french christmas_shoot_a98dab55_1:

    # GCP "(sarcastically) What a treasure trove of moral lessons you've learned today."
    GCP "(sarcastiquement) Quel trésor de leçons morales tu as apprises aujourd'hui."

# game/UpdateScriptChristmasEvent.rpy:2571
translate french christmas_shoot_8f328987_1:

    # R "I really am sorry it didn't work out for you the way you wanted. But if you'll now excuse me. I've got a pretty hot event I need to go spy on."
    R "Je suis vraiment désolé que cela n'ait pas fonctionné pour toi comme tu le voulais. Mais excusez-moi maintenant. J'ai un événement assez chaud que je dois espionner."

# game/UpdateScriptChristmasEvent.rpy:2572
translate french christmas_shoot_57f382dc_1:

    # R "Haha.... yeah, you know what I'm talking about."
    R "Haha.... ouais, tu sais de quoi je parle."

# game/UpdateScriptChristmasEvent.rpy:2573
translate french christmas_shoot_0d29aec3_1:

    # GCP "Yeah, sorry about that, but you won't have time."
    GCP "Ouais, désolé pour ça, mais tu n'auras pas le temps."

# game/UpdateScriptChristmasEvent.rpy:2574
translate french christmas_shoot_4881927b_1:

    # R "What?"
    R "Quoi?"

# game/UpdateScriptChristmasEvent.rpy:2575
translate french christmas_shoot_2af19843_1:

    # GCP "Yeah, it's about time to go."
    GCP "Oui, il est temps de partir."

# game/UpdateScriptChristmasEvent.rpy:2576
translate french christmas_shoot_07a3f443_1:

    # GCP "You can expect the third, and final ghost, when the clock strikes three."
    GCP "Tu peux t'attendre au troisième et dernier fantôme lorsque l'horloge sonnera trois fois."

# game/UpdateScriptChristmasEvent.rpy:2577
translate french christmas_shoot_87163a6f_1:

    # R "No, I don't want to go yet!.... And once again, we don't have a clock tower."
    R "Non, je ne veux pas encore y aller!.... Et encore une fois, nous n'avons pas de tour d'horloge."

# game/UpdateScriptChristmasEvent.rpy:2578
translate french christmas_shoot_228ab36d_1:

    # GCP "In 5.... 4.... 3..."
    GCP "Dans 5.... 4.... 3..."

# game/UpdateScriptChristmasEvent.rpy:2579
translate french christmas_shoot_9e01f8fc_1:

    # R "Nooooo!..."
    R "Nooooonnn!..."

# game/UpdateScriptChristmasEvent.rpy:2580
translate french christmas_shoot_4b36f23f_1:

    # GCP "2.... 1..."
    GCP "2.... 1..."

# game/UpdateScriptChristmasEvent.rpy:2587
translate french christmas_shoot_cca4618a_1:

    # RT "{i}Oh Futa God.... I'm back in my bed?.... Another weird dream?{/i}"
    RT "{i}Oh mon Dieu.... Je suis de retour dans mon lit ?.... Un autre rêve bizarre ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2588
translate french christmas_shoot_636f2d5c_1:

    # RT "{i}At least, I think they were dreams!{/i}"
    RT "{i}Au moins, je pense que c’étaient des rêves ! {/i}"

# game/UpdateScriptChristmasEvent.rpy:2589
translate french christmas_shoot_4ac6c243_1:

    # RT "{i}They were both just so vivid! Huh.... {/i}"
    RT "{i}Ils étaient tous les deux si vifs! Huh.... {/i}"

# game/UpdateScriptChristmasEvent.rpy:2590
translate french christmas_shoot_91c22639_1:

    # RT "{i}I guess I'll have to wait till three o'clock to find out.{/i}"
    RT "{i}Je devrais attendre jusqu'à trois heures pour le savoir.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2591
translate french christmas_shoot_e44c0b82_1:

    # RT "{i}Ohhh.... so tired.... zzzzzzz....{/i}"
    RT "{i}Ohhh.... tellement fatigué.... zzzzzzz....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2614
translate french christmas_club_c46a683d:

    # RT "{i}Holy shit!.... I think I'm going to puke!{/i}"
    RT "{i}Merde!.... Je pense que je vais vomir!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2615
translate french christmas_club_2e99b96e:

    # GP "Just hold it in for a second.... the feeling wears off really quick."
    GP "Tiens-le juste une seconde.... le sentiment s'estompe très rapidement."

# game/UpdateScriptChristmasEvent.rpy:2618
translate french christmas_club_df86fb2a:

    # R "What the fuck!.... Where are we?"
    R "Putain!.... Où sommes-nous ?"

# game/UpdateScriptChristmasEvent.rpy:2619
translate french christmas_club_d1523318:

    # GP "Sssshhhhhh.... try not to speak out loud to me in public. Don't forget, I can hear your thoughts. People will think you're loony if you're talking to someone on your shoulder."
    GP "Sssshhhhhh.... essaies de ne pas me parler à haute voix en public. N'oublie pas, j'entends tes pensées. Les gens penseront que tu es fou si tu parles à quelqu'un sur ton épaule."

# game/UpdateScriptChristmasEvent.rpy:2620
translate french christmas_club_de7f40e6:

    # RT "{i}I have gone loony!.... I'm talking to a ten inch version of my own mom!{/i}"
    RT "{i}Je suis devenu fou !.... Je parle à une version de dix pouces de ma propre mère!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2621
translate french christmas_club_0b621abc:

    # RT "{i}And now I'm just thinking at her!{/i}"
    RT "{i}Et maintenant je pense juste à elle!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2622
translate french christmas_club_91fb80ea:

    # RT "{i}And I assume that I'm the only one who can see or hear you?{/i}"
    RT "{i}Et je suppose que je suis le seul à pouvoir te voir ou t'entendre ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2623
translate french christmas_club_c56ccb17:

    # GP "You have assumed correctly."
    GP "Tu as supposé correctement."

# game/UpdateScriptChristmasEvent.rpy:2624
translate french christmas_club_ce658f9d:

    # RT "{i}So, where the fuck are we?{/i}"
    RT "{i}Alors, où sommes-nous, bordel ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2625
translate french christmas_club_91aa5921:

    # GP "The more appropriate question would be, \"When are we?\""
    GP "La question la plus appropriée serait : 'Quand sommes-nous?'"

# game/UpdateScriptChristmasEvent.rpy:2626
translate french christmas_club_664de4a3:

    # RT "{i}Ok.... I'll bite.... when are we?{/i}"
    RT "{i}Ok.... J'en meurs d'envie.... Quelle heure est-il?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2627
translate french christmas_club_8ad24c45:

    # GP "So glad you asked.... we have gone back to a Christmas 20 years ago."
    GP "Tellement content que tu as demandé.... nous sommes revenus à un Noël il y a 20 ans."

# game/UpdateScriptChristmasEvent.rpy:2628
translate french christmas_club_c4d2d487:

    # GP "To the day that your parents met."
    GP "Jusqu'au jour où tes parents se sont rencontrés."

# game/UpdateScriptChristmasEvent.rpy:2629
translate french christmas_club_d7557e14:

    # RT "{i}Really?.... Cuz Mom told me that they met when she was working at a strip club.{/i}"
    RT "{i}Vraiment?.... Parce que maman m'a dit qu'ils se sont rencontrés quand elle travaillait dans un club de strip-tease.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2630
translate french christmas_club_deee0347:

    # GP "Oh, shit!.... Is that true?.... Dammit.... I knew I should have read your whole file."
    GP "Oh, merde!.... Est-ce vrai ?.... Bon sang.... Je savais que j'aurais dû lire tout ton dossier."

# game/UpdateScriptChristmasEvent.rpy:2631
translate french christmas_club_3f85e9ad:

    # GP "Well, that's not a problem!.... It doesn't matter where two people fall in love.... just that they do."
    GP "Eh bien, ce n'est pas un problème!.... Peu importe où deux personnes tombent amoureuses.... juste ce qu'ils font."

# game/UpdateScriptChristmasEvent.rpy:2632
translate french christmas_club_2355700d:

    # GP "And you're lucky enough to get to see it."
    GP "Et tu as la chance de le voir."

# game/UpdateScriptChristmasEvent.rpy:2635
translate french christmas_club_885e830c:

    # RT "{i}You do realize that my infatuation with my mom and sisters started because I saw my mom stripping at a strip club, right?{/i}"
    RT "{i}Tu te rends compte que mon engouement pour ma mère et mes sœurs a commencé parce que j'ai vu ma mère se déshabiller dans un club de strip-tease, n'est-ce pas?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2636
translate french christmas_club_8094107c:

    # GP "Well, the first time was traumatic, so hopefully this time will be therapeutic."
    GP "Eh bien, la première fois a été traumatisante, alors j'espère que cette fois sera thérapeutique."

# game/UpdateScriptChristmasEvent.rpy:2637
translate french christmas_club_a0151bed:

    # RT "{i}Where is everybody? The club seems empty.{/i}"
    RT "{i}Où est tout le monde? Le club semble vide.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2638
translate french christmas_club_dede6874:

    # GP "Well, it's Christmas! All the men should be home with their families."
    GP "Eh bien, c'est Noël! Tous les hommes devraient être à la maison avec leur famille."

# game/UpdateScriptChristmasEvent.rpy:2639
translate french christmas_club_6157ab87:

    # GP "So, let's look around and see if we can find your parents."
    GP "Alors, regardons autour de nous et voyons si nous pouvons trouver tes parents."

# game/UpdateScriptChristmasEvent.rpy:2640
translate french christmas_club_51c2ee90:

    # VOM "Let's welcome to the stage the only one of our dancers desperate enough to come in on Christmas, to perform for a single patron, Oh, I guess there's two of you now.... I didn't see you come in."
    VOM "Accueillons sur scène la seule de nos danseuses assez désespérée pour venir à Noël, pour jouer pour un seul patron, Oh, je suppose que vous êtes deux maintenant.... Je ne t'ai pas vu entrer."

# game/UpdateScriptChristmasEvent.rpy:2641
translate french christmas_club_e15e5f72:

    # VOM "{i}(Whispering over mic){/i} Honestly Bill, it was a terrible idea to be open on Christmas."
    VOM "{i}(murmure au micro){/i} Honnêtement Bill, c'était une idée terrible d'être ouvert à Noël."

# game/UpdateScriptChristmasEvent.rpy:2642
translate french christmas_club_2e1c5d62:

    # VOM "{i}(Other muffled voice){/i} Ssshhhh.... they can hear you over the mic!"
    VOM "{i}(Autre voix étouffée) {/i} Ssshhhh.... ils peuvent vous entendre au micro!"

# game/UpdateScriptChristmasEvent.rpy:2643
translate french christmas_club_25f6bcb9:

    # VOM "Once again! Let's welcome to the stage, Santa's sexy helper, the one and only, [mom_name]!"
    VOM "Encore une fois! Bienvenue sur la scène, l'aide sexy du Père Noël, le seule et unique, [mom_name]!"

# game/UpdateScriptChristmasEvent.rpy:2646
translate french christmas_club_d39dd80c:

    # R "Oh.... my.... God!..."
    R "Oh.... mon.... Dieu!..."

# game/UpdateScriptChristmasEvent.rpy:2647
translate french christmas_club_8702e544:

    # GP "....Holy Virgin Futa!"
    GP "....Sainte Vierge Futa!"

# game/UpdateScriptChristmasEvent.rpy:2650
translate french christmas_club_602fffc1:

    # RT "{i}We did go through time.... that's my mom!.... Only.... much younger.{/i}"
    RT "{i}Nous avons traversé le temps.... c'est ma mère!.... Seulement.... beaucoup plus jeune.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2651
translate french christmas_club_cf684800:

    # RT "{i}She looks incredible!{/i}"
    RT "{i}Elle a l'air incroyable!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2652
translate french christmas_club_91aca590:

    # GP "Yeah, well.... stop ogling at your mother.... and don't forget why we're here!"
    GP "Ouais, eh bien.... arrête de regarder ta mère.... et n'oublies pas pourquoi nous sommes ici!"

# game/UpdateScriptChristmasEvent.rpy:2653
translate french christmas_club_cdf0b438:

    # RT "{i}Why are we here again?{/i}"
    RT "{i}Pourquoi sommes-nous à nouveau ici?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2654
translate french christmas_club_078ff073:

    # GP "So you can see your parents fall in love! I'm pretty sure If you see the moment they fall in love, you wouldn't possibly want to do anything to ruin their marriage."
    GP "Pour que tu puisses voir tes parents tomber amoureux! Je suis presque sûr que si tu vois le moment où ils tombent amoureux, tu ne voudrais peut-être rien faire pour ruiner leur mariage."

# game/UpdateScriptChristmasEvent.rpy:2655
translate french christmas_club_61ab6a49:

    # GP "Though I'll admit.... I had hoped for a more romantic setting.... I should have done my homework."
    GP "Bien que je l'admette.... J'avais espéré un cadre plus romantique.... J'aurais dû faire mes devoirs."

# game/UpdateScriptChristmasEvent.rpy:2656
translate french christmas_club_57ccd245:

    # GP "Well, let's make the most of it anyways."
    GP "Eh bien, profitons-en de toute façon."

# game/UpdateScriptChristmasEvent.rpy:2657
translate french christmas_club_5f80101e:

    # GP "Look! Is that your dad over there?"
    GP "Regardes! C'est ton père là-bas ?"

# game/UpdateScriptChristmasEvent.rpy:2660
translate french christmas_club_6ac5ee2f:

    # RT "{i}Holy shit.... yeah.... only way younger and thinner.{/i}"
    RT "{i}Merde.... ouais.... seulement en plus jeune et plus mince.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2661
translate french christmas_club_783a7c63:

    # RT "{i}I wonder if he's as much of a douche in the past, as he is in the present.{/i}"
    RT "{i}Je me demande s'il était autant un crétin dans le passé qu'il l'est dans le présent.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2662
translate french christmas_club_ad24f4d6:

    # GP "Well.... let's watch and find out."
    GP "Bien.... regardons et découvrons."

# game/UpdateScriptChristmasEvent.rpy:2665
translate french christmas_club_99cb7158:

    # GP "Oh look, how sweet!.... He's tipping your mom for her hard work!"
    GP "Oh regardes, comme c'est gentil!.... Il donne un pourboire à ta mère pour son travail acharné!"

# game/UpdateScriptChristmasEvent.rpy:2666
translate french christmas_club_1667a823:

    # RT "{i}You think tipping a stripper is sweet?.... Angels have a strange moral code.{/i}"
    RT "{i}Tu penses que le pourboire d'une strip-teaseuse est agréable?.... Les anges ont un code moral étrange.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2667
translate french christmas_club_b32a7574:

    # GP "I'm not an angel, I'm a ghost.... and it's sweet because it's going to lead to them falling in love!"
    GP "Je ne suis pas un ange, je suis un fantôme.... et c'est doux parce que ça va les amener à tomber amoureux!"

# game/UpdateScriptChristmasEvent.rpy:2668
translate french christmas_club_2e5bac34:

    # RT "{i}Well, I know seeing Mom strip made me fall in love.... that is, I already loved her, but now I want to bang her.{/i}"
    RT "{i}Eh bien, je sais que voir le strip-tease de maman m'a fait tomber amoureux.... autrement dit, je l'aimais déjà, mais maintenant je veux la baiser.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2669
translate french christmas_club_49d996c3:

    # GP "Oh, stop thinking with your cock, and try to appreciate this moment. You're about to see them fall in love!"
    GP "Oh, arrête de penser avec ta bite, et essaie d'apprécier ce moment. Tu es sur le point de les voir tomber amoureux!"

# game/UpdateScriptChristmasEvent.rpy:2672
translate french christmas_club_40e8a5e1:

    # D "Hey buddy!.... Are you just going to stare at us all day? Why don't you be a gentleman and tip the nice lady. It is Christmas, you know."
    D "Salut mon pote!.... Tu vas juste nous regarder toute la journée? Sois un gentleman et donne un pourboire à la gentille dame. C'est Noël, tu sais."

# game/UpdateScriptChristmasEvent.rpy:2673
translate french christmas_club_c7314589:

    # R "Uhhhhh..."
    R "Uhhhhh..."

# game/UpdateScriptChristmasEvent.rpy:2674
translate french christmas_club_81c4acfe:

    # RT "{i}Shit!.... What do I do?{/i}"
    RT "{i}Merde!.... Que dois-je faire ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2675
translate french christmas_club_d1cd41da:

    # GP "OK.... don't panic.... I gave you some Christmas magic when I blew that Christmas dust on you."
    GP "D'ACCORD.... pas de panique.... Je t'ai donné de la magie de Noël quand j'ai soufflé cette poussière de Noël sur toi."

# game/UpdateScriptChristmasEvent.rpy:2676
translate french christmas_club_73717b7f:

    # GP "You should be able to pull anything you need out of your pocket.... you know, like money.... all you need to do is think really hard about it."
    GP "Tu devrais pouvoir sortir de ta poche tout ce dont tu as besoin.... tu sais, comme l'argent.... tout ce que tu dois faire est d'y réfléchir très fort."

# game/UpdateScriptChristmasEvent.rpy:2677
translate french christmas_club_2250e01b:

    # M "Oh, be nice!.... Nobody should feel pressured into tipping me, but I'll tell you what I'll do. Whichever of you tips me the most tonight will get a private lap dance from me."
    M "Oh, sois gentil!.... Personne ne devrait se sentir obligé de me donner un pourboire, mais je vais te dire ce que je vais faire. Celui d'entre vous qui me donne le plus de pourboires ce soir aura un tour de danse privé de ma part."

# game/UpdateScriptChristmasEvent.rpy:2678
translate french christmas_club_ba7ef94b:

    # M "Are you both up for it?"
    M "Êtes-vous tous les deux prêts?"

# game/UpdateScriptChristmasEvent.rpy:2679
translate french christmas_club_3d619045:

    # D "I'm in!"
    D "J'y suis!"

# game/UpdateScriptChristmasEvent.rpy:2680
translate french christmas_club_710034af:

    # GP "I'm not sure this is a good idea..."
    GP "Je ne suis pas sûr que ce soit une bonne idée..."

# game/UpdateScriptChristmasEvent.rpy:2681
translate french christmas_club_db059cc9:

    # RT "{i}Why?.... Is it like the movie \"Back to the Future\", where I'll stop existing if my parents don't fall in love?{/i}"
    RT "{i}Pourquoi?.... Est-ce comme le film 'Retour vers le futur', où j'arrêterais d'exister si mes parents ne tombent pas amoureux?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2682
translate french christmas_club_f2ca3f91:

    # GP "No.... It's nothing like that! There isn't anything we can do here that will change the future. The past is already written."
    GP "Non.... Ce n'est rien comme ça ! Il n'y a rien que nous puissions faire ici qui puisse changer l'avenir. Le passé est déjà écrit."

# game/UpdateScriptChristmasEvent.rpy:2683
translate french christmas_club_8c5b85fb:

    # GP "These are just shadows of the past, but with Christmas magic, we can interact with them."
    GP "Ce ne sont que des ombres du passé, mais avec la magie de Noël, nous pouvons interagir avec eux."

# game/UpdateScriptChristmasEvent.rpy:2684
translate french christmas_club_4f5cab17:

    # RT "{i}Well, what can go wrong then?{/i}"
    RT "{i}Eh bien, qu'est-ce qui peut mal tourner alors?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2685
translate french christmas_club_df2b6619:

    # GP "Well, you might not learn the lesson that I'm trying to teach you!"
    GP "Eh bien, tu pourrais ne pas apprendre la leçon que j'essaie de t'enseigner!"

# game/UpdateScriptChristmasEvent.rpy:2686
translate french christmas_club_901c4d6d:

    # R "I'm in too!"
    R "Je suis aussi dedans!"

# game/UpdateScriptChristmasEvent.rpy:2687
translate french christmas_club_27b2494d:

    # GP "Dammit! You better let your dad win!"
    GP "Bon sang! Tu ferais mieux de laisser ton père gagner!"

# game/UpdateScriptChristmasEvent.rpy:2688
translate french christmas_club_3ad9ebed:

    # M "Yay!.... I'll start dancing and you guys start tipping!"
    M "Ouah!.... Je vais commencer à danser et vous les gars commencez à faire des trucs !"

# game/UpdateScriptChristmasEvent.rpy:2689
translate french christmas_club_96d1c805:

    # M "Let the \"tip off\" begin!"
    M "Que le 'pourboire' commence!"

# game/UpdateScriptChristmasEvent.rpy:2692
translate french christmas_club_8e37c225:

    # RT "{i}Wow!.... Look at her!.... And you're trying to teach me that I shouldn't get a hard-on when I look at that?{/i}"
    RT "{i}Wow!.... Regarde-la!.... Et tu essaies de m'apprendre que je ne devrais pas bander quand je regarde ça?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2693
translate french christmas_club_8ff37729:

    # GP "Well, there's a big difference in getting a hard-on and actually trying to fuck her!"
    GP "Eh bien, il y a une grande différence à avoir une bite et à essayer de la baiser!"

# game/UpdateScriptChristmasEvent.rpy:2694
translate french christmas_club_c4d84850:

    # RT "{i}You're right.... I should try to fuck her!....{/i}"
    RT "{i}Tu as raison.... Je devrais essayer de la baiser!....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2697
translate french christmas_club_818b12ce:

    # GP "Dammit!.... No!"
    GP "Bon sang!.... Non!"

# game/UpdateScriptChristmasEvent.rpy:2700
translate french christmas_club_f6562421:

    # GP "Get away you hussy!"
    GP "Éloignes-toi de ton mari !"

# game/UpdateScriptChristmasEvent.rpy:2701
translate french christmas_club_fcc741c3:

    # GP "She needs to be giving her attention to your dad!"
    GP "Elle doit accorder son attention à ton père!"

# game/UpdateScriptChristmasEvent.rpy:2704
translate french christmas_club_c07c0161:

    # RT "{i}Is it accurate to say that this is bringing back memories, if this event technically takes place before I saw Mom in the club for the first time?{/i}"
    RT "{i}Est-il exact de dire que cela rappelle des souvenirs, si cet événement a techniquement lieu avant que je ne voie maman dans le club pour la première fois ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2705
translate french christmas_club_50cb5c4c:

    # GP "I don't know.... don't ask me stupid questions."
    GP "Je ne sais pas.... ne me pose pas de questions stupides."

# game/UpdateScriptChristmasEvent.rpy:2706
translate french christmas_club_5ec0eb33:

    # D "Hey baby!. I've got something for you over here."
    D "Salut bébé!. J'ai quelque chose pour toi ici."

# game/UpdateScriptChristmasEvent.rpy:2709
translate french christmas_club_07e06a6f:

    # GP "That's it [dad_name]! Keep on bidding! Win [mom_name]'s love!"
    GP "C'est ça [dad_name]! Continues à enchérir! Gagnes l'amour de [mom_name]!"

# game/UpdateScriptChristmasEvent.rpy:2710
translate french christmas_club_1a9908fc:

    # RT "{i}Damn! I'm jealous of the view he's getting!{/i}"
    RT "{i}Merde ! Je suis jaloux de la vue qu'il a!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2713
translate french christmas_club_b5c102c1:

    # RT "{i}So, I'm assuming that if I were to win the tip war, you wouldn't let me stay and enjoy it. You'd just zap me back to my room, right?{/i}"
    RT "{i}Donc, je suppose que si je devais gagner la guerre, tu ne me laisserais pas rester en profiter. Tu voudrais juste me virer dans ma chambre, non?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2714
translate french christmas_club_fd06f5a1:

    # GP "Well..."
    GP "Bien..."

# game/UpdateScriptChristmasEvent.rpy:2715
translate french christmas_club_ae392bca:

    # RT "{i}\"Well\" what?{/i}"
    RT "{i}« Eh bien» quoi?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2716
translate french christmas_club_ad0bf686:

    # GP "Weeelllll..."
    GP "Biieennnn..."

# game/UpdateScriptChristmasEvent.rpy:2717
translate french christmas_club_0dde8a6e:

    # RT "{i}What is it you don't want to tell me?{/i}"
    RT "{i}Qu'est-ce que tu ne veux pas me dire?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2718
translate french christmas_club_96d7cac7:

    # GP "Urrrrgggghhhh!.... We're not allowed to lie to our clients, otherwise I damn well would right now."
    GP "Urrrrgggghhhh!.... On n'a pas le droit de mentir à nos clients, sinon je le ferais bien maintenant."

# game/UpdateScriptChristmasEvent.rpy:2719
translate french christmas_club_aaf8e06b:

    # GP "When we go through a time worm hole, it kind of acts like an hour-glass, and we're stuck here until we run out of time."
    GP "Lorsque nous traversons un trou de ver temporel, cela agit en quelque sorte comme un verre d'heure, et nous sommes coincés ici jusqu'à ce que nous manquions de temps."

# game/UpdateScriptChristmasEvent.rpy:2720
translate french christmas_club_67fbc0b6:

    # GP "I was thinking that maybe we could hit the bar while we wait to go back. You know.... after your parents fall in love?"
    GP "Je pensais que nous pourrions peut-être frapper la barre en attendant de rentrer. Tu sais.... après que tes parents soient tombés amoureux?"

# game/UpdateScriptChristmasEvent.rpy:2721
translate french christmas_club_49e788b7:

    # GP "I haven't had a good drink in a few decades!"
    GP "Je n'ai pas bu un bon verre depuis quelques décennies!"

# game/UpdateScriptChristmasEvent.rpy:2722
translate french christmas_club_dcb0f90b:

    # RT "{i}I'm not old enough to buy alcohol.{/i}"
    RT "{i}Je ne suis pas assez vieux pour acheter de l'alcool.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2723
translate french christmas_club_8cf9748d:

    # GP "You'd just have to think about an ID hard enough, and then you could pull it out of your pocket."
    GP "Il te suffirait de penser à une pièce d'identité assez fort, puis de la sortir de ta poche."

# game/UpdateScriptChristmasEvent.rpy:2724
translate french christmas_club_bd99ce18:

    # RT "{i}Haha....Yeah, that's right!{/i}"
    RT "{i}Haha....Oui, c'est vrai !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2725
translate french christmas_club_6d223fbe:

    # RT "{i}Hmmmm.... you know what? Let's just play it by ear.{/i}"
    RT "{i}Hmmmm.... tu sais quoi ? Jouons.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2726
translate french christmas_club_e3a2579b:

    # GP "Shit!!.... I don't like the sound of that."
    GP "Merde!!.... Je n'aime pas le son de ça."

# game/UpdateScriptChristmasEvent.rpy:2729
translate french christmas_club_1026fd34:

    # GP "Alright [ryan].... I think this should be our last tip.... I don't know how much more [dad_name] can afford."
    GP "D'accord [ryan].... Je pense que ça devrait être notre dernier pourboire.... Je ne sais pas combien plus [dad_name] peut se permettre."

# game/UpdateScriptChristmasEvent.rpy:2732
translate french christmas_club_aecdea9b:

    # RT "{i}I've got an endless supply of money in my pocket.... how on earth would I be able to resist tipping that sexy minx?{/i}"
    RT "{i}J'ai une réserve infinie d'argent dans ma poche.... comment diable pourrais-je résister à faire basculer cette sexy coquine ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2733
translate french christmas_club_73b0404b:

    # GP "Don't forget why we're here!"
    GP "N'oublies pas pourquoi nous sommes ici!"

# game/UpdateScriptChristmasEvent.rpy:2734
translate french christmas_club_b406f334:

    # GP "If you don't get to see your parents fall in love than this whole trip will have been for nothing!"
    GP "Si tu ne vois pas tes parents tomber amoureux, tout ce voyage n'aura servi à rien!"

# game/UpdateScriptChristmasEvent.rpy:2735
translate french christmas_club_382aac6e:

    # RT "{i}Alright, I promise I'll try!{/i}"
    RT "{i}D'accord, je promets que je vais essayer!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2738
translate french christmas_club_a63a1621:

    # RT "{i}But the look in those eyes!{/i}"
    RT "{i}Mais le regard dans ces yeux!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2739
translate french christmas_club_d8473410:

    # RT "{i}And those perfect breasts!{/i}"
    RT "{i}Et ces seins parfaits!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2742
translate french christmas_club_8106ad46:

    # RT "{i}Oh, God.... I think the only thing you've succeeded in doing is making me fall deeper in love with Mom.{/i}"
    RT "{i}Oh, mon Dieu.... Je pense que la seule chose que tu as réussi à faire est de me faire tomber plus profondément amoureux de maman.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2743
translate french christmas_club_2ce15aec:

    # GP "Dammit [ryan]! Show some self-restraint, for God's sake!"
    GP "Bon sang [ryan]! Fais preuve de retenue, pour l'amour de Dieu!"

# game/UpdateScriptChristmasEvent.rpy:2744
translate french christmas_club_f08dbc76:

    # GP "Resist that perfect ass!"
    GP "Résistes à ce cul parfait!"

# game/UpdateScriptChristmasEvent.rpy:2747
translate french christmas_club_fa7c958c:

    # RT "{i}I'm trying!{/i}"
    RT "{i}J'essaie!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2748
translate french christmas_club_e491831a:

    # RT "{i}But I've got a hard-on that could cut glass!{/i}"
    RT "{i}Mais j'ai un braquemarre qui pourrait casser du verre!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2749
translate french christmas_club_60112cd2:

    # D "Hey, sexy! I've got something over here that's going to sweeten your whole Christmas!"
    D "Salut, sexy! J'ai quelque chose ici qui va adoucir tout ton Noël!"

# game/UpdateScriptChristmasEvent.rpy:2752
translate french christmas_club_0a923769:

    # D "And just so you know, little boy.... hey!.... Are you even old enough to be in a bar?"
    D "Et juste pour que tu saches, petit garçon.... hé!.... Es-tu même assez vieux pour être dans un bar?"

# game/UpdateScriptChristmasEvent.rpy:2753
translate french christmas_club_f267cc74:

    # R "I've got ID. I've just got one of those baby faces, ya know?"
    R "J'ai une pièce d'identité. Je viens d'avoir un de ces visages de bébé, tu sais?"

# game/UpdateScriptChristmasEvent.rpy:2754
translate french christmas_club_af53819f:

    # D "Well.... I'm going to send you home crying like a baby."
    D "Bien.... Je vais te renvoyer à la maison en pleurant comme un bébé."

# game/UpdateScriptChristmasEvent.rpy:2755
translate french christmas_club_00a12abb:

    # R "Wow.... clever..."
    R "Wow.... intelligent..."

# game/UpdateScriptChristmasEvent.rpy:2756
translate french christmas_club_4e8a53fe:

    # D "Fuck you.... see, I don't ever lose.... and I also like to make grand gestures..."
    D "Va te faire foutre.... tu vois, je ne perds jamais.... et j'aime aussi faire de grands gestes..."

# game/UpdateScriptChristmasEvent.rpy:2757
translate french christmas_club_0370d319:

    # D "And this beauty deserves a real man to dance for tonight, not some little boy!"
    D "Et cette beauté mérite un vrai homme pour danser ce soir, pas un petit garçon!"

# game/UpdateScriptChristmasEvent.rpy:2758
translate french christmas_club_c82a2ab9:

    # D "So, feel free to leave any time, because I can guarantee your Christmas is going to end in disappointment."
    D "Alors, n'hésites pas à partir à tout moment, car je peux te garantir que ton Noël va se terminer en déception."

# game/UpdateScriptChristmasEvent.rpy:2761
translate french christmas_club_17f93351:

    # D "Merry Christmas, beautiful!"
    D "Joyeux Noël, magnifique!"

# game/UpdateScriptChristmasEvent.rpy:2762
translate french christmas_club_bf6d6810:

    # M "Oh, my God!..."
    M "Oh, mon Dieu!..."

# game/UpdateScriptChristmasEvent.rpy:2763
translate french christmas_club_19e32b8b:

    # M "That's got to be at least a thousand dollars!..."
    M "Ça doit être au moins mille dollars!..."

# game/UpdateScriptChristmasEvent.rpy:2764
translate french christmas_club_5447bdcb:

    # D "It's actually one thousand, five hundred, to be exact!"
    D "C'est en fait mille, cinq cents, pour être exact!"

# game/UpdateScriptChristmasEvent.rpy:2765
translate french christmas_club_9ff9f283:

    # M "That's the biggest tip I've ever seen!"
    M "C'est le plus gros pourboire que j'ai jamais vu!"

# game/UpdateScriptChristmasEvent.rpy:2768
translate french christmas_club_dc551855:

    # M "Oh.... I want to rub your huge tip all over my pussy!"
    M "Oh.... Je veux frotter ton énorme bout sur ma chatte!"

# game/UpdateScriptChristmasEvent.rpy:2769
translate french christmas_club_60087b65:

    # D "Holy shit.... that's hot!"
    D "Putain de merde.... c'est chaud !"

# game/UpdateScriptChristmasEvent.rpy:2770
translate french christmas_club_117947e8:

    # GP "Yes! [ryan]! You did it!"
    GP "Oui! [ryan]! Tu l'as fait !"

# game/UpdateScriptChristmasEvent.rpy:2771
translate french christmas_club_b3cb4c16:

    # RT "{i}What did I do?{/i}"
    RT "{i}Qu'ai-je fait?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2772
translate french christmas_club_0185310a:

    # GP "You forced your dad to make a grand gesture to win over your mom!"
    GP "Tu as forcé ton père à faire un grand geste pour gagner ta mère!"

# game/UpdateScriptChristmasEvent.rpy:2773
translate french christmas_club_1b31d6aa:

    # GP "They look like they are falling in love already!"
    GP "Ils ont l'air de tomber amoureux déjà!"

# game/UpdateScriptChristmasEvent.rpy:2774
translate french christmas_club_7365feb5:

    # RT "{i}Who said I'm going to let him win?{/i}"
    RT "{i}Qui a dit que je vais le laisser gagner?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2775
translate french christmas_club_07254025:

    # GP "Oh no, please, [ryan]! It's too perfect! Please!.... Just let it play out from here!"
    GP "Oh non, s'il te plait, [ryan]! C'est trop parfait ! S'il te plaît!.... Laisses-le jouer à partir d'ici!"

# game/UpdateScriptChristmasEvent.rpy:2776
translate french christmas_club_e85ebd6f:

    # RT "{i}Hmmmmm....{/i}"
    RT "{i}Hmmmmm....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2779
translate french christmas_club_3d5a89a6:

    # R "Well, congratulations.... you win..."
    R "Eh bien, félicitations.... tu gagnes..."

# game/UpdateScriptChristmasEvent.rpy:2782
translate french christmas_club_992ac644:

    # D "That's right, I win."
    D "C'est vrai, je gagne."

# game/UpdateScriptChristmasEvent.rpy:2783
translate french christmas_club_0d61ec20:

    # D "Because I always win..."
    D "Parce que je gagne toujours..."

# game/UpdateScriptChristmasEvent.rpy:2784
translate french christmas_club_08f4515e:

    # D "So head on home, back to mommy, little boy."
    D "Alors rentres chez toi, retournes voir maman, petit garçon."

# game/UpdateScriptChristmasEvent.rpy:2785
translate french christmas_club_481f0d54:

    # D "Maybe you can suck on her tits tonight."
    D "Peut-être que tu peux sucer ses seins ce soir."

# game/UpdateScriptChristmasEvent.rpy:2786
translate french christmas_club_16fc3473:

    # RT "{i}Dammit! If he even knew, that he stole that chance from me!....{/i}"
    RT "{i}Bon sang! S'il savait même qu'il m'avait volé cette chance !....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2787
translate french christmas_club_88904fc6:

    # GP "He didn't steal that chance! You weren't even born yet, you idiot!"
    GP "Il n'a pas volé cette chance! Tu n'étais même pas encore né, idiot!"

# game/UpdateScriptChristmasEvent.rpy:2788
translate french christmas_club_00abae6b:

    # RT "{i}UUURRRGGGHHHH.... I still hate to lose to that asshat!{/i}"
    RT "{i}UUURRRGGGHHHH.... Je déteste toujours perdre contre ce trou du cul!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2789
translate french christmas_club_85b61c14:

    # GP "Well, just remember.... If he didn't fuck your mom, you wouldn't have been born."
    GP "Eh bien, rappelles-toi juste.... S'il n'avait pas baisé ta mère, tu ne serais pas née."

# game/UpdateScriptChristmasEvent.rpy:2790
translate french christmas_club_92d107ef:

    # GP "Soooo..."
    GP "Bieeeennn..."

# game/UpdateScriptChristmasEvent.rpy:2793
translate french christmas_club_34121dbf:

    # RT "{i}And there goes that beautiful ass! I wish I was following it!{/i}"
    RT "{i}Et voilà ce beau cul! J'aimerais le suivre !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2796
translate french christmas_club_c15147df:

    # D "Get one good last look, loser! Because that ass is going to be rubbing all over my lap tonight!"
    D "Obtiens un bon dernier regard, de perdant! Parce que ce cul va se frotter sur mes genoux ce soir!"

# game/UpdateScriptChristmasEvent.rpy:2797
translate french christmas_club_f1975969:

    # RT "{i}Ugghh.... what an asshole!{/i}"
    RT "{i}Ugghh.... quel connard !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2800
translate french christmas_club_fa6435fe:

    # RT "{i}How did Mom ever fall in love with that jerk?{/i}"
    RT "{i}Comment maman est-elle tombée amoureuse de ce crétin?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2803
translate french christmas_club_b7680d41:

    # GP "Well?..."
    GP "Eh bien?..."

# game/UpdateScriptChristmasEvent.rpy:2804
translate french christmas_club_adfbbb81:

    # GP "Wasn't that amazing?.... You know.... to see the beginnings of your parents's relationship?"
    GP "N'était-ce pas incroyable?.... Tu sais.... pour voir les débuts de la relation de tes parents?"

# game/UpdateScriptChristmasEvent.rpy:2805
translate french christmas_club_b770b8d4:

    # GP "Should we hit the bar, until our time here runs out?"
    GP "Devrions-nous frapper la barre, jusqu'à ce que notre temps ici s'épuise?"

# game/UpdateScriptChristmasEvent.rpy:2806
translate french christmas_club_d99d6a09:

    # RT "{i}No!.... If anything, it makes me even more convinced that Mom shouldn't stay together with Dad, and that she would be happier with me.{/i}"
    RT "{i}Non !.... Au contraire, cela me rend encore plus convaincu que maman ne devrait pas rester avec papa et qu'elle serait plus heureuse avec moi.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2807
translate french christmas_club_beb3c6c9:

    # GP "No!.... That's not right!"
    GP "Non!.... C'est faux !"

# game/UpdateScriptChristmasEvent.rpy:2808
translate french christmas_club_74f41bc4:

    # GP "This has to be a life changing event for you!"
    GP "Cela doit être un événement qui change ta vie pour toi!"

# game/UpdateScriptChristmasEvent.rpy:2809
translate french christmas_club_eb976d82:

    # GP "This is supposed to make you want to stop trying to fuck your mom, and stop trying to fuck over your dad!"
    GP "Cela est censé te donner envie d'arrêter d'essayer de baiser ta mère et d'arrêter d'essayer de baiser ton père!"

# game/UpdateScriptChristmasEvent.rpy:2810
translate french christmas_club_8d89fd17:

    # GP "Uhhhhh..."
    GP "Uhhhhh..."

# game/UpdateScriptChristmasEvent.rpy:2811
translate french christmas_club_e30af173:

    # GP "I guess.... we need to keep watching."
    GP "Je suppose.... nous devons continuer à regarder."

# game/UpdateScriptChristmasEvent.rpy:2812
translate french christmas_club_c39ddeda:

    # RT "{i}I don't want to see my mom giving my dad a lapdance!{/i}"
    RT "{i}Je ne veux pas voir ma mère donner une lapdance à mon père!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2813
translate french christmas_club_78f6791d:

    # GP "Well, tough shit!"
    GP "Eh bien, merde!"

# game/UpdateScriptChristmasEvent.rpy:2814
translate french christmas_club_a498da8a:

    # GP "You haven't had a life changing event, so we're going to follow them until you do!"
    GP "Tu n'as pas eu d'événement qui a changé ta vie, alors nous allons les suivre jusqu'à ce que tu le fasses!"

# game/UpdateScriptChristmasEvent.rpy:2815
translate french christmas_club_4b2bba9c:

    # GP "Now follow me upstairs!"
    GP "Maintenant suis-moi à l'étage!"

# game/UpdateScriptChristmasEvent.rpy:2818
translate french christmas_club_86651a58:

    # GP "Ok, now see if you can open the door a crack and take a peek."
    GP "Ok, maintenant, vois si tu peux ouvrir la porte légèrement et jeter un coup d'œil."

# game/UpdateScriptChristmasEvent.rpy:2819
translate french christmas_club_b6c233ab:

    # RT "{i}It's locked.{/i}"
    RT "{i}C'est verrouillé.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2820
translate french christmas_club_0d750640:

    # GP "Hmmmm..."
    GP "Hmmmm..."

# game/UpdateScriptChristmasEvent.rpy:2823
translate french christmas_club_08e9dd0b:

    # GP "Well, I don't really hear any talking..."
    GP "Eh bien, je n'entends pas vraiment parler..."

# game/UpdateScriptChristmasEvent.rpy:2824
translate french christmas_club_29b1e385:

    # GP "We're going to have to go in."
    GP "Nous allons devoir entrer."

# game/UpdateScriptChristmasEvent.rpy:2825
translate french christmas_club_c88832fc:

    # RT "{i}Once again.... I don't want to see their lapdance, and I'm not invisible like you.{/i}"
    RT "{i}Encore une fois.... Je ne veux pas voir leur lap-dance, et je ne suis pas invisible.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2826
translate french christmas_club_0efa089c:

    # GP "You're right, that's why I'm going to let you see through my eyes."
    GP "Tu as raison, c'est pourquoi je vais te laisser voir à travers mes yeux."

# game/UpdateScriptChristmasEvent.rpy:2827
translate french christmas_club_3ea60cb0:

    # GP "You'll just wait out here, and I'll magic myself inside, and then you will see everything I see."
    GP "Tu attendras juste ici, et je me ferai de la magie à l'intérieur, et alors tu verras tout ce que je vois."

# game/UpdateScriptChristmasEvent.rpy:2828
translate french christmas_club_dfc7ac17:

    # GP "Here I go"
    GP "J'y vais"

# game/UpdateScriptChristmasEvent.rpy:2835
translate french christmas_club_21a0b683:

    # GP "Ok, can you see anything yet?."
    GP "Ok, peux-tu encore voir quelque chose?."

# game/UpdateScriptChristmasEvent.rpy:2836
translate french christmas_club_671ccd9c:

    # RT "{i}I can only see white!{/i}"
    RT "{i}Je ne vois que du blanc!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2837
translate french christmas_club_fcb26b9e:

    # GP "OK, give it just another second."
    GP "Ok, donnes-lui juste une seconde de plus."

# game/UpdateScriptChristmasEvent.rpy:2842
translate french christmas_club_39c9b5e8:

    # RT "{i}Oh what the hell?{/i}"
    RT "{i}Oh que diable?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2843
translate french christmas_club_d1de4048:

    # RT "{i}I thought Mom was just going to give him a lap dance.{/i}"
    RT "{i}Je pensais que maman allait juste faire une lap-danse.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2844
translate french christmas_club_30c05914:

    # GP "Well, he did just tip her $1,500. Your little competition raised the stakes."
    GP "Eh bien, il lui a juste donné un pourboire de 1'500$. Ta petite compétition a fait monter les enchères."

# game/UpdateScriptChristmasEvent.rpy:2845
translate french christmas_club_9f3ebec8:

    # GP "I'm sure she felt like she owed him a little more for such a generous tip."
    GP "Je suis sûr qu'elle avait l'impression de lui devoir un peu plus pour un pourboire aussi généreux."

# game/UpdateScriptChristmasEvent.rpy:2846
translate french christmas_club_1de3d6b0:

    # GP "I need to check for sure, but I don't even think he got a lap dance in the actual version of the past."
    GP "Je dois vérifier avec certitude, mais je ne pense même pas qu'il ait eu un tour de danse dans la version réelle du passé."

# game/UpdateScriptChristmasEvent.rpy:2847
translate french christmas_club_791ed571:

    # GP "This is all thanks to you."
    GP "C'est grâce à toi."

# game/UpdateScriptChristmasEvent.rpy:2848
translate french christmas_club_fa491e3e:

    # RT "{i}Goddammit, Ghost of Christmas Past! I don't want to watch this.{/i}"
    RT "{i}Putain, Fantôme de Noël passé ! Je ne veux pas regarder ça.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2849
translate french christmas_club_b07661a4:

    # RT "{i}Ahhh.... I can't look away or close my eyes!{/i}"
    RT "{i}Ahhh.... Je ne peux pas détourner le regard ou fermer les yeux!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2850
translate french christmas_club_a3155621:

    # GP "That's because you can only see what I can see."
    GP "C'est parce que tu ne peux voir que ce que je peux voir."

# game/UpdateScriptChristmasEvent.rpy:2851
translate french christmas_club_f2dcc471:

    # RT "{i}Well, look the hell away then!{/i}"
    RT "{i}Eh bien, regarde plus loin alors!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2852
translate french christmas_club_864bec3c:

    # GP "Just a little longer."
    GP "Juste un peu plus longtemps."

# game/UpdateScriptChristmasEvent.rpy:2853
translate french christmas_club_fe0dd1a1:

    # GP "When a man ejaculates, it releases all kinds of chemicals that could make [mom_name] fall in love with [dad_name]."
    GP "Lorsqu'un homme éjacule, il libère toutes sortes de produits chimiques qui pourraient faire tomber [mom_name] amoureuse de [dad_name]."

# game/UpdateScriptChristmasEvent.rpy:2854
translate french christmas_club_caa08817:

    # GP "I need you to see them fall in love! For your own good."
    GP "J'ai besoin que tu les voies tomber amoureux! Pour ton propre bien."

# game/UpdateScriptChristmasEvent.rpy:2855
translate french christmas_club_fc45fbf1:

    # RT "{i}Yuck!.... Do you realize I'm going to need to see a psychologist after you're done helping me?{/i}"
    RT "{i}Beurk!.... Tu te rends compte que je vais avoir besoin de voir un psychologue une fois que tu auras fini de m'aider?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2856
translate french christmas_club_34046993:

    # GP "Just shut up and watch."
    GP "Tais-toi et regarde."

# game/UpdateScriptChristmasEvent.rpy:2857
translate french christmas_club_194e4164:

    # RT "{i}Dad's pretty small though.{/i}"
    RT "{i}Papa est plutôt petit. {/i}"

# game/UpdateScriptChristmasEvent.rpy:2858
translate french christmas_club_2a44c374:

    # RT "{i}Too bad his penis isn't as big as his tip.{/i}"
    RT "{i}Dommage que son pénis ne soit pas aussi gros que son extrémité.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2859
translate french christmas_club_c41507b5:

    # RT "{i}No wonder Mom seems so impressed when she gets a look at me.{/i}"
    RT "{i}Pas étonnant que maman semble si impressionnée quand elle me regarde.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2860
translate french christmas_club_de9021cd:

    # GP "It's not about size."
    GP "Ce n'est pas une question de taille."

# game/UpdateScriptChristmasEvent.rpy:2861
translate french christmas_club_4742e5f8:

    # RT "{i}It kind of is.{/i}"
    RT "{i}C'est comme ça.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2864
translate french christmas_club_3794e86f:

    # GP "Oh, look! He's about to paint her face with his cum!"
    GP "Oh, regarde! Il est sur le point de peindre son visage avec son sperme!"

# game/UpdateScriptChristmasEvent.rpy:2865
translate french christmas_club_62358c65:

    # GP "She looks excited!"
    GP "Elle a l'air excitée!"

# game/UpdateScriptChristmasEvent.rpy:2866
translate french christmas_club_f12e2c7d:

    # RT "{i}Yeah.... she really looks like she can't wait.{/i}"
    RT "{i}Ouais.... elle a vraiment l'air d'avoir hâte.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2867
translate french christmas_club_c3d8d8b1:

    # RT "{i}Shit.... I am going to have to see her fall in love with him.{/i}"
    RT "{i}Merde.... Je vais devoir la voir tomber amoureuse de lui.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2871
translate french christmas_club_ecc316d7:

    # RT "{i}Oh.... haha!.... Look at how small that was.{/i}"
    RT "{i}Oh.... haha!.... Regardez à quel point c'était petit.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2874
translate french christmas_club_a76a1a56:

    # RT "{i}Look at how disappointed Mom looks!{/i}"
    RT "{i}Regarde à quel point Maman a l'air déçue!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2875
translate french christmas_club_c56b939e:

    # RT "{i}Haha!.... He really should have let her fall in love with him before he showed her his little cock!{/i}"
    RT "{i}Haha!.... Il aurait vraiment dû la laisser tomber amoureuse de lui avant qu'il ne lui montre sa petite queue!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2876
translate french christmas_club_a02d1b7e:

    # GP "It's not that little! It's just little compared to yours."
    GP "Ce n'est pas si petit! C'est juste peu comparé au tien."

# game/UpdateScriptChristmasEvent.rpy:2877
translate french christmas_club_4ee2e824:

    # GP "Goddammit!"
    GP "Bon sang!"

# game/UpdateScriptChristmasEvent.rpy:2884
translate french christmas_club_f2f8aaf8:

    # GP "Well, that was a complete waste of time!"
    GP "Eh bien, c'était une perte de temps complète!"

# game/UpdateScriptChristmasEvent.rpy:2885
translate french christmas_club_b6e55499:

    # GP "There's no way God's going to give me my wings now!"
    GP "Il est impossible que Dieu me donne mes ailes maintenant !"

# game/UpdateScriptChristmasEvent.rpy:2886
translate french christmas_club_ee4eba8f:

    # GP "Maybe she'll give me some if I give her a really good blow job."
    GP "Peut-être qu'elle m'en donnera si je lui fais une très bonne fellation."

# game/UpdateScriptChristmasEvent.rpy:2887
translate french christmas_club_6fcfaed5:

    # RT "{i}Wings? I thought you were a ghost, not an angel?{/i}"
    RT "{i}Ailes? Je pensais que tu étais un fantôme, pas un ange ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2888
translate french christmas_club_8a168ed9:

    # GP "Ghosts can get wings too.... you really don't know anything about ghosts."
    GP "Les fantômes peuvent aussi avoir des ailes.... tu ne sais vraiment rien des fantômes."

# game/UpdateScriptChristmasEvent.rpy:2889
translate french christmas_club_0eac5d99:

    # RT "{i}Yeah, I've never really claimed to, either.{/i}"
    RT "{i}Oui, je n'ai jamais vraiment prétendu le faire non plus.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2890
translate french christmas_club_65f048e2:

    # GP "Well, the only thing you need to know about them right now is that you should expect the next ghost when the bell tolls two."
    GP "Eh bien, la seule chose que tu dois savoir à leur sujet en ce moment est que tu devrais t'attendre au prochain fantôme lorsque la cloche sonnera deux fois."

# game/UpdateScriptChristmasEvent.rpy:2891
translate french christmas_club_f39bd6d2:

    # RT "{i}What bell?{/i}"
    RT "{i}Quelle cloche?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2892
translate french christmas_club_b8d56e34:

    # GP "You know.... like the bell in the clock tower."
    GP "Tu sais.... comme la cloche de la tour de l'horloge."

# game/UpdateScriptChristmasEvent.rpy:2893
translate french christmas_club_cd8d1862:

    # RT "{i}Our town doesn't have a clock tower.{/i}"
    RT "{i}Notre ville n'a pas de tour d'horloge.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2894
translate french christmas_club_251a6ba5:

    # GP "Well.... shit!"
    GP "Bien.... Merde!"

# game/UpdateScriptChristmasEvent.rpy:2895
translate french christmas_club_48338516:

    # GP "That isn't nearly as dramatic."
    GP "Ce n'est pas aussi dramatique."

# game/UpdateScriptChristmasEvent.rpy:2896
translate french christmas_club_4f6b00da:

    # GP "I guess just go to sleep and the ghost will wake you up when it gets there."
    GP "Je suppose qu'il suffit d'aller dormir et le fantôme te réveillera quand il y arrivera."

# game/UpdateScriptChristmasEvent.rpy:2899
translate french christmas_club_6a50afa7:

    # RT "{i}But wait!{/i}"
    RT "{i}Mais attends!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2902
translate french christmas_club_318acb32:

    # RT "{i}What the hell?.... I'm back in my bed?.... Was that just a dream?{/i}"
    RT "{i}C'est quoi ce bordel?.... Je suis de retour dans mon lit ?.... C'était juste un rêve ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2903
translate french christmas_club_d6567244:

    # RT "{i}Maybe Sidney's getting revenge and spiking my tea with LSD.{/i}"
    RT "{i}Peut-être que Sidney se venge et édulcore mon thé avec du LSD.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2904
translate french christmas_club_97e9e54e:

    # RT "{i}That was such a crazy dream.... It felt so real.... then again, most do.{/i}"
    RT "{i}C'était un rêve tellement fou.... C'était si réel.... puis encore, la plupart du temps.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2905
translate french christmas_club_2ded6d39:

    # RT "{i}I do feel pretty tired.{/i}"
    RT "{i}Je me sens assez fatigué.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2913
translate french christmas_club_2a436b9c:

    # R "Well played!.... That is quite the grand gesture."
    R "Bien joué!.... C'est tout à fait le grand geste."

# game/UpdateScriptChristmasEvent.rpy:2914
translate french christmas_club_7c10ce97:

    # D "That's right! So, why don't you just head on out of here! [mom_name] has a little private dance she's dying to give me."
    D "C'est vrai! Alors, pourquoi ne pas partir d'ici ! [mom_name] a une petite danse privée qu'elle meurt d'envie de me donner."

# game/UpdateScriptChristmasEvent.rpy:2915
translate french christmas_club_6c25c4ce:

    # R "Yeah, well, I wouldn't be too sure about that."
    R "Ouais, eh bien, je n'en serais pas trop sûr."

# game/UpdateScriptChristmasEvent.rpy:2916
translate french christmas_club_1c3b1adf:

    # GP "[ryan] No!.... Please let [dad_name] win!"
    GP "[ryan] Non!.... Laisses [dad_name] gagner!"

# game/UpdateScriptChristmasEvent.rpy:2917
translate french christmas_club_52b7ac9e:

    # R "And you should stop rubbing that money all over yourself.... you don't know where it's been."
    R "Et tu devrais arrêter de frotter cet argent sur toi-même.... tu ne sais pas où ça a été."

# game/UpdateScriptChristmasEvent.rpy:2920
translate french christmas_club_3a8faf12:

    # R "I can guarantee my money isn't nearly as dirty as his."
    R "Je peux garantir que mon argent n'est pas aussi sale que le sien."

# game/UpdateScriptChristmasEvent.rpy:2921
translate french christmas_club_31219dbf:

    # GP "[ryan]!.... You asshole!"
    GP "[ryan]!.... Espèce de connard !"

# game/UpdateScriptChristmasEvent.rpy:2922
translate french christmas_club_f8aab42e:

    # M "Holy shit!.... How much money is that?"
    M "Putain de merde!.... C'est combien d'argent ?"

# game/UpdateScriptChristmasEvent.rpy:2923
translate french christmas_club_92275895:

    # R "Two thousand dollars, which, unless you have more money, makes me the winner."
    R "Deux mille dollars, ce qui, à moins d'avoir plus d'argent, fait de moi le gagnant."

# game/UpdateScriptChristmasEvent.rpy:2924
translate french christmas_club_728d0f91:

    # R "Oh.... and you should probably get used to losing to me more in the future."
    R "Oh.... et tu devrais probablement t'habituer à perdre plus contre moi à l'avenir."

# game/UpdateScriptChristmasEvent.rpy:2927
translate french christmas_club_55f09327:

    # D "Who the fuck are you even?"
    D "Qui es-tu, putain ?"

# game/UpdateScriptChristmasEvent.rpy:2928
translate french christmas_club_f7010b03:

    # D "And what's that supposed to mean?"
    D "Et qu'est-ce que cela signifie?"

# game/UpdateScriptChristmasEvent.rpy:2929
translate french christmas_club_0670e0f9:

    # R "You'll find out in about 20 years."
    R "Tu le sauras dans environ 20 ans."

# game/UpdateScriptChristmasEvent.rpy:2930
translate french christmas_club_ddd87cc8:

    # GP "Urrrgghhh, you're such an idiot!"
    GP "Urrrgghhh, tu es un idiot!"

# game/UpdateScriptChristmasEvent.rpy:2931
translate french christmas_club_9447f0c7:

    # D "You're not making any sense!"
    D "Tu n'as aucun sens !"

# game/UpdateScriptChristmasEvent.rpy:2932
translate french christmas_club_995ff9b2:

    # D "I'm leaving! There's nothing else for me to stick around here for."
    D "Je pars ! Il n'y a rien d'autre pour lequel je peux rester ici."

# game/UpdateScriptChristmasEvent.rpy:2933
translate french christmas_club_7cefa8f6:

    # R "Good, because we've never needed you anyway!"
    R "Bien, car nous n'avons jamais eu besoin de toi de toute façon!"

# game/UpdateScriptChristmasEvent.rpy:2934
translate french christmas_club_24e5c3c7:

    # D "What the hell are you talking about?"
    D "De quoi diable parles-tu ?"

# game/UpdateScriptChristmasEvent.rpy:2935
translate french christmas_club_1e46c42f:

    # M "Yeah.... how about we finish this conversation, and you come follow me to my dressing room."
    M "Ouais.... et si on finissait cette conversation et que tu viennes me suivre dans ma loge."

# game/UpdateScriptChristmasEvent.rpy:2936
translate french christmas_club_5f1fa3a7:

    # R "Oh, hell yeah!"
    R "Oh, bon sang!"

# game/UpdateScriptChristmasEvent.rpy:2939
translate french christmas_club_f8a63d31:

    # RT "{i}Oh yeah! I'd follow that ass anywhere!{/i}"
    RT "{i}Oh oui! Je suivrais ce cul n'importe où !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2942
translate french christmas_club_cd63c020:

    # RT "{i}Ahhh...Holy shit! You scared me!{/i}"
    RT "{i}Ahhh...Putain de merde! Tu m'as fait peur !{/i}"

# game/UpdateScriptChristmasEvent.rpy:2943
translate french christmas_club_5b419665:

    # RT "{i}Hey.... are you ok?{/i}"
    RT "{i}Salut.... Est-ce que tu vas bien?{/i}"

# game/UpdateScriptChristmasEvent.rpy:2944
translate french christmas_club_66a89c68:

    # GP "Yeah.... not really."
    GP "Ouais.... Pas vraiment."

# game/UpdateScriptChristmasEvent.rpy:2945
translate french christmas_club_c8db3fc7:

    # GP "That's not how this was supposed to go."
    GP "Ce n'est pas comme ça que cela devait se passer."

# game/UpdateScriptChristmasEvent.rpy:2946
translate french christmas_club_4dc4a389:

    # GP "I was supposed to show you the error of your ways, and now, you'll be getting a lap dance from your mom because of me."
    GP "J'étais censé te montrer l'erreur de tes voies, et maintenant, tu vas recevoir un tour de danse de ta mère à cause de moi."

# game/UpdateScriptChristmasEvent.rpy:2947
translate french christmas_club_69d32637:

    # RT "{i}Don't blame yourself!{/i}"
    RT "{i}Ne te blâme pas!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2948
translate french christmas_club_ccb2066b:

    # RT "{i}It's not your fault that I can't stop thinking with my cock.{/i}"
    RT "{i}Ce n'est pas de ta faute si je ne peux pas arrêter de penser avec ma bite.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2949
translate french christmas_club_6702bf57:

    # GP "Yeah, well.... I don't really have a desire to watch you enjoy my failure, so I'm going to go and see if I can steal some booze from the bar while you get a lap dance."
    GP "Ouais, eh bien.... Je n'ai pas vraiment envie de te regarder profiter de mon échec, alors je vais aller voir si je peux voler de l'alcool au bar pendant que tu fais un tour de danse."

# game/UpdateScriptChristmasEvent.rpy:2950
translate french christmas_club_7ad0d7ee:

    # RT "{i}Damn!.... You're making me feel really bad for what I just did.{/i}"
    RT "{i}Merde!.... Tu me fais vraiment mal pour ce que je viens de faire.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2954
translate french christmas_club_9d100de1:

    # M "Are you coming?"
    M "Tu viens ?"

# game/UpdateScriptChristmasEvent.rpy:2955
translate french christmas_club_5f1fa3a7_1:

    # R "Oh, hell yeah!"
    R "Oh, bon sang!"

# game/UpdateScriptChristmasEvent.rpy:2960
translate french christmas_club_a695f80f:

    # M "So, are you ready for that lap dance?"
    M "Alors, es-tu prêt pour ce tour de danse?"

# game/UpdateScriptChristmasEvent.rpy:2961
translate french christmas_club_35f56a55:

    # R "You have no idea!"
    R "Tu n'en as aucune idée!"

# game/UpdateScriptChristmasEvent.rpy:2962
translate french christmas_club_0161667b:

    # M "Are you sure you're 21?"
    M "Es-tu sûr d'avoir 21 ans?"

# game/UpdateScriptChristmasEvent.rpy:2963
translate french christmas_club_61894366:

    # R "Of course! They wouldn't have let me in otherwise."
    R "Bien sûr! Ils ne m'auraient pas laissé entrer autrement."

# game/UpdateScriptChristmasEvent.rpy:2964
translate french christmas_club_f7a245bb:

    # M "I'd have a hard time even believing that you're 18."
    M "J'aurais même du mal à croire que tu as 18."

# game/UpdateScriptChristmasEvent.rpy:2965
translate french christmas_club_393321d4:

    # R "I just look young, ok?"
    R "J'ai juste l'air jeune, ok?"

# game/UpdateScriptChristmasEvent.rpy:2966
translate french christmas_club_2846d664:

    # M "Yeah ok, but you remind me of someone I know."
    M "Ouais ok, mais tu me rappelles quelqu'un que je connais."

# game/UpdateScriptChristmasEvent.rpy:2967
translate french christmas_club_e892e379:

    # M "You kind of look like a young version of my Dad.... and my brother..."
    M "Tu ressembles à une version jeune de mon père.... et mon frère..."

# game/UpdateScriptChristmasEvent.rpy:2968
translate french christmas_club_b2c6cee4:

    # M "You're not from around here are you?"
    M "Tu n'es pas d'ici, n'est-ce pas?"

# game/UpdateScriptChristmasEvent.rpy:2969
translate french christmas_club_4b40a928:

    # R "No.... I'm from.... another country..."
    R "Non.... Je viens de.... un autre pays..."

# game/UpdateScriptChristmasEvent.rpy:2970
translate french christmas_club_f60cabbd:

    # R "Uhhh.... from Idaho, yeah!"
    R "Uhhh.... de l'Idaho, ouais !"

# game/UpdateScriptChristmasEvent.rpy:2971
translate french christmas_club_ec2cc875:

    # M "Huh.... I've never heard of it."
    M "Hein.... Je n'en ai jamais entendu parler."

# game/UpdateScriptChristmasEvent.rpy:2972
translate french christmas_club_3e295e72:

    # R "Yeah.... most people haven't."
    R "Ouais.... la plupart des gens ne connaissent pas en fait."

# game/UpdateScriptChristmasEvent.rpy:2973
translate french christmas_club_a9c37287:

    # M "Alright, well I'm just going to start dancing then."
    M "D'accord, eh bien, je vais juste commencer à danser alors."

# game/UpdateScriptChristmasEvent.rpy:2974
translate french christmas_club_14af55a0:

    # R "Wait!"
    R "Attends!"

# game/UpdateScriptChristmasEvent.rpy:2975
translate french christmas_club_d8bf5ca9:

    # M "What?"
    M "Quoi?"

# game/UpdateScriptChristmasEvent.rpy:2976
translate french christmas_club_c4566c7d:

    # R "I just gave you $2,000. Don't you think I deserve more than a dance?"
    R "Je viens de te donner 2'000$. Tu ne crois pas que je mérite plus qu'une danse ?"

# game/UpdateScriptChristmasEvent.rpy:2977
translate french christmas_club_bd793359:

    # M "I'm going to be rubbing my ass all over your dick. What more do you want?"
    M "Je vais me frotter le cul sur ta bite. Que veux-tu de plus?"

# game/UpdateScriptChristmasEvent.rpy:2978
translate french christmas_club_60592929:

    # R "I was just wondering what you'd be willing to do for another two grand?"
    R "Je me demandais juste ce que tu serais prête à faire pour deux autres milles?"

# game/UpdateScriptChristmasEvent.rpy:2981
translate french christmas_club_fa111f57:

    # M "Holy shit, that's a lot of money in one day!.... But I'm not a prostitute!"
    M "Merde, c'est beaucoup d'argent en une journée!.... Mais je ne suis pas une prostituée!"

# game/UpdateScriptChristmasEvent.rpy:2982
translate french christmas_club_53556959:

    # M "I'm not going to fuck you for any amount of money."
    M "Je ne vais pas te baiser pour une somme d'argent."

# game/UpdateScriptChristmasEvent.rpy:2983
translate french christmas_club_9b84a6f1:

    # R "Oh, no!.... I wasn't trying to put any presssure on you to fuck me. Although I wouldn't object if you were willing."
    R "Oh, non!.... Je n'essayais pas de te pousser à me baiser. Bien que je ne m'y opposerais pas si tu le veux."

# game/UpdateScriptChristmasEvent.rpy:2984
translate french christmas_club_f6027dbd:

    # R "I just meant like a blowjob or something."
    R "Je voulais juste dire comme une fellation ou quelque chose comme ça."

# game/UpdateScriptChristmasEvent.rpy:2985
translate french christmas_club_a3a109c4:

    # M "A blowjob?"
    M "Une fellation?"

# game/UpdateScriptChristmasEvent.rpy:2986
translate french christmas_club_204035d1:

    # M "Hmmmm..."
    M "Hmmmm..."

# game/UpdateScriptChristmasEvent.rpy:2987
translate french christmas_club_edfc1ee0:

    # MT "{i}Holy shit, [mom_name]! What are you thinking?....{/i}"
    MT "{i}Merde, [mom_name]! À quoi penses-tu ?....{/i}"

# game/UpdateScriptChristmasEvent.rpy:2988
translate french christmas_club_9c747162:

    # MT "{i}You swore you would never do anything like this if you became an exotic dancer.{/i}"
    MT "{i}Tu as juré que tu ne ferais jamais rien de tel si tu devenais une danseuse exotique.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2989
translate french christmas_club_35a9edeb:

    # MT "{i}But I never dreamt one guy would offer me $4,000 dollars in one night!{/i}"
    MT "{i}Mais je n'ai jamais rêvé qu'un gars m'offrirait 4'000 $ pour une nuit!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2990
translate french christmas_club_98c490cc:

    # MT "{i}I'll have enough to pay for a semester of modeling school.{/i}"
    MT "{i}J'aurai assez pour payer un semestre d'école de mannequinat.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2991
translate french christmas_club_c0e48b39:

    # MT "{i}That could be enough for me to get my start in the industry.{/i}"
    MT "{i}Cela pourrait me suffire pour démarrer dans l'industrie.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2992
translate french christmas_club_f518ee6d:

    # MT "{i}And I wouldn't ever have to come back to this sleaze hole again{/i}"
    MT "{i}Et je n'aurais plus jamais à revenir dans ce trou sordide {/i}"

# game/UpdateScriptChristmasEvent.rpy:2993
translate french christmas_club_e717013e:

    # MT "{i}It's worth it!{/i}"
    MT "{i}Ça vaut le coup!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2994
translate french christmas_club_10128fa1:

    # MT "{i}Plus, he's cute!{/i}"
    MT "{i}De plus, il est mignon!{/i}"

# game/UpdateScriptChristmasEvent.rpy:2995
translate french christmas_club_23ca0433:

    # MT "{i}Even if he does look like he could be your brother.{/i}"
    MT "{i}Même s'il a l'air d'être ton frère.{/i}"

# game/UpdateScriptChristmasEvent.rpy:2996
translate french christmas_club_7c39d5bc:

    # M "Take your pants off!"
    M "Enlève ton pantalon !"

# game/UpdateScriptChristmasEvent.rpy:2997
translate french christmas_club_1c553c11:

    # R "Really!?"
    R "Vraiment!?"

# game/UpdateScriptChristmasEvent.rpy:2998
translate french christmas_club_d85670c9:

    # M "Hurry before I change my mind..."
    M "Dépêches-toi avant que je change d'avis..."

# game/UpdateScriptChristmasEvent.rpy:3002
translate french christmas_club_58cf727c:

    # MT "{i}Wow! Look at the size of this cock!{/i}"
    MT "{i}Wow ! Regarde la taille de cette bite !{/i}"

# game/UpdateScriptChristmasEvent.rpy:3003
translate french christmas_club_8168fb5e:

    # MT "{i}He's as big as the men in my family!{/i}"
    MT "{i}Il est aussi grand que les hommes de ma famille!{/i}"

# game/UpdateScriptChristmasEvent.rpy:3007
translate french christmas_club_5d6d2490:

    # RT "{i}Holy shit! Mom's sucking my cock!{/i}"
    RT "{i}Merde! Maman suce ma bite!{/i}"

# game/UpdateScriptChristmasEvent.rpy:3008
translate french christmas_club_b82c20d7:

    # RT "{i}It might just be a dream, but what an amazing dream!{/i}"
    RT "{i}C'est peut-être juste un rêve, mais quel rêve incroyable!{/i}"

# game/UpdateScriptChristmasEvent.rpy:3009
translate french christmas_club_01e9d6b7:

    # MT "{i}Shit! This is so weird!{/i}"
    MT "{i}Merde! C'est tellement bizarre !{/i}"

# game/UpdateScriptChristmasEvent.rpy:3010
translate french christmas_club_66e98a77:

    # MT "{i}Why does it feel like I'm giving a blowjob to my brother?{/i}"
    MT "{i}Pourquoi ai-je l'impression de faire une fellation à mon frère?{/i}"

# game/UpdateScriptChristmasEvent.rpy:3011
translate french christmas_club_ab3ce303:

    # MT "{i}Shit, [mom_name]! Stop thinking so much about your brother!{/i}"
    MT "{i}Merde, [mom_name]! Arrête de penser à ton frère !{/i}"

# game/UpdateScriptChristmasEvent.rpy:3019
translate french christmas_club_9d6cd058:

    # GP "Oh, hey.... I'm done drinking, and I wanted to tell you that it's almost time to go!"
    GP "Oh, salut.... J'ai fini de boire, et je voulais te dire qu'il est presque temps d'y aller!"

# game/UpdateScriptChristmasEvent.rpy:3020
translate french christmas_club_1f713dd5:

    # RT "{i}Not now I'm almost done!{/i}"
    RT "{i}Pas maintenant j'ai presque fini!{/i}"

# game/UpdateScriptChristmasEvent.rpy:3021
translate french christmas_club_4df355ad:

    # GP "Oh, sorry. I didn't know you would still be busy.... I'll just let you get on with it."
    GP "Oh, désolé. Je ne savais pas que tu serais encore occupé.... Je te laisse faire."

# game/UpdateScriptChristmasEvent.rpy:3029
translate french christmas_club_bdf00d39:

    # RT "{i}I'm so close!{/i}"
    RT "{i}Je suis si proche!{/i}"

# game/UpdateScriptChristmasEvent.rpy:3030
translate french christmas_club_973c525a:

    # RT "{i}Oh my God!.... I'm going to explode down my mom's throat!{/i}"
    RT "{i}Oh mon Dieu!.... Je vais exploser dans la gorge de ma mère!{/i}"

# game/UpdateScriptChristmasEvent.rpy:3038
translate french christmas_club_bc12f2a8:

    # GP "Wait! What the hell are you doing! You were just supposed to get a lap dance! Not let your own mother suck your cock!"
    GP "Attends! Qu'est-ce que tu fais, bordel ! Tu devais juste faire un tour de danse! Ne laisses pas ta propre mère sucer ta bite!"

# game/UpdateScriptChristmasEvent.rpy:3039
translate french christmas_club_739bf879:

    # GP "What a tramp! No wonder you're so messed up!"
    GP "Quel piège ! Pas étonnant que tu sois si enfoiré !"

# game/UpdateScriptChristmasEvent.rpy:3042
translate french christmas_club_bd3d21cd:

    # GP "Good thing it's time to go!"
    GP "Heureusement qu'il est temps d'y aller!"

# game/UpdateScriptChristmasEvent.rpy:3043
translate french christmas_club_f6b249ac:

    # RT "{i}No not yet! I'm sooooo close!{/i}"
    RT "{i}Non pas encore ! Je suis telllllemmmmennnnt proche d'arriver!{/i}"

# game/UpdateScriptChristmasEvent.rpy:3044
translate french christmas_club_531604e5:

    # RT "{i}I'm....{/i}"
    RT "{i}Je suis....{/i}"

# game/UpdateScriptChristmasEvent.rpy:3049
translate french christmas_club_a89b9863:

    # RT "{i}Cuuuumming!{/i}"
    RT "{i}Je joooouuuiiiiiss!{/i}"

# game/UpdateScriptChristmasEvent.rpy:3053
translate french christmas_club_f5579268:

    # GP "Oh, shit!.... What the fuck!"
    GP "Oh, merde!.... C'est quoi ce bordel !"

# game/UpdateScriptChristmasEvent.rpy:3057
translate french christmas_club_0888da39:

    # GP "[ryan]! You mother-fucking son of a bitch!"
    GP "[ryan]! Espèce de fils de pute!"

# game/UpdateScriptChristmasEvent.rpy:3058
translate french christmas_club_2ae61c97:

    # GP "In all the centuries I've been doing this, I've never failed to change the course of a client!"
    GP "Depuis tous les siècles que je fais cela, je n'ai jamais manqué de changer le cours d'un client!"

# game/UpdateScriptChristmasEvent.rpy:3059
translate french christmas_club_d97998d8:

    # GP "And this, sure as shit, is the first time I've ever been drenched in their cum!"
    GP "Et c'est bien sûr la première fois que je suis trempé de leur sperme!"

# game/UpdateScriptChristmasEvent.rpy:3060
translate french christmas_club_b92b0e39:

    # GP "I think you have a problem! You're like a cum fire hose!.... That much cum is not normal!"
    GP "Je pense que tu as un problème! Tu es comme un tuyau d'incendie!.... Autant de sperme ce n'est pas normal!"

# game/UpdateScriptChristmasEvent.rpy:3061
translate french christmas_club_b6e55499_1:

    # GP "There's no way God's going to give me my wings now!"
    GP "Il est impossible que Dieu me donne mes ailes maintenant !"

# game/UpdateScriptChristmasEvent.rpy:3062
translate french christmas_club_ddabb200:

    # GP "Maybe she'll give me some if I let her fuck my ass."
    GP "Peut-être qu'elle m'en donnera si je la laisse me baiser le cul."

# game/UpdateScriptChristmasEvent.rpy:3063
translate french christmas_club_6fcfaed5_1:

    # RT "{i}Wings? I thought you were a ghost, not an angel?{/i}"
    RT "{i}Ailes? Je pensais que tu étais un fantôme, pas un ange ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:3064
translate french christmas_club_8a168ed9_1:

    # GP "Ghosts can get wings too.... you really don't know anything about ghosts."
    GP "Les fantômes peuvent aussi avoir des ailes.... tu ne sais vraiment rien des fantômes."

# game/UpdateScriptChristmasEvent.rpy:3065
translate french christmas_club_0eac5d99_1:

    # RT "{i}Yeah, I've never really claimed to, either.{/i}"
    RT "{i}Oui, je n'ai jamais vraiment prétendu le faire non plus.{/i}"

# game/UpdateScriptChristmasEvent.rpy:3066
translate french christmas_club_65f048e2_1:

    # GP "Well, the only thing you need to know about them right now is that you should expect the next ghost when the bell tolls two."
    GP "Eh bien, la seule chose que tu dois savoir à leur sujet en ce moment est que tu devrais t'attendre au prochain fantôme lorsque la cloche sonnera deux fois."

# game/UpdateScriptChristmasEvent.rpy:3067
translate french christmas_club_f39bd6d2_1:

    # RT "{i}What bell?{/i}"
    RT "{i}Quelle cloche?{/i}"

# game/UpdateScriptChristmasEvent.rpy:3068
translate french christmas_club_b8d56e34_1:

    # GP "You know.... like the bell in the clock tower."
    GP "Tu sais.... comme la cloche de la tour de l'horloge."

# game/UpdateScriptChristmasEvent.rpy:3069
translate french christmas_club_cd8d1862_1:

    # RT "{i}Our town doesn't have a clock tower.{/i}"
    RT "{i}Notre ville n'a pas de tour d'horloge.{/i}"

# game/UpdateScriptChristmasEvent.rpy:3070
translate french christmas_club_251a6ba5_1:

    # GP "Well.... shit!"
    GP "Bien.... Merde!"

# game/UpdateScriptChristmasEvent.rpy:3071
translate french christmas_club_48338516_1:

    # GP "That isn't nearly as dramatic."
    GP "Ce n'est pas aussi dramatique."

# game/UpdateScriptChristmasEvent.rpy:3072
translate french christmas_club_4f6b00da_1:

    # GP "I guess just go to sleep and the ghost will wake you up when it gets there."
    GP "Je suppose qu'il suffit d'aller dormir et le fantôme te réveillera quand il y arrivera."

# game/UpdateScriptChristmasEvent.rpy:3073
translate french christmas_club_90f79fae:

    # GP "I pray to our Futa God that she'll have better luck than I did!"
    GP "Je prie notre Dieu Futa pour qu'elle ait plus de chance que moi!"

# game/UpdateScriptChristmasEvent.rpy:3074
translate french christmas_club_1490ba47:

    # GP "Goodbye [ryan]!.... Maybe I'll see you in hell!"
    GP "Au revoir [ryan]!.... Je te verrai peut-être en enfer!"

# game/UpdateScriptChristmasEvent.rpy:3079
translate french christmas_club_6a50afa7_1:

    # RT "{i}But wait!{/i}"
    RT "{i}Mais attends!{/i}"

# game/UpdateScriptChristmasEvent.rpy:3080
translate french christmas_club_e4811f54:

    # RT "{i}What the hell?.... She's gone!.... Was that just a dream?{/i}"
    RT "{i}C'est quoi ce bordel?.... Elle est partie !.... C'était juste un rêve ?{/i}"

# game/UpdateScriptChristmasEvent.rpy:3081
translate french christmas_club_d6567244_1:

    # RT "{i}Maybe Sidney's getting revenge and spiking my tea with LSD.{/i}"
    RT "{i}Peut-être que Sidney se venge et édulcore mon thé avec du LSD.{/i}"

# game/UpdateScriptChristmasEvent.rpy:3082
translate french christmas_club_97e9e54e_1:

    # RT "{i}That was such a crazy dream.... It felt so real.... then again, most do.{/i}"
    RT "{i}C'était un rêve tellement fou.... C'était si réel.... puis encore, la plupart du temps.{/i}"

# game/UpdateScriptChristmasEvent.rpy:3083
translate french christmas_club_2ded6d39_1:

    # RT "{i}I do feel pretty tired.{/i}"
    RT "{i}Je me sens assez fatigué.{/i}"

translate french strings:

    # game/UpdateScriptChristmasEvent.rpy:6
    old "Ghost of XXX-mas Past"
    new ""
#--- ""

    # game/UpdateScriptChristmasEvent.rpy:7
    old "Ghost of XXX-mas Present"
    new ""
#--- ""

    # game/UpdateScriptChristmasEvent.rpy:8
    old "Ghost of XXX-mas Future"
    new ""
#--- ""

